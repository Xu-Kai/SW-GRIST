...
lower_reduction_clauses (tree clauses, gimple_seq *stmt_seqp, omp_context *ctx)
{
  ...
  if (gimple_code (ctx->stmt) == GIMPLE_OMP_FOR
      && gimple_omp_for_kind (ctx->stmt) & GF_OMP_FOR_SIMD)
    return;
	omp_context *target_ctx;
	for (target_ctx = ctx; target_ctx; target_ctx = target_ctx->outer) {
		if (gimple_code(target_ctx->stmt) == GIMPLE_OMP_TARGET) break;
	}
  ...
  for (c = clauses; c ; c = OMP_CLAUSE_CHAIN (c))
	...
	new_var = build_simple_mem_ref_loc (clause_loc, new_var);
	 ...
	 identically to reduction(+:var).  */
      if (code == MINUS_EXPR)
        code = PLUS_EXPR;
		/*改动：由于SW的flush比较慢，这里修改后多个变量也用原子操作实现，并重定位到libswgomp的swgomp_reduce_函数*/
      if (count == 1 || (target_ctx
				&& TREE_CODE(TYPE_SIZE(TREE_TYPE(new_var))) == INTEGER_CST
				&& TREE_INT_CST_LOW(TYPE_SIZE(TREE_TYPE(new_var))) <= 64))
	{
		// puts(get_tree_code_name(TREE_CODE(TYPE_SIZE(TREE_TYPE(new_var)))));
	  tree addr = build_fold_addr_expr_loc (clause_loc, ref);
		if (target_ctx) {
			tree fntype = build_function_type_list(void_type_node, TREE_TYPE(addr), TREE_TYPE(new_var), NULL_TREE);
			char *exp_name = ASTRDUP(get_tree_code_name(code));
			exp_name[strlen(exp_name) - 5] = 0;
			tree fndecl = build_fn_decl(ACONCAT(("swgomp_reduce_", exp_name, "_", mode_name[TYPE_MODE(TREE_TYPE(new_var))], TYPE_UNSIGNED((TREE_TYPE(new_var))) ? "_unsigned": "", NULL)), fntype);
		  tree updt = build_call_expr_loc(clause_loc, fndecl, 2, addr, new_var);
			gimplify_and_add(updt, stmt_seqp);
		} else {
			addr = save_expr (addr);
...
class pass_lower_swgomp : public gimple_opt_pass
{
public:
  pass_lower_swgomp (gcc::context *ctxt)
...
void disable_omp_low(void *gcc_data, void *user_data){
	if (!strcmp(current_pass->name, "omplower")){
		*(int*)gcc_data = 0;
	}
}
#include <dumpfile.h>
void register_swgomp_lower(){
	pass_lower_swgomp *new_pass = new pass_lower_swgomp(g);
	register_pass(new_pass, PASS_POS_INSERT_BEFORE, "omplower", 1);
	register_callback("swgomp", PLUGIN_OVERRIDE_GATE, disable_omp_low, nullptr);
	register_one_dump_file(new_pass);
	dump_file_info *dump_info = g->get_dumps()->get_dump_file_info(new_pass->static_pass_number);
	dump_info->pstate = 1;
}
gimple_opt_pass *
make_pass_lower_omp (gcc::context *ctxt)
{
  return new pass_lower_swgomp (ctxt);
}
