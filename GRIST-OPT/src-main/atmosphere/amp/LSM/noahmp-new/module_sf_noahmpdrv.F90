...
contains
  ...
  subroutine noahmplsm(itimestep,        yr,   julian,   coszin,   xlatin,  & 
   ...
   !out 
    ...
    !$omp target parallel do private(cosz,lat,z_ml,vegtyp,soiltyp,fveg,fvgmax,tbot,t_ml,q_ml,u_ml,v_ml,swdn,lwdn,p_ml,psfc,prcp,prcpconv,prcpnonc,prcpshcv,prcpsnow,prcpgrpl,prcphail,prcpothr,fsh,ssoil,salb,fsno,smceq,smc,smh2o,stc,swe,sndpth,emissi,qsfc1d,isnow,tv,tg,canice,canliq,eah,tah,cm,ch,fwet,sneqvo,albold,qsnow,wslake,zwt,wa,wt,smcwtd,deeprech,rech,zsnso,snice,snliq,lfmass,rtmass,stmass,wood,grain,gdd,stblcp,fastcp,plai,psai,tauss,z0wrf,t2mv,t2mb,q2mv,q2mb,trad,nee,gpp,npp,fvegmp,runsf,runsb,ecan,etran,esoil,fsa,fira,apar,psn,sav,sag,rssun,rssha,bgap,wgap,tgv,tgb,chv,chb,irc,irg,shc,shg,evg,ghv,irb,shb,evb,ghb,tr,evc,chleaf,chuc,chv2,chb2,pahv,pahg,pahb,pah,fpice,fcev,fgev,fctr,qsnbot,ponding,ponding1,ponding2,fsr,ficeold,co2pp,o2pp,zsoil,foln,qc,pblh,dz8w1d,taux, tauy, fire,albd_out, albi_out,i,k,ice,slopetyp,soilcolor,ist)
   iloop : do i = its, ite 
...
! xinyf evaluate the runoff 
             sfcrunoff(i,j)                =   runsf * dt
             udrunoff (i,j)                =   runsb * dt
!<---------------------------------------------------------------------             
             if ( salb > -999 ) then
                albedo(i,j)                = salb
             endif
