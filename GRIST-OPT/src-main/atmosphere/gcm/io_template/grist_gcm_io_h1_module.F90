 ...
 module grist_gcm_io_h1_module
  ...
  use grist_domain_types,     only: global_domain, group_comm, isolated_comm, block_structure
  ...
  use grist_nml_module,       only: fname_output,          &
                                    ...
                                    outdir, nlev, ntracer, &
                                    comm_group_size,       &
                                    start_ymd, start_tod, grid_info, h1_history_unordered
...
!================================================
  ...
  subroutine gcm_output_init(mesh)
    ...
    type(isolated_comm), pointer                 :: isocomm
    integer                                      :: nprocs, ierr
    ...
    isocomm=>mesh%icomm_write
    isocomm%cnt_local_e = size(local_block%output%e)
    isocomm%cnt_local_v = size(local_block%output%v)
    isocomm%cnt_local_t = size(local_block%output%t)
    allocate(isocomm%output_local_e(isocomm%cnt_local_e))
    allocate(isocomm%output_local_v(isocomm%cnt_local_v))
    allocate(isocomm%output_local_t(isocomm%cnt_local_t))
    call mpi_scan(isocomm%cnt_local_e, isocomm%off_local_e, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr)
    call mpi_scan(isocomm%cnt_local_v, isocomm%off_local_v, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr)
    call mpi_scan(isocomm%cnt_local_t, isocomm%off_local_t, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, ierr)
    isocomm%off_local_e = isocomm%off_local_e - isocomm%cnt_local_e
    isocomm%off_local_v = isocomm%off_local_v - isocomm%cnt_local_v
    isocomm%off_local_t = isocomm%off_local_t - isocomm%cnt_local_t
    do i = 1, isocomm%cnt_local_e
        isocomm%output_local_e(i) = mesh%map_g2l_e%find(local_block%output%e(i))
    end do
    do i = 1, isocomm%cnt_local_v
        isocomm%output_local_v(i) = mesh%map_g2l_v%find(local_block%output%v(i))
    end do
    do i = 1, isocomm%cnt_local_t
        isocomm%output_local_t(i) = mesh%map_g2l_t%find(local_block%output%t(i))
    end do
    ! if (mpi_rank() == 5) then
    !     print *, "off and cnt e:", mpi_rank(), isocomm%off_local_e, isocomm%cnt_local_e, minval(isocomm%output_local_e), maxval(isocomm%output_local_e)
    !     print *, "off and cnt v:", mpi_rank(), isocomm%off_local_v, isocomm%cnt_local_v, minval(isocomm%output_local_v), maxval(isocomm%output_local_v)
    !     print *, "off and cnt t:", mpi_rank(), isocomm%off_local_t, isocomm%cnt_local_t, minval(isocomm%output_local_t), maxval(isocomm%output_local_t)
    ! end if
    ! stop
...
#ifdef SPIO
    if (h1_history_unordered) then
        call wrap_output_3d_unordered_sp(mesh, outdir,fname_output)
    else
        call wrap_output_3d_group_sp(mesh, outdir,fname_output)
    end if
#else
    if (h1_history_unordered) then
        call t_startf("isoio3d")
        call wrap_output_3d_unordered(   mesh, outdir,fname_output)
        call t_stopf("isoio3d")
    else
        call t_startf("groupio3d")
        call wrap_output_3d_group(   mesh, outdir,fname_output)
        call t_stopf("groupio3d")
    end if
#endif
...
#ifdef SPIO
    if (h1_history_unordered) then
        call wrap_output_2d_unordered_sp(mesh, outdir, fname_output)
    else
        call wrap_output_2d_group_sp(mesh, outdir, fname_output)
    endif
#else
    if (h1_history_unordered) then
        call wrap_output_2d_unordered(mesh, outdir, fname_output)
    else
...
! outputing
    ...
    ! 
    if (h1_history_unordered) then
        call wrap_output_1d_unordered_sp(mesh, outdir, fname_output)
    else
    ...
    ! 
    if (h1_history_unordered) then
        call wrap_output_1d_unordered(mesh, outdir, fname_output)
    else
