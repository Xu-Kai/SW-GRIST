...
#define CHK_DTP_P12D
#if defined(CHK_DTP_P12D) || defined(CHK_ALL)
#include "data_check.inc"
    real(r8)   ,  allocatable  :: ref_tend_dunDt_at_edge_full_level(:,:)
    real(r8)   ,  allocatable  :: ref_tend_dptmDt_at_pc_full_level(:,:)
    real(r8)   ,  allocatable  :: ref_tend_dqDt_at_pc_full_level(:,:,:)
    allocate(ref_tend_dunDt_at_edge_full_level, source=tend_dunDt_at_edge_full_level)
    allocate(ref_tend_dptmDt_at_pc_full_level, source=tend_dptmDt_at_pc_full_level)
    allocate(ref_tend_dqDt_at_pc_full_level, source=tend_dqDt_at_pc_full_level)
#endif
!
! transform u,v tendency from cell to normal wind tendency at edge
!
!$omp target parallel do private(ie, icell1, icell2, point_cart_coort, vector_velocity, ilev, tend_U_wind_at_edge_full_level, tend_V_wind_at_edge_full_level)
        do ie = 1, mesh%ne
        ...
        do ie = 1, mesh%ne
           ...
           do ilev = 1, nlev
              ...
              call convert_vector_sph2cart(tend_U_wind_at_edge_full_level,&
                                           tend_V_wind_at_edge_full_level,&
                                           point_cart_coort, vector_velocity)
              ref_tend_dunDt_at_edge_full_level(ilev,ie) = dot_product(vector_velocity, mesh%edp_nr(1:3,ie))
           end do
        end do
        call data_check(tend_dunDt_at_edge_full_level, ref_tend_dunDt_at_edge_full_level)
#endif

!
! find mxrt and thetam tendency
!
!$omp target parallel do private(iv, ilev)
        do iv = 1, ncell
           do ilev = 1, nlev
!
...
!             !------------ for regress tc_a, which is a special case)
                   ...
                   ref_tend_dqDt_at_pc_full_level(1:ntracer,ilev,iv) = tend_dsDt_at_pc_full_level(1:ntracer,ilev,iv)/scalar_mif_at_pc_full_level(ilev,iv) ! for general purpose
!
! partial
!
                   ref_tend_dptmDt_at_pc_full_level(ilev,iv) = ((p00/scalar_mpressure_at_pc_full_level(ilev,iv))**(rdry/cp))*&
                                                          (tend_dtDt_at_pc_full_level(ilev,iv)*(one+(rvap/rdry)*scalar_tracer_mxrt_at_pc_full_level(1,ilev,iv))+&
                                                           scalar_temp_at_pc_full_level(ilev,iv)*(rvap/rdry)*ref_tend_dqDt_at_pc_full_level(1,ilev,iv))
           end do
        end do
        call data_check(tend_dqDt_at_pc_full_level, ref_tend_dqDt_at_pc_full_level)
        call data_check(tend_dptmDt_at_pc_full_level, ref_tend_dptmDt_at_pc_full_level)
#endif
     return
