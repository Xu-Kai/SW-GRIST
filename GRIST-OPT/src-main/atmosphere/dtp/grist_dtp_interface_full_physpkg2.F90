...
! #define CHK_DTP_P22D
#if defined(CHK_DTP_P22D) || defined(CHK_ALL)
...
!
!$omp target parallel do private(ie,icell1,icell2,point_cart_coort,ilev,tend_U_wind_at_edge_full_level,tend_V_wind_at_edge_full_level,vector_velocity)  
        do ie = 1, mesh%ne
...
!
      if(flag)then
        !$omp target parallel do private(iv, ilev) collapse(2)
        do iv = 1, ncell
           do ilev = 1, nlev
      ...
      do ie = 1, mesh%ne
         ...
         do ilev = 1, nlev
            ...
            call convert_vector_sph2cart(tend_U_wind_at_edge_full_level,&
                                         tend_V_wind_at_edge_full_level,&
                                         point_cart_coort, vector_velocity)
            ref_tend_dunDt_at_edge_full_level(nlev+1-ilev,ie) = dot_product(vector_velocity, mesh%edp_nr(1:3,ie))
         end do
      end do
!$omp end do nowait
!$omp end parallel 
...
!
    if(flag)then
      do iv = 1, ncell
         do ilev = 1, nlev
               ref_tend_dptmDt_at_pc_full_level(ilev,iv) = (one+rvap/rdry*scalar_tracer_mxrt_at_pc_full_level(1,ilev,iv))*&
                                                        tend_dptDt_at_pc_full_level(nlev+1-ilev,iv)+&
                                                       (rvap/rdry*scalar_pt_at_pc_full_level(ilev,iv)*&
                                                        tend_dqDt_at_pc_full_level(1,nlev+1-ilev,iv))
         end do
