...
#include "data_check.inc"
          real(r8), allocatable :: ref_scalar_tracer_hori_crnum
          real(r8), allocatable :: ref_scalar_tracer_vert_crnum
#endif
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_scalar_tracer_hori_crnum, source=scalar_tracer_hori_crnum)
#endif
call t_startf("hori_crnum_target_1")
!$omp target
...
!$omp end parallel
!$omp end target 
call t_stopf("hori_crnum_target_1")

#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("hori_crnum_1")
!$omp parallel  private(ie,ilev)
!$omp do 
     do ie = 1, mesh%ne
          do ilev = 1, nlev
          ref_scalar_tracer_hori_crnum = max(ref_scalar_tracer_hori_crnum,&
                                        abs(tracerVarEdgeFull%scalar_normal_velocity_avg_adv%f(ilev,ie)*&
                                             dtime/(rearth*mesh%edt_leng(ie))))
          end do
     end do
...
!$omp end do nowait
!$omp end parallel
call t_stopf("hori_crnum_1")
call data_check(ref_scalar_tracer_hori_crnum,scalar_tracer_hori_crnum)
deallocate(ref_scalar_tracer_hori_crnum)
#endif


#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_scalar_tracer_vert_crnum, source=scalar_tracer_vert_crnum)
#endif
call t_startf("vert_crnum_target_1")
!$omp target
!$omp parallel  private(ie,ilev)
!$omp do 
...
!$omp end parallel
!$omp end target 
call t_stopf("vert_crnum_target_1")

#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("vert_crnum_1")
!$omp parallel  private(ie,ilev)
!$omp do 
...
do iv = 1, mesh%nv
     do ilev = 2, nlev
        ref_scalar_tracer_vert_crnum = max(ref_scalar_tracer_vert_crnum,&
                                       abs(tracerVarCellFace%scalar_eta_mass_flux_avg_adv%f(ilev,iv)*&
                                           dtime/(0.5_r8*(dycoreVarCellFull%scalar_delhp_n%f(ilev-1,iv)+&
                                                          dycoreVarCellFull%scalar_delhp_n%f(ilev,iv)))))
     end do
...
!$omp end do nowait
!$omp end parallel
call t_stopf("vert_crnum_1")
call data_check(ref_scalar_tracer_vert_crnum, scalar_tracer_vert_crnum)
deallocate(ref_scalar_tracer_vert_crnum)
#endif
