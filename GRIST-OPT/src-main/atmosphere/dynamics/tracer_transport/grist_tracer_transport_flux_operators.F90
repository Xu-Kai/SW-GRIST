...
! #define CHK_TRAC_TRAN_FLUX_AT_EDGE
#if defined(CHK_TRAC_TRAN_FLUX_AT_EDGE) || defined(CHK_ALL)
#include <data_check.inc>
   real(r8), allocatable                :: ref_scalar_normal_mxrt_at_edge_full_level(:,:,:)

   allocate(ref_scalar_normal_mxrt_at_edge_full_level, source=scalar_normal_mxrt_at_edge_full_level)
#endif
   select case(adv_flag)

   case (1)  ! 1st-order upwind flux
      call t_startf("tracer_transport_normal_flux_at_edge_target_1")
!$omp target
...
!$omp end parallel 
!$omp end target
       call t_stopf("tracer_transport_normal_flux_at_edge_target_1")
      ...
      do ie = 1, mesh%ne
         ...
         do ilev = 1, nlev
            wind_edge  = flag*scalar_normal_velocity_at_edge_full_level(ilev,ie)
            do itracer = 1, ntracer
               ref_scalar_normal_mxrt_at_edge_full_level(itracer,ilev,ie) = &
                      half*(scalar_tracer_mxrt_at_pc_full_level(itracer,ilev,v0)+scalar_tracer_mxrt_at_pc_full_level(itracer,ilev,v1))-&
                      half*sign(one,wind_edge)*(scalar_tracer_mxrt_at_pc_full_level(itracer,ilev,v1)-scalar_tracer_mxrt_at_pc_full_level(itracer,ilev,v0))
            end do
         end do
      ...
      !$omp end do nowait
      !$omp end parallel 
      call data_check(scalar_normal_mxrt_at_edge_full_level, ref_scalar_normal_mxrt_at_edge_full_level)
#endif
...
!$omp end parallel 
    ...
    case(33)  ! purely upwind O3
      call t_startf("tracer_transport_normal_flux_at_edge_target_2")
!$omp target
...
!$omp end parallel 
!$omp end target
       call t_stopf("tracer_transport_normal_flux_at_edge_target_2")
         ...
         do ilev   = 1, nlev
            ...
            do itracer = 1, ntracer
               ...
               part1 = (scalar_tracer_mxrt_at_pc_full_level(itracer,ilev,v0)+&
                        scalar_tracer_mxrt_at_pc_full_level(itracer,ilev,v1))*0.5_r8
               part2 = -1._r8*((mesh%edt_leng(ie)**2)/6._r8)*der0
               ref_scalar_normal_mxrt_at_edge_full_level(itracer,ilev,ie) = part1+part2
            end do
         end do
      end do
!$omp end do nowait
