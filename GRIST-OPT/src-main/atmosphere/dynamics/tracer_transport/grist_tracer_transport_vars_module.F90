...
#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
#include "data_check.inc"
      real(r8), allocatable :: ref_scalar_mif_n_f(:,:)
#endif
     call t_startf("tracer_transport_evaluate_mif")
! simple test of dry core
       if(trim(working_mode).eq.'dycore')then
          call t_startf("tracer_transport_drycore")
          !$omp target
          ...
          !$omp end target
          call t_stopf("tracer_transport_drycore")
          call t_stopf("tracer_transport_evaluate_mif")
          return
       end if

#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
       allocate(ref_scalar_mif_n_f, source=tracerVarCellFull%scalar_mif_n%f)
#endif
! evaluate forcing at cell, which persists across dycore steps
       call t_startf("tracer_transport_forcing_cell_target")
!$omp target
...
!$omp end parallel
!$omp end target
       call t_stopf("tracer_transport_forcing_cell_target")

#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
call t_startf("tracer_transport_forcing_cell")
   do iv = 1, mesh%nv_full
      do ilev = 1, nlev
            ref_scalar_mif_n_f(ilev,iv) = one/(one+sum(tracerVarCellFull%scalar_tracer_mxrt_n%f(mif_index(1:nmif),ilev,iv)))
      end do
   end do
   call t_stopf("tracer_transport_forcing_cell")
   call data_check(tracerVarCellFull%scalar_mif_n%f, ref_scalar_mif_n_f)
   deallocate(ref_scalar_mif_n_f)
#endif



#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
       allocate(ref_scalar_mif_n_f, source=tracerVarEdgeFull%scalar_mif_n%f)
#endif
! cell to edge
       call t_startf("tracer_transport_cell_to_edge_target")
!$omp target
...
!$omp end parallel
!$omp end target
       call t_stopf("tracer_transport_cell_to_edge_target")

#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
call t_startf("tracer_transport_cell_to_edge")
...
do ie = 1, mesh%ne
      ...
      do ilev = 1, nlev
#ifdef MIFAVG
      ref_scalar_mif_n_f(ilev,ie) = half*(ref_scalar_mif_n_f(ilev,icell1)+&
                                          ref_scalar_mif_n_f(ilev,icell2))
#else
         tmp  = half*(sum(tracerVarCellFull%scalar_tracer_mxrt_n%f(mif_index(1:nmif),ilev,icell1))+&
                      sum(tracerVarCellFull%scalar_tracer_mxrt_n%f(mif_index(1:nmif),ilev,icell2)))
      ref_scalar_mif_n_f(ilev,ie) = one/(one+tmp)
...
!$omp end do nowait
!$omp end parallel
   call t_stopf("tracer_transport_cell_to_edge")
   call data_check(tracerVarEdgeFull%scalar_mif_n%f, ref_scalar_mif_n_f)
   deallocate(ref_scalar_mif_n_f)

#endif
#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
      allocate(ref_scalar_mif_n_f, source=tracerVarCellFace%scalar_mif_n%f)
#endif
...
#else
       call t_startf("tracer_transport_full_to_face_target")
!$omp target
!$omp parallel private(iv, ilev, tmp)
!$omp do
       do iv = 1, mesh%nv_full
       do ilev = 2, nlev
...
!$omp end parallel 
!$omp end target
       call t_stopf("tracer_transport_full_to_face_target")

#if defined(CHK_TRANS_EVAL_MIF) || defined(CHK_ALL)
      call t_startf("tracer_transport_full_to_face")

      do iv = 1, mesh%nv_full
      do ilev = 2, nlev
              tmp = 0.5*(deta_full(ilev-1)/deta_face(ilev)*sum(tracerVarCellFull%scalar_tracer_mxrt_n%f(mif_index(1:nmif),ilev,iv))+&
                         deta_full(ilev)  /deta_face(ilev)*sum(tracerVarCellFull%scalar_tracer_mxrt_n%f(mif_index(1:nmif),ilev-1,iv)))
            ref_scalar_mif_n_f(ilev,iv) = one/(one+tmp)
      end do
      end do

      call t_stopf("tracer_transport_full_to_face")
      call data_check(tracerVarCellFace%scalar_mif_n%f, ref_scalar_mif_n_f)
      deallocate(ref_scalar_mif_n_f)
#endif
#endif
! these bdy conditions actually donot matter, but we set here for consistency
       call t_startf("tracer_transport_final_copy")
       ...
       call t_stopf("tracer_transport_final_copy")
       call t_stopf("tracer_transport_evaluate_mif")
       return
