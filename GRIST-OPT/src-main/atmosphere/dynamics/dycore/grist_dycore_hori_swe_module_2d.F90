...
#include "data_check.inc"
         real(r8), allocatable :: ref_scalar_avor_at_dc_full_level(:, :)
         real(r8), allocatable :: ref_scalar_rvor_at_dc_full_level(:, :)
...
do it = 1, mesh%nt
   ...
   ref_scalar_avor_at_dc_full_level(1:nlev,it) = ref_scalar_rvor_at_dc_full_level(1:nlev,it)+2._r8*omega*sin(mesh%tri_c_lat(it))
end do
!$omp end do nowait
!$omp end parallel
...
#include "data_check.inc"
      ...
      real(r8), allocatable :: ref_tend_grad_ke_at_edge_full_level(:,:)
...
do ie = 1, mesh%ne
   ...
   v2      = mesh%edt_v(2, ie)
   do ilev = 1, nlev
      ref_tend_grad_ke_at_edge_full_level(ilev,ie) = -mesh%edt_edpNr_edtTg(ie)*(kinetic_energy(ilev,v2)-kinetic_energy(ilev,v1))/(rearth*mesh%edt_leng(ie))
   end do
end do
!$omp end do nowait
!$omp end parallel
...
#include "data_check.inc"
   real(r8), allocatable                  :: ref_scalar_pv_at_edge_full_level(:,:)
