...
! local
   ...
   integer(i4)                          :: ii
!#define CHK_PRIMAL_FLUX
#if defined(CHK_PRIMAL_FLUX) || defined(CHK_ALL)
#include <data_check.inc>
   real(r8), allocatable                :: my_scalar1_normal_flux_at_edge_full_level(:,:) ! also q
   real(r8), allocatable                :: my_scalar2_normal_flux_at_edge_full_level(:,:) ! also q
   allocate(my_scalar1_normal_flux_at_edge_full_level, source=scalar1_normal_flux_at_edge_full_level)
   allocate(my_scalar2_normal_flux_at_edge_full_level, source=scalar2_normal_flux_at_edge_full_level)
#endif
   select case(adv_flag)

   case(2)  ! 2nd-order center difference 
      call t_startf("primal_flux_operators_2d_2")
       ...
       call t_stopf("primal_flux_operators_2d_2")
...
call t_startf("primal_flux_operators_2d_34567")
...
!$omp end do nowait
!$omp end parallel 
   call t_stopf("primal_flux_operators_2d_34567")
   case(33)  ! pure 3ord
   call t_startf("primal_flux_operators_2d_33")
!$omp target
...
! inline_opt_here
            ...
            !                real(mesh%edt_leng(ie),r8))
               ...
               my_scalar1_normal_flux_at_edge_full_level(ilev,ie) = (scalar1_height_at_pc_full_level(ilev,v1)+&
                                                                  scalar1_height_at_pc_full_level(ilev,v2))*0.5_r8-&
                                                                 ((real(mesh%edt_leng(ie),r8)**2)/6._r8)*der1_0

               my_scalar2_normal_flux_at_edge_full_level(ilev,ie) = (scalar2_height_at_pc_full_level(ilev,v1)+&
                                                                  scalar2_height_at_pc_full_level(ilev,v2))*0.5_r8-&
                                                                 ((real(mesh%edt_leng(ie),r8)**2)/6._r8)*der2_0
! inline_opt_here
...
!$omp end do nowait
!$omp end parallel 
    call data_check(scalar1_normal_flux_at_edge_full_level, my_scalar1_normal_flux_at_edge_full_level)
    call data_check(scalar2_normal_flux_at_edge_full_level, my_scalar2_normal_flux_at_edge_full_level)
    deallocate(my_scalar1_normal_flux_at_edge_full_level)
    deallocate(my_scalar2_normal_flux_at_edge_full_level)
#endif
   call t_stopf("primal_flux_operators_2d_33")
   case default
        print*,"you must select an advection scheme/flux divergence operator in gcm_para"
   end select

   call t_startf("primal_normal_flux_at_edge_var2_last")
#ifndef REGRESSION
#if defined(CHK_PRIMAL_FLUX) || defined(CHK_ALL) 
   allocate(my_scalar2_normal_flux_at_edge_full_level, source=scalar2_normal_flux_at_edge_full_level)
#endif 
call t_startf("primal_normal_flux_at_edge_var2_target_5")
!$omp target
!$omp parallel private(ie, v1, v2)
!$omp  do 
   do ie = 1, mesh%ne
      v1      = mesh%edt_v(1, ie)
      v2      = mesh%edt_v(2, ie)
...
!$omp end parallel 
!$omp end target
call t_stopf("primal_normal_flux_at_edge_var2_target_5")

#if defined(CHK_PRIMAL_FLUX) || defined(CHK_ALL) 
call t_startf("primal_normal_flux_at_edge_var2_5")
do ie = 1, mesh%ne
   v1      = mesh%edt_v(1, ie)
   v2      = mesh%edt_v(2, ie)
   my_scalar2_normal_flux_at_edge_full_level(nlev,ie) = 0.5_r8*(scalar2_height_at_pc_full_level(nlev,v1)+scalar2_height_at_pc_full_level(nlev,v2))
end do
call t_stopf("primal_normal_flux_at_edge_var2_5")
call data_check(scalar2_normal_flux_at_edge_full_level, my_scalar2_normal_flux_at_edge_full_level)
deallocate(my_scalar2_normal_flux_at_edge_full_level)
#endif
#endif
...
#endif
call t_stopf("primal_normal_flux_at_edge_var2_last")
...
! local
   ...
   integer(i4)                          :: flag, mark
   integer(i4)                          :: ii
#if defined(PRIMAL_NORMAL_FLUX_AT_EDGE) || defined(CHK_ALL)
#include "data_check.inc"
   real(r8), allocatable :: ref_scalar_normal_flux_at_edge_full_level(:, :)
#endif
   select case(adv_flag)

   case(2)  ! 2nd-order center difference
#if defined(PRIMAL_NORMAL_FLUX_AT_EDGE) || defined(CHK_ALL)
allocate(ref_scalar_normal_flux_at_edge_full_level, source = scalar_normal_flux_at_edge_full_level)
#endif

call t_startf("primal_normal_flux_at_edge_target_1")
!$omp target
...
!$omp end parallel 
!$omp end target
call t_stopf("primal_normal_flux_at_edge_target_1")
#if defined(PRIMAL_NORMAL_FLUX_AT_EDGE) || defined(CHK_ALL)
call t_startf("primal_normal_flux_at_edge_1")
!$omp parallel  private(ie,ilev,v1,v2)
!$omp  do 
       do ie = 1, mesh%ne
          v1      = mesh%edt_v(1, ie)
          v2      = mesh%edt_v(2, ie)
          do ilev = 1, nlev
             ref_scalar_normal_flux_at_edge_full_level(ilev, ie) = 0.5*(scalar_height_at_pc_full_level(ilev,v1)+&
                                                                    scalar_height_at_pc_full_level(ilev,v2))
          end do
       end do
!$omp end do nowait
!$omp end parallel 
call t_stopf("primal_normal_flux_at_edge_1")
call data_check(scalar_normal_flux_at_edge_full_level, ref_scalar_normal_flux_at_edge_full_level)
deallocate(ref_scalar_normal_flux_at_edge_full_level)
#endif
   case(3,4,5,6,7)  ! WRF3/4, damping depends on beta value
...
allocate(ref_scalar_normal_flux_at_edge_full_level, source=scalar_normal_flux_at_edge_full_level)
#endif
call t_startf("primal_normal_flux_at_edge_target_2")
!$omp target
...
!$omp end parallel
!$omp end target
call t_stopf("primal_normal_flux_edge_target_2")

#if defined(PRIMAL_NORMAL_FLUX_AT_EDGE) || defined(CHK_ALL)
call t_startf("primal_normal_flux_at_edge_2")
!$omp parallel  private(ii,ie,ilev,v1,v2,wind_edge,der0,der1) 
!$omp  do  
    do ii = 1, mesh%ne*nlev,1
        ie = ceiling(ii/real(nlev,r8))
...
!          do ilev  = 1, nlev
             ...
             call flux_wrf34(wind_edge, scalar_height_at_pc_full_level(ilev,v1),&
                                        scalar_height_at_pc_full_level(ilev,v2),&
                             der0,der1, ref_scalar_normal_flux_at_edge_full_level(ilev,ie),&
                             real(mesh%edt_leng(ie),r8),beta)
!          end do
!    end do
    end do
!$omp end do nowait
!$omp end parallel
call t_stopf("primal_normal_flux_edge_2")
call data_check(ref_scalar_normal_flux_at_edge_full_level, scalar_normal_flux_at_edge_full_level)
deallocate(ref_scalar_normal_flux_at_edge_full_level)
#endif
   case(33)  ! pure 3ord
#if defined(PRIMAL_NORMAL_FLUX_AT_EDGE) || defined(CHK_ALL)
allocate(ref_scalar_normal_flux_at_edge_full_level, source=scalar_normal_flux_at_edge_full_level)
#endif
call t_startf("primal_normal_flux_edge_target_3`")
...
!$omp end parallel
!$omp end target
call t_stopf("primal_normal_flux_edge_target_3`")


#if defined(PRIMAL_NORMAL_FLUX_AT_EDGE) || defined(CHK_ALL)
call t_startf("primal_normal_flux_edge_3")
!$omp parallel  private(ii,ie,ilev,v1,v2,wind_edge,der0,mark,v_upwind)
!$omp  do 
do ii = 1, mesh%ne*nlev,1
   ie=ceiling(ii/real(nlev,r8))
...
!          do ilev   = 1, nlev
        ...
        call flux_wrf3(scalar_height_at_pc_full_level(ilev,v1),&
                       scalar_height_at_pc_full_level(ilev,v2),&
                       der0, ref_scalar_normal_flux_at_edge_full_level(ilev,ie),&
                       real(mesh%edt_leng(ie),r8))
!          end do
!    end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("primal_normal_flux_edge_3")
call data_check(ref_scalar_normal_flux_at_edge_full_level, scalar_normal_flux_at_edge_full_level)
deallocate(ref_scalar_normal_flux_at_edge_full_level)
#endif
   case default
