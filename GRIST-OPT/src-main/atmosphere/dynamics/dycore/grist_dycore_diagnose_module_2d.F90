...
#if defined(DIAGNOSE_TOTAL_ENERGY_MOIST_E2) || defined(CHK_ALL)
#include "data_check.inc"
     real(r8) :: ref_scalar_energy_at_pc
     real(r8) :: ref_scalar_kinetic_energy
     real(r8) :: ref_scalar_potential_energy
     real(r8) :: ref_scalar_internal_energy
     real(r8) :: ref_scalar_energy_at_edge
#endif
!
...
!
     ...
     ref_scalar_energy_at_pc     = 0._r8
     ref_scalar_energy_at_edge   = 0._r8
     ref_scalar_potential_energy = 0._r8
     ref_scalar_internal_energy  = 0._r8
     ref_scalar_kinetic_energy   = 0._r8
#endif
call t_startf("diagnose_total_energy_moist_e2_target_1")
!$omp target map(tofrom: scalar_energy_at_pc, scalar_kinetic_energy, scalar_potential_energy, scalar_internal_energy)
!$omp parallel private(iv, ilev, partx, part0, part1, part2, delmp, gcv)
!$omp do 
     do iv = 1, mesh%nv
!
! compute phis*ps
...
!$omp end parallel 
!$omp end target
     call t_stopf("diagnose_total_energy_moist_e2_target_1")
#if defined(DIAGNOSE_TOTAL_ENERGY_MOIST_E2) || defined(CHK_ALL)

call t_startf("diagnose_total_energy_moist_e2_1")
     do iv = 1, mesh%nv
     !
     ! compute phis*ps
     !
     ...
     ! temperature part
              ...
              ref_scalar_energy_at_pc     = ref_scalar_energy_at_pc    +(partx+part0+part1+part2)*mesh%plg_areag(iv) !*rearth*rearth
              ref_scalar_kinetic_energy   = ref_scalar_kinetic_energy  +part0*mesh%plg_areag(iv)
              ref_scalar_potential_energy = ref_scalar_potential_energy+partx*mesh%plg_areag(iv)         ! not a real PE
              ref_scalar_internal_energy  = ref_scalar_internal_energy +(part1+part2)*mesh%plg_areag(iv) ! not a real IE
      
          end do
call t_stopf("diagnose_total_energy_moist_e2_1")
#endif 
! KE at edges

call t_startf("diagnose_total_energy_moist_e2_target_2")
!$omp target map(tofrom: scalar_energy_at_edge)
!$omp parallel private(ie, ilev, icell1, icell2, delmp_c1, delmp_c2, part3)
!$omp  do  reduction(+: scalar_energy_at_edge)
     do ie = 1, mesh%ne
...
!$omp end parallel 
!$omp end target
     call t_stopf("diagnose_total_energy_moist_e2_target_2")

#if defined(DIAGNOSE_TOTAL_ENERGY_MOIST_E2) || defined(CHK_ALL)
call t_startf("diagnose_total_energy_moist_e2_2")
do ie = 1, mesh%ne
     ...
     if(icell1>mesh%nv_compute.or.icell2>mesh%nv_compute) then
        ref_scalar_energy_at_edge = ref_scalar_energy_at_edge+part3*mesh%edt_leng(ie)*mesh%edp_leng(ie)*0.5_r8  !*rearth*rearth ! rhombi area doubled to!*rearth*rearth
     else                                                                                               ! account two velocity component;  
          ref_scalar_energy_at_edge = ref_scalar_energy_at_edge+part3*mesh%edt_leng(ie)*mesh%edp_leng(ie)         ! see R10; resulting the same
     endif                                                                                              ! meaning as we sum over cell-based
  end do  
  call t_stopf("diagnose_total_energy_moist_e2_2")
  call data_check(scalar_energy_at_edge, ref_scalar_energy_at_edge)
#endif 
...
#if defined(DIAGNOSE_TOTAL_ENERGY_MOIST_E3) || defined(CHK_ALL)
#include "data_check.inc"
     real(r8) :: ref_scalar_energy_at_pc
     real(r8) :: ref_scalar_kinetic_energy
     real(r8) :: ref_scalar_potential_energy
...
call t_startf("diagnose_total_energy_moist_e3_target_1")
!$omp target map(tofrom: scalar_energy_at_pc, scalar_kinetic_energy, scalar_potential_energy, scalar_internal_energy)
!$omp parallel private(iv, ilev, partx, part0, part1, part2, delmp, gcv)
!$omp  do  reduction(+: scalar_energy_at_pc, scalar_kinetic_energy, scalar_potential_energy, scalar_internal_energy)
     do iv = 1, mesh%nv
!
! compute phis*ps
!
...
!$omp end parallel 
!$omp end target
     call t_stopf("diagnose_total_energy_moist_e3_target_1")
#if defined(DIAGNOSE_TOTAL_ENERGY_MOIST_E3) || defined(CHK_ALL)
...
! temperature part
     ...
     call t_stopf("diagnose_total_energy_moist_e3_target_1")
     call data_check(scalar_energy_at_pc, ref_scalar_energy_at_pc)
     call data_check(scalar_kinetic_energy, ref_scalar_kinetic_energy)
     call data_check(scalar_potential_energy, ref_scalar_potential_energy)
     call data_check(scalar_internal_energy, ref_scalar_internal_energy)
#endif
! KE at edges
call t_startf("diagnose_total_energy_moist_e3_target_2")
!$omp target map(tofrom: scalar_energy_at_edge)
!$omp parallel private(ie, ilev, icell1, icell2, delmp_c1, delmp_c2, part3)
!$omp  do   reduction(+: scalar_energy_at_edge)
     do ie = 1, mesh%ne
...
!$omp end parallel 
!$omp end target
     call t_stopf("diagnose_total_energy_moist_e3_target_2")

#if defined(DIAGNOSE_TOTAL_ENERGY_MOIST_E3) || defined(CHK_ALL)
call t_startf("diagnose_total_energy_moist_e3_2")
 ...
 do ie = 1, mesh%ne
  ...
  call t_stopf("diagnose_total_energy_moist_e3_2")
  call data_check(scalar_energy_at_edge, ref_scalar_energy_at_edge)
#endif 
...
! local
   ...
   real(r8)                           :: scalar_kinetic_energy,scalar_kinetic_energy_all
   real(r8)                           :: scalar_total_energy,scalar_total_energy_all
#if defined(CHK_DY_DIAGNOSE_TOTAL_ENERGY) || defined(CHK_ALL)
#include "data_check.inc"
    real(r8)  :: ref_scalar_energy_at_pc
    real(r8)  :: ref_scalar_potential_energy
    real(r8)  :: ref_scalar_internal_energy
...
call t_startf("dycore_diagnose_total_energy_target_1")
#if defined(USE_OMP_FOR_DIAGNOSE_TOTAL_ENERGY)
!$omp target map(tofrom: scalar_energy_at_pc, scalar_potential_energy, scalar_internal_energy, scalar_kinetic_energy)
!$omp parallel private(iv, ilev, part0, part1, part2) 
!$omp  do  reduction(+: scalar_energy_at_pc, scalar_potential_energy, scalar_internal_energy, scalar_kinetic_energy)
#endif
     do iv = 1, mesh%nv
!
! compute phis*ps
...
!$omp end target
#endif
     call t_stopf("dycore_diagnose_total_energy_target_1")

#if defined(CHK_DY_DIAGNOSE_TOTAL_ENERGY) || defined(CHK_ALL)
     call t_startf("dycore_diagnose_total_energy_1")
     do iv = 1, mesh%nv
     !
     ! compute phis*ps
     !
     ...
     ! below for dev
              ...
              ref_scalar_energy_at_pc     = ref_scalar_energy_at_pc+(part0+part1+part2)*mesh%plg_areag(iv) !*rearth*rearth
              ref_scalar_potential_energy = ref_scalar_potential_energy+part1*mesh%plg_areag(iv)
              ref_scalar_internal_energy  = ref_scalar_internal_energy +part2*mesh%plg_areag(iv) ! actually enthalpy
              ref_scalar_kinetic_energy   = ref_scalar_kinetic_energy  +part0*mesh%plg_areag(iv)
      
     end do
     call t_stopf("dycore_diagnose_total_energy_1")
     call data_check(scalar_energy_at_pc, ref_scalar_energy_at_pc)
     call data_check(scalar_potential_energy, ref_scalar_potential_energy)
     call data_check(scalar_internal_energy, ref_scalar_internal_energy)
     call data_check(scalar_kinetic_energy, ref_scalar_kinetic_energy)
...
! KE at edges
     ...
     call t_startf("dycore_diagnose_total_energy_target_2")
!$omp target map(tofrom: scalar_energy_at_edge)
!$omp parallel private(ie, ilev, icell1, icell2, part3) 
!$omp  do  reduction(+: scalar_energy_at_edge)
     do ie = 1, mesh%ne
...
!$omp end parallel 
!$omp end target
     call t_stopf("dycore_diagnose_total_energy_target_2")

#if defined(CHK_DY_DIAGNOSE_TOTAL_ENERGY) || defined(CHK_ALL)
     call t_startf("dycore_diagnose_total_energy_2")
     do ie = 1, mesh%ne
          !icell1 = mesh%edt(ie)%v(1)
          !icell2 = mesh%edt(ie)%v(2)
          icell1 = mesh%edt_v(1,ie)
          ...
          if(icell1>mesh%nv_compute.or.icell2>mesh%nv_compute) then ! this is a bdy edge, remove double count
             ref_scalar_energy_at_edge = ref_scalar_energy_at_edge+part3*mesh%edt_leng(ie)*mesh%edp_leng(ie)*0.5_r8 !*rearth*rearth
          else                                                                                      
             ref_scalar_energy_at_edge = ref_scalar_energy_at_edge+part3*mesh%edt_leng(ie)*mesh%edp_leng(ie) !*rearth*rearth
          endif
       end do
     call t_stopf("dycore_diagnose_total_energy_2")
     call data_check(scalar_energy_at_edge, ref_scalar_energy_at_edge)
#endif 
