...
real(r8), allocatable :: ref_scalar_mass_eta_velocity(:,:)
...
!
         ...
         do ilev = 1, nlev
            ...
            ref_scalar_mass_eta_velocity(ilev+1,iv) = eta_face_b(ilev+1)*(-1._r8*scalar_ps_tendency(iv))-vert_sum
         end do
!
! this should be, or the above routine is bugged
!
        if (r8 .eq. 8) then
          if( abs(ref_scalar_mass_eta_velocity(nlev+1,iv)).gt.1e-15) print*,"sth wrong inside calc_hpe_tend_continuity_2d m*etadot"
        elseif(r8 .eq. 4) then
          if( abs(ref_scalar_mass_eta_velocity(nlev+1,iv)).gt.1e-7) print*,"sth wrong inside calc_hpe_tend_continuity_2d m*etadot"
        end if
