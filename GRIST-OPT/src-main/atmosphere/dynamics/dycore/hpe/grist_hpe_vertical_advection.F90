...
#if defined(CHK_HPE_VERT_MASS_FLUX_2D) || defined(CHK_ALL)
#include "data_check.inc"
    real(r8), allocatable :: ref_tend_vert_mass_flux(:, :)
    allocate(ref_tend_vert_mass_flux, source=tend_vert_mass_flux)
#endif

call t_startf("hpe_tend_vert_mass_flux_2d_target")
!$omp target
...
!$omp end parallel 
!$omp end target
call t_stopf("hpe_tend_vert_mass_flux_2d_target")


#if defined(CHK_HPE_VERT_MASS_FLUX_2D) || defined(CHK_ALL)
call t_startf("hpe_tend_vert_mass_flux_2d")
!$omp parallel private(icell, ilev)
!$omp do
do icell = 1, ncell_do
   call calc_hpe_vert_flux_operator(scalar_at_full_level(1:nlev,icell)   ,&
                                    ...
                                    scalar_at_face_level(1:nlevp,icell)   )
   do ilev =  1, nlev
      ref_tend_vert_mass_flux(ilev,icell) = -(scalar_eta_velocity(ilev+1,icell)*scalar_at_face_level(ilev+1,icell)-&
                                          scalar_eta_velocity(ilev,icell)  *scalar_at_face_level(ilev,icell))
...
!$omp end do nowait
!$omp end parallel 
call t_stopf("hpe_tend_vert_mass_flux_2d")

call data_check(tend_vert_mass_flux, ref_tend_vert_mass_flux)
deallocate(ref_tend_vert_mass_flux)
#endif
     return
...
!
     select case(order)
     case(2) 
      ! call t_startf("hpe_vert_flux_operator_1")
      ...
      !   call t_stopf("hpe_vert_flux_operator_1")

     case(3) 
      ! call t_startf("hpe_vert_flux_operator_2")
        scalar_at_face_level(1)       = 0._r8
      ...
      !   call t_stopf("hpe_vert_flux_operator_2")
     ...
     select case(order)
     case(2)
      ! call t_startf("hpe_vert_flux_operator_3")
        do ilev = 2, nlev
      ...
      !   call t_stopf("hpe_vert_flux_operator_3")

      case(3)
      ! call t_startf("hpe_vert_flux_operator_4")
...
! full level for this face level
      ...
      !   call t_stopf("hpe_vert_flux_operator_4")
     case default
        print*, "you must select a vertical order, stop"
        stop
     end select
