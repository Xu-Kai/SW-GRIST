...
subroutine getfil(fulpath, locfn, iflag, lexist)
   ...
   if ((klen-i) > maxlen) then
      if (abort_on_failure) then
         ...
         if (mpi_rank()==0) print*, '(GETFIL): local filename variable is too short for path length',klen-i,maxlen
   ...
   if (len_trim(locfn) == 0) then
      ...
      print*, '(GETFIL): attempting to find local file ', trim(locfn)
   end if
 
   inquire(file=locfn, exist=lexist_in)
   if (present(lexist)) lexist = lexist_in
   if (lexist_in) then
      if (mpi_rank()==0) print*, '(GETFIL): using ',trim(locfn), ' in current working directory'
      return
   end if
   ...
   if (klen > maxlen) then
      if (abort_on_failure) then
         ...
         if (mpi_rank()==0) print*, '(GETFIL): local filename variable is too short for path length',klen,maxlen
         if (present(lexist)) lexist = .false.
         return
      end if
   end if
   ...
   if (present(lexist)) lexist = lexist_in
   if (lexist_in) then
      if (mpi_rank()==0) print*, '(GETFIL): using ',trim(fulpath)
      return
   else
      if (mpi_rank()==0) print*, '(GETFIL): all tries to get file have been unsuccessful: ',trim(fulpath)
      if (abort_on_failure) then
...
! --------------------------------------------------------------------
   ...
   if (ioe /= 0) then
      if(mpi_rank()==0) print*, '(OPNFIL): failed to open file ',trim(locfn), ' on unit ',iun,' ierr=',ioe
      call endrun ('opnfil') 
   else
      if(mpi_rank()==0) print*, '(OPNFIL): Successfully opened file ',trim(locfn), ' on unit= ',iun
   end if
