...
module grist_wrf_radiation_driver
  ...
  use grist_wrfphys_nml_module,only: wrfphys_cf_scheme, wrfphys_rasw_scheme, wrfphys_ralw_scheme, step_ra, wphys_has_req, unuse_ra
...
!----------------------------------------------------------------------
   ...
   case('RRTMGDNN')
        call rrtmg_dnn_lw_init(ptend_wrf%rthraten(   1:ncell, 1:nLevel, 1:1),   &
                    ptend_wrf%rthraten_lw(1:ncell, 1:nLevel, 1:1),   &
                    pstate_wrf%cldfra(    1:ncell, 1:nLevel, 1:1),   &
                    restart,                 &
                    1, ncell, 1, 1, 1, nLevel, &
...
!----------------------------------------------------------------------
      ...
      case('RRTMGV341','RRTMGV381')
       call rrtmg_swinit(.true. ,                    &
                          ...
                          1, ncell, 1, 1, 1, nLevel-1) 
case('RRTMGDNN')
        call rrtmg_dnn_sw_init(ptend_wrf%rthraten(   1:ncell, 1:nLevel, 1:1),   &
                            ptend_wrf%rthraten_lw(1:ncell, 1:nLevel, 1:1),   &
                            pstate_wrf%cldfra(    1:ncell, 1:nLevel, 1:1),   &
                            restart,                 &
                            1, ncell, 1, 1, 1, nLevel, &
...
!
   ...
   !Added switch for radiation scheme by wangym
   if ((.not.unuse_ra) .and. (itimestep.eq.0 .or. itimestep .eq. 1 .or.  mod(itimestep,stepra)  .eq. 0)) call_radiation = .true.
...
!ccc
    ...
    case('RRTMGDNN')
#ifdef RRTMG_DNN
      call rrtmg_dnn_lwrad(p3d        = pstate_wrf%p_phy( 1:ncell,1:nLevel, 1:1),   & ! in 
                           ...
                           emiss  = psurf_wrf%emiss( 1:ncell,         1:1) ,        & ! in
                           xcoszen = xcoszen(1:ncell, 1:1),                          & ! in
                           lat      = dycoreVarGeography%scalar_lat_at_pc%f(1:ncell),& ! in
                           coszr  = pstate_wrf%coszr  (1:ncell,          1:1),      & ! out
                           ids=1,ide=ncell, jds=1,jde=1, kds=1,kde=nLevel  ,    &
                           ims=1,ime=ncell, jms=1,jme=1, kms=1,kme=nLevel  ,    &
                           its=1,ite=ncell, jts=1,jte=1, kts=1,kte=nLevel-1,    &
                           glw        = psurf_wrf%glw(1:ncell,1:1),            &   ! out, 
                           gsw        = psurf_wrf%gsw(1:ncell,1:1),            &   ! out, 
                           swdnb      = pstate_wrf%swdnb(1:ncell,1:1))             ! out
#endif
    case default
      if(mpi_rank().eq.0) print*,"you must select a radiation LW scheme in WRFphys: wrfphys_ralw_scheme"
      call endrun()
...
! wym added for reduce ra tendence
    ptend_wrf%rthraten_lw(1:ncell,1:nLevel,1:1)=zero
    ptend_wrf%rthraten_sw(1:ncell,1:nLevel,1:1)=zero
! wym added for reduce ra tendency
