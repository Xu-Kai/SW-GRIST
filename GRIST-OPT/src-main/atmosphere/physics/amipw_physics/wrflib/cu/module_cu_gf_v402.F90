...
MODULE module_cu_gf_v402
!use module_gfs_physcons, g => con_g,                           &
!                         cp => con_cp,                         &
!                         xlv => con_hvap,                      &
!                         r_v => con_rv
use grist_constants, only: g=>gravity, cp=>cp, r_v=>rvap
use module_wrfmodel_constants, only: xlv
use module_cu_gf_deep_v402,    only: cup_gf,neg_check,autoconv,aeroevap
use module_cu_gf_sh_v402,      only: cup_gf_sh
...
!-------------------------------------------------------------
   SUBROUTINE GFDRV(spp_conv,pattern_spp_conv,field_conv,       &
              ...
              ,rqvblten,rthblten                                &
              ,dudt_phy,dvdt_phy, ndc, omega_hdc                &
#if ( WRF_DFI_RADAR == 1 )
...
!                                                                      
     ...
     DO I= its,itf
! Stochastic
        if (spp_conv==1.and.present(pattern_spp_conv)) then
        do n=1,4
...
!
        ...
        IF(PRESENT(k22_shallow)) THEN
         ...
         ENDIF
            DO I=ibegc,iendc
              cuten(i)=0.
              if(present(ktop_deep)) ktop_deep(i,j) = ktop(i)
              if(pret(i).gt.0.)then
                 cuten(i)=1.
              else
                 cuten(i)=0.
