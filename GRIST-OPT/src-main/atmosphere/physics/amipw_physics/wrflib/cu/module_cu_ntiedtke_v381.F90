!-----------------------------------------------------------------------
! This source module is taken from WRF-V3.8.1, (upbl, ztau)
!
...
module module_cu_ntiedtke_v381

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     use module_wrfmodel_constants, only:rd=>r_d, rv=>r_v, &
...
!   
!-------------LiXH Test--------------
      parameter(cprcon = 1.4e-3)    !default
!      parameter(cprcon = 1.0e-3)
!-------------LiXH Test--------------
!     -------
!
...
!-----------------------------------------------------------------------
     subroutine cu_ntiedtke(                                    &
                ...
                ndc, omega, cmfmc                              ) ! added by yiz for grist
!-------------------------------------------------------------------
      implicit none
!-------------------------------------------------------------------
...
!
     ...
     real,    dimension(ims:ime, kms:kme, jms:jme), intent(in) ::    omega
     real,    dimension(ims:ime, kms:kme, jms:jme), intent(out)::    cmfmc
...
!########################################################################
      call tiecnvn(u1,v1,t1,q1,q2,q3,q1b,t1b,ghtl,ghti,omg,prsl,prsi,evap,heatflux,  &
                  rn,slimsk,im,kx,kx1,delt,dx,npmf)
...
!-----------------------------------------------------------------
     ...
     &         pap,paph,evap,hfx,zprecc,lndj,lq,km,km1,dt,dx, npmf)
!-----------------------------------------------------------------
...
!
     ...
     &     zlu(lq,km),     zlude(lq,km), zmfu(lq,km),  zmfd(lq,km), npmf(lq,km),&
...
!-----------------------------------------------------------------
     ...
     &         pmfu(klon,klev),        pmfd(klon,klev), npmf(klon,klev), &
     &         prain(klon),&
...
! LiXH Test----------------->
!if(dx(jl).lt.30000.)then
! ztau = ztauc(jl) * (1.+log(3e4/dx(jl))**2)
! ztau = ztauc(jl) * 2.5
!end if
! LiXH Test<-----------------

#ifdef SCALE_TBCU
           if(dx(jl).lt.8000.)then
              ztau = ztauc(jl) * (1.+log(1e4/dx(jl))**2)
           end if
#endif
           ztau = max(ztmst,ztau)
           !ztau = max(720.,ztau)
...
end module module_cu_ntiedtke_v381
