...
#endif
    if( kind .ne. r8 ) then
        print*, 'wrong KIND of reals passed to init_diffusvity -- exiting.'
        call endrun('init_eddy_diff')
!#ifdef WRF_PORT
...
!        stop 'init_eddy_diff'
    ...
    if(vdiff_select(fieldlist_wet,'s').ne.'')   print*, 'error: ', vdiff_select(fieldlist_wet,'s')
!#ifdef WRF_PORT
!        call wrf_message(iulog)
!#endif 
    if(vdiff_select(fieldlist_wet,'q',1).ne.'') print*, 'error: ', vdiff_select(fieldlist_wet,'q',1)
!#ifdef WRF_PORT
!        call wrf_message(iulog)
!#endif 
    if(vdiff_select(fieldlist_wet,'u').ne.'')   print*, 'error: ', vdiff_select(fieldlist_wet,'u')
!#ifdef WRF_PORT
!        call wrf_message(iulog)
!#endif 
    if(vdiff_select(fieldlist_wet,'v').ne.'')   print*, 'error: ', vdiff_select(fieldlist_wet,'v')
!#ifdef WRF_PORT
!        call wrf_message(iulog)
!#endif 
...
!    call addfld('UW_leng',          'm/s',     pver+1, 'A',  'Turbulence length scale, I',                            phys_decomp )
  ...
  subroutine compute_eddy_diff( lchnk  ,                                                            &
                                ...
                                ipbl   , kpblh  , wstarPBL , turbtype, sm_aw    , lengi )
    ...
    !                       May.    2008.                                 !
    !-------------------------------------------------------------------- !
    use module_cam_diffusion_solver, only: compute_vdiff
#ifndef WRF_PORT 
    use cam_history,      only: outfld, addfld, phys_decomp
...
!    use module_cam_support,   only: outfld, addfld, phys_decomp 
  ...
  ! type(physics_state)     :: state                     ! Physics state variables
    ...
    real(r8), intent(out)   :: lengi(pcols,pver+1)       ! Turbulence length scale at all interfaces [ m ]
          ...
          do k = 1, pver
             do i = 1, ncol
              ...
              !                I should understand why the variation of ice, qi is neglected during diffusion.
                ...
                call qsat( templ, pmid(i,k), es(1), qs(1), gam(1))
                ep2       =  .622_r8 
                temps     =   templ + ( qtfd(i,k) - qs(1) ) / ( cpair / latvap + latvap * qs(1) / ( rair * templ**2 ) )
                call qsat( temps, pmid(i,k), es(1), qs(1), gam(1))
...
!    call outfld( 'UW_leng',        lengi,      pcols,   lchnk )
  ...
  subroutine trbintd( pcols   , pver    , ncol    ,                               &
    ...
    do k = ntop_turb, nbot_turb
       call qsat( t(1,k), pmid(1,k), es(1,k), qs(1,k), gam(1,k))
       do i = 1, ncol
...
!#endif 
                             do ks = 1, ncvmax
                                print*, 'ncv =', ks, ' ', kbase_o(i,ks), ktop_o(i,ks), kbase_mg(i,ks), ktop_mg(i,ks)
!#ifdef WRF_PORT
!                                call wrf_message(iulog)
!#endif 
                             end do
       ...
       do k = 2, pver
          ...
          if( belongst(i,k) ) then
              ...
              if( det .lt. 0._r8 ) then
                  print*, 'The det < 0. for the STL in UW eddy_diff'             
                  call endrun()
!#ifdef WRF_PORT
!                  call wrf_message(iulog)
!#endif                   
     ...
     ! do while ( -dl2n2 .gt. (-rinc*dzinc*l2n2/(lint+(1._r8-rinc)*dzinc)) )  ! Merging test based on l2n2 
       ...
       do while ( -dl2n2 .gt. (-rinc*l2n2/(1._r8-rinc)) .and. kt-1 .gt. ntop_turb )                     ! Integral merging test
          ...
          if( kt .eq. ntop_turb ) then
              print*, 'zisocl: Error: Tried to extend CL to the model top'
              call endrun()
!#ifdef WRF_PORT
!              call wrf_message(iulog)
!#endif 
       ...
       if( kb .ne. pver + 1 ) then
         ...
         !             .and.(kb.ne.pver+1))
          ...
          if( (kb.eq.pver+1) .and. (ncv.ne.1) ) then 
               print*, 'Major mistake zisocl: the CL based at surface is not indexed 1'
               call endrun()
!#ifdef WRF_PORT
!               call wrf_message(iulog)
!#endif 
...
END MODULE module_cam_eddy_diff
