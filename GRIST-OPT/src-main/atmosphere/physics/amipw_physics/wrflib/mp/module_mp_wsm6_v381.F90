...
MODULE module_mp_wsm6_v381
!
#ifndef AMIPW_PHYSICS
   USE module_utility, ONLY: WRFU_Clock, WRFU_Alarm
   USE module_domain, ONLY : HISTORY_ALARM, Is_alarm_tstep
#endif
   USE module_mp_radar_v381
   USE grist_mpi
!
  ...
  REAL, DIMENSION(ims:ime, kms:kme, jms:jme),                     &
        INTENT(INOUT)::                                           &
                                                        re_cloud, &
                                                         ...
                                                         re_snow
!+---+-----------------------------------------------------------------+
  REAL, DIMENSION(ims:ime, kms:kme, jms:jme), OPTIONAL, INTENT(INOUT)::     &  ! GT
                                                       refl_10cm
!+---+-----------------------------------------------------------------+
...
! to calculate effective radius for radiation
  ...
  REAL, DIMENSION( kts:kte ) :: re_qc, re_qi, re_qs
      ...
      DO j=jts,jte
        !$omp target parallel do private(i,k) collapse(2)
         DO k=kts,kte
         DO i=its,ite
...
!+---+-----------------------------------------------------------------+
         ...
         if (diagflag .and. do_radar_ref == 1) then
            !$omp target parallel do private(i,k) private(t1d, p1d, qv1d,qr1d,qs1d,qg1d)
            DO I=its,ite
               DO K=kts,kte
        ...
        if (has_reqc.ne.0 .and. has_reqi.ne.0 .and. has_reqs.ne.0) then
          !$omp target parallel do private(i,k) private(re_qc,re_qi,re_qs,t1d,qc1d,qi1d,qs1d,den1d)
          do i=its,ite
            do k=kts,kte
              re_qc(k) = 2.51E-6
              re_qi(k) = 10.01E-6
...
! #define CHK_WSM62D
#if defined(CHK_WSM62D) || defined(CHK_ALL)
      !inout
      REAL, dimension(:,:),  allocatable     :: ref_t
      REAL, dimension(:,:,:),  allocatable  :: ref_qci
      REAL, dimension(:,:,:),  allocatable  :: ref_qrs
      REAL, dimension(:,:),  allocatable     :: ref_q
      REAL, dimension(:),  allocatable               :: ref_rain, ref_rainncv, ref_sr
      !optional inout
      REAL, dimension(:,:), allocatable      :: ref_snow, ref_snowncv
      REAL, dimension(:,:), allocatable      :: ref_graupel, ref_graupelncv
#include "data_check.inc"


      allocate(ref_t, source=t)
      allocate(ref_qci, source=qci)
      allocate(ref_qrs, source=qrs)
      allocate(ref_q, source=q)
      allocate(ref_rain, source=rain)
      allocate(ref_rainncv, source=rainncv)
      allocate(ref_sr, source=sr)
      if (present(snow)) allocate(ref_snow, source=snow)
      if (present(snowncv)) allocate(ref_snowncv, source=snowncv)
      if (present(graupel)) allocate(ref_graupel, source=graupel)
      if (present(graupelncv)) allocate(ref_graupelncv, source=graupelncv)
#endif
!
!
      idim = ite-its+1
...
!
!$omp target parallel do private(k,i) collapse(2) 
      do k = kts, kte
        do i = its, ite
...
!!!$omp end do nowait
!$omp end target parallel do
!$omp target parallel do private(k,i) 
!!!$omp do  
      do k = kts, kte
        do i = its, ite
...
!
!$omp target parallel do private(i)
      do i = its, ite
        rainncv(i) = 0.
...
!       enddo
!     enddo
!$omp target parallel do private(k,i,tvec1) device(1)
!!!$omp do  
      do k = kts, kte
...
!         qs(i,k,2) = fpvs(t(i,k),1,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
      ...
      xai=-dldti/rv
      xbi=xai+hsub/(rv*ttp)
!$omp target parallel do private(k,i,tr) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
          tr=ttp/t(i,k)
...
!-------------------------------------------------------------
!$omp target parallel do private(k,i,temp) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
...
!$omp end target parallel do
      call slope_wsm6(qrs_tmp,den_tmp,denfac,t,rslope,rslopeb,rslope2,rslope3, &
                     work1,its,ite,kts,kte)
!
!$omp target parallel do private(k,i,supcol,coeres) collapse(2)
!!!$omp do  
      do k = kte, kts, -1 
        do i = its, ite
          supcol = t0c-t(i,k)
...
!---------------------------------------------------------------
!$omp target parallel do private(k,i,xmi,diameter) collapse(2)
!!!$omp do  
      do k = kte, kts, -1
        do i = its, ite
...
!
!$omp target parallel do private(i,fallsum,fallsum_qsi,fallsum_qg)
!!!$omp do  
      do i = its, ite
...
!       (T>T0: I->C)
!---------------------------------------------------------------
!$omp target parallel do private(i,k,supcol,xlf,supcolt,pfrzdtc,temp,pfrzdtr) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
          supcol = t0c-t(i,k)
...
!===============================================================
!
!$omp target parallel do private(i,k,supsat,satdt,coeres) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
...
!===============================================================
!
!$omp target parallel do private(i,k,supcol,supsat,satdt,ifsat,temp,eacrs,xmi,diameter,vt2i,vt2r,vt2s,vt2g,vt2ave,acrfac,egi,xlf,supice,xni0,roqi0,qimax,alpha2,coeres) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
          supcol = t0c-t(i,k)
...
!     large scale
!
!$omp target parallel do private(i,k,delta2,delta3,value,source,factor,xlf,xlwork2) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
!
...
!         qs(i,k,2) = fpvs(t(i,k),1,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
      ...
      xai=-dldti/rv
      xbi=xai+hsub/(rv*ttp)
!$omp target parallel do private(i,k,tr) collapse(2)
!!!$omp do  
      do k = kts, kte
        do i = its, ite
          tr=ttp/t(i,k)
...
do k = kts, kte
  do i = its, ite
    ref_qci(i,k,1) = max(ref_qci(i,k,1),0.0)
    ref_qrs(i,k,1) = max(ref_qrs(i,k,1),0.0)
    ref_qci(i,k,2) = max(ref_qci(i,k,2),0.0)
    ref_qrs(i,k,2) = max(ref_qrs(i,k,2),0.0)
    ref_qrs(i,k,3) = max(ref_qrs(i,k,3),0.0)
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
...
do k = kts, kte
  do i = its, ite
    cpm(i,k) = cpmcal(ref_q(i,k))
    xl(i,k) = xlcal(ref_t(i,k))
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
...
!
!----------------------------------------------------------------
!    initialize the surface ref_rain, ref_snow, ref_graupel
!
do i = its, ite
  ref_rainncv(i) = 0.
  if(present(snowncv) .AND. present(snow)) ref_snowncv(i,lat) = 0.
  if(present(graupelncv) .AND. present(graupel)) ref_graupelncv(i,lat) = 0.
  ref_sr(i) = 0.
! new local array to catch step ref_snow and ref_graupel
  tstepsnow(i) = 0.
  tstepgraup(i) = 0.
enddo
!
...
!       enddo
!     enddo
!!!$omp parallel  private(k,i,tvec1) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  CALL VREC( tvec1(its), den(its,k), ite-its+1)
  do i = its, ite
...
!
! Inline expansion for fpvs
!         qs(i,k,1) = fpvs(ref_t(i,k),0,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
!         qs(i,k,2) = fpvs(ref_t(i,k),1,rd,rv,cpv,cliq,cice,xlv0,xls,psat,t0c)
hsub = xls
hvap = xlv0
cvap = cpv
ttp=t0c+0.01
...
do k = kts, kte
  do i = its, ite
    ...
    qs(i,k,1) = ep2 * qs(i,k,1) / (p(i,k) - qs(i,k,1))
    qs(i,k,1) = max(qs(i,k,1),qmin)
    rh(i,k,1) = max(ref_q(i,k) / qs(i,k,1),qmin)
    tr=ttp/ref_t(i,k)
    if(ref_t(i,k).lt.ttp) then
      qs(i,k,2)=psat*exp(log(tr)*(xai))*exp(xbi*(1.-tr))
    else
      qs(i,k,2)=psat*exp(log(tr)*(xa))*exp(xb*(1.-tr))
    endif
    ...
    qs(i,k,2) = ep2 * qs(i,k,2) / (p(i,k) - qs(i,k,2))
    qs(i,k,2) = max(qs(i,k,2),qmin)
    rh(i,k,2) = max(ref_q(i,k) / qs(i,k,2),qmin)
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
...
! Ni: ice crystal number concentraiton   [HDC 5c]
!-------------------------------------------------------------
!!!$omp parallel  private(k,i,temp) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
    temp = (den(i,k)*max(ref_qci(i,k,2),qmin))
    temp = sqrt(sqrt(temp*temp*temp))
    xni(i,k) = min(max(5.38e7*temp,1.e3),1.e6)
  enddo
enddo
...
do k = kts, kte
  do i = its, ite
    qrs_tmp(i,k,1) = ref_qrs(i,k,1)
    qrs_tmp(i,k,2) = ref_qrs(i,k,2)
    qrs_tmp(i,k,3) = ref_qrs(i,k,3)
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
call slope_wsm6(qrs_tmp,den_tmp,denfac,ref_t,rslope,rslopeb,rslope2,rslope3, & 
               work1,its,ite,kts,kte)
!
!!!$omp parallel  private(k,i) 
!!!$omp do schedule(static,5) 
do k = kte, kts, -1
  do i = its, ite
    workr(i,k) = work1(i,k,1)
    qsum(i,k) = max( (ref_qrs(i,k,2)+ref_qrs(i,k,3)), 1.E-15)
    IF ( qsum(i,k) .gt. 1.e-15 ) THEN
      worka(i,k) = (work1(i,k,2)*ref_qrs(i,k,2) + work1(i,k,3)*ref_qrs(i,k,3)) &
                /qsum(i,k)
    ELSE
      worka(i,k) = 0.
    ENDIF
    denqrs1(i,k) = den(i,k)*ref_qrs(i,k,1)
    denqrs2(i,k) = den(i,k)*ref_qrs(i,k,2)
    denqrs3(i,k) = den(i,k)*ref_qrs(i,k,3)
    if(ref_qrs(i,k,1).le.0.0) workr(i,k) = 0.0
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
call nislfv_rain_plm(idim,kdim,den_tmp,denfac,ref_t,delz_tmp,workr,denqrs1,  &
                     delqrs1,dtcld,1,1)
call nislfv_rain_plm6(idim,kdim,den_tmp,denfac,ref_t,delz_tmp,worka,         & 
                     denqrs2,denqrs3,delqrs2,delqrs3,dtcld,1,1)
!!!$omp parallel  private(k,i) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
    ref_qrs(i,k,1) = max(denqrs1(i,k)/den(i,k),0.)
    ref_qrs(i,k,2) = max(denqrs2(i,k)/den(i,k),0.)
    ref_qrs(i,k,3) = max(denqrs3(i,k)/den(i,k),0.)
    fall(i,k,1) = denqrs1(i,k)*workr(i,k)/delz(i,k)
    fall(i,k,2) = denqrs2(i,k)*worka(i,k)/delz(i,k)
    fall(i,k,3) = denqrs3(i,k)*worka(i,k)/delz(i,k)
  enddo
...
call slope_wsm6(qrs_tmp,den_tmp,denfac,ref_t,rslope,rslopeb,rslope2,rslope3, &
               work1,its,ite,kts,kte)
!
!!!$omp parallel  private(k,i,supcol,coeres) 
!!!$omp do schedule(static,5) 
do k = kte, kts, -1 
  do i = its, ite
    supcol = t0c-ref_t(i,k)
    n0sfac(i,k) = max(min(exp(alpha*supcol),n0smax/n0s),1.)
    if(ref_t(i,k).gt.t0c) then
!---------------------------------------------------------------
! psmlt: melting of ref_snow [HL A33] [RH83 A25]
!       (ref_t>T0: S->R)
!---------------------------------------------------------------
      xlf = xlf0
      work2(i,k) = venfac(p(i,k),ref_t(i,k),den(i,k))
      if(ref_qrs(i,k,2).gt.0.) then
        coeres = rslope2(i,k,2)*sqrt(rslope(i,k,2)*rslopeb(i,k,2))
        psmlt(i,k) = xka(ref_t(i,k),den(i,k))/xlf*(t0c-ref_t(i,k))*pi/2.       &
                   *n0sfac(i,k)*(precs1*rslope2(i,k,2)                 &
                   +precs2*work2(i,k)*coeres)/den(i,k)
        psmlt(i,k) = min(max(psmlt(i,k)*dtcld/mstep(i),                &
                    -ref_qrs(i,k,2)/mstep(i)),0.)
        ref_qrs(i,k,2) = ref_qrs(i,k,2) + psmlt(i,k)
        ref_qrs(i,k,1) = ref_qrs(i,k,1) - psmlt(i,k)
        ref_t(i,k) = ref_t(i,k) + xlf/cpm(i,k)*psmlt(i,k)
      endif
!---------------------------------------------------------------
! pgmlt: melting of ref_graupel [HL A23]  [LFO 47]
!       (ref_t>T0: G->R)
!---------------------------------------------------------------
      if(ref_qrs(i,k,3).gt.0.) then
        coeres = rslope2(i,k,3)*sqrt(rslope(i,k,3)*rslopeb(i,k,3))
        pgmlt(i,k) = xka(ref_t(i,k),den(i,k))/xlf                          &
                   *(t0c-ref_t(i,k))*(precg1*rslope2(i,k,3)                &
                   +precg2*work2(i,k)*coeres)/den(i,k)
        pgmlt(i,k) = min(max(pgmlt(i,k)*dtcld/mstep(i),                &
                    -ref_qrs(i,k,3)/mstep(i)),0.)                          
        ref_qrs(i,k,3) = ref_qrs(i,k,3) + pgmlt(i,k)
        ref_qrs(i,k,1) = ref_qrs(i,k,1) - pgmlt(i,k)
        ref_t(i,k) = ref_t(i,k) + xlf/cpm(i,k)*pgmlt(i,k)
      endif
    endif
  enddo
enddo
...
! Vice [ms-1] : fallout of ice crystal [HDC 5a]
!---------------------------------------------------------------
!!!$omp parallel  private(k,i,xmi,diameter) 
!!!$omp do schedule(static,5) 
do k = kte, kts, -1
  do i = its, ite
    if(ref_qci(i,k,2).le.0.) then
      work1c(i,k) = 0.
    else
      xmi = den(i,k)*ref_qci(i,k,2)/xni(i,k)
      diameter  = max(min(dicon * sqrt(xmi),dimax), 1.e-25)
      work1c(i,k) = 1.49e4*exp(log(diameter)*(1.31))
    endif
  enddo
...
do k = kte, kts, -1
  do i = its, ite
    denqci(i,k) = den(i,k)*ref_qci(i,k,2)
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
call nislfv_rain_plm(idim,kdim,den_tmp,denfac,ref_t,delz_tmp,work1c,denqci,  &
                     delqi,dtcld,1,0)
!!!$omp parallel  private(k,i) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
    ref_qci(i,k,2) = max(denqci(i,k)/den(i,k),0.)
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
...
!
!----------------------------------------------------------------
!      ref_rain (unit is mm/sec;kgm-2s-1: /1000*delt ===> m)==> mm for wrf
!
!!!$omp parallel  private(i,fallsum,fallsum_qsi,fallsum_qg) 
!!!$omp do schedule(static,5) 
do i = its, ite
  fallsum = fall(i,kts,1)+fall(i,kts,2)+fall(i,kts,3)+fallc(i,kts)
  fallsum_qsi = fall(i,kts,2)+fallc(i,kts)
  fallsum_qg = fall(i,kts,3)
  if(fallsum.gt.0.) then
    ref_rainncv(i) = fallsum*delz(i,kts)/denr*dtcld*1000. + ref_rainncv(i)
    ref_rain(i) = fallsum*delz(i,kts)/denr*dtcld*1000. + ref_rain(i)
  endif
  if(fallsum_qsi.gt.0.) then
    tstepsnow(i)   = fallsum_qsi*delz(i,kts)/denr*dtcld*1000.            &
                     +tstepsnow(i)
  IF ( present(snowncv) .AND. present(snow)) THEN
    ref_snowncv(i,lat) = fallsum_qsi*delz(i,kts)/denr*dtcld*1000.            & 
                     +ref_snowncv(i,lat)
    ref_snow(i,lat) = fallsum_qsi*delz(i,kts)/denr*dtcld*1000. + ref_snow(i,lat)
  ENDIF
  endif
  if(fallsum_qg.gt.0.) then
    tstepgraup(i)  = fallsum_qg*delz(i,kts)/denr*dtcld*1000.            &
                     +tstepgraup(i)
  IF ( present(graupelncv) .AND. present(graupel)) THEN
    ref_graupelncv(i,lat) = fallsum_qg*delz(i,kts)/denr*dtcld*1000.          &   
                        + ref_graupelncv(i,lat)
    ref_graupel(i,lat) = fallsum_qg*delz(i,kts)/denr*dtcld*1000. + ref_graupel(i,lat)
  ENDIF
  endif
  IF ( present(snowncv)) THEN
    if(fallsum.gt.0.)ref_sr(i)=(ref_snowncv(i,lat) + ref_graupelncv(i,lat))/(ref_rainncv(i)+1.e-12)
  ELSE
    if(fallsum.gt.0.)ref_sr(i)=(tstepsnow(i) + tstepgraup(i))/(ref_rainncv(i)+1.e-12)
  ENDIF
enddo
!!!$omp end do nowait
!!!$omp end parallel 
...
!       (ref_t>T0: I->C)
!---------------------------------------------------------------
!!!$omp parallel  private(i,k,supcol,xlf,supcolt,pfrzdtc,temp,pfrzdtr) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
    supcol = t0c-ref_t(i,k)
    xlf = xls-xl(i,k)
    if(supcol.lt.0.) xlf = xlf0
    if(supcol.lt.0.and.ref_qci(i,k,2).gt.0.) then
      ref_qci(i,k,1) = ref_qci(i,k,1) + ref_qci(i,k,2)
      ref_t(i,k) = ref_t(i,k) - xlf/cpm(i,k)*ref_qci(i,k,2)
      ref_qci(i,k,2) = 0.
    endif
!---------------------------------------------------------------
! pihmf: homogeneous freezing of cloud water below -40c [HL A45]
!        (ref_t<-40C: C->I)
!---------------------------------------------------------------
    if(supcol.gt.40..and.ref_qci(i,k,1).gt.0.) then
      ref_qci(i,k,2) = ref_qci(i,k,2) + ref_qci(i,k,1)
      ref_t(i,k) = ref_t(i,k) + xlf/cpm(i,k)*ref_qci(i,k,1)
      ref_qci(i,k,1) = 0.
    endif
!---------------------------------------------------------------
! pihtf: heterogeneous freezing of cloud water [HL A44]
!        (T0>ref_t>-40C: C->I)
!---------------------------------------------------------------
    if(supcol.gt.0..and.ref_qci(i,k,1).gt.qmin) then
!           pfrzdtc = min(pfrz1*(exp(pfrz2*supcol)-1.)                         &
!              *den(i,k)/denr/xncr*ref_qci(i,k,1)**2*dtcld,ref_qci(i,k,1))
      supcolt=min(supcol,50.)
      pfrzdtc = min(pfrz1*(exp(pfrz2*supcolt)-1.)                        &
      *den(i,k)/denr/xncr*ref_qci(i,k,1)*ref_qci(i,k,1)*dtcld,ref_qci(i,k,1))
      ref_qci(i,k,2) = ref_qci(i,k,2) + pfrzdtc
      ref_t(i,k) = ref_t(i,k) + xlf/cpm(i,k)*pfrzdtc
      ref_qci(i,k,1) = ref_qci(i,k,1)-pfrzdtc
    endif
!---------------------------------------------------------------
! pgfrz: freezing of ref_rain water [HL A20] [LFO 45]
!        (ref_t<T0, R->G)
!---------------------------------------------------------------
    if(supcol.gt.0..and.ref_qrs(i,k,1).gt.0.) then
!           pfrzdtr = min(20.*pi**2*pfrz1*n0r*denr/den(i,k)                    &
!                 *(exp(pfrz2*supcol)-1.)*rslope3(i,k,1)**2                    &
!                 *rslope(i,k,1)*dtcld,ref_qrs(i,k,1))
      temp = rslope3(i,k,1)
      temp = temp*temp*rslope(i,k,1)
      supcolt=min(supcol,50.)
      pfrzdtr = min(20.*(pi*pi)*pfrz1*n0r*denr/den(i,k)                  &
            *(exp(pfrz2*supcolt)-1.)*temp*dtcld,                         &
            ref_qrs(i,k,1))
      ref_qrs(i,k,3) = ref_qrs(i,k,3) + pfrzdtr
      ref_t(i,k) = ref_t(i,k) + xlf/cpm(i,k)*pfrzdtr
      ref_qrs(i,k,1) = ref_qrs(i,k,1)-pfrzdtr
    endif
  enddo
enddo
!!!$omp end do nowait
...
do k = kts, kte
  do i = its, ite
    work1(i,k,1) = diffac(xl(i,k),p(i,k),ref_t(i,k),den(i,k),qs(i,k,1))
    work1(i,k,2) = diffac(xls,p(i,k),ref_t(i,k),den(i,k),qs(i,k,2))
    work2(i,k) = venfac(p(i,k),ref_t(i,k),den(i,k))
  enddo
enddo
!!!$omp end do nowait
...
!===============================================================
!
! warm ref_rain processes
!
! - follows the processes in RH83 and LFO except for autoconcersion
!
!===============================================================
!
!!!$omp parallel  private(i,k,supsat,satdt,coeres) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
    supsat = max(ref_q(i,k),qmin)-qs(i,k,1)
    satdt = supsat/dtcld
!---------------------------------------------------------------
! praut: auto conversion rate from cloud to ref_rain [HDC 16]
!        (C->R)
!---------------------------------------------------------------
    if(ref_qci(i,k,1).gt.qc0) then
      praut(i,k) = qck1*ref_qci(i,k,1)**(7./3.)
      praut(i,k) = min(praut(i,k),ref_qci(i,k,1)/dtcld)
    endif
!---------------------------------------------------------------
! pracw: accretion of cloud water by ref_rain [HL A40] [LFO 51]
!        (C->R)
!---------------------------------------------------------------
    if(ref_qrs(i,k,1).gt.qcrmin.and.ref_qci(i,k,1).gt.qmin) then
      pracw(i,k) = min(pacrr*rslope3(i,k,1)*rslopeb(i,k,1)               &
                  *ref_qci(i,k,1)*denfac(i,k),ref_qci(i,k,1)/dtcld)
    endif
!---------------------------------------------------------------
! prevp: evaporation/condensation rate of ref_rain [HDC 14]
!        (V->R or R->V)
!---------------------------------------------------------------
    if(ref_qrs(i,k,1).gt.0.) then
      coeres = rslope2(i,k,1)*sqrt(rslope(i,k,1)*rslopeb(i,k,1))
      prevp(i,k) = (rh(i,k,1)-1.)*(precr1*rslope2(i,k,1)                 &
                   +precr2*work2(i,k)*coeres)/work1(i,k,1)
      if(prevp(i,k).lt.0.) then
        prevp(i,k) = max(prevp(i,k),-ref_qrs(i,k,1)/dtcld)
        prevp(i,k) = max(prevp(i,k),satdt/2)
      else
        prevp(i,k) = min(prevp(i,k),satdt/2)
      endif
...
!===============================================================
!
! cold ref_rain processes
!
! - follows the revised ice microphysics processes in HDC
! - the processes same as in RH83 and RH84  and LFO behave
!   following ice crystal hapits defined in HDC, inclduing
!   intercept parameter for ref_snow (n0s), ice crystal number
!   concentration (ni), ice nuclei number concentration
!   (n0i), ice diameter (d)
!
!===============================================================
!
!!!$omp parallel private(i,k,supcol,supsat,satdt,ifsat,temp,eacrs,xmi,diameter,vt2i,vt2r,vt2s,vt2g,vt2ave,acrfac,egi,xlf,supice,xni0,roqi0,qimax,alpha2,coeres) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
    supcol = t0c-ref_t(i,k)
    n0sfac(i,k) = max(min(exp(alpha*supcol),n0smax/n0s),1.)
    supsat = max(ref_q(i,k),qmin)-qs(i,k,2)
    satdt = supsat/dtcld
    ifsat = 0
!-------------------------------------------------------------
! Ni: ice crystal number concentraiton   [HDC 5c]
!-------------------------------------------------------------
!         xni(i,k) = min(max(5.38e7*(den(i,k)                                  &
!                      *max(ref_qci(i,k,2),qmin))**0.75,1.e3),1.e6)
    temp = (den(i,k)*max(ref_qci(i,k,2),qmin))
    temp = sqrt(sqrt(temp*temp*temp))
    xni(i,k) = min(max(5.38e7*temp,1.e3),1.e6)
    eacrs = exp(0.07*(-supcol))
!
    ...
    qsum(i,k) = max( (ref_qrs(i,k,2)+ref_qrs(i,k,3)), 1.E-15)
    if(qsum(i,k) .gt. 1.e-15) then
    vt2ave=(vt2s*ref_qrs(i,k,2)+vt2g*ref_qrs(i,k,3))/(qsum(i,k))
    else
    vt2ave=0.
    endif
    if(supcol.gt.0.and.ref_qci(i,k,2).gt.qmin) then
      if(ref_qrs(i,k,1).gt.qcrmin) then
!-------------------------------------------------------------
! praci: Accretion of cloud ice by ref_rain [HL A15] [LFO 25]
!        (ref_t<T0: I->R)
!-------------------------------------------------------------
        acrfac = 2.*rslope3(i,k,1)+2.*diameter*rslope2(i,k,1)            &
                +diameter**2*rslope(i,k,1)
        praci(i,k) = pi*ref_qci(i,k,2)*n0r*abs(vt2r-vt2i)*acrfac/4.
        praci(i,k) = min(praci(i,k),ref_qci(i,k,2)/dtcld)
!-------------------------------------------------------------
! piacr: Accretion of ref_rain by cloud ice [HL A19] [LFO 26]
!        (ref_t<T0: R->S or R->G)
!-------------------------------------------------------------
        piacr(i,k) = pi**2*avtr*n0r*denr*xni(i,k)*denfac(i,k)            &
                    *g6pbr*rslope3(i,k,1)*rslope3(i,k,1)                 &
                    *rslopeb(i,k,1)/24./den(i,k)
        piacr(i,k) = min(piacr(i,k),ref_qrs(i,k,1)/dtcld)
      endif
!-------------------------------------------------------------
! psaci: Accretion of cloud ice by ref_snow [HDC 10]
!        (ref_t<T0: I->S)
!-------------------------------------------------------------
      if(ref_qrs(i,k,2).gt.qcrmin) then
        acrfac = 2.*rslope3(i,k,2)+2.*diameter*rslope2(i,k,2)            &
                +diameter**2*rslope(i,k,2)
        psaci(i,k) = pi*ref_qci(i,k,2)*eacrs*n0s*n0sfac(i,k)                 &
                    *abs(vt2ave-vt2i)*acrfac/4.
        psaci(i,k) = min(psaci(i,k),ref_qci(i,k,2)/dtcld)
      endif
!-------------------------------------------------------------
! pgaci: Accretion of cloud ice by ref_graupel [HL A17] [LFO 41]
!        (ref_t<T0: I->G)
!-------------------------------------------------------------
      if(ref_qrs(i,k,3).gt.qcrmin) then
        egi = exp(0.07*(-supcol))
        acrfac = 2.*rslope3(i,k,3)+2.*diameter*rslope2(i,k,3)            &
                +diameter**2*rslope(i,k,3)
        pgaci(i,k) = pi*egi*ref_qci(i,k,2)*n0g*abs(vt2ave-vt2i)*acrfac/4.
        pgaci(i,k) = min(pgaci(i,k),ref_qci(i,k,2)/dtcld)
      endif
    endif
!-------------------------------------------------------------
! psacw: Accretion of cloud water by ref_snow  [HL A7] [LFO 24]
!        (ref_t<T0: C->S, and ref_t>=T0: C->R)
!-------------------------------------------------------------
    if(ref_qrs(i,k,2).gt.qcrmin.and.ref_qci(i,k,1).gt.qmin) then
      psacw(i,k) = min(pacrc*n0sfac(i,k)*rslope3(i,k,2)*rslopeb(i,k,2)   &    
                  *ref_qci(i,k,1)*denfac(i,k),ref_qci(i,k,1)/dtcld)
    endif
!-------------------------------------------------------------
! pgacw: Accretion of cloud water by ref_graupel [HL A6] [LFO 40]
!        (ref_t<T0: C->G, and ref_t>=T0: C->R)
!-------------------------------------------------------------
    if(ref_qrs(i,k,3).gt.qcrmin.and.ref_qci(i,k,1).gt.qmin) then
      pgacw(i,k) = min(pacrg*rslope3(i,k,3)*rslopeb(i,k,3)               &
                  *ref_qci(i,k,1)*denfac(i,k),ref_qci(i,k,1)/dtcld)
    endif
!-------------------------------------------------------------
! paacw: Accretion of cloud water by averaged ref_snow/ref_graupel 
!        (ref_t<T0: C->G or S, and ref_t>=T0: C->R) 
!-------------------------------------------------------------
    if(qsum(i,k) .gt. 1.e-15) then
      paacw(i,k) = (ref_qrs(i,k,2)*psacw(i,k)+ref_qrs(i,k,3)*pgacw(i,k))         & 
                  /(qsum(i,k))
    endif      
!-------------------------------------------------------------
! pracs: Accretion of ref_snow by ref_rain [HL A11] [LFO 27]
!         (ref_t<T0: S->G)
!-------------------------------------------------------------
    if(ref_qrs(i,k,2).gt.qcrmin.and.ref_qrs(i,k,1).gt.qcrmin) then
      if(supcol.gt.0) then
        acrfac = 5.*rslope3(i,k,2)*rslope3(i,k,2)*rslope(i,k,1)          &
                +2.*rslope3(i,k,2)*rslope2(i,k,2)*rslope2(i,k,1)         &
                +.5*rslope2(i,k,2)*rslope2(i,k,2)*rslope3(i,k,1)
        pracs(i,k) = pi**2*n0r*n0s*n0sfac(i,k)*abs(vt2r-vt2ave)          &
                    *(dens/den(i,k))*acrfac
        pracs(i,k) = min(pracs(i,k),ref_qrs(i,k,2)/dtcld)
      endif
!-------------------------------------------------------------
! psacr: Accretion of ref_rain by ref_snow [HL A10] [LFO 28]
!         (ref_t<T0:R->S or R->G) (ref_t>=T0: enhance melting of ref_snow)
!-------------------------------------------------------------
      acrfac = 5.*rslope3(i,k,1)*rslope3(i,k,1)*rslope(i,k,2)            &
              +2.*rslope3(i,k,1)*rslope2(i,k,1)*rslope2(i,k,2)           &
              +.5*rslope2(i,k,1)*rslope2(i,k,1)*rslope3(i,k,2)
      psacr(i,k) = pi**2*n0r*n0s*n0sfac(i,k)*abs(vt2ave-vt2r)            &
                  *(denr/den(i,k))*acrfac
      psacr(i,k) = min(psacr(i,k),ref_qrs(i,k,1)/dtcld)
    endif
!-------------------------------------------------------------
! pgacr: Accretion of ref_rain by ref_graupel [HL A12] [LFO 42]
!         (ref_t<T0: R->G) (ref_t>=T0: enhance melting of ref_graupel)
!-------------------------------------------------------------
    if(ref_qrs(i,k,3).gt.qcrmin.and.ref_qrs(i,k,1).gt.qcrmin) then
      acrfac = 5.*rslope3(i,k,1)*rslope3(i,k,1)*rslope(i,k,3)            &
              +2.*rslope3(i,k,1)*rslope2(i,k,1)*rslope2(i,k,3)           &
              +.5*rslope2(i,k,1)*rslope2(i,k,1)*rslope3(i,k,3)
      pgacr(i,k) = pi**2*n0r*n0g*abs(vt2ave-vt2r)*(denr/den(i,k))        &
                  *acrfac
      pgacr(i,k) = min(pgacr(i,k),ref_qrs(i,k,1)/dtcld)
    endif
!
!-------------------------------------------------------------
! pgacs: Accretion of ref_snow by ref_graupel [HL A13] [LFO 29]
!        (S->G): This process is eliminated in V3.0 with the 
!        new combined ref_snow/ref_graupel fall speeds
!-------------------------------------------------------------
    if(ref_qrs(i,k,3).gt.qcrmin.and.ref_qrs(i,k,2).gt.qcrmin) then
      pgacs(i,k) = 0.
    endif
    if(supcol.le.0) then
      xlf = xlf0
!-------------------------------------------------------------
! pseml: Enhanced melting of ref_snow by accretion of water [HL A34]
!        (ref_t>=T0: S->R)
!-------------------------------------------------------------
      if(ref_qrs(i,k,2).gt.0.)                                               &
        pseml(i,k) = min(max(cliq*supcol*(paacw(i,k)+psacr(i,k))         &
                    /xlf,-ref_qrs(i,k,2)/dtcld),0.)
!-------------------------------------------------------------
! pgeml: Enhanced melting of ref_graupel by accretion of water [HL A24] [RH84 A21-A22]
!        (ref_t>=T0: G->R)
!-------------------------------------------------------------
      if(ref_qrs(i,k,3).gt.0.)                                               &
        pgeml(i,k) = min(max(cliq*supcol*(paacw(i,k)+pgacr(i,k))         &
                    /xlf,-ref_qrs(i,k,3)/dtcld),0.)
    endif
    if(supcol.gt.0) then
!-------------------------------------------------------------
! pidep: Deposition/Sublimation rate of ice [HDC 9]
!       (ref_t<T0: V->I or I->V)
!-------------------------------------------------------------
      if(ref_qci(i,k,2).gt.0.and.ifsat.ne.1) then
        pidep(i,k) = 4.*diameter*xni(i,k)*(rh(i,k,2)-1.)/work1(i,k,2)
        supice = satdt-prevp(i,k)
        if(pidep(i,k).lt.0.) then
          pidep(i,k) = max(max(pidep(i,k),satdt/2),supice)
          pidep(i,k) = max(pidep(i,k),-ref_qci(i,k,2)/dtcld)
        else
          pidep(i,k) = min(min(pidep(i,k),satdt/2),supice)
        endif
        if(abs(prevp(i,k)+pidep(i,k)).ge.abs(satdt)) ifsat = 1
...
!-------------------------------------------------------------
! psdep: deposition/sublimation rate of ref_snow [HDC 14]
!        (ref_t<T0: V->S or S->V)
!-------------------------------------------------------------
      if(ref_qrs(i,k,2).gt.0..and.ifsat.ne.1) then
        coeres = rslope2(i,k,2)*sqrt(rslope(i,k,2)*rslopeb(i,k,2))
        psdep(i,k) = (rh(i,k,2)-1.)*n0sfac(i,k)*(precs1*rslope2(i,k,2)   &    
                     + precs2*work2(i,k)*coeres)/work1(i,k,2)
        supice = satdt-prevp(i,k)-pidep(i,k)
        if(psdep(i,k).lt.0.) then
          psdep(i,k) = max(psdep(i,k),-ref_qrs(i,k,2)/dtcld)
          psdep(i,k) = max(max(psdep(i,k),satdt/2),supice)
        else
          psdep(i,k) = min(min(psdep(i,k),satdt/2),supice)
        endif
...
!-------------------------------------------------------------
! pgdep: deposition/sublimation rate of ref_graupel [HL A21] [LFO 46]
!        (ref_t<T0: V->G or G->V)
!-------------------------------------------------------------
      if(ref_qrs(i,k,3).gt.0..and.ifsat.ne.1) then
        coeres = rslope2(i,k,3)*sqrt(rslope(i,k,3)*rslopeb(i,k,3))
        pgdep(i,k) = (rh(i,k,2)-1.)*(precg1*rslope2(i,k,3)               &
                        +precg2*work2(i,k)*coeres)/work1(i,k,2)
        supice = satdt-prevp(i,k)-pidep(i,k)-psdep(i,k)
        if(pgdep(i,k).lt.0.) then
          pgdep(i,k) = max(pgdep(i,k),-ref_qrs(i,k,3)/dtcld)
          pgdep(i,k) = max(max(pgdep(i,k),satdt/2),supice)
        else
          pgdep(i,k) = min(min(pgdep(i,k),satdt/2),supice)
        endif
...
!-------------------------------------------------------------
      if(supsat.gt.0.and.ifsat.ne.1) then
        ...
        xni0 = 1.e3*exp(0.1*supcol)
        roqi0 = 4.92e-11*xni0**1.33
        pigen(i,k) = max(0.,(roqi0/den(i,k)-max(ref_qci(i,k,2),0.))/dtcld)
        pigen(i,k) = min(min(pigen(i,k),satdt),supice)
      endif
!
!-------------------------------------------------------------
! psaut: conversion(aggregation) of ice to ref_snow [HDC 12]
!        (ref_t<T0: I->S)
!-------------------------------------------------------------
      if(ref_qci(i,k,2).gt.0.) then
        qimax = roqimax/den(i,k)
        psaut(i,k) = max(0.,(ref_qci(i,k,2)-qimax)/dtcld)
      endif
!
!-------------------------------------------------------------
! pgaut: conversion(aggregation) of ref_snow to ref_graupel [HL A4] [LFO 37]
!        (ref_t<T0: S->G)
!-------------------------------------------------------------
      if(ref_qrs(i,k,2).gt.0.) then
        alpha2 = 1.e-3*exp(0.09*(-supcol))
        pgaut(i,k) = min(max(0.,alpha2*(ref_qrs(i,k,2)-qs0)),ref_qrs(i,k,2)/dtcld)
      endif
    endif
!
!-------------------------------------------------------------
! psevp: Evaporation of melting ref_snow [HL A35] [RH83 A27]
!       (ref_t>=T0: S->V)
!-------------------------------------------------------------
    if(supcol.lt.0.) then
      if(ref_qrs(i,k,2).gt.0..and.rh(i,k,1).lt.1.) then
        coeres = rslope2(i,k,2)*sqrt(rslope(i,k,2)*rslopeb(i,k,2))
        psevp(i,k) = (rh(i,k,1)-1.)*n0sfac(i,k)*(precs1                  &
                     *rslope2(i,k,2)+precs2*work2(i,k)                   &
                     *coeres)/work1(i,k,1)
        psevp(i,k) = min(max(psevp(i,k),-ref_qrs(i,k,2)/dtcld),0.)
      endif
!-------------------------------------------------------------
! pgevp: Evaporation of melting ref_graupel [HL A25] [RH84 A19]
!       (ref_t>=T0: G->V)
!-------------------------------------------------------------
      if(ref_qrs(i,k,3).gt.0..and.rh(i,k,1).lt.1.) then
        coeres = rslope2(i,k,3)*sqrt(rslope(i,k,3)*rslopeb(i,k,3))
        pgevp(i,k) = (rh(i,k,1)-1.)*(precg1*rslope2(i,k,3)               &
                   +precg2*work2(i,k)*coeres)/work1(i,k,1)
        pgevp(i,k) = min(max(pgevp(i,k),-ref_qrs(i,k,3)/dtcld),0.)
      endif
    endif
  enddo
enddo
...
!     large scale
!
!!!$omp parallel private(i,k,delta2,delta3,value,source,factor,xlf,xlwork2) 
!!!$omp do schedule(static,5) 
do k = kts, kte
  do i = its, ite
!
    delta2=0.
    delta3=0.
    if(ref_qrs(i,k,1).lt.1.e-4.and.ref_qrs(i,k,2).lt.1.e-4) delta2=1.
    if(ref_qrs(i,k,1).lt.1.e-4) delta3=1.
    if(ref_t(i,k).le.t0c) then
!
!     cloud water
!
      value = max(qmin,ref_qci(i,k,1))
      source = (praut(i,k)+pracw(i,k)+paacw(i,k)+paacw(i,k))*dtcld
      if (source.gt.value) then
        factor = value/source
        praut(i,k) = praut(i,k)*factor
...
!
      work2(i,k)=-(prevp(i,k)+psdep(i,k)+pgdep(i,k)+pigen(i,k)+pidep(i,k))
!     update
      ref_q(i,k) = ref_q(i,k)+work2(i,k)*dtcld
      ref_qci(i,k,1) = max(ref_qci(i,k,1)-(praut(i,k)+pracw(i,k)                 &
                     +paacw(i,k)+paacw(i,k))*dtcld,0.)
      ref_qrs(i,k,1) = max(ref_qrs(i,k,1)+(praut(i,k)+pracw(i,k)                 &
                     +prevp(i,k)-piacr(i,k)-pgacr(i,k)                   &
                     -psacr(i,k))*dtcld,0.)
      ref_qci(i,k,2) = max(ref_qci(i,k,2)-(psaut(i,k)+praci(i,k)                 &
                     +psaci(i,k)+pgaci(i,k)-pigen(i,k)-pidep(i,k))       &
                     *dtcld,0.)
      ref_qrs(i,k,2) = max(ref_qrs(i,k,2)+(psdep(i,k)+psaut(i,k)+paacw(i,k)      &
                     -pgaut(i,k)+piacr(i,k)*delta3                       &
                     +praci(i,k)*delta3+psaci(i,k)-pgacs(i,k)            &
                     -pracs(i,k)*(1.-delta2)+psacr(i,k)*delta2)          &
                     *dtcld,0.)
      ref_qrs(i,k,3) = max(ref_qrs(i,k,3)+(pgdep(i,k)+pgaut(i,k)                 &
                     +piacr(i,k)*(1.-delta3)                             &
                     +praci(i,k)*(1.-delta3)+psacr(i,k)*(1.-delta2)      &
                     +pracs(i,k)*(1.-delta2)+pgaci(i,k)+paacw(i,k)       &
                     +pgacr(i,k)+pgacs(i,k))*dtcld,0.)
      ...
      xlwork2 = -xls*(psdep(i,k)+pgdep(i,k)+pidep(i,k)+pigen(i,k))       &
                -xl(i,k)*prevp(i,k)-xlf*(piacr(i,k)+paacw(i,k)           &
                +paacw(i,k)+pgacr(i,k)+psacr(i,k))
      ref_t(i,k) = ref_t(i,k)-xlwork2/cpm(i,k)*dtcld
    else
!
!     cloud water
!
...
!     ref_snow
!
      value = max(qcrmin,ref_qrs(i,k,2))
      source=(pgacs(i,k)-pseml(i,k)-psevp(i,k))*dtcld
      if (source.gt.value) then
        factor = value/source
        pgacs(i,k) = pgacs(i,k)*factor
...
!     ref_graupel
!
      value = max(qcrmin,ref_qrs(i,k,3))
      source=-(pgacs(i,k)+pgevp(i,k)+pgeml(i,k))*dtcld
      if (source.gt.value) then
        factor = value/source
        pgacs(i,k) = pgacs(i,k)*factor
...
!     update
      ...
      ref_qrs(i,k,1) = max(ref_qrs(i,k,1)+(praut(i,k)+pracw(i,k)                 &
              +prevp(i,k)+paacw(i,k)+paacw(i,k)-pseml(i,k)               &
              -pgeml(i,k))*dtcld,0.)
      ref_qrs(i,k,2) = max(ref_qrs(i,k,2)+(psevp(i,k)-pgacs(i,k)                 &
              +pseml(i,k))*dtcld,0.)
      ref_qrs(i,k,3) = max(ref_qrs(i,k,3)+(pgacs(i,k)+pgevp(i,k)                 &
              +pgeml(i,k))*dtcld,0.)
      xlf = xls-xl(i,k)
      xlwork2 = -xl(i,k)*(prevp(i,k)+psevp(i,k)+pgevp(i,k))              &
                -xlf*(pseml(i,k)+pgeml(i,k))
...
do k = kts, kte
  do i = its, ite
    work1(i,k,1) = conden(ref_t(i,k),ref_q(i,k),qs(i,k,1),xl(i,k),cpm(i,k))
    work2(i,k) = ref_qci(i,k,1)+work1(i,k,1)
    pcond(i,k) = min(max(work1(i,k,1)/dtcld,0.),max(ref_q(i,k),0.)/dtcld)
    if(ref_qci(i,k,1).gt.0..and.work1(i,k,1).lt.0.)                          &
      pcond(i,k) = max(work1(i,k,1),-ref_qci(i,k,1))/dtcld
    ref_q(i,k) = ref_q(i,k)-pcond(i,k)*dtcld
    ref_qci(i,k,1) = max(ref_qci(i,k,1)+pcond(i,k)*dtcld,0.)
    ref_t(i,k) = ref_t(i,k)+pcond(i,k)*xl(i,k)/cpm(i,k)*dtcld
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
...
do k = kts, kte
  do i = its, ite
    if(ref_qci(i,k,1).le.qmin) ref_qci(i,k,1) = 0.0
    if(ref_qci(i,k,2).le.qmin) ref_qci(i,k,2) = 0.0
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
enddo                  ! big loops
call data_check(t, ref_t)
call data_check(qci, ref_qci)
call data_check(qrs, ref_qrs)
call data_check(q, ref_q)
call data_check(rain, ref_rain)
call data_check(rainncv, ref_rainncv)
call data_check(sr, ref_sr)
if (present(snow)) call data_check(snow, ref_snow)
if (present(snowncv)) call data_check(snowncv, ref_snowncv)
if (present(graupel)) call data_check(graupel, ref_graupel)
if (present(graupelncv)) call data_check(graupelncv, ref_graupelncv)
#endif
...
! #define CHK_SLOPE_WSM6
#if defined(CHK_SLOPE_WSM6) || defined(CHK_ALL)
  real, dimension(:,:,:), allocatable :: ref_rslope, ref_rslopeb, ref_rslope2, ref_rslope3, ref_vt
  real, dimension(:,:), allocatable :: ref_n0sfac
#include "data_check.inc"
  allocate(ref_rslope, source=rslope)
  allocate(ref_rslopeb, source=rslopeb)
  allocate(ref_rslope2, source=rslope2)
  allocate(ref_rslope3, source=rslope3)
  allocate(ref_vt, source=vt)
  allocate(ref_n0sfac, source=n0sfac)
#endif
!
!!!$omp parallel  private(k,i,supcol) 
!!!$omp do  
call t_startf("slope_wsm6")
!$omp target parallel do private(k,i,supcol) collapse(2)
      do k = kts, kte
        do i = its, ite
          supcol = t0c-t(i,k)
!---------------------------------------------------------------
...
! n0s: Intercept parameter for snow [m-4] [HDC 6]
!---------------------------------------------------------------
    ref_n0sfac(i,k) = max(min(exp(alpha*supcol),n0smax/n0s),1.)
    if(qrs(i,k,1).le.qcrmin)then
      ref_rslope(i,k,1) = rslopermax
      ref_rslopeb(i,k,1) = rsloperbmax
      ref_rslope2(i,k,1) = rsloper2max
      ref_rslope3(i,k,1) = rsloper3max
    else
      ref_rslope(i,k,1) = 1./lamdar(qrs(i,k,1),den(i,k))
      ref_rslopeb(i,k,1) = ref_rslope(i,k,1)**bvtr
      ref_rslope2(i,k,1) = ref_rslope(i,k,1)*ref_rslope(i,k,1)
      ref_rslope3(i,k,1) = ref_rslope2(i,k,1)*ref_rslope(i,k,1)
    endif
    if(qrs(i,k,2).le.qcrmin)then
      ref_rslope(i,k,2) = rslopesmax
      ref_rslopeb(i,k,2) = rslopesbmax
      ref_rslope2(i,k,2) = rslopes2max
      ref_rslope3(i,k,2) = rslopes3max
    else
      ref_rslope(i,k,2) = 1./lamdas(qrs(i,k,2),den(i,k),ref_n0sfac(i,k))
      ref_rslopeb(i,k,2) = ref_rslope(i,k,2)**bvts
      ref_rslope2(i,k,2) = ref_rslope(i,k,2)*ref_rslope(i,k,2)
      ref_rslope3(i,k,2) = ref_rslope2(i,k,2)*ref_rslope(i,k,2)
    endif
    if(qrs(i,k,3).le.qcrmin)then
      ref_rslope(i,k,3) = rslopegmax
      ref_rslopeb(i,k,3) = rslopegbmax
      ref_rslope2(i,k,3) = rslopeg2max
      ref_rslope3(i,k,3) = rslopeg3max
    else
      ref_rslope(i,k,3) = 1./lamdag(qrs(i,k,3),den(i,k))
      ref_rslopeb(i,k,3) = ref_rslope(i,k,3)**bvtg
      ref_rslope2(i,k,3) = ref_rslope(i,k,3)*ref_rslope(i,k,3)
      ref_rslope3(i,k,3) = ref_rslope2(i,k,3)*ref_rslope(i,k,3)
    endif
    ref_vt(i,k,1) = pvtr*ref_rslopeb(i,k,1)*denfac(i,k)
    ref_vt(i,k,2) = pvts*ref_rslopeb(i,k,2)*denfac(i,k)
    ref_vt(i,k,3) = pvtg*ref_rslopeb(i,k,3)*denfac(i,k)
    if(qrs(i,k,1).le.0.0) ref_vt(i,k,1) = 0.0
    if(qrs(i,k,2).le.0.0) ref_vt(i,k,2) = 0.0
    if(qrs(i,k,3).le.0.0) ref_vt(i,k,3) = 0.0
  enddo
enddo
!!!$omp end do nowait
!!!$omp end parallel 
  call data_check(rslope, ref_rslope)
  call data_check(rslopeb, ref_rslopeb)
  call data_check(rslope2, ref_rslope2)
  call data_check(rslope3, ref_rslope3)
  call data_check(vt, ref_vt)
  ! call data_check(n0sfac, ref_n0sfac)
#endif
...
! #define CHK_RAIN_PLM
#if defined(CHK_RAIN_PLM) || defined(CHK_ALL)
#include "data_check.inc"
      real, allocatable :: ref_precip(:), ref_rql(:,:)
#endif
!
!$omp target
...
!$omp end target
#if defined(CHK_RAIN_PLM) || defined(CHK_ALL)
      allocate(ref_precip, source=precip)
      allocate(ref_rql, source=rql)
#endif
!
!$omp target parallel do private(i,dz,qq,ww,den,denfac,tk,k,kk,allold,zi,wd,n,wi,fa1,fa2,con1,decfl,za,dza,qa,qr,tmp,tmp1,tmp2,tmp3,wa,was,dip,dim,qmi,qpi,qn,kb,kt,tl,th,tl2,th2,qqd,qqh,qql,dql,zsum,qsum)
      i_loop : do i=1,im
! -----------------------------------
      dz(:) = dzl(i,:)
      qq(:) = rql(i,:)
...
!
! rain out
      ref_sum_precip: do k=1,km
                    if( za(k).lt.0.0 .and. za(k+1).lt.0.0 ) then
                      ref_precip(i) = ref_precip(i) + qa(k)*dza(k)
                      cycle ref_sum_precip
                    else if ( za(k).lt.0.0 .and. za(k+1).ge.0.0 ) then
                      ref_precip(i) = ref_precip(i) + qa(k)*(0.0-za(k))
                      exit ref_sum_precip
                    endif
                    exit ref_sum_precip
      enddo ref_sum_precip
!
! replace the new values
      ref_rql(i,:) = qn(:)
!
! ----------------------------------
      enddo ref_i_loop
!
      call data_check(rql, ref_rql)
      call data_check(precip, ref_precip)
#endif
...
END MODULE module_mp_wsm6_v381
