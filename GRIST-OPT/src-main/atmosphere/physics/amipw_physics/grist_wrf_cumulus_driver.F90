...
module grist_wrf_cumulus_driver
   ...
   use grist_nml_module,         only: nh_dynamics,run_type
   use grist_mpi
!
! module physics 
!
...
!ai-phys
#ifdef Resnet
   use module_cu_ResNet_v1,      only: ResNet_init, ResNet_tend
   use module_cu_ResNet_hybrid_v2
#endif
!resnet used
   use grist_dycore_vars_module,     only: dycoreVarCellFull,dycoreVarGeography
   use grist_gcm_diagnose_h1_module,only: diag_phys_accu_vars_h1_2d
...
!
...
#ifdef Resnet
      case('ResNet')
      call ResNet_init(rthcuten= ptend_wrf%rthcuten,&
                   rqvcuten= ptend_wrf%rqvcuten,&
                   restart = restart,                               &
                   ids=1,ide=ncell, jds=1,jde=1, kds=1, kde=nLevel, &
                   ims=1,ime=ncell, jms=1,jme=1, kms=1, kme=nLevel, &
      ...
      case('ResNet_hybrid')
      call ResNet_hybrid_init(rthcuten= ptend_wrf%rthcuten,&
                   rqvcuten= ptend_wrf%rqvcuten,&
                   restart = restart,                               &
                   ids=1,ide=ncell, jds=1,jde=1, kds=1, kde=nLevel, &
                   ims=1,ime=ncell, jms=1,jme=1, kms=1, kme=nLevel, &
...
!wym add
   real(r8)      :: rthraten (ncell,nLevel,1)
!wym
...
!      rthften(1:ncell,1:nLevel,1:1) = ptend_wrf%rthblten(1:ncell,1:nLevel,1:1)+ptend_wrf%rthdyten(1:ncell,1:nLevel,1:1)+ptend_wrf%rthraten(1:ncell,1:nLevel,1:1)
      !rqvften(1:ncell,1:nLevel,1:1) = ptend_wrf%rqvblten(1:ncell,1:nLevel,1:1)
      !rthften(1:ncell,1:nLevel,1:1) = ptend_wrf%rthblten(1:ncell,1:nLevel,1:1)+ptend_wrf%rthraten(1:ncell,1:nLevel,1:1)
       rthraten(1:ncell,1:nLevel,1:1)=ptend_wrf%rthraten(1:ncell,1:nLevel,1:1)
      ...
      call ResNet_tend(&
                ...
                u3d      = pstate_wrf%u_phy( 1:ncell,1:nLevel,1:1), & ! in
                v3d      = pstate_wrf%v_phy( 1:ncell,1:nLevel,1:1), & ! in
                w        = pstate_wrf%omega(1:ncell,1:nLevel,1:1), & ! in, not used
                th3d      = pstate_wrf%t_phy( 1:ncell,1:nLevel,1:1), & ! in
                qv3d     = pstate_wrf%moist( 1:ncell,1:nLevel,1:1,p_qv), & ! in
                therad3d   =diag_phys_accu_vars_h1_2d%rad_thten%f(1:nLevel-1,1:ncell),  & ! in, th tend from other
                dz8w     = pstate_wrf%dz8w(  1:ncell,1:nLevel,1:1) , & ! in
                pcps     = pstate_wrf%p_phy( 1:ncell,1:nLevel,1:1) , & ! in
                p8w      = pstate_wrf%p8w(   1:ncell,1:nLevel,1:1) , & ! in
                raincv    = psurf_wrf%raincv(  1:ncell,         1:1) , &
                cu_act_flag = cu_act_flag,                           &
                dx       = pstate_wrf%dxmean(1:ncell),               &
                ids=1,ide=ncell, jds=1,jde=1, kds=1,kde=nLevel  ,    &
                ims=1,ime=ncell, jms=1,jme=1, kms=1,kme=nLevel  ,    &
                ...
                rthcuten = ptend_wrf%rthcuten(1:ncell,1:nLevel,1:1), & ! out
                rqvcuten = ptend_wrf%rqvcuten(1:ncell,1:nLevel,1:1)) ! out
      ptend_wrf%rthcuten(1:ncell,1:nLevel-11,1:1)=ptend_wrf%rthcuten(1:ncell,1:nLevel-11,1:1)-ptend_wrf%rthblten(1:ncell,1:nLevel-11,1:1)
      ptend_wrf%rqvcuten(1:ncell,5:nLevel-7,1:1)=ptend_wrf%rqvcuten(1:ncell,5:nLevel-7,1:1)-ptend_wrf%rqvblten(1:ncell,5:nLevel-7,1:1)
      psurf_wrf%raincv=psurf_wrf%raincv*dtime

    case('ResNet_hybrid')

       if((trim(run_type).eq.'init') .and. (itimestep.eq.0 .or. itimestep .eq. 1)) then
                ptend_wrf%rthcuten(1:ncell,1:nLevel,1:1)=zero
                ptend_wrf%rqvcuten(1:ncell,1:nLevel,1:1)=zero
       else
...
!      rthften(1:ncell,1:nLevel,1:1) = ptend_wrf%rthblten(1:ncell,1:nLevel,1:1)+ptend_wrf%rthdyten(1:ncell,1:nLevel,1:1)+ptend_wrf%rthraten(1:ncell,1:nLevel,1:1)
      ...
      call ResNet_hybrid_tend(&
                ...
                th3d     = pstate_wrf%t_phy( 1:ncell,1:nLevel,1:1), & ! in
                qv3d     = pstate_wrf%moist( 1:ncell,1:nLevel,1:1,p_qv), & ! in
                qdyn     = ptend_wrf%rqvdyten (1:ncell,1:nLevel,1:1),  & ! in, th tend from other
                tdyn     = ptend_wrf%rthdyten (1:ncell,1:nLevel,1:1) , & ! in
                lh       = psurf_wrf%lh(1:ncell,1)                  , &
                hfx      = psurf_wrf%hfx (1:ncell,1)                , &
                coszr    = pstate_wrf%coszr(1:ncell,1)              , &
                pcps     = pstate_wrf%p_phy( 1:ncell,1:nLevel,1:1) , & ! in
                p8w      = pstate_wrf%p8w(   1:ncell,1:nLevel,1:1) , & ! in
                lat      = dycoreVarGeography%scalar_lat_at_pc%f(1:ncell),&
                raincv    = psurf_wrf%raincv(  1:ncell,         1:1) , &
                cu_act_flag = cu_act_flag,                           &
                dx       = pstate_wrf%dxmean(1:ncell),               &
                ids=1,ide=ncell, jds=1,jde=1, kds=1,kde=nLevel  ,    &
 ...
 !------------wym test-------------               
 !     ptend_wrf%rthcuten(1:ncell,1:nLevel-11,1:1)=ptend_wrf%rthcuten(1:ncell,1:nLevel-11,1:1)-ptend_wrf%rthblten(1:ncell,1:nLevel-11,1:1)
 !     ptend_wrf%rqvcuten(1:ncell,1:nLevel-13,1:1)=ptend_wrf%rqvcuten(1:ncell,5:nLevel-13,1:1)-ptend_wrf%rqvblten(1:ncell,5:nLevel-13,1:1)
    ptend_wrf%rthcuten(1:ncell,1:4,1:1)=zero
    ptend_wrf%rqvcuten(1:ncell,1:1,1:1)=zero
 !---------------------------------
    psurf_wrf%raincv=psurf_wrf%raincv*dtime
    end if
#endif
...
!
!wym added 
!       print*,'rthcuten: ',  ptend_wrf%rthcuten(1,:,1)
