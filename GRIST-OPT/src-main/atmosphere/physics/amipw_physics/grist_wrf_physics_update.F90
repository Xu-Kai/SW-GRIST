...
!--------------------------------------------------------------------------------------
 ...
 module grist_wrf_physics_update
    ...
    use grist_wrf_data_structure,          only: pstate_wrf, ptend_wrf, p_qv, p_qc, p_qr, p_qi, p_qs, &
                                                 p_qg, p_nr, p_ni, p_ns, p_ng, psurf_wrf
    use grist_mpi
...
!------------------------------------------------------------
  ...
  subroutine phys_update_p2(dtime, ncell, process_name, scheme_name)
    ...
    character(len=*),      intent(in)   :: process_name
    character(len=*),      intent(in)   :: scheme_name
!wym add for diag rainc
#ifdef Diag_ai_rain
    real(r8) :: qck1,qc0,pi,ilev
    integer(i4)                         :: icell
    ...
    REAL, DIMENSION(1:ncell,1:nlev) :: praut
    !wym
#endif
    ...
    CASE('CU')
       ...
       case('NTDKV381','TDK','ResNet','ResNet_hybrid')
          ptend_f3%tend_u_wind_at_pc_full_level%f(1:nlev,1:ncell) =  &
...
!wym add for diag rain
#ifdef Diag_ai_rain
          ptend_wrf%rqccuten(1:ncell,1:nlev,1)=ptend_wrf%rqvcuten(1:ncell,1:nlev,1)
          pstate_wrf%moist(  1:ncell,1:nlev,1,p_qv) = pstate_wrf%moist(1:ncell,1:nlev,1,p_qv)-dtime*ptend_wrf%rqvcuten(1:ncell,1:nlev,1)
          pstate_wrf%th_phy( 1:ncell,1:nlev,1)      = pstate_wrf%th_phy( 1:ncell,1:nlev,1)-dtime*ptend_wrf%rthcuten(1:ncell,1:nlev,1)
          !wym add
#endif
...
!---------------------------------------------------------------
          do icell=1,ncell
             do ilev=1,30
                if(pstate_wrf%moist(icell,ilev,1,p_qc) .gt. qc0) then
                    praut(icell,ilev) = qck1*pstate_wrf%moist(icell,ilev,1,p_qc)**(7./3.)
                    praut(icell,ilev) = min(praut(icell,ilev),pstate_wrf%moist(icell,ilev,1,p_qc)/dtcldcr)
                endif
          
                psurf_wrf%raincv(icell,1)=0
          
                psurf_wrf%raincv(icell,1:1)=psurf_wrf%raincv(icell,1:1)+praut(icell,ilev)/gravity*abs(pstate_wrf%p8w(icell,ilev,1:1)-pstate_wrf%p8w(icell,ilev+1,1:1))
             end do
          end do
          pstate_wrf%moist(  1:ncell,1:nlev,1,p_qc) = pstate_wrf%moist(  1:ncell,1:nlev,1,p_qc)-dtime*ptend_wrf%rqccuten(1:ncell,1:nlev,1)
#endif
...
!---------------------------------------------------------------------
  ...
  subroutine phys_update_p3(dtime, ncell, process_name, scheme_name)
    ...
    CASE('CU')
      ...
      select case(trim(scheme_name))
        case('TDK','NTDKV381','ResNet','ResNet_hybrid')
          ptend_f1%tend_u_wind_at_pc_full_level%f(1:nlev,1:ncell) =  &
          ptend_f1%tend_u_wind_at_pc_full_level%f(1:nlev,1:ncell) + transpose(ptend_wrf%ruucuten(1:ncell,1:nlev,1))
