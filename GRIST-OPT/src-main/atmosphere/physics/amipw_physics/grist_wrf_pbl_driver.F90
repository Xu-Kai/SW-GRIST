...
module grist_wrf_pbl_driver
  ...
  use grist_wrfphys_nml_module, only: wrfphys_bl_scheme, use_gwdo, step_bl, unuse_bl
...
!Added switch for boundary layer scheme by wangym
IF( (.not.unuse_bl) .and. (itimestep.eq.0 .or. itimestep .eq. 1 .or.  mod(itimestep,stepbl) .eq. 0)) then
...
! assume no-slip sea surface
     ...
     call ysu_v381(u3d         = pstate_wrf%u_phy(  1:ncell,1:nLevel,1:1), &
              ...
              !optional, ! not used
                   !regime )
!wym add
     ptend_wrf%rthblten(1:ncell,5:nLevel,1:1)=zero
     ptend_wrf%rqvblten(1:ncell,2:nLevel,1:1)=zero
!wym add
