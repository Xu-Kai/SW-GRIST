...
!
    ...
    use grist_data_types,   only: scalar_1d_field, scalar_2d_field, exchange_field_list_1d, scalar_3d_field, &
                                  wrap_allocate_data1d, wrap_deallocate_data1d, wrap_deallocate_data2d, wrap_deallocate_data3d
    use grist_constants,    only: rearth, i4, i8, r8, pi, one, zero, t00, r4, gravity
    use grist_nml_module,   only: nlev, staticFilePath, sftopoFilePath, sstFilePath, aqua_sst_style, real_sst_style, numMonSST,test_real_case, topo_type, mlFilePath, &
...
! wangym Resnet
#ifdef Resnet
   type(scalar_1d_field), public   :: mldata_bias0
   type(scalar_1d_field), public   :: mldata_bias1
   type(scalar_2d_field), public   :: mldata_biases
   ...
   type(scalar_3d_field), public   :: mldata_weight0
   type(scalar_3d_field), public   :: mldata_weights
   type(scalar_1d_field), public   :: mldata_plev
   type(scalar_1d_field), public   :: mldata_ps
   type(scalar_2d_field), public   :: mldata_qqq
   type(scalar_3d_field), public   :: mldata_inre
   type(scalar_3d_field), public   :: mldata_pred
   type(scalar_2d_field), public   :: mldata_weight1
   type(scalar_3d_field), public   :: mldata_weight1_hybrid
...
#ifdef Resnet
    if(.not.allocated(mldata_bias0%f)) allocate(mldata_bias0%f(128))
    if(.not.allocated(mldata_biases%f)) allocate(mldata_biases%f(128,20))
    if(.not.allocated(mldata_weight0%f)) allocate(mldata_weight0%f(3,5,128))
    if(.not.allocated(mldata_weights%f)) allocate(mldata_weights%f(60,128,128))
    if(.not.allocated(mldata_imax%f)) allocate(mldata_imax%f(30,5))
    if(.not.allocated(mldata_imin%f)) allocate(mldata_imin%f(30,5))
    if(.not.allocated(mldata_plev%f)) allocate(mldata_plev%f(29))
    if(.not.allocated(mldata_qqq%f))allocate(mldata_qqq%f(29,mesh%nv));mldata_qqq%pos = 0
    if(.not.allocated(mldata_ps%f))allocate(mldata_ps%f(mesh%nv));mldata_ps%pos = 0
    if(.not.allocated(mldata_pred%f)) allocate(mldata_pred%f(2720,29,2))
#ifdef aiplev
       if(.not.allocated(mldata_bias1%f)) allocate(mldata_bias1%f(58))
       if(.not.allocated(mldata_weight1%f)) allocate(mldata_weight1%f(128,58))
       if(.not.allocated(mldata_omax%f)) allocate(mldata_omax%f(29,2))
       if(.not.allocated(mldata_omin%f)) allocate(mldata_omin%f(29,2))
       if(.not.allocated(mldata_pred%f)) allocate(mldata_pred%f(2720,29,2))
#else
       if(.not.allocated(mldata_bias1%f)) allocate(mldata_bias1%f(2))
       if(.not.allocated(mldata_weight1_hybrid%f)) allocate(mldata_weight1_hybrid%f(3,128,2))
       if(.not.allocated(mldata_omax%f)) allocate(mldata_omax%f(30,2))
       if(.not.allocated(mldata_omin%f)) allocate(mldata_omin%f(30,2))
       if(.not.allocated(mldata_pred%f)) allocate(mldata_pred%f(2720,30,2))
#endif
#endif
...
#ifdef Resnet
    dim_3 = 5
    call wrap_read_1d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights0.nc','bias',10,mldata_bias0%f)
    call wrap_read_3d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights0.nc','kernels',dim_3,9,mldata_weight0%f)
    dim_3 = 128
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights-sum.nc','bias',dim_3,10,mldata_biases%f)
    call wrap_read_3d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights-sum.nc','kernels',dim_3,8,mldata_weights%f)
    dim_3 = 30
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32.nc','inputsmax',dim_3,9,mldata_imax%f)
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32.nc','inputmin',dim_3,9,mldata_imin%f)
!    call wrap_read_1d_group_rst(mesh%gcomm_read,trim(mlFilePath),'mldata.splev.nc','PS',0,mldata_ps%f)
!    call wrap_read_1d_group_rst(mesh%gcomm_read,trim(mlFilePath),'mldata.splev.nc','plev',12,mldata_plev%f)
!    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'mldata.splev.nc','Q', 22, 0,mldata_qqq%f)
!    call wrap_read_3d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32n.nc','inre0',30,10,mldata_inre%f)
#ifdef aiplev
    dim_3 = 128
    call wrap_read_1d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights1.nc','bias',11,mldata_bias1%f)
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights1.nc','kernels',dim_3,11,mldata_weight1%f)
    dim_3 = 29
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32.nc','outputsmax',dim_3,8,mldata_omax%f)
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32.nc','outputmin',dim_3,8,mldata_omin%f)
   !  call wrap_read_3d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32n.nc','ypred0',29,11,mldata_pred%f)
#else
    call wrap_read_1d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights1.nc','bias',13,mldata_bias1%f)
    dim_3 = 128
    call wrap_read_3d_group_rst(mesh%gcomm_read,trim(mlFilePath),'weights1.nc','kernels',dim_3,13,mldata_weight1_hybrid%f)
    dim_3 = 30
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32.nc','outputsmax',dim_3,12,mldata_omax%f)
    call wrap_read_2d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32.nc','outputmin',dim_3,12,mldata_omin%f)
!    call wrap_read_3d_group_rst(mesh%gcomm_read,trim(mlFilePath),'test_k2f_32n.nc','ypred0',30,12,mldata_pred%f)
#endif
#endif
    return
...
!Resnet data
#ifdef Resnet
      call wrap_deallocate_data1d(mldata_bias0)
      call wrap_deallocate_data1d(mldata_bias1)
      call wrap_deallocate_data2d(mldata_biases)
      call wrap_deallocate_data3d(mldata_weight0)
#ifdef aiplev
      call wrap_deallocate_data2d(mldata_weight1)
#endif
      call wrap_deallocate_data3d(mldata_weight1_hybrid)
      call wrap_deallocate_data3d(mldata_weights)
      call wrap_deallocate_data2d(mldata_imax)
      call wrap_deallocate_data2d(mldata_imin)
      call wrap_deallocate_data2d(mldata_omax)
      call wrap_deallocate_data2d(mldata_omin)
      call wrap_deallocate_data1d(mldata_plev)
      call wrap_deallocate_data1d(mldata_ps)
      call wrap_deallocate_data2d(mldata_qqq)
      call wrap_deallocate_data3d(mldata_inre)
      call wrap_deallocate_data3d(mldata_pred)
#endif
