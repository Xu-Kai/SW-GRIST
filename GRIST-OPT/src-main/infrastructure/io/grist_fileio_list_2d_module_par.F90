...
!================================================
   ...
   subroutine wrap_output_2d_unordered(mesh, outdir, filename)
...
! possible dims for 2nd
 ...
 ! lsm restart, added by yiz 2020dec
        call def_var_2d_pnetcdf(ptr%varname, fst_dim_id(i1),sec_dim_id(i2), ncid, ptr%var_idlist)
        ptr => ptr%next
      end do
 ...
 !================================================    
      ptr => scalar_2d_list
      do while (associated(ptr))
        call put_att_2d_pnetcdf( ncid, ptr%var_idlist ,ptr%varname,ptr%longname,ptr%units,ptr%var_pos)
        ptr => ptr%next
      end do 
 ...
 !================================================
      ...
      return
    end subroutine wrap_output_2d_unordered
...
!================================================
   ...
   subroutine wrap_output_2d_unordered_sp(mesh, outdir, filename)
 ...
 ! lsm restart, added by yiz 2020dec
        call def_var_2d_pnetcdf_sp(ptr%varname, fst_dim_id(i1),sec_dim_id(i2), ncid, ptr%var_idlist)
        ptr => ptr%next
      end do
 ...
 !================================================
      ...
      do while (associated(ptr))      
        ...
        allocate(temp_var(len_dim1, size(idx_local)))
        call find_output_var_2d(ptr%var, ptr%var_level, idx_local, temp_var)
        allocate(buff_put_sp(len_dim1, size(idx_local)))
      ...
      call wrap_close(ncid)
      return
    end subroutine wrap_output_2d_unordered_sp
     ...
     if (option .le. 6) then
       ...
       !-------------------Wangym add for other data levels------------------
         if (option .eq. 8) then
            count(1)=29
            count(2)=2
            allocate(local_data_r8(count(1),count(2)))
            call wrap_get_vara_realx(ncid, varid, start, count, local_data_r8)
       ...
       !-------------------LiXH add for other data levels------------------
         if (option .eq. 8) then
            count(1)=29
            count(2)=2
            allocate(local_data_r4(count(1),count(2)))
            call wrap_get_vara_real(ncid, varid, start, count, local_data_r4)
         else if(option .eq. 9)then
            count(1)=30
            count(2)=5
            allocate(local_data_r4(count(1),count(2)))
