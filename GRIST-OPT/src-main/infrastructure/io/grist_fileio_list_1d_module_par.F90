...
!================================================
  ...
  subroutine wrap_output_1d_unordered(mesh, outdir, filename)
 ...
 !================================================
      cmode = IOR(NF_CLOBBER,NF_64BIT_DATA)
      call wrap_create(MPI_COMM_WORLD,trim(outdir)//"unordered-"//trim(filename),cmode, nfid) ! large-file support 
      if(mpi_rank() .eq. 0) print*,trim(outdir)//"unordered-"//trim(filename)
 !================================================
 ! 1st: define dim
 !================================================
      ! if(comm%color==0)then
 ...
 !========================================================
      ...
      do while (associated(ptr))     
        ...
        if( ptr%var_pos.eq.8) i1 = 4
        if( ptr%var_pos.eq.9) i1 = 5
        ! print *,ptr%varname, location_id(i1)
        call def_var_1d_pnetcdf(nfid, ptr%varname, location_id(i1), ptr%var_idlist)
        ptr => ptr%next
      end do
 ...
 !================================================
      ...
      ptr => scalar_1d_list
      do while (associated(ptr))
        call put_att_1d_pnetcdf(nfid, ptr%var_idlist, ptr%varname,ptr%longname,ptr%units, ptr%var_pos)
        ptr => ptr%next
      end do
 
      call wrap_enddef(nfid)
 ...
 !================================================
    ...
    do while (associated(ptr))
      if(ptr%var_pos<7)then
        if (ptr%var_pos .eq. 0) then
          ...
          idx_recv    => comm%idx_recvv
          idx_local   => comm%idx_localv
          mystart     => mesh%icomm_write%off_local_v
          mycount     => mesh%icomm_write%cnt_local_v
        else if (ptr%var_pos .eq. 1) then 
          displs_send => comm%displs_sendt
          displs_recv => comm%displs_recvt
          counts_send => comm%counts_sendt
          ...
          idx_recv    => comm%idx_recvt
          idx_local   => comm%idx_localt
          mystart     => mesh%icomm_write%off_local_t
          mycount     => mesh%icomm_write%cnt_local_t
        else if (ptr%var_pos .eq. 6) then 
          displs_send => comm%displs_sende
          displs_recv => comm%displs_recve
          counts_send => comm%counts_sende
          ...
          idx_recv    => comm%idx_recve
          idx_local   => comm%idx_locale
          mystart     => mesh%icomm_write%off_local_e
          mycount     => mesh%icomm_write%cnt_local_e
        end if

        start(1) = mystart+1
        count(1) = mycount
        ! print *,ptr%varname
        ! print *, mesh_nv, start(1)+1, count(1)
        allocate(output_var(size(idx_local)))
        

        call find_output_var(ptr%var,idx_local,output_var)
        call wrap_put_vara_realx_req(nfid, ptr%var_idlist, start, count, output_var, req)
        err = nfmpi_wait_all(nfid, 1, req, st)
        deallocate(output_var)
      else if(ptr%var_pos>7)then
        if(mpi_rank()==0)then
    ...
    return
  end subroutine wrap_output_1d_unordered
...
!================================================
   ...
   subroutine wrap_output_1d_unordered_sp(mesh, outdir, filename)
 ...
 !================================================
      cmode = IOR(NF_CLOBBER,NF_64BIT_DATA)
      call wrap_create(comm%gcomm,trim(outdir)//"unordered-sp-"//trim(filename),cmode, nfid) ! large-file support 
      if(mpi_rank() .eq. 0) print*,trim(outdir)//"unordered-sp-"//trim(filename)
 !================================================
 ! 1st: define dim
 !================================================
        call wrap_def_dim(nfid, 'location_nt', mesh_nt , location_id(1))
 ...
 !========================================================
      ...
      do while (associated(ptr))     
        ...
        if( ptr%var_pos.eq.8) i1 = 4
        if( ptr%var_pos.eq.9) i1 = 5
        call def_var_1d_pnetcdf_sp(nfid, ptr%varname, location_id(i1), ptr%var_idlist)
        ptr => ptr%next
      end do
 ...
 !================================================
      ...
      do while (associated(ptr))
        if(ptr%var_pos<7)then
          ...
          allocate(output_var(size(idx_local)))
          call find_output_var(ptr%var,idx_local,output_var)
          allocate(buff_put_sp(size(idx_local)))
          buff_put_sp(:) = output_var(:)
          call wrap_put_vara_real_req(nfid, ptr%var_idlist, start, count, buff_put_sp, req)
          err = nfmpi_wait_all(nfid, 1, req, st)
          deallocate(output_var)
          deallocate(buff_put_sp)
        else if(ptr%var_pos>7)then
      ...
      call wrap_close(nfid)
      return
    end subroutine wrap_output_1d_unordered_sp
     ...
     if (option .le. 6) then
       ...
       !-------------------LiXH add for other data levels------------------
         if (option .eq. 8) then
            ...
            allocate(local_data_r8(count(3)))
            call wrap_get_vara_realx(ncid, varid, start(3), count(3), local_data_r8)
         else if(option .eq. 11)then
            allocate(local_data_r8(count(4)))
            call wrap_get_vara_realx(ncid, varid, start(4), count(4), local_data_r8)
         else if(option .eq. 12)then
            allocate(local_data_r8(count(5)))
            call wrap_get_vara_realx(ncid, varid, start(5), count(5), local_data_r8)
         else if(option .eq. 13)then
            allocate(local_data_r8(count(6)))
            call wrap_get_vara_realx(ncid, varid, start(6), count(6), local_data_r8)
#endif
         end if
       ...
       !-------------------LiXH add for other data levels------------------
         if (option .eq. 8) then
            ...
            allocate(local_data_r4(count(3)))
            call wrap_get_vara_real(ncid, varid, start(3), count(3), local_data_r4)
         else if(option .eq. 11)then
            allocate(local_data_r4(count(4)))
            call wrap_get_vara_real(ncid, varid, start(4), count(4), local_data_r4)
         else if(option .eq. 12)then
            allocate(local_data_r4(count(5)))
            call wrap_get_vara_real(ncid, varid, start(5), count(5), local_data_r4)
         else if(option .eq. 13)then
            allocate(local_data_r4(count(6)))
            call wrap_get_vara_real(ncid, varid, start(6), count(6), local_data_r4)
#endif
         end if
