...
!================================================
   ...
   subroutine wrap_output_3d_unordered(mesh, outdir, filename, dim3_len)
 ...
 !================================================
      cmode = IOR(NF_CLOBBER,NF_64BIT_DATA)
      call wrap_create(comm%gcomm,trim(outdir)//"unordered-"//trim(filename),cmode, ncid) ! large-file support 
...
! possible dims for 3rd
 ...
 !========================================================
      ...
      do while (associated(ptr))
        ...
        call def_var_3d_pnetcdf(ptr%varname, fst_dim_id(1), sec_dim_id(i1), thr_dim_id(i2), ncid, ptr%var_idlist)
        ptr => ptr%next
      end do
 ...
 !================================================    
      ptr => scalar_3d_list
      do while (associated(ptr))
        call put_att_3d_pnetcdf( ncid, ptr%var_idlist ,ptr%varname,ptr%longname,ptr%units,ptr%var_pos)
        ptr => ptr%next
      end do 
 ...
 !================================================
      ...
      do while (associated(ptr))      
        ...
        start(1) = 1
        count(1) = ndim3
        call wrap_put_vara_realx_req(ncid, ptr%var_idlist, start, count, temp_var, req)
        err = nfmpi_wait_all(ncid, 1, req, st)
        deallocate(temp_var)
        ptr => ptr%next
      end do
      ...
      return
    end subroutine wrap_output_3d_unordered
...
!================================================
   ...
   subroutine wrap_output_3d_unordered_sp(mesh, outdir, filename, dim3_len)
 ...
 !================================================
      cmode = IOR(NF_CLOBBER,NF_64BIT_DATA)
      call wrap_create(comm%gcomm,trim(outdir)//"unordered-sp-"//trim(filename),cmode, ncid) ! large-file support 
 ...
 !========================================================
      ...
      do while (associated(ptr))
        ...
        call def_var_3d_pnetcdf_sp(ptr%varname, fst_dim_id(1), sec_dim_id(i1), thr_dim_id(i2), ncid, ptr%var_idlist)
        ptr => ptr%next
      end do
 ...
 !================================================
      ...
      do while (associated(ptr))      
        ...
        len_dim1 = ptr%var_level
        allocate(temp_var(ndim3, len_dim1, size(idx_local)))
        allocate(buff_put_sp(ndim3, len_dim1, size(idx_local)))
        call find_output_var_3d(ptr%var, ptr%var_level, idx_local, temp_var, ndim3)
        buff_put_sp(:,:,:) = temp_var(:,:,:)
        start(3) = mystart+1
        count(3) = mycount
        start(2) = 1
        count(2) = len_dim1
        start(1) = 1
        count(1) = ndim3
        call wrap_put_vara_real_req(ncid, ptr%var_idlist, start, count, buff_put_sp, req)
        err = nfmpi_wait_all(ncid, 1, req, st)
        deallocate(buff_put_sp)
        deallocate(temp_var)
        ptr => ptr%next
      ...
      return
    end subroutine wrap_output_3d_unordered_sp
...
! local
...
#ifdef Resnet
     real(4),          allocatable             :: local_data_r4r(:,:,:)
     real(8),          allocatable             :: local_data_r8r(:,:,:)
#endif 
     ...
     if (option .le. 6) then
       ...
       call wrap_inq_vartype(ncid, varid, xtype)
       start = 1
       !-------------------Wangym add for other data levels-----------------
       if (xtype .eq. NF_DOUBLE) then
       !-------------------Wangym add for other data levels------------------
         if (option .eq. 8) then
            count(1)=60
            count(2)=128
            count(3)=128
            allocate(local_data_r8r(count(1),count(2),count(3)))
            call wrap_get_vara_realx(ncid, varid, start, count, local_data_r8r)
       ...
       !-------------------LiXH add for other data levels------------------
         if (option .eq. 8) then
            ...
            count(2)=128
            count(3)=128
            allocate(local_data_r4r(count(1),count(2),count(3)))
            call wrap_get_vara_real(ncid, varid, start, count, local_data_r4r)
         else if(option .eq. 9)then
            count(1)=3
            count(2)=5
            count(3)=128
