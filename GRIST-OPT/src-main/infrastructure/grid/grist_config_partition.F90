...
! Yi Zhang, 2017, SEP
!================================================
#define PART_ENDIAN 'little_endian'
! #define CHK_ALL
...
contains
  ...
  subroutine search_io(gmesh, local_block, comm)
    ...
    if (.not. read_partition) then
      ...
      if(iblock .eq. 0) then
        open(101, file=trim(pardir)//"output-"//i2s(nprocs), form='unformatted', access="stream", convert=PART_ENDIAN)
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      allocate(buffer_ref(sums))
      if (mpi_rank() == 0) then
        print *, "counts=", counts
        print *, "displs=", displs
      endif
      call MPI_Scatterv(buffer_all, counts, displs, MPI_INTEGER, buffer_ref, &
                        sums, MPI_INTEGER, 0, local_block%comm, ierr)
#endif
      call MPI_Bcast(foff, 1, MPI_LONG, 0, local_block%comm,ierr)
      cdisp = displ*4_i8 + foff - 1
      ccnt = sums*4_i8
      call get_bin_range(buffer, trim(pardir)//"output-"//i2s(nprocs)//c_null_char, cdisp, ccnt)
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      print *, "buffer check=", all(buffer_ref == buffer)
      if (mpi_rank() == 0) then
        print *, cdisp + foff - 1, foff
        print *, "buffer check=", all(buffer_ref == buffer)
        print *, "buffer_all(:10)", buffer_all(:10)
        print *, "buffer_ref(:10)", buffer_ref(:10)
...
!    call write_output(local_block)
  ...
  subroutine search_compute(mesh,local_block)
    ...
    integer,               allocatable           :: counts(:), displs(:), buffer_all(:), buffer_ref(:)
    integer(i8)                                  :: cdisp, ccnt, foff
    integer                                      :: displ
    ...
    if ( .not. read_partition) then
      ...
      if(iblock .eq. 0) then
        open(101, file=trim(pardir)//"compute-"//i2s(nprocs), form='unformatted', access="stream", convert=PART_ENDIAN)
        allocate(counts(nprocs))
        read(101) counts 
        sums = sum(counts)
        inquire(101, pos=foff)
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      allocate(buffer_ref(compute%nv))
      call MPI_Scatterv(buffer_all, counts, displs, MPI_INTEGER, buffer_ref, &
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      print *, "compute check=", mpi_rank(), all(buffer_ref == compute%v_index(:compute%nv))
      if (mpi_rank() == 1) then
        print *, cdisp + foff - 1, foff
        print *, "buffer check=", all(buffer_ref == compute%v_index(:compute%nv))
        print *, "buffer_all(:10)", buffer_all(:10)
        print *, "buffer_ref(:10)", buffer_ref(:10)
        print *, "buffer(:10)", compute%v_index(:10)
      end if
#endif
      deallocate(counts)
      deallocate(displs)
  ...
  subroutine search_bdry_halo(mesh, local_block)
    ...
    integer,               allocatable           :: counts(:), displs(:), ele3(:), &
                                                    ele3_all(:), buffer_all(:), buffer(:), buffer_ref(:)
    ...
    if ( .not. read_partition) then
      ...
      if(iblock .eq. 0) then
        open(101, file=trim(pardir)//"full-"//i2s(nprocs), form='unformatted', access="stream", convert=PART_ENDIAN)
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      print *, "full check=", all(buffer_ref == buffer)
      if (mpi_rank() == 1) then
        print *, cdisp + foff - 1, foff
        print *, "buffer check=", all(buffer_ref == buffer)
        ! print *, "buffer_all(:10)", buffer_all(:10)
...
!
  ...
  subroutine search_exchange(mesh,local_block)
    ...
    integer,               allocatable           :: counts(:), displs(:), nbcpus(:), ele6(:), &
                                                    ...
                                                    counts_idx(:), displs_idx(:), buffer_ref(:)
    integer(i8)                                  :: cdisp, ccnt, foff
    integer                                      :: displ
    iblock = local_block%cpuid
    nprocs = local_block%nprocs
    if (.not. read_partition) then
      ...
      if(iblock .eq. 0) then
        open(1, file=trim(pardir)//"comm-"//i2s(nprocs), form='unformatted', access="stream",convert=PART_ENDIAN)
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      allocate(buffer_ref(sums))
      call MPI_Scatterv(buffer_all, counts_idx, displs_idx, MPI_INTEGER, buffer_ref, &
                        sums, MPI_INTEGER, 0, local_block%comm, ierr)
#endif
...
#if defined(CHK_PARTREAD) || defined(CHK_ALL)
      print *, "comm check=", all(buffer_ref == buffer)
      if (mpi_rank() == 1) then
        print *, cdisp + foff - 1, foff
        print *, "buffer check=", all(buffer_ref == buffer)
        ! print *, "buffer_all(:10)", buffer_all(:10)
...
call t_startf("exchange1d_make_pack")
    !get recv_data value
    do c = 1, local_block%nnb
      tmpf => field_head
      iv_list=0; it_list=0; ie_list=0
...
call t_stopf("exchange1d_make_pack")
...
#define EXCHANGE2D_OPT
#ifdef EXCHANGE2D_OPT
  subroutine exchange_data_2d(local_block,field_head)
    ...
    integer,  allocatable                                :: reqs_send(:), reqs_recv(:), reqs(:)
    integer                                              :: ierr, c, iv, ie, it, dst_s, dst_r
    integer                                              :: i_list, iblock, iv_list, it_list, ie_list, n, nr, ns, ip, ix, jx
    integer                                              :: list_n, tmp_n, n_v, n_t, n_e, list_nv, list_nt, list_ne
    integer                                              :: maxdim1, sizedim1

    INTEGER,  allocatable                                :: status(:,:) !STATUS(MPI_STATUS_SIZE)

    real(r8), allocatable, target                        :: field_data_nv_send(:,:,:), my_field_data_nv_send(:,:,:)!bdry
    real(r8), allocatable, target                        :: field_data_ne_send(:,:,:), my_field_data_ne_send(:,:,:)
    real(r8), allocatable, target                        :: field_data_nt_send(:,:,:), my_field_data_nt_send(:,:,:)
! #include <data_check.inc>
    real(r8), allocatable, target                        :: field_data_nv_recv(:,:,:)!halo
    real(r8), allocatable, target                        :: field_data_ne_recv(:,:,:)
    real(r8), allocatable, target                        :: field_data_nt_recv(:,:,:)
    type(exchange_field_list_2d), pointer                :: tmpf
    type :: pack_task
      ! type(scalar_2d_field), pointer :: field
      real(r8), pointer :: buffer(:,:), field(:,:)
      integer(i4), pointer :: list(:)
      integer(i4) :: dim1, n
    end type pack_task
    type(pack_task), allocatable                         :: pack_tasks(:), unpack_tasks(:)
    integer(i4)                                          :: npack, nunpack, row, col

    call t_startf("exchange2d")
    call t_startf("exchange2d_init_alloc")
    tmpf => field_head
    maxdim1 = 0
    do while(associated(tmpf))
    ...
    if(list_nv.gt.0) then
      allocate(field_data_nv_send(maxdim1,list_nv*local_block%max_nv_send,local_block%nnb))
      allocate(field_data_nv_recv(maxdim1,list_nv*local_block%max_nv_recv,local_block%nnb))
      !$omp target
      !$omp parallel workshare
      field_data_nv_send(:,:,:) = 0
      !$omp end parallel workshare
      !$omp end target
    endif
    if(list_nt.gt.0) then
      allocate(field_data_nt_send(maxdim1,list_nt*local_block%max_nt_send,local_block%nnb))
      allocate(field_data_nt_recv(maxdim1,list_nt*local_block%max_nt_recv,local_block%nnb))
      !$omp target
      !$omp parallel workshare
      field_data_nt_send(:,:,:) = 0
      !$omp end parallel workshare
      !$omp end target
    endif
    if(list_ne.gt.0) then
      allocate(field_data_ne_send(maxdim1,list_ne*local_block%max_ne_send,local_block%nnb))
      allocate(field_data_ne_recv(maxdim1,list_ne*local_block%max_ne_recv,local_block%nnb))
      !$omp target
      !$omp parallel workshare
      field_data_ne_send(:,:,:) = 0
      !$omp end parallel workshare
      !$omp end target
    endif
    ! if(list_nv.gt.0) allocate(my_field_data_nv_send(maxdim1,list_nv*local_block%max_nv_send,local_block%nnb), source=0._r8)
    ! if(list_nt.gt.0) allocate(my_field_data_nt_send(maxdim1,list_nt*local_block%max_nt_send,local_block%nnb), source=0._r8)
    ! if(list_ne.gt.0) allocate(my_field_data_ne_send(maxdim1,list_ne*local_block%max_ne_send,local_block%nnb), source=0._r8)
    ! print *, maxdim1, list_nv*local_block%max_nv_send
    ! call data_check(my_field_data_nv_send, field_data_nv_send)
    !assign send_data value
    allocate(reqs_send(local_block%nnb * n))
    allocate(reqs_recv(local_block%nnb * n))
    allocate(status(MPI_STATUS_SIZE, local_block%nnb*n))

    allocate(pack_tasks(local_block%nnb*list_n))
    call t_stopf("exchange2d_init_alloc")
    call t_startf("exchange2d_make_pack")
    npack = 0
    do c = 1, local_block%nnb
      dst_s = local_block%send_sites(c)%cpuid
      dst_r = local_block%recv_sites(c)%cpuid
      ...
      do i_list = 0, list_n-1
        ...
        case(0)
          n_v = local_block%send_sites(c)%nv
          sizedim1 = tmpf%dim1
          pack_tasks(npack)%field  => tmpf%field_data%f
          pack_tasks(npack)%buffer => field_data_nv_send(:,n_v*iv_list+1:,c)
          pack_tasks(npack)%list   => local_block%send_sites(c)%local_v(:)
          pack_tasks(npack)%n      =  n_v
          pack_tasks(npack)%dim1   =  sizedim1
          iv_list = iv_list + 1
        !nt
        case(1)
          n_t = local_block%send_sites(c)%nt
          sizedim1 = tmpf%dim1
          pack_tasks(npack)%field  => tmpf%field_data%f(:,:)
          pack_tasks(npack)%buffer => field_data_nt_send(:,n_t*it_list+1:,c)
          pack_tasks(npack)%list   => local_block%send_sites(c)%local_t(:)
          pack_tasks(npack)%n      =  n_t
          pack_tasks(npack)%dim1   =  sizedim1
          it_list = it_list + 1
        !ne
        case(6)
          ...
          sizedim1 = tmpf%dim1
          pack_tasks(npack)%field  => tmpf%field_data%f
          pack_tasks(npack)%buffer => field_data_ne_send(:,n_e*ie_list+1:,c)
          pack_tasks(npack)%list   => local_block%send_sites(c)%local_e(:)
          pack_tasks(npack)%n      =  n_e
          pack_tasks(npack)%dim1   =  sizedim1
          ie_list = ie_list + 1
        case default
          print*,"Invalid scalar field data type"
    ...
    call t_stopf("exchange2d_make_pack")
    call t_startf("exchange2d_pack")
    !$omp target
    !$omp parallel private(row, col, ip, ix, tmp_n)
    row = omp_get_thread_num() / 8
    col = MOD(omp_get_thread_num(), 8)
    do ip = row+1, npack, 8
      do ix = col+1, pack_tasks(ip)%n, 8
        tmp_n = pack_tasks(ip)%list(ix)
        pack_tasks(ip)%buffer(1:pack_tasks(ip)%dim1,ix) = pack_tasks(ip)%field(1:pack_tasks(ip)%dim1, tmp_n)
      end do
    end do
    !$omp end parallel
    !$omp end target
    call t_stopf("exchange2d_pack")

    ns=1; nr=1
    call t_startf("exchange2d_comm")
    do c = 1, local_block%nnb
      dst_s = local_block%send_sites(c)%cpuid
      dst_r = local_block%recv_sites(c)%cpuid
      if (list_nv .gt. 0) then
    ...
    call t_stopf("exchange2d_comm")
    call t_startf("exchange2d_make_unpack")
    !get recv_data value
    allocate(unpack_tasks(local_block%nnb*list_n))
    nunpack = 0
    do c = 1, local_block%nnb
      tmpf => field_head
      iv_list=0; it_list=0; ie_list=0;
      do i_list = 0, list_n-1
        ...
        case(0)
          n_v = local_block%recv_sites(c)%nv
          sizedim1 = tmpf%dim1
          unpack_tasks(nunpack)%field  => tmpf%field_data%f
          unpack_tasks(nunpack)%buffer => field_data_nv_recv(:, n_v*iv_list+1:, c)
          unpack_tasks(nunpack)%list   => local_block%recv_sites(c)%local_v
          unpack_tasks(nunpack)%n      =  n_v
          unpack_tasks(nunpack)%dim1   =  sizedim1
          iv_list = iv_list + 1
        !nt
        case(1)
          n_t = local_block%recv_sites(c)%nt
          sizedim1 = tmpf%dim1
          unpack_tasks(nunpack)%field  => tmpf%field_data%f
          unpack_tasks(nunpack)%buffer => field_data_nt_recv(:, n_t*it_list+1:, c)
          unpack_tasks(nunpack)%list   => local_block%recv_sites(c)%local_t
          unpack_tasks(nunpack)%n      =  n_t
          unpack_tasks(nunpack)%dim1   =  sizedim1
          it_list = it_list + 1
        !ne
        case(6)
          ...
          sizedim1 = tmpf%dim1
          unpack_tasks(nunpack)%field  => tmpf%field_data%f
          unpack_tasks(nunpack)%buffer => field_data_ne_recv(:, n_e*ie_list+1:, c)
          unpack_tasks(nunpack)%list   => local_block%recv_sites(c)%local_e
          unpack_tasks(nunpack)%n      =  n_e
          unpack_tasks(nunpack)%dim1   =  sizedim1
          ie_list = ie_list + 1
        case default
          print*,"Invalid scalar field data type"
    ...
    call t_stopf("exchange2d_make_unpack")
    call t_startf("exchange2d_wait")
    call MPI_WaitAll(n*local_block%nnb, reqs_send, status, ierr)
    call MPI_WaitAll(n*local_block%nnb, reqs_recv, status, ierr)
    call t_stopf("exchange2d_wait")
    
    call t_startf("exchange2d_unpack")
    !$omp target
    !$omp parallel private(row, col, ip, ix, tmp_n)
    row = omp_get_thread_num() / 8
    col = MOD(omp_get_thread_num(), 8)
    do ip = row+1, nunpack, 8
      do ix = col+1, unpack_tasks(ip)%n, 8
        tmp_n = unpack_tasks(ip)%list(ix)
        unpack_tasks(ip)%field(1:unpack_tasks(ip)%dim1,tmp_n) = unpack_tasks(ip)%buffer(1:unpack_tasks(ip)%dim1,ix)
      end do
    end do
    !$omp end parallel
    !$omp end target
    call t_stopf("exchange2d_unpack")

    call t_startf("exchange2d_clean")
    ...
    deallocate(reqs_recv)
    deallocate(status)
    deallocate(pack_tasks)
    deallocate(unpack_tasks)
    call t_stopf("exchange2d_clean")
    call t_stopf("exchange2d")
...
#define EXCHANGE3D_OPT
#if defined(EXCHANGE3D_OPT)
  subroutine exchange_data_3d(local_block,field_head)
    ...
    real(r8),allocatable, target                                 :: field_data_nv_send(:,:,:,:)!bdry
    real(r8),allocatable, target                                 :: field_data_ne_send(:,:,:,:)
    real(r8),allocatable, target                                 :: field_data_nt_send(:,:,:,:)
    ...
    type :: pack_task
      real(r8), pointer :: buffer(:,:,:), field(:,:,:)
      integer(i4), pointer :: list(:)
      integer(i4) :: dim1, dim2, n, n_list
    end type pack_task
    type(pack_task), allocatable                         :: pack_tasks(:), unpack_tasks(:)
    integer(i4)                                          :: npack, nunpack, row, col

    call t_startf("exchange3d_init_alloc")
    ...
    if(list_nv.gt.0) then
      allocate(field_data_nv_send(maxdim1,maxdim2,list_nv*local_block%max_nv_send,local_block%nnb))
      allocate(field_data_nv_recv(maxdim1,maxdim2,list_nv*local_block%max_nv_recv,local_block%nnb))
!$omp target
!$omp parallel workshare
      field_data_nv_send(:,:,:,:) = 0
!$omp end parallel workshare
!$omp end target
    ...
    if(list_nt.gt.0) then
      allocate(field_data_nt_send(maxdim1,maxdim2,list_nt*local_block%max_nt_send,local_block%nnb))
      allocate(field_data_nt_recv(maxdim1,maxdim2,list_nt*local_block%max_nt_recv,local_block%nnb))
!$omp target
!$omp parallel workshare
      field_data_nt_send(:,:,:,:) = 0
!$omp end parallel workshare
!$omp end target
    ...
    if(list_ne.gt.0) then
      allocate(field_data_ne_send(maxdim1,maxdim2,list_ne*local_block%max_ne_send,local_block%nnb))
      allocate(field_data_ne_recv(maxdim1,maxdim2,list_ne*local_block%max_ne_recv,local_block%nnb))
!$omp target
!$omp parallel workshare
      field_data_ne_send(:,:,:,:) = 0
!$omp end parallel workshare
!$omp end target
    ...
    call t_stopf("exchange3d_init_alloc")

    call t_startf("exchange3d_make_pack")
    allocate(pack_tasks(local_block%nnb*list_n))
    npack = 0
    do c = 1, local_block%nnb
      dst_s = local_block%send_sites(c)%cpuid
      ...
      do i_list = 0, list_n-1
        ...
        case(0)
          ...
          ! do iv = 1, n_v
            ...
            pack_tasks(npack)%n      =  n_v
            pack_tasks(npack)%dim1   =  sizedim1
            pack_tasks(npack)%dim2   =  sizedim2
            pack_tasks(npack)%n_list = iv_list
          ! end do
          iv_list = iv_list + 1
        !nt
        case(1)
          ...
          pack_tasks(npack)%dim1   =  sizedim1
          pack_tasks(npack)%dim2   =  sizedim2
          pack_tasks(npack)%n_list = it_list
          it_list = it_list + 1
        !ne
        case(6)
          n_e = local_block%send_sites(c)%ne
          ...
          pack_tasks(npack)%dim1   =  sizedim1
          pack_tasks(npack)%dim2   =  sizedim2
          pack_tasks(npack)%n_list = ie_list
          ie_list = ie_list + 1
        case default
          print*,"Invalid scalar field data type"
        end select
    ...
    call t_stopf("exchange3d_make_pack")
    call t_startf("exchange3d_pack")


!$omp target
!$omp parallel private(row, col, ip, ix, tmp_n, sizedim1, sizedim2)
    row = omp_get_thread_num() / 8
    col = MOD(omp_get_thread_num(), 8)
    do ip = row+1, npack,8
      do ix = col+1, pack_tasks(ip)%n,8
        tmp_n = pack_tasks(ip)%list(ix)
        sizedim1 = pack_tasks(ip)%dim1
        sizedim2 = pack_tasks(ip)%dim2
        ! write(*,*) lbound(pack_tasks(ip)%buffer, 1), ubound(pack_tasks(ip)%buffer, 1),lbound(pack_tasks(ip)%buffer, 3), ubound(pack_tasks(ip)%buffer, 3), pack_tasks(ip)%n
         pack_tasks(ip)%buffer(1:sizedim1,1:sizedim2,ix) = pack_tasks(ip)%field(1:sizedim1,1:sizedim2,tmp_n)
      enddo
    enddo 
!$omp end parallel
!$omp end target
...
! !       do ix = col+1, pack_tasks(ip)%n, 8
! !         tmp_n = pack_tasks(ip)%list(ix)
! !         pack_tasks(ip)%buffer(1:pack_tasks(ip)%dim1,1:pack_tasks(ip)%dim2, ix) = pack_tasks(ip)%field(1:pack_tasks(ip)%dim1, 1:pack_tasks(ip)%dim2, tmp_n)
! !       end do
! !     end do
! ! !$omp end parallel
! ! !$omp end target

    call t_stopf("exchange3d_pack")
    ...
    call t_stopf("exchange3d_comm")

    call t_startf("exchange3d_make_unpack")
    !get recv_data value
    allocate(unpack_tasks(local_block%nnb*list_n))
    nunpack = 0
    do c = 1, local_block%nnb
      ...
      do i_list = 0, list_n-1
        ...
        case(0)
          ...
          unpack_tasks(nunpack)%n      =  n_v
          unpack_tasks(nunpack)%dim1   =  sizedim1
          unpack_tasks(nunpack)%dim2   =  sizedim2
          unpack_tasks(nunpack)%n_list = iv_list
        ...
        case(1)
          ...
          unpack_tasks(nunpack)%dim1   =  sizedim1
          unpack_tasks(nunpack)%dim2   =  sizedim2
          unpack_tasks(nunpack)%n_list = it_list
        ...
        case(6)
          ...
          unpack_tasks(nunpack)%dim1   =  sizedim1
          unpack_tasks(nunpack)%dim2   =  sizedim2
          unpack_tasks(nunpack)%n_list = ie_list
    ...
    call t_stopf("exchange3d_make_unpack")
    call t_startf("exchange3d_wait")

    call MPI_WaitAll(n*local_block%nnb, reqs_send, status, ierr)
    call MPI_WaitAll(n*local_block%nnb, reqs_recv, status, ierr)
    call t_stopf("exchange3d_wait")

    call t_startf("exchange3d_unpack")
...
!$omp parallel private(row, col, ip, ix, tmp_n, sizedim1, sizedim2)
    ...
    do ip = row + 1, nunpack, 8
      do ix = col + 1, unpack_tasks(ip)%n, 8
        tmp_n = unpack_tasks(ip)%list(ix)
        sizedim1 = unpack_tasks(ip)%dim1
        sizedim2 = unpack_tasks(ip)%dim2
        unpack_tasks(ip)%field(1:sizedim1,1:sizedim2,tmp_n) = unpack_tasks(ip)%buffer(1:sizedim1,1:sizedim2,ix)
      enddo
    enddo 
!$omp end parallel
!$omp end target
...
!       do ix = col+1, unpack_tasks(ip)%n, 8
!         tmp_n = unpack_tasks(ip)%list(ix)
!         unpack_tasks(ip)%field(1:unpack_tasks(ip)%dim1,1:unpack_tasks(ip)%dim2, tmp_n) = unpack_tasks(ip)%buffer(1:unpack_tasks(ip)%dim1,1:unpack_tasks(ip)%dim2, ix)
!       end do
!     end do
! !$omp end parallel
! !$omp end target
    call t_stopf("exchange3d_unpack")
    ...
    !   end do
    ! end do
    call t_startf("exchange3d_clean")
  ...
  subroutine exchange_data_3d(local_block,field_head)
    ...
    ns=1; nr=1
    call t_startf("exchange3d_pack_comm")
    ...
    call t_stopf("exchange3d_pack_comm")
