...
! #include <data_check.inc>
    ...
    if(list_nv.gt.0) then
      ...
      call t_startf("exchange_data_2d_copy_1")
      !$omp target
      !$omp parallel workshare
      field_data_nv_send(:,:,:) = 0
      !$omp end parallel workshare
      !$omp end target
      call t_stopf("exchange_data_2d_copy_1")
    ...
    if(list_nt.gt.0) then
      ...
      call t_startf("exchange_data_2d_copy_2")
      !$omp target
      !$omp parallel workshare
      field_data_nt_send(:,:,:) = 0
      !$omp end parallel workshare
      !$omp end target
      call t_stopf("exchange_data_2d_copy_2")
    ...
    if(list_ne.gt.0) then
      ...
      call t_startf("exchange_data_2d_copy_3")
      !$omp target
      !$omp parallel workshare
      field_data_ne_send(:,:,:) = 0
      !$omp end parallel workshare
      !$omp end target
      call t_stopf("exchange_data_2d_copy_3")
...
#define EXCHANGE2D_OPT_R4
#ifdef EXCHANGE2D_OPT_R4
subroutine exchange_data_2d_r4(local_block,field_head)
  ...
  real(r4), allocatable, target                                :: field_data_nv_send(:,:,:)!bdry
  real(r4), allocatable, target                                :: field_data_ne_send(:,:,:)
  real(r4), allocatable, target                                :: field_data_nt_send(:,:,:)

  real(r4), allocatable, target                                :: field_data_nv_recv(:,:,:)!halo
  real(r4), allocatable, target                                :: field_data_ne_recv(:,:,:)
  real(r4), allocatable, target                                :: field_data_nt_recv(:,:,:)
  type(exchange_field_list_2d), pointer                :: tmpf
  type :: pack_task
  ! type(scalar_2d_field), pointer :: field
  real(r4), pointer :: buffer(:,:)
...
do c = 1, local_block%nnb
  ...
  do i_list = 0, list_n-1
    ...
    case(0)
      n_v = local_block%send_sites(c)%nv
      sizedim1 = tmpf%dim1
      pack_tasks(npack)%field  => tmpf%field_data%f_r4
      pack_tasks(npack)%buffer => field_data_nv_send(:,n_v*iv_list+1:,c)
      pack_tasks(npack)%list   => local_block%send_sites(c)%local_v(:)
      pack_tasks(npack)%n      =  n_v
      pack_tasks(npack)%dim1   =  sizedim1
...
do c = 1, local_block%nnb
  ...
  do i_list = 0, list_n-1
    ...
    case(0)
      n_v = local_block%recv_sites(c)%nv
      sizedim1 = tmpf%dim1
      unpack_tasks(nunpack)%field  => tmpf%field_data%f_r4
      unpack_tasks(nunpack)%buffer => field_data_nv_recv(:, n_v*iv_list+1:, c)
      unpack_tasks(nunpack)%list   => local_block%recv_sites(c)%local_v
      unpack_tasks(nunpack)%n      =  n_v
      unpack_tasks(nunpack)%dim1   =  sizedim1
...
call t_startf("exchange_data_3d_allocate")
    if(list_nv.gt.0) then
...
!$omp end target
    ...
    call t_stopf("exchange_data_3d_allocate")
...
#define EXCHANGE3D_OPT_R4
#if defined(EXCHANGE3D_OPT_R4)
