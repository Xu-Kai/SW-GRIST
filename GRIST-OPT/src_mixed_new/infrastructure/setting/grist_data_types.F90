...
CONTAINS
  ...
  subroutine wrap_allocate_data1d(ncell,var)
      ...
      if(.not.allocated(var%f)) allocate(var%f(max(ncell, MAGIC)))
      var%f    = zero
      var%pos  = 0
      return
  ...
  subroutine wrap_allocate_data2d(ncell,nLevel,var)
      ...
      if(.not.allocated(var%f)) allocate(var%f(nLevel,max(ncell, MAGIC)))
      var%f    = zero
      var%pos  = 0
      return
  ...
  subroutine wrap_allocate_data3d(ncell,nLevel,ntracer,var)
      ...
      if(.not.allocated(var%f)) allocate(var%f(ntracer,nLevel,max(ncell, MAGIC)))
      var%f    = zero
      var%pos  = 0
      return
