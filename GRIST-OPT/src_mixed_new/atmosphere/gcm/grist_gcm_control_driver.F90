...
! dycore
!!!   use grist_dycore_time_integration_2d_mixed, only:dycore_time_integration_run
...
! dycore-diffusion
   use grist_dycore_diffusion_module_mixed,    only: grist_diffusion_init
! dycore-hpe
...
! Tracer Transport
   ...
   use grist_tracer_transport_time_integration_hvsplit_mixed, only: tracer_transport_time_integration_init_r8
...
!
     ...
     if(mpi_rank() == 0) print *,"grist_dycore_vars_construct"
!
! tracer data
!
     call tracer_transport_vars_construct(local_block%full_domain)
     if(mpi_rank() == 0) print *,"tracer_transport_vars_construct"
!
...
! tracer transport
!
     call tracer_transport_time_integration_init_r8(local_block%full_domain)
...
!---------------------------------------------------
 ...
 subroutine grist_gcm_control_run_amipc(mesh)
   ...
   logical      :: is_first_restart
    ...
    print *, "this is mixed, not test for amipc"
     call mpi_abort()
! local
...
!
  ...
  subroutine grist_gcm_final(mesh)
...
#ifdef MIXCODE
   use grist_dycore_diffusion_module_mixed,      only: grist_diffusion_destruct_mixed=>grist_diffusion_destruct
#else
   ...
   use grist_tracer_transport_time_integration_hvsplit_mixed, only: tracer_transport_time_integration_final
