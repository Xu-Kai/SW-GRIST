...
!+---+-----------------------------------------------------------------+
!
 MODULE module_mp_thompson_v381

#ifdef AMIPW_PHYSICS
      use module_mp_radar_v381
      use grist_handle_error, only: showinfo, endrun, grist_message
      use grist_mpi
      use grist_wrfphys_nml_module, only: force_read_thompson, write_thompson_tables
#else
...
!+---+-----------------------------------------------------------------+
      ...
      enddo
#ifdef AMIPW_PHYSICS
      CALL showinfo('CREATING MICROPHYSICS LOOKUP TABLES ... ', 150)
      WRITE (grist_message, '(a, f5.2, a, f5.2, a, f5.2, a, f5.2)') &
          ' using: mu_c=',mu_c,' mu_i=',mu_i,' mu_r=',mu_r,' mu_g=',mu_g
      CALL showinfo(grist_message, 150)
#else
...
!.. further changes by Eidhammer and Kriedenweis.
      if (is_aerosol_aware) then
#ifdef AMIPW_PHYSICS
         CALL showinfo('  calling table_ccnAct routine', 200)
#else
...
#ifdef AMIPW_PHYSICS
      CALL showinfo('  creating qc collision eff tables', 200)
#else
...
!..Drop evaporation.
#ifdef AMIPW_PHYSICS
      CALL showinfo('  creating rain evap table', 200)
#else
...
#ifdef AMIPW_PHYSICS
      CALL showinfo('  creating rain collecting graupel table', 200)
#else
...
#ifdef AMIPW_PHYSICS
      CALL showinfo('  creating rain collecting snow table', 200)
#else
...
#ifdef AMIPW_PHYSICS
      CALL showinfo('  creating freezing of water drops table',200)
#else
...
#ifdef AMIPW_PHYSICS
      CALL showinfo('  creating ice converting to snow table', 200)
#else
...
#ifdef AMIPW_PHYSICS
      CALL showinfo(' ... DONE microphysical lookup tables', 150)
#else
...
!+---+
      ...
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF ( lexist ) THEN
#ifdef AMIPW_PHYSICS
          CALL showinfo("ThompMP: read qr_acr_qg.dat stead of computing")
#else
 ...
 1234     CONTINUE
          IF ( good .NE. 1 ) THEN
...
#ifdef AMIPW_PHYSICS
                CALL endrun("Error reading qr_acr_qg.dat. Aborting because force_read_thompson is .true.")
#else
            ...
            ELSE
              IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
                CALL endrun("Error opening qr_acr_qg.dat. Aborting because force_read_thompson is .true.")
#else
        ...
        ELSE
          IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
            CALL endrun("Non-existent qr_acr_qg.dat. Aborting because force_read_thompson is .true.")
#else
...
#ifdef AMIPW_PHYSICS
      CALL mpi_bcast(good,  1,  MPI_INTEGER,0, comm,ierr)
#else
#if defined(DM_PARALLEL) && !defined(STUBMPI)
      ...
      IF ( good .EQ. 1 ) THEN
#ifdef AMIPW_PHYSICS
        CALL mpi_bcast(tcg_racg,SIZE(tcg_racg), MPI_REAL8,  0, comm,ierr)
        CALL mpi_bcast(tmr_racg,SIZE(tmr_racg), MPI_REAL8,  0, comm,ierr)
        CALL mpi_bcast(tcr_gacr,SIZE(tcr_gacr), MPI_REAL8,  0, comm,ierr)
        CALL mpi_bcast(tmg_gacr,SIZE(tmg_gacr), MPI_REAL8,  0, comm,ierr)
        CALL mpi_bcast(tnr_racg,SIZE(tnr_racg), MPI_REAL8,  0, comm,ierr)
        CALL mpi_bcast(tnr_gacr,SIZE(tnr_gacr), MPI_REAL8,  0, comm,ierr)
#else
#if defined(DM_PARALLEL) && !defined(STUBMPI)
      ...
      ELSE
#ifdef AMIPW_PHYSICS
        CALL showinfo("ThompMP: computing qr_acr_qg")
#else
...
! to be added
...
#ifdef AMIPW_PHYSICS
        IF ( write_thompson_tables .AND. mpi_rank().eq.0 ) THEN
          CALL showinfo("Writing qr_acr_qg.dat in Thompson MP init")
#else
 ...
 9234     CONTINUE
#ifdef AMIPW_PHYSICS
          CALL endrun("Error writing qr_acr_qg.dat")
#else
...
!+---+
      ...
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF ( lexist ) THEN
#ifdef AMIPW_PHYSICS
          CALL showinfo("ThompMP: read qr_acr_qs.dat instead of computing")
#else
 ...
 1234     CONTINUE
          IF ( good .NE. 1 ) THEN
            ...
            IF (lopen) THEN
              IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
                CALL endrun("Error reading qr_acr_qs.dat. Aborting because force_read_thompson is .true.")
#else
            ...
            ELSE
              IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
                CALL endrun("Error opening qr_acr_qs.dat. Aborting because force_read_thompson is .true.")
#else
        ...
        ELSE
          IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
            CALL endrun("Non-existent qr_acr_qs.dat. Aborting because force_read_thompson is .true.")
#else
      ...
      ELSE
#ifdef AMIPW_PHYSICS
        CALL showinfo("ThompMP: computing qr_acr_qs")
#else
...
!..Note wrf_dm_gatherv expects zero-based km_s, km_e (J. Michalakes, 2009Oct30).
...
#ifdef AMIPW_PHYSICS
        IF ( write_thompson_tables .AND. mpi_rank().eq.0 ) THEN
          CALL showinfo("Writing qr_acr_qs.dat in Thompson MP init")
#else
        IF ( write_thompson_tables .AND. wrf_dm_on_monitor() ) THEN
 ...
 9234     CONTINUE
#ifdef AMIPW_PHYSICS
          CALL endrun("Error writing qr_acr_qs.dat")
#else
...
!+---+
      ...
      IF ( wrf_dm_on_monitor() ) THEN
        ...
        IF ( lexist ) THEN
#ifdef AMIPW_PHYSICS
          CALL showinfo("ThompMP: read freezeH2O.dat stead of computing")
#else
 ...
 1234     CONTINUE
          IF ( good .NE. 1 ) THEN
            ...
            IF (lopen) THEN
              IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
                CALL endrun("Error reading freezeH2O.dat. Aborting because force_read_thompson is .true.")
#else
            ...
            ELSE
              IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
                CALL endrun("Error opening freezeH2O.dat. Aborting because force_read_thompson is .true.")
#else
        ...
        ELSE
          IF( force_read_thompson ) THEN
#ifdef AMIPW_PHYSICS
            CALL endrun("Non-existent freezeH2O.dat. Aborting because force_read_thompson is .true.")
#else
      ...
      ELSE
#ifdef AMIPW_PHYSICS
        CALL showinfo("ThompMP: computing freezeH2O")
#else
...
!         print*, ' Freezing water for temp = ', -k
...
#ifdef AMIPW_PHYSICS
        IF ( write_thompson_tables .AND. mpi_rank().eq.0 ) THEN
          CALL showinfo("Writing freezeH2O.dat in Thompson MP init")
#else
        IF ( write_thompson_tables .AND. wrf_dm_on_monitor() ) THEN
 ...
 9234     CONTINUE
#ifdef AMIPW_PHYSICS
          CALL endrun("Error writing freezeH2O.dat")
#else
...
#ifdef AMIPW_PHYSICS
        CALL endrun ( 'module_mp_thompson: table_ccnAct: '//   &
#else
...
#ifdef AMIPW_PHYSICS
      IF ( mpi_rank().eq.0 ) READ(iunit_mp_th1,ERR=9010) tnccn_act
#else
...
!+---+-----------------------------------------------------------------+
 END MODULE module_mp_thompson_v381
!+---+-----------------------------------------------------------------+
