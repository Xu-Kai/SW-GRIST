...
!
module module_bl_shinhong_v381
!
contains
!
!-------------------------------------------------------------------------------
...
!
   ...
   ! SSW change scalar to 1D array
   real,     dimension( ims:ime), intent(in   )   ::      dx,dy
!
...
!
   ...
   ! SSW change from scalar to array
   real,     dimension( its:ite )            ::                         delxy
...
!
   do i = its,ite
     delxy(i)=sqrt(dx(i)*dy(i))
   enddo
!
   do i = its,ite
     pu1=pu(delxy(i),cslen(i))
     pq1=pq(delxy(i),cslen(i))
     if(pblflg(i)) then
       hgamu(i) = hgamu(i)*pu1
       hgamv(i) = hgamv(i)*pu1
       hgamq(i) = hgamq(i)*pq1
...
!
       ...
       delxy(i)=sqrt(dx(i)*dy(i))*max(zfacdx,1.0)
       pth1=pthl(delxy(i),hpbl(i))
       if(pblflg(i).and.k.lt.kpbl(i)) then
         au(i,k) = au(i,k)*pth1
         al(i,k) = al(i,k)*pth1
       endif
...
! SSW add this if sentences according to YSU PBL, to deal with optional input ctopo
...
!
   do i = its,ite
     if(present(ctopo).and.present(ctopo2)) then ! SSW add according to YSUV381
...
!
     call prodq2(lmh,dt,ust(i),s2,rig,q2xk,el,zqk,akmk,akhk                    &
                     ,uxk,vxk,thxk,thvxk                                       &
                     ,hgamu(i),hgamv(i),hgamq(i),delxy(i)                      &
...
!
!-------------------------------------------------------------------------------
end module module_bl_shinhong_v381
!-------------------------------------------------------------------------------
