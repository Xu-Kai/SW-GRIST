...
!-------------------------------------------------------------
   SUBROUTINE GFDRV(                                            &
              ...
              ,rqvblten,rthblten                                &
              ,F_QV    ,F_QC    ,F_QR    ,F_QI    ,F_QS, ndc, omega_hdc&
#if ( WRF_DFI_RADAR == 1 )
...
!                   ! HBOT>HTOP follow physics leveling convention
   ...
   logical, intent(in) :: ndc 
   real, intent(in)    :: omega_hdc( ims:ime , kms:kme , jms:jme )
...
!
   ...
   REAL    :: tcrit,tscl_KF(its:ite),dp,dq,sub_spread,subcenter
...
!   write(0,*)'ishallow = ',ishallow_g3
   ...
   IF ( periodic_x ) THEN
      ...
      iendc=min(ite,ide-1)
   ELSE
      ibeg =its !max(its,ids)
      iend =ite !min(ite,ide-1)
      ibegc=its !max(its,ids+4)
      iendc=ite !min(ite,ide-5)
   END IF
   IF ( periodic_y ) THEN
      jbeg=max(jts,jds)
      jend=min(jte,jde-1)
      ...
      jendc=min(jte,jde-1)
   ELSE
      jbeg =jts !max(jts,jds)
      jend =jte !min(jte,jde-1)
      jbegc=jts !max(jts,jds+4)
      jendc=jte !min(jte,jde-5)
   END IF
   do j=jts,jte
   do i=its,ite
     k22_shallow(i,j)=0
   ...
   itf=ite !MIN(ite,ide-1)
   ktf=kte !MIN(kte,kde-1)
   jtf=jte !MIN(jte,jde-1)
!                                                                      
     DO 100 J = jts,jtf  
     DO n= 1,ensdim
     DO I= its,itf
...
!    if(j.eq.jpr .and. (ipr.gt.its .and. ipr.lt.itf))write(0,*)psur(ipr),ter11(ipr),kpbli(ipr)
     ...
     if(ndc)then
        ! corrected as in mpas
        !DO K=kts,ktf
        DO K=kts+1,ktf ! 2->nlev, grist face level
        DO I=ITS,ITF
           !omeg(I,K,:)= -g*rho(i,k,j)*w(i,k,j)
            omeg(I,K,:)= -g*0.5*(rho(i,k,j)+rho(i,k-1,j))*w(i,k,j)
        enddo
        enddo
     else ! hydrostatic added
        DO K=kts+1,ktf
        DO I=ITS,ITF
           omeg(I,K,:)= 0.5*(omega_hdc(i,k,j)+omega_hdc(i,k-1,j))
        enddo
        enddo
     end if
...
!       kpbl
  ...
  !
     ...
     real, dimension (its:ite,1:ens4)                                         &
       ...
       real                                                            &
        ,intent (in   )                   ::                           &
        beta,dx(its:ite),ccnclean,dtime,tcrit,xl,cp,rv,g
...
!
  ...
  ! edt     = epsilon
     ...
     real                                 ::                           &
      day,dz,dzo,mbdt,entr_rate(its:ite),radius,entrd_rate(its:ite),mentrd_rate(its:ite),  &
      zcutdown,edtmax,edtmin,depth_min,zkbmax,z_detr,zktop,      &
      massfld,dh,cap_maxs,trash,frh,xlamdd(its:ite)
...
!--------------------------------------------------------------------
  ...
  !
     ...
     real, dimension (its:ite)                                         &
       ...
       real                                                            &
        ,intent (in   )                   ::                           &
        tscl_kf(its:ite),dx(its:ite),ccnclean,dtime,tcrit,xl,cp,rv,g
