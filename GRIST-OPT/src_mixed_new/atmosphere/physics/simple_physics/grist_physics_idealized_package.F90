...
#include "data_check.inc"
   real(r8), allocatable :: ref_CAm(:, :)
   real(r8), allocatable :: ref_CCm(:, :)
   real(r8), allocatable :: ref_CA(:,:)
   real(r8), allocatable :: ref_CC(:,:)
   real(r8), allocatable :: ref_CFu(:, :)
   real(r8), allocatable :: ref_CFv(:, :)
   real(r8), allocatable :: ref_CFt(:,:)
   real(r8), allocatable :: ref_CFq(:,:)
   real(r8), allocatable :: ref_dudt(:, :)
   real(r8), allocatable :: ref_dvdt(:, :)
   real(r8), allocatable :: ref_u(:,:)
   real(r8), allocatable :: ref_v(:,:)
   real(r8), allocatable :: ref_dtdt(:, :)
   real(r8), allocatable :: ref_t(:, :)
   real(r8), allocatable :: ref_dqdt(:,:)
   real(r8), allocatable :: ref_q(:,:)    
   real(r8), allocatable :: ref_za(:)
   real(r8), allocatable :: ref_zi(:,:)
   real(r8), allocatable :: ref_precl(:)
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_za, source=za)
allocate(ref_zi, source=zi)
#endif
call t_startf("Calculate_hydrostatic_height_target")
!$omp target
!$omp parallel  private(i,dlnpint)
!$omp do 
     do i=1,pcols
...
!$omp end parallel
!$omp end target 
call t_stopf("Calculate_hydrostatic_height_target")

#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Calculate_hydrostatic_height")
!$omp parallel  private(i,dlnpint)
!$omp do 
do i=1,pcols
   dlnpint = log(ps(i)) - log(pint(i,pver))  ! ps(i) is identical to pint(i,pver+1), note: this is the correct sign (corrects typo in JAMES paper) 
...
!$omp end do nowait
!$omp end parallel
call t_stopf("Calculate_hydrostatic_height")
call data_check(ref_za, za)
call data_check(ref_zi, zi)
deallocate(ref_za)
deallocate(ref_zi)
#endif
...
!
!===============================================================================
call t_startf("Set_initial_physics_time_target")
!$omp target
...
!$omp end target
call t_stopf("Set_initial_physics_time_target")
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_dtdt, source=dtdt)
allocate(ref_dqdt, source=dqdt)
allocate(ref_precl, source=precl)
#endif
call t_startf("Calculate_tendencies_target")
!$omp target
!$omp parallel  private(k,i,qsat,tmp)
!$omp do 
      do k=1,pver
         do i=1,pcols
...
!$omp end parallel
!$omp end target 
call t_stopf("Calculate_tendencies_target")
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Calculate_tendencies")
!$omp parallel  private(k,i,qsat,tmp)
!$omp do 
do k=1,pver
   do i=1,pcols
      ...
      if (q(i,k) > qsat) then
         tmp  = 1._r8/dtime*(q(i,k)-qsat)/(1._r8+(latvap/cpair)*(epsilo*latvap*qsat/(rair*t(i,k)**2)))
         ref_dtdt(i,k) = ref_dtdt(i,k)+latvap/cpair*tmp
         ref_dqdt(i,k) = ref_dqdt(i,k)-tmp
         ref_precl(i)  = ref_precl(i)+tmp*pdel(i,k)/(gravit*rhow)
      end if
   end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("Calculate_tendencies")
call data_check(ref_dtdt, dtdt)
call data_check(ref_dqdt, dqdt)
call data_check(ref_precl, precl)
deallocate(ref_dtdt)
deallocate(ref_dqdt)
deallocate(ref_precl)
#endif
...
#endif
call t_startf("Update_moisture_and_temperature_fields_target")
!$omp target
...
!$omp end parallel
!$omp end target 
call t_stopf("Update_moisture_and_temperature_fields_target")
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Update_moisture_and_temperature_fields")
!$omp parallel  private(k,i)
!$omp do 
do k=1,pver
   do i=1,pcols
      ref_t(i,k) =  ref_t(i,k) + dtdt(i,k)*dtime
      ref_q(i,k) =  ref_q(i,k) + dqdt(i,k)*dtime
   end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("Update_moisture_and_temperature_fields")
call data_check(ref_t, t)
call data_check(ref_q, q)
deallocate(ref_t)
deallocate(ref_q)
#endif
...
call t_startf("Compute_turbulence_scheme_target_1")     
   !    do i=1,pcols
...
!$omp end parallel
!$omp end target 
call t_stopf("Compute_turbulence_scheme_target_1")

     if (TC_PBL_mod) then !Bryan TC PBL Modification 
call t_startf("Compute_turbulence_scheme_target_2") 
     do k=pver,1,-1
        do i=1,pcols
...
call t_stopf("Compute_turbulence_scheme_target_2")  
     else ! Reed and Jablonowski (2012) Configuration
call t_startf("Compute_turbulence_scheme_target_3")
!$omp target
!$omp parallel  private(k,i)
!$omp do     
     do k=1,pver
...
!$omp end parallel
!$omp end target 
call t_stopf("Compute_turbulence_scheme_target_3") 
     end if
!===============================================================================
...
!===============================================================================
#ifndef NO_TSURF
call t_startf("moist_baroclinic_wave_test_target")
!$omp target
!$omp parallel  private(i)
!$omp do    
     do i=1,pcols
...
!$omp end parallel
!$omp end target 
call t_stopf("moist_baroclinic_wave_test_target")
#endif
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_CAm, source=CAm)
allocate(ref_CCm, source=CCm)
allocate(ref_CA, source=CA)
allocate(ref_CC, source=CC)
#endif
call t_startf("Calculate_diagonal_variables_target_1")
!$omp target
!$omp parallel  private(k,i,rho)
!$omp do 
      do k=1,pver-1
         do i=1,pcols
...
!$omp end parallel
!$omp end target 
call t_stopf("Calculate_diagonal_variables_target_1")
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Calculate_diagonal_variables_1")
!$omp parallel  private(k,i,rho)
!$omp do 
do k=1,pver-1
   do i=1,pcols
      rho = (pint(i,k+1)/(rair*(t(i,k+1)*(1._r8+zvir*q(i,k+1))+t(i,k)*(1._r8+zvir*q(i,k)))/2.0_r8)) 
      ref_CAm(i,k) = rpdel(i,k)*dtime*gravit*gravit*Km(i,k+1)*rho*rho/(pmid(i,k+1)-pmid(i,k))    
      ref_CCm(i,k+1) = rpdel(i,k+1)*dtime*gravit*gravit*Km(i,k+1)*rho*rho/(pmid(i,k+1)-pmid(i,k))
      ref_CA(i,k) = rpdel(i,k)*dtime*gravit*gravit*Ke(i,k+1)*rho*rho/(pmid(i,k+1)-pmid(i,k))
      ref_CC(i,k+1) = rpdel(i,k+1)*dtime*gravit*gravit*Ke(i,k+1)*rho*rho/(pmid(i,k+1)-pmid(i,k))
   end do
end do
!$omp end do nowait
!$omp end parallel

call data_check(ref_CAm,CAm)
call data_check(ref_CCm,CCm)
call data_check(ref_CA,CA)
call data_check(ref_CC,CC)
deallocate(ref_CAm)
deallocate(ref_CCm)
deallocate(ref_CA)
deallocate(ref_CC)
call t_stopf("Calculate_diagonal_variables_1")
#endif
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_CFu, source=CFu)
allocate(ref_CFv, source=CFv)
allocate(ref_CFt, source=CFt)
allocate(ref_CFq, source=CFq)
#endif
call t_startf("Calculate_diagonal_variables_target_2")
...
!$omp end parallel
!$omp end target 
call t_stopf("Calculate_diagonal_variables_target_2")

#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Calculate_diagonal_variables_2")
!$omp parallel  private(k,i)
!$omp do 
do i=1,pcols
   do k=pver,1,-1
      CE(i,k) = CC(i,k)/(1._r8+CA(i,k)+CC(i,k)-CA(i,k)*CE(i,k+1)) 
      CEm(i,k) = CCm(i,k)/(1._r8+CAm(i,k)+CCm(i,k)-CAm(i,k)*CEm(i,k+1))
      ref_CFu(i,k) = (u(i,k)+CAm(i,k)*ref_CFu(i,k+1))/(1._r8+CAm(i,k)+CCm(i,k)-CAm(i,k)*CEm(i,k+1))
      ref_CFv(i,k) = (v(i,k)+CAm(i,k)*ref_CFv(i,k+1))/(1._r8+CAm(i,k)+CCm(i,k)-CAm(i,k)*CEm(i,k+1))
      ref_CFt(i,k) = ((p0/pmid(i,k))**(rair/cpair)*t(i,k)+CA(i,k)*ref_CFt(i,k+1))/(1._r8+CA(i,k)+CC(i,k)-CA(i,k)*CE(i,k+1)) 
      ref_CFq(i,k) = (q(i,k)+CA(i,k)*ref_CFq(i,k+1))/(1._r8+CA(i,k)+CC(i,k)-CA(i,k)*CE(i,k+1))
 end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("Calculate_diagonal_variables_2")
call data_check(ref_CFu,CFu)
call data_check(ref_CFv,CFv)
call data_check(ref_CFt,CFt)
call data_check(ref_CFq,CFq)
deallocate(ref_CFu)
deallocate(ref_CFv)
deallocate(ref_CFt)
deallocate(ref_CFq)
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_dudt, source=dudt)
allocate(ref_dvdt, source=dvdt)
allocate(ref_u, source=u)
allocate(ref_v, source=v)
allocate(ref_dtdt, source=dtdt)
allocate(ref_t, source=t)
allocate(ref_dqdt, source=dqdt)
allocate(ref_q, source=q)
#endif
call t_startf("Calculate_temperaure_and_specific_humidity_target_1")
...
!$omp end parallel
!$omp end target 
call t_stopf("Calculate_temperaure_and_specific_humidity_target_1")
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Calculate_temperaure_and_specific_humidity_1")
!$omp parallel private(i)
!$omp do 
do i=1,pcols
   dudt(i,1)  = dudt(i,1)+(CFu(i,1)-u(i,1))/dtime
...
!$omp end do nowait
!$omp end parallel
call t_stopf("Calculate_temperaure_and_specific_humidity_1")
call data_check(ref_dudt,dudt)
call data_check(ref_dvdt,dvdt)
call data_check(ref_u,u)
call data_check(ref_v,v)
call data_check(ref_dtdt,dtdt)
call data_check(ref_t,t)
call data_check(ref_dqdt,dqdt)
call data_check(ref_q,q)
...
allocate(ref_q, source=q)
#endif
call t_startf("Calculate_temperaure_and_specific_humidity_target_2")
...
!$omp end parallel
!$omp end target 
call t_stopf("Calculate_temperaure_and_specific_humidity_target_2")

#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
call t_startf("Calculate_temperaure_and_specific_humidity_2")
!$omp parallel  private(i,k)
!$omp do 
do i=1,pcols
   do k=2,pver
      ref_dudt(i,k)  = ref_dudt(i,k)+(CEm(i,k)*ref_u(i,k-1)+CFu(i,k)-ref_u(i,k))/dtime
      ref_dvdt(i,k)  = ref_dvdt(i,k)+(CEm(i,k)*ref_v(i,k-1)+CFv(i,k)-ref_v(i,k))/dtime
      ref_u(i,k)    = CEm(i,k)*ref_u(i,k-1)+CFu(i,k) 
      ref_v(i,k)    = CEm(i,k)*ref_v(i,k-1)+CFv(i,k)
      ref_dtdt(i,k)  = ref_dtdt(i,k)+((CE(i,k)*ref_t(i,k-1)*(p0/pmid(i,k-1))**(rair/cpair)+CFt(i,k))*(pmid(i,k)/p0)**(rair/cpair)-ref_t(i,k))/dtime 
      ref_t(i,k)    = (CE(i,k)*ref_t(i,k-1)*(p0/pmid(i,k-1))**(rair/cpair)+CFt(i,k))*(pmid(i,k)/p0)**(rair/cpair)
      ref_dqdt(i,k)  = ref_dqdt(i,k)+(CE(i,k)*ref_q(i,k-1)+CFq(i,k)-ref_q(i,k))/dtime
      ref_q(i,k)  = CE(i,k)*ref_q(i,k-1)+CFq(i,k)
   end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("Calculate_temperaure_and_specific_humidity_2")
call data_check(ref_dudt,dudt)
call data_check(ref_dvdt,dvdt)
call data_check(ref_u,u)
call data_check(ref_v,v)
