...
!
! 'The University of Washington shallow convection and  moist turbulent schemes 
...
!===================================================================================
 ...
 module grist_uwshcu
    ...
    real(r8) :: rpen          !  For penetrative entrainment efficiency, 1-10
...
contains
    ...
    subroutine read_nml_uwshcu(nlfile)
    ...
    if (ierr /= 0) call endrun(" error reading uwshcu namelist")
    close(unitn)
   
    rpen=uwshcu_rpen
  
    end subroutine read_nml_uwshcu


    ! Purpose: Initialize key constants for the shallow convection package.
    subroutine init_uwshcu( xlv_in, cp_in, xlf_in, zvir_in, r_in, g_in, ep2_in )
    ...
    if (rpen == unset_r8) then
       call endrun("uwshcu_rpen must be set in the namelist, init_uwshcu")
    end if

    end subroutine init_uwshcu
 

    subroutine compute_uwshcu_inv( mkx    , iend       , ncnst         , dt        ,             & 
...
! local
    ...
    call compute_uwshcu( mkx  , iend   , ncnst     , dt    ,        &
                         ps0  , zs0    , p0        , z0    , dp0  , &
                         u0   , v0     , qv0       , ql0   , qi0  , & 
                         t0   , s0     , tr0       ,                & 
    ...
    do k = 0, mkx
       k_inv                  = mkx + 1 - k
       umf_inv(k_inv,:iend)   = umf(k,:iend)       
       slflx_inv(k_inv,:iend) = slflx(k,:iend)     
       qtflx_inv(k_inv,:iend) = qtflx(k,:iend)
       flxprc1_inv(k_inv,:iend) = flxprc1(k,:iend)     ! reversed for output to cam
       flxsnow1_inv(k_inv,:iend) = flxsnow1(k,:iend)   ! ""
    end do

    do k = 1, mkx
       k_inv                         = mkx + 1 - k
       qvten_inv(k_inv,:iend)        = qvten(k,:iend)   
       qlten_inv(k_inv,:iend)        = qlten(k,:iend)   
       qiten_inv(k_inv,:iend)        = qiten(k,:iend)   
       sten_inv(k_inv,:iend)         = sten(k,:iend)    
       uten_inv(k_inv,:iend)         = uten(k,:iend)    
       vten_inv(k_inv,:iend)         = vten(k,:iend)    
       qrten_inv(k_inv,:iend)        = qrten(k,:iend)   
       qsten_inv(k_inv,:iend)        = qsten(k,:iend)   
       evapc_inv(k_inv,:iend)        = evapc(k,:iend)
       cufrc_inv(k_inv,:iend)        = cufrc(k,:iend)   
       qcu_inv(k_inv,:iend)          = qcu(k,:iend)     
       qlu_inv(k_inv,:iend)          = qlu(k,:iend)     
       qiu_inv(k_inv,:iend)          = qiu(k,:iend)     
       qc_inv(k_inv,:iend)           = qc(k,:iend)      
       do m = 1, ncnst
          trten_inv(m,k_inv,:iend)   = trten(m,k,:iend) 
       enddo
    enddo

    !for Lin Macro ------------------------>
    qtu_inv(:,:iend)  = qtu(:,:iend) 
    thlu_inv(:,:iend) = thlu(:,:iend)
    !for Lin Macro <------------------------

    end subroutine compute_uwshcu_inv

    subroutine compute_uwshcu( mkx    , iend      , ncnst        , dt       ,             &
...
!    real(r8)  thlu_out(0:mkx,iend)                             !  Updraft thl [ K ]
    ...
    real(r8), dimension(iend)        :: cush_o   , precip_o , snow_o    , rliq_o, cbmf_o, cnt_o, cnb_o
    ...
    real(r8), dimension(ncnst,mkx)   :: trten_o, sstr0_o  
    real(r8), dimension(ncnst,0:mkx) :: trflx_o
...
!   parameter (rkm = 16.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit] 
    parameter (rkm = 14.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit], 8-16, default: 14
    ...
    ! kevp     : Evaporative efficiency , 1e-6 - 20e-6, default: 2e-6             !
...
25     continue
       ...
       do iter_xc = 1, niter_xc
        ...
        ! rei(k) = ( rkm / scaleh / g / rho0j )        ! Default.
          ...
          if( fer(k)*dpe .lt. 1.e-4_r8 ) then
              ...
              do m = 1, ncnst
                 tru(m,k)  =  tru(m,km1) + ( tre(m)  + sstr0(m,k) * dpe / 2._r8  -  tru(m,km1) ) * fer(k) * dpe
              enddo
          else
              ...
              do m = 1, ncnst
                 tru(m,k)  = ( tre(m)  + sstr0(m,k) / fer(k) - sstr0(m,k) * dpe / 2._r8 ) - &  
                             ( tre(m)  + sstr0(m,k) * dpe / 2._r8 - tru(m,km1) + sstr0(m,k) / fer(k) ) * exp(-fer(k) * dpe)
              enddo
          end if
 ...
 45    continue
       ...
       do m = 1, ncnst
          xsrc  = trsrc(m)
          xmean = tr0(m,kinv)
          xtop  = tr0(m,kinv+1) + sstr0(m,kinv+1) * ( ps0(kinv)   - p0(kinv+1) )
          xbot  = tr0(m,kinv-1) + sstr0(m,kinv-1) * ( ps0(kinv-1) - p0(kinv-1) )        
          call fluxbelowinv( cbmf, ps0(0:mkx), mkx, kinv, dt, xsrc, xmean, xtop, xbot, xflx )
          trflx(m,0:kinv-1) = xflx(0:kinv-1)
       enddo
       ...
       !       do m = 1, ncnst                                                                                              !
       !          trflx(m,k) = umf(k) * ( tru(m,k) - ( tr0(m,kp1) + sstr0(m,kp1) * ( ps0(k) - p0(kp1) ) ) ) + &             ! 
       !                       emf(k) * ( tru_emf(m,k) - ( tr0(m,k) + sstr0(m,k) * ( ps0(k) - p0(k) ) ) )                   ! 
       !       enddo                                                                                                        !
       ! ------------------------------------------------------------------------------------------------------------------ !
 ...
 333 if(id_exit) then ! Exit without cumulus convection
  ...
  ! ----------------------------------------------------------------- !
    ...
    if( rhi .le. 0.01_r8 ) then
        print*, 'Source air is too dry and pLCL is set to psmin in uwshcu.F90', 'rank=',mpi_rank() 
        qsinvert = psmin
        return
    end if
