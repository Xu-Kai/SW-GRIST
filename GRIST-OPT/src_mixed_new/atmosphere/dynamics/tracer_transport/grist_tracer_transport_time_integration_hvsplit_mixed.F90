...
#endif
! V prepared
if(.not.allocated(scalar_tracer_mxrt_at_pc_full_level_rk%f_r4))    allocate(scalar_tracer_mxrt_at_pc_full_level_rk%f_r4(1:ubound(tracerVarCellFull%scalar_tracer_mxrt_np1%f_r4, 1),1:ubound(tracerVarCellFull%scalar_tracer_mxrt_np1%f_r4, 2),1:ubound(tracerVarCellFull%scalar_tracer_mxrt_np1%f_r4, 3)))
...
!$omp do
    do itn = 1, ubound(scalar_normal_mxrt_at_edge_full_level_rk%f_r4, 3)
        do ite = 1, nlev
            do itracer = 1, ntracer
                scalar_normal_mxrt_at_edge_full_level_rk%f_r4(itracer,ite,itn) = scalar_normal_mxrt_at_edge_full_level_rk%f_r4(itracer,ite,itn)*&
...
!$omp parallel private(itracer, itn, ite)
!$omp do
         do itn = 1, ubound(scalar_normal_mxrt_at_edge_full_level_lo%f_r4, 3)
            do ite = 1, nlev
                do itracer = 1, ntracer
                    scalar_normal_mxrt_at_edge_full_level_lo%f_r4(itracer,ite,itn) = scalar_normal_mxrt_at_edge_full_level_lo%f_r4(itracer,ite,itn)*&
                                                                              tracerVarEdgeFull%scalar_normal_mass_flux_avg_adv%f(ite,itn)
                end do
            end do
        end do
...
!$omp do
      do itn = 1, mesh%nv 
         do ite = 1, nlev
            do itracer = 1, ntracer
               tracerVarCellFull%scalar_tracer_mxrt_np1%f_r4(itracer,ite,itn) = tracerVarCellFull%scalar_tracer_mass_np1%f_r4(itracer,ite,itn)/tracerVarCellFull%scalar_delhp_end_adv%f_r4(ite,itn)
            end do
        end do
    end do
!$omp end do nowait
...
!
        call t_startf("tracer_transport_time_integration_init_target")
        ...
        call t_stopf("tracer_transport_time_integration_init_target")
   ...
   subroutine tracer_transport_time_integration_init_r8(mesh)
    use grist_dycore_vars_module, only: dycoreVarCellFull
! io
     type(global_domain),  intent(inout) :: mesh
...
! no need for init any more
!
        call t_startf("tracer_transport_time_integration_init_r8_target")
        ...
        call t_stopf("tracer_transport_time_integration_init_r8_target")
      return
   end subroutine tracer_transport_time_integration_init_r8
...
!----------------------------------------------------------------------------
    ...
    subroutine tracer_transport_time_integration_vert_aimp(mesh,dtime)
        use omp_lib
        use grist_tridiagonal_solver_mixed, only: solve_tridiagonal1
! io
        type(global_domain),  intent(inout) :: mesh
        real(r8)          ,   intent(in)    :: dtime
        integer(i4) :: ite, itn
...
!$omp do
    do itn = 1, mesh%nv
        do ite = 1, nlev
            do itracer = 1, ntracer
                tracerVarCellFull%tend_tracer_mass_vert%f_r4(itracer,ite,itn) = &
...
!$omp parallel private(itn, ite, itracer)
!$omp do
      do itn = 1, ubound(tracerVarCellFull%scalar_tracer_mxrt_np1%f_r4, 3)
         do ite = 1, nlev
            do itracer = 1, ntracer
               tracerVarCellFull%scalar_tracer_mxrt_np1%f_r4(itracer,ite,itn) = tracerVarCellFull%scalar_tracer_mass_np1%f_r4(itracer,ite,itn)/tracerVarCellFull%scalar_delhp_end_adv%f_r4(ite,itn)
            end do
