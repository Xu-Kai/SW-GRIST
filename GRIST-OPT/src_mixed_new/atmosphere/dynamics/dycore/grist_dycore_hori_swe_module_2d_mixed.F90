...
#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
#include "data_check.inc"
         real(r4), allocatable :: ref_scalar_avor_at_dc_full_level(:, :)
         real(r4), allocatable :: ref_scalar_rvor_at_dc_full_level(:, :)
         real(r8), allocatable :: ref_scalar_height_at_dc_full_level(:, :)
#endif
#ifdef USE_HALO2
   ...
   call t_startf("tend_nct_at_edge_full_level_copy_1")
   !$omp target
...
!$omp end target
   call t_stopf("tend_nct_at_edge_full_level_copy_1")
!
...
#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
      allocate(ref_scalar_avor_at_dc_full_level, source=scalar_avor_at_dc_full_level)
      allocate(ref_scalar_rvor_at_dc_full_level, source=scalar_rvor_at_dc_full_level)
#endif 
    call t_startf("tend_nct_at_edge_target_1")
 !$omp target
...
!$omp end parallel
!$omp end target 
      call t_stopf("tend_nct_at_edge_target_1")
#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
call t_startf("tend_nct_at_edge_1")
!$omp parallel  private(it,sum_tmp,ie,ilev,index_of_edge,t_edge_v)
!$omp  do 
do it = 1, mesh%nt
   sum_tmp(:) = 0._r4
   ...
   ref_scalar_rvor_at_dc_full_level(1:nlev,it) = sum_tmp(1:nlev)/((rearth**2)*mesh%tri_areag(it))
   ref_scalar_avor_at_dc_full_level(1:nlev,it) =ref_scalar_rvor_at_dc_full_level(1:nlev,it)+2._r4*omega_r4*sin(tri_c_lat(it))
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("tend_nct_at_edge_1")
   call data_check(scalar_avor_at_dc_full_level, ref_scalar_avor_at_dc_full_level)
   call data_check(scalar_rvor_at_dc_full_level, ref_scalar_rvor_at_dc_full_level)
   deallocate(ref_scalar_avor_at_dc_full_level)
   deallocate(ref_scalar_rvor_at_dc_full_level)
#endif
...
! inline_opt_here
      ...
      !                                     nlev)
#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
      allocate(ref_scalar_height_at_dc_full_level, source=scalar_height_at_dc_full_level)
#endif 
call t_startf("tend_nct_at_edge_target_2")
!$omp target 
...
!$omp end parallel
!$omp end target
      call t_stopf("tend_nct_at_edge_target_2")

#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
call t_startf("tend_nct_at_edge_2")
!$omp parallel  private(it,sum_tmp,ie,ilev,cell_index)
!$omp  do 
      do it = 1, mesh%nt
         sum_tmp(:) = 0._r4
         do ie = 1, mesh%tri_nnb(it)
            cell_index       = mesh%tri_v(ie,it)
            do ilev = 1, nlev
               sum_tmp(ilev) = sum_tmp(ilev)+(mesh%tri_kite_area(ie,it))*scalar_height_at_pc_full_level%f_r4(ilev,cell_index)
            end do
         end do
         ref_scalar_height_at_dc_full_level(1:nlev,it) = sum_tmp(1:nlev)/(mesh%tri_areag(it))
      end do
!$omp end do nowait
!$omp end parallel
call t_stopf("tend_nct_at_edge_2")
   call data_check(scalar_height_at_dc_full_level, ref_scalar_height_at_dc_full_level)
   deallocate(ref_scalar_height_at_dc_full_level)
#endif
call t_startf("tend_nct_at_edge_full_level_copy_2")
!$omp target
!$omp parallel workshare
...
!$omp end parallel workshare
!$omp end target
        call t_stopf("tend_nct_at_edge_full_level_copy_2")
     else
call t_startf("tend_nct_at_edge_full_level_copy_3")
!$omp target
!$omp parallel workshare
        scalar_pv_at_dc_full_level%f_r4(:,:) = scalar_avor_at_dc_full_level(:,:)
!$omp end parallel workshare
!$omp end target
        call t_stopf("tend_nct_at_edge_full_level_copy_3")  
     end if
...
!
     if(pv_order(irk_step).eq.3)then
        ...
        call t_startf("tend_nct_at_edge_full_level_copy_m3")
...
!$omp end parallel workshare
!$omp end target  
        call t_stopf("tend_nct_at_edge_full_level_copy_m3")
...
!
     ...
     call t_startf("tend_nct_at_edge_full_level_copy_4")
!$omp target
!$omp parallel workshare
...
!$omp end parallel workshare
!$omp end target
      call t_stopf("tend_nct_at_edge_full_level_copy_4")
...
#if defined(CHK_GRAD_KINETIC_ENERGY) || defined(CHK_ALL)
#include "data_check.inc"
      real(r8), allocatable :: ref_kinetic_energy(:,:)
      real(r8), allocatable :: ref_kinetic_energy_hx(:,:)
      real(r8), allocatable :: ref_kinetic_energy_tr(:,:)
      real(r4), allocatable :: ref_tend_grad_ke_at_edge_full_level(:,:)
      allocate(ref_kinetic_energy, source=kinetic_energy)
      allocate(ref_kinetic_energy_tr, source=kinetic_energy_tr)
      allocate(ref_kinetic_energy_hx, source=kinetic_energy_hx)
      allocate(ref_tend_grad_ke_at_edge_full_level, source=tend_grad_ke_at_edge_full_level)
#endif
...
!   if(.not.allocated(kinetic_energy)) allocate(kinetic_energy(nlev,mesh%nv))
   ...
   case (0)   ! R10's formulation
      call t_startf("grad_kinetic_energy_target_case_0")
      ...
      call t_stopf("grad_kinetic_energy_target_case_0")
...
!
call t_startf("grad_kinetic_energy_target_1")
!$omp target
...
!$omp end parallel
!$omp end target
        call t_stopf("grad_kinetic_energy_target_1") 
#if defined(CHK_GRAD_KINETIC_ENERGY) || defined(CHK_ALL)
call t_startf("grad_kinetic_energy_1")
!$omp parallel  private(iv,tmp_sum,ie,index_edge,length_of_triangle_edge,length_of_voronoi_edge,ilev,wind) 
!$omp do
do iv = 1, mesh%nv
   tmp_sum(:)  = 0._r8
   ...
   ref_kinetic_energy(:,iv) = tmp_sum(:)/((rearth**2)*mesh%plg_areag(iv))
end do
!$omp end do nowait
!$omp end parallel 
...
do it = 1, mesh%nt
   ...
   ref_kinetic_energy_tr(:,it) = tmp_sum(:)/mesh%tri_areag(it)
end do 
!$omp end do nowait
!$omp end parallel
!
...
do iv = 1, mesh%nv
   ...
   do ie = 1, mesh%vtx_nnb(iv)  ! local index of each corner/tr
      do ilev = 1, nlev
         tmp_sum(ilev) = tmp_sum(ilev)+ref_kinetic_energy_tr(ilev,mesh%vtx_tr(ie, iv))*mesh%plg_kite_area(ie, iv)
      end do
   end do
   ref_kinetic_energy_hx(:,iv) = tmp_sum(:)
! merge
   ref_kinetic_energy(:,iv)    = ke_alpha*ref_kinetic_energy(:,iv)+(1._r8-ke_alpha)*ref_kinetic_energy_hx(:,iv)
end do
!$omp end do nowait
!$omp end parallel
   call t_stopf("grad_kinetic_energy_1")
   call data_check(kinetic_energy, ref_kinetic_energy)
   call data_check(kinetic_energy_tr, ref_kinetic_energy_tr)
   call data_check(kinetic_energy_hx, ref_kinetic_energy_hx)
   deallocate(ref_kinetic_energy)
   deallocate(ref_kinetic_energy_tr)
   deallocate(ref_kinetic_energy_hx)
#endif
...
!
   ...
   !   end do
   !end do
   call t_startf("grad_kinetic_energy_target_2")
!$omp target
...
!$omp end target
   ...
   call t_stopf("grad_kinetic_energy_target_2")

#if defined(CHK_GRAD_KINETIC_ENERGY) || defined(CHK_ALL)
call t_startf("grad_kinetic_energy_2")
!$omp parallel  private(ie,v1,v2,ilev)  
!$omp  do 
do ie = 1, mesh%ne
   v1      = mesh%edt_v(1, ie)
   v2      = mesh%edt_v(2, ie)
   do ilev = 1, nlev
      ref_tend_grad_ke_at_edge_full_level(ilev,ie) = -mesh%edt_edpNr_edtTg(ie)*(kinetic_energy(ilev,v2)-kinetic_energy(ilev,v1))/(rearth_r4*mesh%edt_leng(ie))
   end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("grad_kinetic_energy_2")
call data_check(tend_grad_ke_at_edge_full_level, ref_tend_grad_ke_at_edge_full_level)
deallocate(ref_tend_grad_ke_at_edge_full_level)
#endif
...
!   deallocate(kinetic_energy)
  ...
  subroutine calc_coriolis_term_r8(mesh,scalar_pv_at_edge_full_level           ,&
   scalar_normal_flux_at_edge_full_level  ,&
...
integer(i4)                            :: ie, ilev
!#define CHK_CORIOLIS
#if defined(CHK_CORIOLIS) || defined(CHK_ALL)
#include "data_check.inc"
real(r8), allocatable                  :: my_scalar_coriolis_term_at_edge_full_level(:,:)
allocate(my_scalar_coriolis_term_at_edge_full_level, source=scalar_coriolis_term_at_edge_full_level)
#endif
!
...
!     end do
!  end do
call t_startf("calc_coriolis_term")
!$omp target
...
!$omp end parallel 
!$omp end target
call t_stopf("calc_coriolis_term")
#if defined(CHK_CORIOLIS) || defined(CHK_ALL)
!$omp parallel  private(ie,length_of_triangle,ilev,pv_at_edge,cell_sum) 
!$omp  do 
do ie = 1, mesh%ne  ! for each edge e
...
scalar_normal_flux_at_edge_full_level(ilev,mesh%edp_edge_on_edge(1:mesh%edp_nedge(ie), ie))*&
(pv_at_edge(1:mesh%edp_nedge(ie))+scalar_pv_at_edge_full_level(ilev,mesh%edp_edge_on_edge(1:mesh%edp_nedge(ie), ie)))/2._r8
my_scalar_coriolis_term_at_edge_full_level(ilev,ie) = sum(cell_sum(1:mesh%edp_nedge(ie)))/length_of_triangle

end do
end do
!$omp end do nowait
!$omp end parallel 
call data_check(scalar_coriolis_term_at_edge_full_level, my_scalar_coriolis_term_at_edge_full_level)
deallocate(my_scalar_coriolis_term_at_edge_full_level)
#endif
...
return
end subroutine calc_coriolis_term_r8
...
#if defined(CHK_CORIOLIS) || defined(CHK_ALL)
#include "data_check.inc"
   real(r4), allocatable                  :: my_scalar_coriolis_term_at_edge_full_level(:,:)
   allocate(my_scalar_coriolis_term_at_edge_full_level, source=scalar_coriolis_term_at_edge_full_level)
#endif
!
! te conserving scheme
...
! local
   ...
   integer(i4)                          :: ii
#if defined(CHK_NORMAL_FLUX_TR_EDGE) || defined(CHK_ALL)
#include "data_check.inc"
   real(r4), allocatable                  :: ref_scalar_pv_at_edge_full_level(:,:)
   allocate(ref_scalar_pv_at_edge_full_level, source=scalar_pv_at_edge_full_level)
#endif
...
!================================================
      ...
      case(2) ! PV2 
call t_startf("normal_flux_tr_edge_case_2")
!$omp target
!$omp  parallel do private(ie, v1, v2, ilev) 
        do ie = 1, mesh%ne
             !v1 = mesh%edp(ie)%v(1)
             !v2 = mesh%edp(ie)%v(2)
             v1 = mesh%edp_v(1, ie)
...
!$omp end target
        ...
        do ie = 1, mesh%ne
             ...
             v2 = mesh%edp_v(2, ie)
             do ilev = 1, nlev
                ref_scalar_pv_at_edge_full_level(ilev,ie) = 0.5*(scalar_pv_at_dc_full_level(ilev,v1)+&
                                                             scalar_pv_at_dc_full_level(ilev,v2))
             end do
        end do
#endif
        call t_stopf("normal_flux_tr_edge_case_2")
      case(3) ! PV3
call t_startf("normal_flux_tr_edge_target")
!$omp target
...
!$omp end parallel 
!$omp end target 
     call t_stopf("normal_flux_tr_edge_target")

#if defined(CHK_NORMAL_FLUX_TR_EDGE) || defined(CHK_ALL)
call t_startf("normal_flux_tr_edge")
!$omp parallel  private(ii,ie,ilev,v1,v2,wind_edge,mark,v_upwind,der0) 
!$omp  do  
do ii = 1, mesh%ne*nlev,1
   ie=ceiling(ii/real(nlev,r8))
...
! inline written here
          der0   = 2._r8*dot_product(mesh%edp_v_weight_dual_cell(1:mesh%tri_stencil_number_2nd(v_upwind), mark, ie), &
                         scalar_pv_at_dc_full_level(ilev,mesh%tri_stencil_index_2nd(1:mesh%tri_stencil_number_2nd(v_upwind),v_upwind))-&
                         scalar_pv_at_dc_full_level(ilev,v_upwind))
         ref_scalar_pv_at_edge_full_level(ilev,ie) = (scalar_pv_at_dc_full_level(ilev,v1)+scalar_pv_at_dc_full_level(ilev,v2))*0.5_r8-&
                       ((edp_leng(ie)**2)/6._r8)*der0
! inline_opt_here
...
!$omp end do nowait
!$omp end parallel
call t_stopf("normal_flux_tr_edge")
   call data_check(scalar_pv_at_edge_full_level, ref_scalar_pv_at_edge_full_level)
   deallocate(ref_scalar_pv_at_edge_full_level)
#endif
      case default
