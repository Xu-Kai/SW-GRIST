...
! operator
  use grist_dycore_gcd_recon_module_2d_mixed,   only: divergence_operator_2d_r8, curl_operator_2d 
  use grist_mpi
#ifndef SEQ_GRIST
...
!
       ...
       case('SPLIKE') ! sponge-like
           ...
           call t_stopf("diffusion_spike")
       case('SMG')    ! Smagorinsky
call t_startf("diffusion_SMG")
!$omp target
!$omp parallel workshare
   www_k2coef(1:nexpdif_level+1,1:mesh%ne_halo(1)) = scalar_smg_eddy_coef_at_edge_face_level(1:nexpdif_level+1,1:mesh%ne_halo(1))
!$omp end parallel workshare
!$omp end target
call t_stopf("diffusion_SMG")
       case default   ! SMG
           call t_startf("diffusion_default")
           www_k2coef(1:nexpdif_level+1,1:mesh%ne_halo(1)) = scalar_smg_eddy_coef_at_edge_face_level(1:nexpdif_level+1,1:mesh%ne_halo(1))
           call t_stopf("diffusion_default")
       end select
       call t_startf("diffusion_2nd_fullface")
       call calc_scalar_laplacian_2nd_FullFace_1(mesh,dycoreVarEdgeFull%scalar_normal_velocity_n%f,&
                                                    ...
                                                    nexpdif_level+1)
       call t_stopf("diffusion_2nd_fullface")
       end if
       call t_stopf("diffusion_2nd")
       call t_startf("diffusion_4th")
...
!
       ...
       call t_stopf("diffusion_4th")
       call t_startf("diffusion_6th")
...
! local
      ...
      integer(i4)            :: ie, iv, inb,ilev, v1, v2
      real(r8)               :: v1v2(3), flag, div_sum
!#define CHK_SCALAR_LAPLACIAN_2ND_FULLFACE
#if defined(CHK_SCALAR_LAPLACIAN_2ND_FULLFACE) || defined(CHK_ALL)
      real(r8), allocatable  :: ref_gradient_at_prime_edge(:,:)
      real(r8), allocatable  :: ref_tend_laplacian_2nd_at_pc(:,:)
#include <data_check.inc>
      allocate(ref_gradient_at_prime_edge, source=gradient_at_prime_edge)
      allocate(ref_tend_laplacian_2nd_at_pc, source=tend_laplacian_2nd_at_pc)
#endif

!
! gradient at edge, counter edge's normal direction
!       
call t_startf("calc_scalar_laplacian_2nd_fullface_normal")
!$omp target
!$omp  parallel do private(ie, v1, v2, v1v2, flag, ilev) 
do ie = 1, mesh%ne_halo(1)     ! global index
   v1      = mesh%edt_v(1,ie)
   v2      = mesh%edt_v(2,ie)
   v1v2    = mesh%vtx_p(1:3,v2)-mesh%vtx_p(1:3,v1)
...
do ie = 1, mesh%ne_halo(1)     ! global index
   ...
   flag    = sign(1._r8,dot_product(v1v2,real(mesh%edp_nr(1:3,ie),r8)))
   do ilev = 1, nLevel
      ref_gradient_at_prime_edge(ilev,ie) = flag*(scalar_at_pc(ilev,v2)-scalar_at_pc(ilev,v1))/(rearth*mesh%edt_leng(ie))
      ref_gradient_at_prime_edge(ilev,ie) = scalar_smg_eddy_coef_at_edge(ilev,ie)*ref_gradient_at_prime_edge(ilev,ie)
   end do
end do
!$omp end parallel do
call data_check(gradient_at_prime_edge, ref_gradient_at_prime_edge)
deallocate(ref_gradient_at_prime_edge)
#endif
        call t_stopf("calc_scalar_laplacian_2nd_fullface_normal")

!
! divergence at cell
!
        call t_startf("calc_scalar_laplacian_2nd_fullface_divergence")
!$omp target
!$omp  parallel do private(iv, ilev, div_sum, inb, ie) 
do iv = 1, mesh%nv_halo(1)
...
!$omp end target
        ...
        !$omp  parallel do private(iv, ilev, div_sum, inb, ie) 
        do iv = 1, mesh%nv_halo(1)
           ref_tend_laplacian_2nd_at_pc(:,iv) = zero ! default zero
           do ilev = 1, nLevel
              div_sum = zero
              do inb = 1, mesh%vtx_nnb(iv)
                 ie  = mesh%vtx_ed(inb,iv)
              ...
              ref_tend_laplacian_2nd_at_pc(ilev,iv) = div_sum/((rearth**2)*mesh%plg_areag(iv))
           end do
        end do
        !$omp end parallel do
        call data_check(tend_laplacian_2nd_at_pc, ref_tend_laplacian_2nd_at_pc)
        deallocate(ref_tend_laplacian_2nd_at_pc)
#endif
        call t_stopf("calc_scalar_laplacian_2nd_fullface_divergence")
        return
...
#if defined(CHK_SCALAR_LAPLACIAN_2ND_FULLFACE) || defined(CHK_ALL)
      real(r8), allocatable  :: ref_gradient_at_prime_edge(:,:)
      real(r4), allocatable  :: ref_tend_laplacian_2nd_at_pc(:,:)
#include <data_check.inc>
      allocate(ref_gradient_at_prime_edge, source=gradient_at_prime_edge)
      allocate(ref_tend_laplacian_2nd_at_pc, source=tend_laplacian_2nd_at_pc)
#endif
...
#if defined(CHK_HRW_LAP_2ND_FULL) || defined(CHK_ALL)
#include <data_check.inc>
      real(r8), allocatable                :: ref_tend_laplacian_2nd_at_pc_full_level(:,:)
      allocate(ref_tend_laplacian_2nd_at_pc_full_level, source=tend_laplacian_2nd_at_pc_full_level)
#endif
!
! if use laplacian vi, will overwrite that from smg_coef
!
call t_startf("hrwind_laplacian_2nd_full")
call t_startf("hrwind_laplacian_2nd_full_vi")
      if(use_laplacian_vi) call calc_hrwind_laplacian_2nd_full_vi(mesh,scalar_normal_velocity_at_edge_full_level)
call t_stopf("hrwind_laplacian_2nd_full_vi")
call t_startf("hrwind_laplacian_2nd_full_1")
!$omp target
!$omp  parallel do private(ie, ilev) 
do ie = 1, mesh%ne_compute
...
#if defined(CHK_HRW_LAP_2ND_FULL) || defined(CHK_ALL)
   do ie = 1, mesh%ne_compute
      ref_tend_laplacian_2nd_at_pc_full_level(:,ie) = 0._r8  ! default zero
      do ilev = 1, nexpdif
            ref_tend_laplacian_2nd_at_pc_full_level(ilev,ie) = 4._r8*scalar_smg_eddy_coef_at_edge_full_level(ilev,ie)*&
                                                               scalar_hwind_laplace_at_edge_full_level(ilev,ie)
   #ifdef DCMIP_SUPERCELL
            ref_tend_laplacian_2nd_at_pc_full_level(ilev,ie) = 4._r4*500._r4*&
                                                               scalar_hwind_laplace_at_edge_full_level(ilev,ie)
   #endif
      end do
   end do
   call data_check(tend_laplacian_2nd_at_pc_full_level, ref_tend_laplacian_2nd_at_pc_full_level)
#endif
      call t_stopf("hrwind_laplacian_2nd_full_1")
!
! add top-nsponge layer with real 2nd diffusion
!
      if(nsponge.gt.0)then ! default should be 5
        call t_startf("hrwind_laplacian_2nd_full_2")
        do ie = 1, mesh%ne_compute
...
! from top to bottom
      ...
      call t_stopf("hrwind_laplacian_2nd_full_2")
   end if
   call t_stopf("hrwind_laplacian_2nd_full")
   return
...
#if defined(CHK_HRW_LAP_2ND_FULL) || defined(CHK_ALL)
#include <data_check.inc>
      real(r4), allocatable                :: ref_tend_laplacian_2nd_at_pc_full_level(:,:)
#endif
!
! if use laplacian vi, will overwrite that from smg_coef
!
...
!$omp end target
      ...
      do ie = 1, mesh%ne_compute
         ref_tend_laplacian_2nd_at_pc_full_level(:,ie) = 0._r8  ! default zero
         do ilev = 1, nexpdif
               ref_tend_laplacian_2nd_at_pc_full_level(ilev,ie) = 4._r4*scalar_smg_eddy_coef_at_edge_full_level(ilev,ie)*&
                                                                  scalar_hwind_laplace_at_edge_full_level(ilev,ie)
#ifdef DCMIP_SUPERCELL
         ref_tend_laplacian_2nd_at_pc_full_level(ilev,ie) = 4._r4*500._r4*&
                                                            scalar_hwind_laplace_at_edge_full_level(ilev,ie)
...
call data_check(tend_laplacian_2nd_at_pc_full_level, ref_tend_laplacian_2nd_at_pc_full_level)
call deallocate(ref_tend_laplacian_2nd_at_pc_full_level)
#endif
call t_stopf("hrwind_laplacian_2nd_full_1")
...
allocate(ref_tend_laplacian_2nd_at_pc_full_level, source=tend_laplacian_2nd_at_pc_full_level)
         ...
         call t_startf("hrwind_laplacian_2nd_full_target_2")
!$omp target
!$omp  parallel do private(ie, ilev, cell1, cell2)  
        do ie = 1, mesh%ne_compute
           cell1 = mesh%edt_v(1,ie)
           cell2 = mesh%edt_v(2,ie)
! from top to bottom
...
!$omp end parallel do
!$omp end target
        call t_stopf("hrwind_laplacian_2nd_full_target_2")
   ...
   ! from top to bottom
      do ilev = 1, nsponge
         ref_tend_laplacian_2nd_at_pc_full_level(ilev,ie) = ref_tend_laplacian_2nd_at_pc_full_level(ilev,ie)+ &
                                       4._r8*scalar_hwind_laplace_at_edge_full_level(ilev,ie)*&
                                 0.5_r8*(mesh%vtxCellLeng(cell1)+mesh%vtxCellLeng(cell2))*rearth*(7-ilev)*spcoef/6._r8
      end do
   end do
...
#if defined(CHK_SMG_EDDY_COEF) || defined(CHK_ALL)
#include "data_check.inc"
        real(r8), allocatable  :: chk_velocity_vector_at_pc_full_level(:,:,:)
        real(r8), allocatable  :: chk_velocity_vector_at_dc_full_level(:,:,:)
        real(r8), allocatable  :: chk_scalar_smg_eddy_coef_at_edge_face_level(:,:)
        real(r8), allocatable  :: chk_scalar_smg_eddy_coef_at_edge_full_level(:,:)
        allocate(chk_scalar_smg_eddy_coef_at_edge_face_level, source=scalar_smg_eddy_coef_at_edge_face_level)
        allocate(chk_scalar_smg_eddy_coef_at_edge_full_level, source=scalar_smg_eddy_coef_at_edge_full_level)
#endif
...
#if defined(CHK_SMG_EDDY_COEF) || defined(CHK_ALL)
      allocate(chk_velocity_vector_at_pc_full_level, source=velocity_vector_at_pc_full_level)
      allocate(chk_velocity_vector_at_dc_full_level, source=velocity_vector_at_dc_full_level)
#endif 

   call t_startf("smg_eddy_coef_target_1")
!$omp target      
...
!$omp end target 
   ...
   call t_stopf("smg_eddy_coef_target_1")
#if defined(CHK_SMG_EDDY_COEF) || defined(CHK_ALL) 
   call t_startf("smg_eddy_coef_1")

   
!$omp parallel  private(iv,inb,ie,ilev)    
!$omp  do  
      do iv  = 1, mesh%nv_halo(2)
         chk_velocity_vector_at_pc_full_level(:,:,iv) = 0._r8
         do inb  = 1, mesh%vtx_nnb(iv)
            ie = mesh%vtx_ed(inb,iv)
            do ilev = 1, nlev
               chk_velocity_vector_at_pc_full_level(:,ilev,iv) = chk_velocity_vector_at_pc_full_level(:,ilev,iv)+perot_weight_at_pc(:,inb,iv)*&
                                                   scalar_normal_velocity_at_edge_full_level(ilev,ie)
            end do
         end do
      end do
!$omp end do nowait
!$omp end parallel 
      call t_stopf("smg_eddy_coef_1")

      call data_check(velocity_vector_at_pc_full_level, chk_velocity_vector_at_pc_full_level)
      deallocate(chk_velocity_vector_at_pc_full_level)
#endif


call t_startf("smg_eddy_coef_target_2")
!$omp target
...
!$omp end parallel 
!$omp end target 
call t_stopf("smg_eddy_coef_target_2")

#if defined(CHK_SMG_EDDY_COEF) || defined(CHK_ALL)
call t_startf("smg_eddy_coef_2")

!$omp parallel  private(it,inb,ilev,ie) 
!$omp  do  
      do it  = 1, mesh%nt_halo(1)
         chk_velocity_vector_at_dc_full_level(:,:,it) = 0._r8
         do inb = 1, mesh%tri_nnb(it)
            ie = mesh%tri_ed(inb,it)
            do ilev = 1, nlev
               chk_velocity_vector_at_dc_full_level(:,ilev,it) = chk_velocity_vector_at_dc_full_level(:,ilev,it)+perot_weight_at_dc(:,inb,it)*&
                                                   scalar_normal_velocity_at_edge_full_level(ilev,ie)
            end do
         end do
      end do
!$omp end do nowait
!$omp end parallel
call t_stopf("smg_eddy_coef_2")

      call data_check(velocity_vector_at_dc_full_level, chk_velocity_vector_at_dc_full_level)
      deallocate(chk_velocity_vector_at_dc_full_level) 
#endif 
...
!         pc1
!-----------------------------------------------------------------------
call t_startf("smg_eddy_coef_target_3")
!$omp target
...
!$omp end parallel 
!$omp end target 
call t_stopf("smg_eddy_coef_target_3")


#if defined(CHK_SMG_EDDY_COEF) || defined(CHK_ALL)
call t_startf("smg_eddy_coef_3")
...
!$omp  do  
       DO ie = 1, mesh%ne_halo(1)
         ...
         do ilev = 1, nlev
            ...
            if(vr_mode)then
               chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*(rearth**2)*mesh%edt_leng(ie)*mesh%edp_leng(ie)*sqrt((un_pc1pc2-ut_dc1dc2)**2+(un_dc1dc2+ut_pc1pc2)**2)
            else ! QU
               chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*((rearth*mesh%mean_edt_dist)**2)*sqrt((un_pc1pc2-ut_dc1dc2)**2+(un_dc1dc2+ut_pc1pc2)**2)
            end if

            !----------------------------------------depreciated------------------------------------------------
!#if (defined VRVIS)
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*(ref_leng**2)*mesh%edt_scale_2nd(ie)*&
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*((rearth*mesh%min_edt_dist)**2)*mesh%edt_scale_2nd(ie)*&
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*((rearth*mesh%edt_leng(ie))**2)*&
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*(ref_leng**2)*mesh%edt_scale_2nd(ie)*&
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*(rearth**2)*mesh%edt_leng(ie)*mesh%edp_leng(ie)*&
!#elif (defined VRSMG)
        ! old setup for exact regression
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*((rearth*mesh%edt_leng(ie))**2)*&
!#else
            !chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = smg_coef*((rearth*mesh%mean_edt_dist)**2)*&
!#endif
            !----------------------------------------------------------------------------------------------------------

#ifdef DCMIP_SUPERCELL
            chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie) = 1500._r8
#endif
     ! second-order laplacian of normal wind, because we should use 1/2(edt&edp's length) for double center differences but not,
     ! so this laplacian needs a *4 when using outside, for laplacian vi, we also normalize it like this;
     ! why not *4 here? currently just for bit-bit regression!
...
! 
         do ilev = 2, nlev
            chk_scalar_smg_eddy_coef_at_edge_face_level(ilev,ie) = &
                                    0.5*(deta_full(ilev-1)/deta_face(ilev)*chk_scalar_smg_eddy_coef_at_edge_full_level(ilev,ie)+&
                                         deta_full(ilev)  /deta_face(ilev)*chk_scalar_smg_eddy_coef_at_edge_full_level(ilev-1,ie))
         end do
!#ifndef REGRESSION
! bdy extrapolation
!         chk_scalar_smg_eddy_coef_at_edge_face_level(1,ie)     = extrapolate_bdy(chk_scalar_smg_eddy_coef_at_edge_full_level(1,ie),&
!                                                                             chk_scalar_smg_eddy_coef_at_edge_full_level(2,ie),&
!                                                                             chk_scalar_smg_eddy_coef_at_edge_full_level(3,ie),&
!                                                                             eta_full(1), eta_full(2), eta_full(3), eta_face(1))
 
!         chk_scalar_smg_eddy_coef_at_edge_face_level(nlevp,ie)= extrapolate_bdy(chk_scalar_smg_eddy_coef_at_edge_full_level(nlev-2,ie),&
!                                                                             chk_scalar_smg_eddy_coef_at_edge_full_level(nlev-1,ie),&
!                                                                             chk_scalar_smg_eddy_coef_at_edge_full_level(nlev,ie)  ,&
!                                                                             eta_full(nlev-2),eta_full(nlev-1), eta_full(nlev), eta_face(nlevp))
!#else
         chk_scalar_smg_eddy_coef_at_edge_face_level(1,ie)     = 2.*chk_scalar_smg_eddy_coef_at_edge_full_level(1,ie)-chk_scalar_smg_eddy_coef_at_edge_face_level(2,ie)
         chk_scalar_smg_eddy_coef_at_edge_face_level(nlevp,ie) = 2.*chk_scalar_smg_eddy_coef_at_edge_full_level(nlev,ie)-chk_scalar_smg_eddy_coef_at_edge_face_level(nlev,ie)
!#endif
#ifdef DCMIP_SUPERCELL
         chk_scalar_smg_eddy_coef_at_edge_face_level(:,:) = 500._r8
#endif

       END DO
!$omp end do nowait
!$omp end parallel 
call t_stopf("smg_eddy_coef_3")

       call data_check(chk_scalar_smg_eddy_coef_at_edge_face_level, scalar_smg_eddy_coef_at_edge_face_level)
       call data_check(chk_scalar_smg_eddy_coef_at_edge_full_level, scalar_smg_eddy_coef_at_edge_full_level)
       deallocate(chk_scalar_smg_eddy_coef_at_edge_face_level)
       deallocate(chk_scalar_smg_eddy_coef_at_edge_full_level)
#endif 
...
! local
     ...
     real(r8)                          :: un_pc1, un_pc2, un_dc1, un_dc2  ! normal wind follow edp's nr
     real(r8)                          :: ut_pc1, ut_pc2, ut_dc1, ut_dc2  ! tangent wind follow edp's tg
!#define CHK_HRWIND_LAP4
#if defined(CHK_HRWIND_LAP4) || defined(CHK_ALL)
#include <data_check.inc>
     real(r8), allocatable             :: ref_velocity_vector_at_pc_full_level(:,:,:)
     real(r8), allocatable             :: ref_velocity_vector_at_dc_full_level(:,:,:)
     real(r8), allocatable             :: ref_scalar_hwind_lap4th_at_edge_full_level(:,:)
     real(r8), allocatable             :: ref_scalar_hwind_laplace4th_at_edge_full_level_f(:,:)
#endif
!
...
#if defined(CHK_HRWIND_LAP4) || defined(CHK_ALL)
      allocate(ref_velocity_vector_at_pc_full_level, source=velocity_vector_at_pc_full_level)
      allocate(ref_velocity_vector_at_dc_full_level, source=velocity_vector_at_dc_full_level)
      allocate(ref_scalar_hwind_lap4th_at_edge_full_level, source=scalar_hwind_lap4th_at_edge_full_level)
      if(flag.and.do_dycore_laplacian_6th) allocate(ref_scalar_hwind_laplace4th_at_edge_full_level_f, source=scalar_hwind_laplace4th_at_edge_full_level%f)
#endif
      call t_startf("hrwind_laplacian_4th_full_velocity_vector_at_pc")
!$omp target
!$omp  parallel do private(iv, inb, ie, ilev) 
do iv  = 1, mesh%nv_halo(1)
   velocity_vector_at_pc_full_level(:,:,iv) = 0._r8
   do inb  = 1, mesh%vtx_nnb(iv)
      ie = mesh%vtx_ed(inb,iv)
...
#if defined(CHK_HRWIND_LAP4) || defined(CHK_ALL)
      do iv  = 1, mesh%nv_halo(1)
         ref_velocity_vector_at_pc_full_level(:,:,iv) = 0._r8
         do inb  = 1, mesh%vtx_nnb(iv)
            ie = mesh%vtx_ed(inb,iv)
            do ilev = 1, nlev
                ref_velocity_vector_at_pc_full_level(:,ilev,iv) = ref_velocity_vector_at_pc_full_level(:,ilev,iv)+perot_weight_at_pc(:,inb,iv)*&
                                                   scalar_hwind_lap2nd_at_edge_full_level(ilev,ie)
            end do
         end do
      end do
      call data_check(velocity_vector_at_pc_full_level, ref_velocity_vector_at_pc_full_level)
      deallocate(ref_velocity_vector_at_pc_full_level)
#endif
      call t_stopf("hrwind_laplacian_4th_full_velocity_vector_at_pc")
      call t_startf("hrwind_laplacian_4th_full_velocity_vector_at_dc")
!$omp target
!$omp  parallel do private(it, inb, ie, ilev) 
do it  = 1, mesh%nt
   velocity_vector_at_dc_full_level(:,:,it) = 0._r8
   do inb = 1, mesh%tri_nnb(it)
      ie = mesh%tri_ed(inb,it)
...
#if defined(CHK_HRWIND_LAP4) || defined(CHK_ALL)
      do it  = 1, mesh%nt
         ref_velocity_vector_at_dc_full_level(:,:,it) = 0._r8
         do inb = 1, mesh%tri_nnb(it)
            ie = mesh%tri_ed(inb,it)
            do ilev = 1, nlev
               ref_velocity_vector_at_dc_full_level(:,ilev,it) = ref_velocity_vector_at_dc_full_level(:,ilev,it)+perot_weight_at_dc(:,inb,it)*&
                                                   scalar_hwind_lap2nd_at_edge_full_level(ilev,ie)
            end do
         end do
      end do
      call data_check(velocity_vector_at_dc_full_level, ref_velocity_vector_at_dc_full_level)
      deallocate(ref_velocity_vector_at_dc_full_level)
#endif
      call t_stopf("hrwind_laplacian_4th_full_velocity_vector_at_dc")
!-----------------------------------------------------------------------
!         pc2
!          |
!    dc1---e---dc2
...
!         pc1
!-----------------------------------------------------------------------
      call t_startf("hrwind_laplacian_4th_full_wind")
!$omp target
!$omp  parallel do private(ie, pc1, pc2, dc1, dc2, un_pc1, un_pc2, un_dc1, un_dc2)  
do ie = 1, mesh%ne
...
!$omp end target
   ...
   ! high-order laplacian of normal wind for hyperviscosity
   !
         ref_scalar_hwind_lap4th_at_edge_full_level(ilev,ie) = (-1)**(order+1)*(4._r8**order)*coef*scaling_factor(ie)*&  ! coef and constant

                                                            (un_pc1+un_pc2-2._r8*scalar_hwind_lap2nd_at_edge_full_level(ilev,ie))/((rearth*mesh%edt_leng(ie))**2)+&
                                                            (un_dc1+un_dc2-2._r8*scalar_hwind_lap2nd_at_edge_full_level(ilev,ie))/((rearth*mesh%edp_leng(ie))**2)
            if(flag.and.do_dycore_laplacian_6th)then ! store 4th for later 6th
               ref_scalar_hwind_laplace4th_at_edge_full_level_f(ilev,ie) = (un_pc1+un_pc2-2._r8*scalar_hwind_lap2nd_at_edge_full_level(ilev,ie))/((rearth*mesh%edt_leng(ie))**2)+&
                                                                     (un_dc1+un_dc2-2._r8*scalar_hwind_lap2nd_at_edge_full_level(ilev,ie))/((rearth*mesh%edp_leng(ie))**2)
            end if
      end do

   end do
   call data_check(scalar_hwind_lap4th_at_edge_full_level, ref_scalar_hwind_lap4th_at_edge_full_level)
   deallocate(ref_scalar_hwind_lap4th_at_edge_full_level)
   if(flag.and.do_dycore_laplacian_6th) then
      call data_check(scalar_hwind_laplace4th_at_edge_full_level%f, ref_scalar_hwind_laplace4th_at_edge_full_level_f)
      deallocate(ref_scalar_hwind_laplace4th_at_edge_full_level_f)
   endif
#endif
      call t_stopf("hrwind_laplacian_4th_full_wind")
      if(allocated(velocity_vector_at_pc_full_level)) deallocate(velocity_vector_at_pc_full_level)
      if(allocated(velocity_vector_at_dc_full_level)) deallocate(velocity_vector_at_dc_full_level)
...
#include <data_check.inc>
      real(r8), allocatable             :: ref_velocity_vector_at_pc_full_level(:,:,:)
      real(r8), allocatable             :: ref_velocity_vector_at_dc_full_level(:,:,:)
      real(r4), allocatable             :: ref_scalar_hwind_lap4th_at_edge_full_level(:,:)
      real(r8), allocatable             :: ref_scalar_hwind_laplace4th_at_edge_full_level_f(:,:)
#endif
...
!  
      ...
      call divergence_operator_2d_r8(mesh, scalar_normal_velocity_at_edge_full_level, &
...
!        
      ...
      call divergence_operator_2d_r8(mesh, scalar_hwind_lap2nd_at_edge_full_level, &
                                        div_at_pc_full_level, &
                                        nlev)
      mesh%nv = mesh%nv_compute
