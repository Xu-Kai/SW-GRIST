...
! local
     ...
     real(r4) :: scalar_normal_velocity_n_c(nlev), &
     ...
     real(r8) :: tmp1_c(nlev), tmp2_c(nlev)
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
#include "data_check.inc"
        real(r4), allocatable :: ref_scalar_normal_mass_flux_n_f(:, :)
        real(r4), allocatable :: ref_tend_mass_hori_f(:, :)
        real(r4), allocatable :: ref_tend_normal_velocity_vadv_f(:,:)
        ! real(r8), allocatable :: ref_scalar_mpressure_at_edge_full_level_n_f(:,:)
        real(r8), allocatable :: ref_tend_mass_pt_hori_f(:, :)
        real(r4), allocatable :: ref_scalar_mpressure_at_edge_full_level_n(:, :)
        real(r8), allocatable :: ref_tend_pidpiv_at_pc_full_level_hori(:, :)
        real(r4), allocatable :: ref_tend_normal_velocity_pgf_f(:, :)
...
if(.not.allocated(scalar_www_at_pc_face_level_rk%f_r4))    allocate(scalar_www_at_pc_face_level_rk%f_r4(1:ubound(dycoreVarCellFace%scalar_www_n%f, 1),1:ubound(dycoreVarCellFace%scalar_www_n%f, 2)))
if(.not.allocated(scalar_phi_at_pc_face_level_rk%f_r4))    allocate(scalar_phi_at_pc_face_level_rk%f_r4(1:ubound(dycoreVarCellFace%scalar_phi_n%f, 1),1:ubound(dycoreVarCellFace%scalar_phi_n%f, 2)))
      if(nh_dynamics)then
...
!$omp end target
      ...
      if(.not.allocated(scalar_normal_velocity_at_edge_full_level_rk%f_r4))    allocate(scalar_normal_velocity_at_edge_full_level_rk%f_r4(1:ubound(dycoreVarEdgeFull%scalar_normal_velocity_n%f, 1),1:ubound(dycoreVarEdgeFull%scalar_normal_velocity_n%f, 2)))
      if(.not.allocated(scalar_delhp_at_pc_full_level_rk%f_r4))    allocate(scalar_delhp_at_pc_full_level_rk%f_r4(1:ubound(dycoreVarCellFull%scalar_delhp_n%f, 1),1:ubound(dycoreVarCellFull%scalar_delhp_n%f, 2)))
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_scalar_normal_mass_flux_n_f, source=dycoreVarEdgeFull%scalar_normal_mass_flux_n%f_r4)
#endif
...
!$omp do
      do ie = 1, mesh%ne_halo(1)
        do ilev = 1, nlev
        ref_scalar_normal_mass_flux_n_f(ilev, ie) = 0.5_r4*(scalar_delhp_at_pc_full_level_rk%f_r4(ilev,mesh%edt_v(1,ie))+&
                scalar_delhp_at_pc_full_level_rk%f_r4(ilev,mesh%edt_v(2,ie)))
        end do
     end do
...
!================================================
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_scalar_normal_mass_flux_n_f, source= dycoreVarEdgeFace%scalar_normal_mass_flux_n%f_r4)
#endif
...
!$omp do
        do itv = 1, ubound(dycoreVarEdgeFace%scalar_normal_mass_flux_n%f, 2)  
           do ilev = 2, nlev
             dycoreVarEdgeFace%scalar_normal_mass_flux_n%f_r4(ilev,itv) = &
                                     0.5*(deta_full(ilev-1)/deta_face(ilev)*dycoreVarEdgeFull%scalar_normal_mass_flux_n%f_r4(ilev,itv)+&
                                          deta_full(ilev)  /deta_face(ilev)*dycoreVarEdgeFull%scalar_normal_mass_flux_n%f_r4(ilev-1,itv))
            end do
        end do
!$omp end do nowait
!$omp end parallel
...
call data_check(dycoreVarEdgeFace%scalar_normal_mass_flux_n%f_r4, ref_scalar_normal_mass_flux_n_f)
deallocate(ref_scalar_normal_mass_flux_n_f)
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_tend_normal_velocity_vadv_f, source=dycoreVarEdgeFull%tend_normal_velocity_vadv%f_r4)
#endif
...
!$omp do
        ...
        !
                  ...
                  ref_tend_normal_velocity_vadv_f(:,ie) = scalar_template_1d_nlev_c
              
        end do
!$omp end do nowait
!$omp end parallel
call t_stopf("dycore_time_intergration_5")
call data_check(dycoreVarEdgeFull%tend_normal_velocity_vadv%f_r4, ref_tend_normal_velocity_vadv_f)
...
#else
              ref_scalar_omega_n_f(ilev,iv) = dycoreVarCellFull%scalar_eta_mass_flux_n%f_r4(ilev,iv)/tracerVarCellFull%scalar_mif_n%f(ilev,iv)
#endif
           end if
        end do
     end do
...
#endif
      if(.not.allocated(dycoreVarCellFace%scalar_www_n%f_r4))    allocate(dycoreVarCellFace%scalar_www_n%f_r4(1:ubound(dycoreVarCellFace%scalar_www_n%f, 1),1:ubound(dycoreVarCellFace%scalar_www_n%f, 2)))
      if(.not.allocated(tracerVarCellFace%scalar_mif_n%f_r4))    allocate(tracerVarCellFace%scalar_mif_n%f_r4(1:ubound(dycoreVarCellFull%scalar_delhp_n%f, 1),1:ubound(dycoreVarCellFull%scalar_delhp_n%f, 2)))
      if(.not.allocated(dycoreVarCellFull%scalar_delhp_np1%f_r4))    allocate(dycoreVarCellFull%scalar_delhp_np1%f_r4(1:ubound(dycoreVarCellFull%scalar_delhp_np1%f, 1),1:ubound(dycoreVarCellFull%scalar_delhp_np1%f, 2)))
!$omp target
!$omp parallel workshare
...
#if defined(CHK_DYCORE_TIME_INTERGRATION) || defined(CHK_ALL)
allocate(ref_tend_normal_velocity_pgf_f, source=dycoreVarEdgeFull%tend_normal_velocity_pgf%f_r4)
#endif
...
call data_check(ref_tend_normal_velocity_pgf_f, dycoreVarEdgeFull%tend_normal_velocity_pgf%f_r4)
...
!$omp target parallel do private(scalar_normal_velocity_n_c, tend_normal_velocity_vadv_c, tend_normal_velocity_pgf_c, tend_normal_velocity_gz_c, tend_normal_velocity_ke_c, tend_normal_velocity_nct_c, tmp1_c, tmp2_c)
       do ie = 1, mesh%ne
                call omnicopy(tmp1_c(:), dycoreVarEdgeFull%scalar_normal_velocity_n%f(:,ie))
                call omnicopy(tend_normal_velocity_vadv_c(:), dycoreVarEdgeFull%tend_normal_velocity_vadv%f_r4(:,ie))
                call omnicopy(tend_normal_velocity_pgf_c(:), dycoreVarEdgeFull%tend_normal_velocity_pgf%f_r4(:,ie))
                call omnicopy(tend_normal_velocity_gz_c(:), dycoreVarEdgeFull%tend_normal_velocity_gz%f_r4(:,ie))
                call omnicopy(tend_normal_velocity_ke_c(:), dycoreVarEdgeFull%tend_normal_velocity_ke%f_r4(:,ie))
                call omnicopy(tend_normal_velocity_nct_c(:), dycoreVarEdgeFull%tend_normal_velocity_nct%f_r4(:,ie))
                ...
                tmp1_c(:) = tmp1_c(:) + dtime_r4/rk_number * tmp2_c(:)
...
!$omp do
        do iv = 1, mesh%nv_halo(1)
                ...
                ! the definition of model layer
                 ...
                 dfullf(:,iv) = scalar_template_1d_nlev_c  ! output
                 np1f(:,iv)        = facef(:,iv)
...
#if defined(CHK_INT_RENEW_MASS_STATE) || defined(CHK_ALL)
#include "data_check.inc"
real(r4), allocatable :: ref_scalar_hpressure_at_pc_face_level_f(:, :)
real(r8), allocatable :: ref_scalar_delhp_at_pc_full_level_f(:, :)
real(r8), allocatable :: ref_scalar_hpressure_at_pc_full_level_f(:, :)
real(r8), allocatable :: ref_scalar_delhp_at_pc_face_level_f(:, :)
allocate(ref_scalar_hpressure_at_pc_face_level_f, source=scalar_hpressure_at_pc_face_level%f_r4)
allocate(ref_scalar_delhp_at_pc_full_level_f, source=scalar_delhp_at_pc_full_level%f)
allocate(ref_scalar_hpressure_at_pc_full_level_f, source=scalar_hpressure_at_pc_full_level%f)
allocate(ref_scalar_delhp_at_pc_face_level_f, source=scalar_delhp_at_pc_face_level%f)
#endif 
