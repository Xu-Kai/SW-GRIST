...
!----------------------------------------------------------------------------
 ...
 module grist_dtp_interface_module
   ...
   use grist_dycore_gcd_recon_module_2d_mixed, only: vector_recon_perot_edge2cell_uv_2d
...
! local
    ...
    real(r8)                            :: local_pint(ncell,nlev+1)  ! hpressure plus moisture
    real(r8)                            :: local_pdel(ncell,nlev)    ! hpressure plus moisture
    real(r8)                            :: local_pdel_temp(nlev,ncell)    ! hpressure plus moisture
    real(r8)                            :: local_rpdel(ncell,nlev)   ! hpressure plus moisture
    real(r8)                            :: local_ps(ncell)           ! hpressure plus moisture
    real(r8)                            :: local_uuu(ncell,nlev)
    real(r8)                            :: local_vvv(ncell,nlev)
    ...
    real(r8)                            :: local_dudt(ncell,nlev)
    real(r8)                            :: local_dvdt(ncell,nlev)
    real(r8)                            :: local_dudt_temp(nlev,ncell)
    real(r8)                            :: local_dvdt_temp(nlev,ncell)
    real(r8)                            :: local_dtdt(ncell,nlev)
    real(r8)                            :: local_dqdt(ncell,nlev)
    real(r8)                            :: local_dtdt_temp(nlev,ncell)
    real(r8)                            :: local_dqdt_temp(nlev,ncell)
    real(r8)                            :: tend_U_wind_at_edge_full_level, tend_V_wind_at_edge_full_level 
    real(r8)                            :: point_cart_coort(3), vector_velocity(3)
    integer(i4)                         :: ie, iv, ilev, icell1, icell2, nlevp
...
#include "data_check.inc"
         real(r8), allocatable :: ref_tend_tracer_mxrt_at_pc_full_level(:, :, :)
         real(r8), allocatable :: ref_tend_potential_temp_at_pc_full_level(:, :)
         real(r8), allocatable :: ref_local_pmid(:, :)
         real(r8), allocatable :: ref_local_rpdel(:, :)
         real(r8), allocatable :: ref_local_ps(:)
         real(r8), allocatable :: ref_tend_normal_velocity_at_edge_full_level(:, :)
#endif
...
! from dry to moist
call t_startf("dtp_interface_dcmip2016_tc_a_target_1")        
!$omp target
...
!$omp end target   
call t_stopf("dtp_interface_dcmip2016_tc_a_target_1")
! evaluate moist hydro-pressure for physics
        
#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
   allocate(ref_local_pmid, source=local_pmid)
   allocate(ref_local_rpdel, source=local_rpdel)
   allocate(ref_local_ps, source=local_ps)
#endif  
call t_startf("dtp_interface_dcmip2016_tc_a_target_2")
!$omp target
...
!$omp do     
        do iv = 1, ncell
           do ilev = 1, nlev
            ...
            local_pdel_temp(ilev,iv) = scalar_mpressure_at_pc_face_level(ilev+1,iv)-scalar_mpressure_at_pc_face_level(ilev,iv)
         !      local_rpdel(iv,ilev)= one/local_pdel(iv,ilev)
           end do
         !   do ilev = 1, nlevp
...
!$omp target
!$omp parallel workshare
        local_pmid(1:ncell,1:nlev) = transpose(scalar_mpressure_at_pc_full_level(1:nlev,1:ncell))
        local_pdel(1:ncell,1:nlev) = transpose(local_pdel_temp(1:nlev,1:ncell))
        local_pint(1:ncell,1:nlevp)= transpose(scalar_mpressure_at_pc_face_level(1:nlevp,1:ncell))
        
!$omp end parallel workshare
!$omp end target  
!$omp target
!$omp parallel workshare   
        local_ps(1:ncell)  = local_pint(1:ncell,nlevp)      
!$omp end parallel workshare
!$omp end target
!$omp target
!$omp parallel workshare      
        local_rpdel(1:ncell,1:nlev)= one/local_pdel(1:ncell,1:nlev)
!$omp end parallel workshare
!$omp end target
        
call t_stopf("dtp_interface_dcmip2016_tc_a_target_2")

#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
call t_startf("dtp_interface_dcmip2016_tc_a_2")
!$omp parallel  private(iv,ilev)
!$omp do     
do iv = 1, ncell
   do ilev = 1, nlev
      ref_local_pmid(iv,ilev) = scalar_mpressure_at_pc_full_level(ilev,iv)
      local_pdel(iv,ilev) = scalar_mpressure_at_pc_face_level(ilev+1,iv)-scalar_mpressure_at_pc_face_level(ilev,iv)
      ref_local_rpdel(iv,ilev)= one/local_pdel(iv,ilev)
   end do
   do ilev = 1, nlevp
      local_pint(iv,ilev)= scalar_mpressure_at_pc_face_level(ilev,iv)
   end do
   ref_local_ps(iv)  = local_pint(iv,nlevp)
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("dtp_interface_dcmip2016_tc_a_2")
   call data_check(local_pmid, ref_local_pmid)
   call data_check(local_rpdel, ref_local_rpdel)
   call data_check(local_ps, ref_local_ps)
   deallocate(ref_local_pmid)
   deallocate(ref_local_rpdel)
   deallocate(ref_local_ps)
#endif
...
!
   ...
   allocate(ref_tend_normal_velocity_at_edge_full_level, source=tend_normal_velocity_at_edge_full_level)
#endif
call t_startf("dtp_interface_dcmip2016_tc_a_target_3")

!$omp target
!$omp parallel workshare
local_dudt_temp(1:nlev,1:ncell) = transpose(local_dudt(1:ncell,1:nlev))
local_dvdt_temp(1:nlev,1:ncell) = transpose(local_dvdt(1:ncell,1:nlev))
...
!$omp do 
        do ie = 1, mesh%ne
           ...
           point_cart_coort(:) = mesh%edt_c_p(1:3,ie)
           do ilev = 1, nlev
              tend_U_wind_at_edge_full_level = half*(local_dudt_temp(ilev,icell1)+local_dudt_temp(ilev,icell2))
              tend_V_wind_at_edge_full_level = half*(local_dudt_temp(ilev,icell1)+local_dudt_temp(ilev,icell2))
              call convert_vector_sph2cart(tend_U_wind_at_edge_full_level,&
                                           tend_V_wind_at_edge_full_level,&
                                           point_cart_coort, vector_velocity)
              tend_normal_velocity_at_edge_full_level(ilev,ie) = dot_product(vector_velocity, mesh%edp_nr(1:3,ie))
...
!$omp end parallel
!$omp end target 
call t_stopf("dtp_interface_dcmip2016_tc_a_target_3")

#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
call t_startf("dtp_interface_dcmip2016_tc_a_3")
!$omp parallel  private(ie,icell1,icell2,point_cart_coort,ilev,tend_U_wind_at_edge_full_level,tend_V_wind_at_edge_full_level,vector_velocity)
!$omp do 
do ie = 1, mesh%ne
   !icell1 = mesh%edt(ie)%v(1)
   ...
   do ilev = 1, nlev
      ...
      call convert_vector_sph2cart(tend_U_wind_at_edge_full_level,&
                                   tend_V_wind_at_edge_full_level,&
                                   point_cart_coort, vector_velocity)
      ref_tend_normal_velocity_at_edge_full_level(ilev,ie) = dot_product(vector_velocity, mesh%edp_nr(1:3,ie))
   end do
end do
!$omp end do nowait
!$omp end parallel
call t_stopf("dtp_interface_dcmip2016_tc_a_3")
   call data_check(ref_tend_normal_velocity_at_edge_full_level, tend_normal_velocity_at_edge_full_level)
   deallocate(ref_tend_normal_velocity_at_edge_full_level)
#endif
...
#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
   allocate(ref_tend_tracer_mxrt_at_pc_full_level, source=tend_tracer_mxrt_at_pc_full_level)
   allocate(ref_tend_potential_temp_at_pc_full_level, source=tend_potential_temp_at_pc_full_level)
#endif 
call t_startf("dtp_interface_dcmip2016_tc_a_target_4")
!$omp target
!$omp parallel workshare
local_dqdt_temp(1:nlev,1:ncell) = transpose(local_dqdt(1:ncell,1:nlev))
local_dtdt_temp(1:nlev,1:ncell) = transpose(local_dtdt(1:ncell,1:nlev))
...
do iv = 1, ncell
   do ilev = 1, nlev
      tend_tracer_mxrt_at_pc_full_level(1,ilev,iv) = local_dqdt_temp(ilev,iv)/((one-scalar_qqq_at_pc_full_level(1,ilev,iv))**2)

      tend_potential_temp_at_pc_full_level(ilev,iv) = ((p00/scalar_mpressure_at_pc_full_level(ilev,iv))**(rdry/cp))*&
                                                      (local_dtdt_temp(ilev,iv)*(one+(rvap/rdry)*scalar_tracer_mxrt_at_pc_full_level(1,ilev,iv))+&
...
!$omp end parallel
!$omp end target 
call t_stopf("dtp_interface_dcmip2016_tc_a_target_4")

#if defined(CHK_TEND_NCT_AT_EDGE) || defined(CHK_ALL)
call t_startf("dtp_interface_dcmip2016_tc_a_4")
!$omp parallel  private(iv,ilev)
!$omp do 
do iv = 1, ncell
   do ilev = 1, nlev
      ref_tend_tracer_mxrt_at_pc_full_level(1,ilev,iv) = local_dqdt(iv,ilev)/((one-scalar_qqq_at_pc_full_level(1,ilev,iv))**2)

      ref_tend_potential_temp_at_pc_full_level(ilev,iv) = ((p00/local_pmid(iv,ilev))**(rdry/cp))*&
...
!$omp end do nowait
!$omp end parallel
call t_stopf("dtp_interface_dcmip2016_tc_a_4")
   call data_check(tend_tracer_mxrt_at_pc_full_level,ref_tend_tracer_mxrt_at_pc_full_level)
   call data_ckeck(tend_potential_temp_at_pc_full_level,ref_tend_potential_temp_at_pc_full_level)
   deallocate(ref_tend_tracer_mxrt_at_pc_full_level)
   deallocate(ref_tend_potential_temp_at_pc_full_level)
#endif
      return
