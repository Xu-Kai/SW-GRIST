...
!----------------------------------------------------------------------------
 ...
 module grist_dtp_coupling_module
   ...
   use grist_constants,                  only: r8, i4, pi, zero, one, r4
#ifdef MIXCODE
...
! data module, used vars should be explicitly stated here to avoid any potential conflication
   ...
   use grist_tracer_transport_utils_module_mixed, only: tracer_transport_check_mxrt, &
   ...
   do itracer2 = 1,ubound(ptend_f2%tend_tracer_mxrt_at_pc_full_level%f, 3)
      do itracer1 = 1,ubound(ptend_f2%tend_tracer_mxrt_at_pc_full_level%f, 2)      
         do itracer = 1, ntracer
            ptend_f1%tend_tracer_mass_at_pc_full_level%f(itracer,itracer1,itracer2) = ptend_f1%tend_tracer_mxrt_at_pc_full_level%f(itracer,itracer1,itracer2)*&
...
!$omp parallel  private(itracer2,itracer1,itracer)
!$omp do 
do itracer2 = 1,ubound(tracerVarCellFull%scalar_tracer_mass_n%f_r4, 3)
   do itracer1 = 1,ubound(tracerVarCellFull%scalar_tracer_mass_n%f_r4, 2)
         do itracer = 1, ntracer
            tracerVarCellFull%scalar_tracer_mxrt_n%f_r4(itracer,itracer1,itracer2)  = tracerVarCellFull%scalar_tracer_mass_n%f_r4(itracer,itracer1,itracer2)/&
                                                                    dycoreVarCellFull%scalar_delhp_n%f(itracer1,itracer2)
         end do
      end do
   end do
...
!$omp parallel  private(itracer2,itracer1,itracer)
!$omp do 
   do itracer2 = 1,ubound(tracerVarCellFull%scalar_tracer_mxrt_n%f_r4, 3)
      do itracer1 = 1,ubound(tracerVarCellFull%scalar_tracer_mxrt_n%f_r4, 2)   
        do itracer = 1, ntracer
           tracerVarCellFull%scalar_tracer_mass_n%f_r4(itracer,itracer1,itracer2) = tracerVarCellFull%scalar_tracer_mxrt_n%f_r4(itracer,itracer1,itracer2)*tracerVarCellFull%scalar_delhp_end_adv%f_r4(itracer1,itracer2)
        end do
      end do
   end do
!$omp end do nowait
...
do itracer2 = 1,ubound(tracerVarCellFull%scalar_tracer_mass_n%f_r4, 3)
   do itracer1 = 1,ubound(tracerVarCellFull%scalar_tracer_mass_n%f_r4, 2) 
        do itracer = 1, ntracer
           tracerVarCellFull%scalar_tracer_mxrt_n%f_r4(itracer,itracer1,itracer2) = tracerVarCellFull%scalar_tracer_mass_n%f_r4(itracer,itracer1,itracer2)/tracerVarCellFull%scalar_delhp_end_adv%f_r4(itracer1,itracer2)
        end do
      end do
   end do
!$omp end do nowait
...
#include "data_check.inc"
   ...
   real(r4), allocatable :: ref_scalar_normal_velocity_avg_adv_f(:,:)
#endif
      field_head_2d=>null()
...
!$omp end target
      ...
      deallocate(ref_scalar_normal_mass_flux_avg_adv_f)
#endif
#if defined(D2T_COUPLING_DRIVER) || defined(CHK_ALL)
...
#if defined(D2T_COUPLING_DRIVER) || defined(CHK_ALL)
   allocate(ref_scalar_normal_velocity_avg_adv_f, source = tracerVarEdgeFull%scalar_normal_velocity_avg_adv%f_r4)
#endif 
