...
module grist_amp_shr_flux_mod

! !USES:
   use grist_constants,    only: i4, r8
   use grist_handle_error, only: endrun
!   use shr_kind_mod    ! shared kinds
   use grist_ncar_shr_const_mod   ! shared constants
...
! !PUBLIC DATA MEMBERS:
  ...
  integer(i4),parameter,public :: shr_flux_MOwScales   = 1 ! w scales  option
  integer(i4),parameter,public :: shr_flux_MOfunctions = 2 ! functions option
...
!   integer,parameter :: R8 = r8  ! 8 byte real
   integer,parameter :: IN = i4  ! native/default integer

   integer,parameter :: debug = 0 ! internal debug level
   integer(i4),  parameter :: s_logunit = 6
   integer(i4),  parameter :: s_loglev  = 0
...
!-------------------------------------------------------------------------------
   ...
   if (debug > 1 .and. s_loglev > 0) then
      ...
      if ( option == shr_flux_MOwScales .and. present(arg5) ) then
         ...
         call endrun(subName//"option inconsistant with arguments")
         ...
         call endrun(subName//"invalid option")
      end if
   end if
...
end module grist_amp_shr_flux_mod
