...
!
!=======================================================================
     use grist_constants, only: omega, pi, rearth, rvap, rdry, gravity, cp, r8
...
!=======================================================================
   private
   public :: baroclinic_wave_test, evaluate_pressure_temperature

  real(r8), PARAMETER ::               &
       a     = rearth , & !6371220.0d0,           & ! Reference Earth's Radius (m)
       Rd    = rdry   , & !287.0d0,               & ! Ideal gas const dry air (J kg^-1 K^1)
       g     = gravity, & !9.80616d0,             & ! Gravity (m s^2)
       !cp    = cp !1004.5d0,              & ! Specific heat capacity (J kg^-1 K^1)
...
END MODULE grist_dtp_dcmip2016_bw 
