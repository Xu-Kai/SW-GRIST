...
! linhan add----------------------------------------------------------------------------
      subroutine cldprmc_4DDA_sw(nlayers, inflag, iceflag, liqflag, cldfmc, &
                            ciwpmc, clwpmc, reicmc, dgesmc, relqmc, &
                            taormc, taucmc, ssacmc, asmcmc, fsfcmc,&
...
!-------------linhan-----------------
      real(kind=r8), intent(out) :: asmcmc2(:,:)     ! second parameter of omega (ww) (delta scaled)
                                                        !    Dimensions: (jpband,nlayers)
      real(kind=r8), intent(out) :: asmcmc3(:,:)     ! third parameter of omega (ww) (delta scaled)
!-------------linhan end-------------
...
! Apply delta-M scaling here (using Henyey-Greenstein approximation)
                  ...
                  asmcmc(ig,lay) = (asmcmc(ig,lay) - ffp) / (ffp1)
                  !linhan
                  asmcmc2(ig,lay) = (asmcmc(ig,lay)**2 - ffp) / (ffp1)
                  asmcmc3(ig,lay) = (asmcmc(ig,lay)**3 - ffp) / (ffp1)
                  !linhan end
...
!-------------linhan-----------------
                     istr = 2
                     asmcmc2(ig,lay) = (1.0_r8/(scatliq+scatice))* &
                        (scatliq*(gliq(ig)**istr - forwliq(ig)) / &
                        (1.0_r8 - forwliq(ig)) + scatice * ((gice(ig)-forwice(ig))/ &
                        (1.0_r8 - forwice(ig)))**istr)
                     istr = 3
                     asmcmc3(ig,lay) = (1.0_r8/(scatliq+scatice))* &
                        (scatliq*(gliq(ig)**istr - forwliq(ig)) / &
                        (1.0_r8 - forwliq(ig)) + scatice * ((gice(ig)-forwice(ig))/ &
                        (1.0_r8 - forwice(ig)))**istr)
!-------------linhan end-------------
...
!-------------linhan-----------------
                     istr = 2
                     asmcmc2(ig,lay) = (scatliq *  &
                        (gliq(ig)**istr - forwliq(ig)) / &
                        (1.0_r8 - forwliq(ig)) + scatice * (gice(ig)**istr - forwice(ig)) / &
                        (1.0_r8 - forwice(ig)))/(scatliq + scatice)
                     istr = 3
                     asmcmc3(ig,lay) = (scatliq *  &
                        (gliq(ig)**istr - forwliq(ig)) / &
                        (1.0_r8 - forwliq(ig)) + scatice * (gice(ig)**istr - forwice(ig)) / &
                        (1.0_r8 - forwice(ig)))/(scatliq + scatice)
!-------------linhan end-------------
...
! End layer loop
      ...
      end subroutine cldprmc_4DDA_sw
