...
!      use parkind, only : jpim, jprb
      ...
      use rrtmg_sw_cldprmc, only: cldprmc_sw, cldprmc_4DDA_sw
...
!      use rrtmg_sw_init, only: rrtmg_sw_ini
      ...
      use rrtmg_sw_spcvmc, only: spcvmc_sw, spcvmc_4DDA_sw

      implicit none

! public interfaces/functions/subroutines
!      public :: rrtmg_sw, inatm_sw, earth_sun
      public :: rrtmg_sw,rrtmg_4DDA_sw
...
!      real(kind=r8) :: solvar(jpband)           ! solar constant scaling factor from rrtmg_sw
      ...
      real(kind=r8) :: taua(nbndsw,nlay)      ! Aerosol optical depth
      real(kind=r8) :: ssaa(nbndsw,nlay)      ! Aerosol single scattering albedo
      real(kind=r8) :: asma(nbndsw,nlay)      ! Aerosol asymmetry parameter
...
! Atmosphere/clouds/aerosol - spcvrt,spcvmc
      real(kind=r8) :: ztauc(nbndsw,nlay)     ! cloud optical depth
      real(kind=r8) :: ztaucorig(nbndsw,nlay) ! unscaled cloud optical depth
      real(kind=r8) :: zasyc(nbndsw,nlay)     ! cloud asymmetry parameter 
                                                !  (first moment of phase function)
      real(kind=r8) :: zomgc(nbndsw,nlay)     ! cloud single scattering albedo
      real(kind=r8) :: ztaua(nbndsw,nlay)     ! total aerosol optical depth
      real(kind=r8) :: zasya(nbndsw,nlay)     ! total aerosol asymmetry parameter 
      real(kind=r8) :: zomga(nbndsw,nlay)     ! total aerosol single scattering albedo

      real(kind=r8) :: zcldfmc(ngptsw,nlay)   ! cloud fraction [mcica]
      real(kind=r8) :: ztaucmc(ngptsw,nlay)   ! cloud optical depth [mcica]
      real(kind=r8) :: ztaormc(ngptsw,nlay)   ! unscaled cloud optical depth [mcica]
      real(kind=r8) :: zasycmc(ngptsw,nlay)   ! cloud asymmetry parameter [mcica] 
      real(kind=r8) :: zomgcmc(ngptsw,nlay)   ! cloud single scattering albedo [mcica]
...
! Clouds
         ...
         elseif (icld.ge.1) then
            do i=1,nlay
               do ig=1,ngptsw
                  zcldfmc(ig,i) = cldfmc(ig,i)
                  ztaucmc(ig,i) = taucmc(ig,i)
                  ztaormc(ig,i) = taormc(ig,i)
                  zasycmc(ig,i) = asmcmc(ig,i)
                  zomgcmc(ig,i) = ssacmc(ig,i)
               enddo
            enddo
...
!                  zomga(ib,i) = 1._r8
!                  do ia = 1, naerec
!                     ztaua(ib,i) = ztaua(ib,i) + rsrtaua(ib,ia) * ecaer(ia,i,iplon)
!                     zomga(ib,i) = zomga(ib,i) + rsrtaua(ib,ia) * ecaer(ia,i,iplon) * &
!                                   rsrpiza(ib,ia)
!                     zasya(ib,i) = zasya(ib,i) + rsrtaua(ib,ia) * ecaer(ia,i,iplon) * &
!                                   rsrpiza(ib,ia) * rsrasya(ib,ia)
!                  enddo
!                  if (zomga(ib,i) /= 0._r8) then
!                     zasya(ib,i) = zasya(ib,i) / zomga(ib,i)
!                  endif
!                  if (ztaua(ib,i) /= 0._r8) then
!                     zomga(ib,i) = zomga(ib,i) / ztaua(ib,i)
!                  endif
!               enddo
!            enddo
...
! IAER=10: Direct specification of aerosol optical properties from GCM
         elseif (iaer.eq.10) then
            ...
            do i = 1 ,nlay
               do ib = 1 ,nbndsw
                  ztaua(ib,i) = taua(ib,i)
                  zasya(ib,i) = asma(ib,i)
                  zomga(ib,i) = ssaa(ib,i)
               enddo
            enddo
...
! Call the 2-stream radiation transfer model
         ...
         call spcvmc_sw &
             (iplon, nlay, istart, iend, icpr, idelm, iout, &
...
! Vertical indexing goes from bottom to top
         ...
         do i = 1, nlay+1
            swuflxc(i,iplon) = zbbcu(i)
            swdflxc(i,iplon) = zbbcd(i)
            swuflx(i,iplon) = zbbfu(i)
            swdflx(i,iplon) = zbbfd(i)
            swuflxs(:,i,iplon) = zbbfsu(:,i)
            swdflxs(:,i,iplon) = zbbfsd(:,i)
            uvdflx(i) = zuvfd(i)
            nidflx(i) = znifd(i)
!  Direct/diffuse fluxes
            dirdflux(i) = zbbfddir(i)
            difdflux(i) = swdflx(i,iplon) - dirdflux(i)
!  UV/visible direct/diffuse fluxes
            dirdnuv(i,iplon) = zuvfddir(i)
            difdnuv(i,iplon) = zuvfd(i) - dirdnuv(i,iplon)
!  Near-IR direct/diffuse fluxes
            dirdnir(i,iplon) = znifddir(i)
            difdnir(i,iplon) = znifd(i) - dirdnir(i,iplon)
!  Added for net near-IR diagnostic
            ninflx(i,iplon) = znifd(i) - znifu(i)
            ninflxc(i,iplon) = znicd(i) - znicu(i)
         enddo

!  Total and clear sky net fluxes
         do i = 1, nlay+1
            swnflxc(i) = swdflxc(i,iplon) - swuflxc(i,iplon)
            swnflx(i) = swdflx(i,iplon) - swuflx(i,iplon)
         enddo
...
!  Heating units are in K/d. Flux units are in W/m2.
         do i = 1, nlay
            ...
            swhrc(i,iplon) = (swnflxc(i+1) - swnflxc(i)) * zdpgcp
            swhr(i,iplon) = (swnflx(i+1) - swnflx(i)) * zdpgcp
         enddo
         swhrc(nlay,iplon) = 0._r8
         swhr(nlay,iplon) = 0._r8
...
! Atmosphere/clouds - cldprmc [mcica]
      ...
      real(kind=r8) :: asmcmc(ngptsw,nlay)    ! cloud asymmetry parameter [mcica]
      !linhan 
      real(kind=r8) :: asmcmc2(ngptsw,nlay)    ! second parameter [mcica]
      real(kind=r8) :: asmcmc3(ngptsw,nlay)    ! third parameter [mcica]
      !linhan end
      real(kind=r8) :: fsfcmc(ngptsw,nlay)    ! cloud forward scattering fraction [mcica]
...
! Atmosphere/clouds/aerosol - spcvrt,spcvmc
      ...
      real(kind=r8) :: zasycmc(ngptsw,nlay)   ! cloud asymmetry parameter [mcica] 
      !linhan
      real(kind=r8) :: zasycmc2(ngptsw,nlay)   ! 2nd parameter [mcica] 
      real(kind=r8) :: zasycmc3(ngptsw,nlay)   ! 3rd parameter [mcica] 
      !linhan end
      real(kind=r8) :: zomgcmc(ngptsw,nlay)   ! cloud single scattering albedo [mcica]
...
! Initializations
      ...
      istart = jpb1
      iend = jpb2-1   !linhan deal the overlap
      icpr = 0
      ims = 2
...
!  optical properties are transferred to rrtmg_sw arrays in cldprop.  
         ...
         call cldprmc_4DDA_sw(nlay, inflag, iceflag, liqflag, cldfmc, &
                         ciwpmc, clwpmc, reicmc, dgesmc, relqmc, &
                         taormc, taucmc, ssacmc, asmcmc, fsfcmc, &
                         asmcmc2, asmcmc3) !linhan
         icpr = 1
         ...
         elseif (icld.ge.1) then
            do i=1,nlay
               do ig=1,ngptsw
                  ...
                  ztaormc(ig,i) = taormc(ig,i)
                  zasycmc(ig,i) = asmcmc(ig,i)
                  zasycmc2(ig,i) = asmcmc2(ig,i)
                  zasycmc3(ig,i) = asmcmc3(ig,i)
                  zomgcmc(ig,i) = ssacmc(ig,i)
               enddo
            enddo
...
! Call the 2-stream radiation transfer model
         ...
         call spcvmc_4DDA_sw &
             (iplon, nlay, istart, iend, icpr, idelm, iout, &
              pavel, tavel, pz, tz, tbound, albdif, albdir, &
              zcldfmc, ztaucmc, zasycmc, zasycmc2, zasycmc3, zomgcmc, ztaormc, & !linhan
              ztaua, zasya, zomga, cossza, coldry, wkl, adjflux, &	 
              laytrop, layswtch, laylow, jp, jt, jt1, &
              co2mult, colch4, colco2, colh2o, colmol, coln2o, colo2, colo3, &
              fac00, fac01, fac10, fac11, &
...
!  from GCM fields should be reversed here if necessary.
      ...
      pz(0) = plev(nlay+1,iplon)
      tz(0) = tlev(nlay+1,iplon)
      do l = 1, nlay
         pavel(l) = play(nlay-l+1,iplon)
         tavel(l) = tlay(nlay-l+1,iplon)
         pz(l) = plev(nlay-l+1,iplon)
         tz(l) = tlev(nlay-l+1,iplon)
         pdp(l) = pz(l-1) - pz(l)
! For h2o input in vmr:
         wkl(1,l) = h2ovmr(nlay-l+1,iplon)
...
!         wkl(1,l) = (h2o(iplon,nlayers-l)/(1._r8 - h2o(iplon,nlayers-l)))*amdw
         wkl(2,l) = co2vmr(nlay-l+1,iplon)
         wkl(3,l) = o3vmr(nlay-l+1,iplon)
         wkl(4,l) = n2ovmr(nlay-l+1,iplon)
         wkl(6,l) = ch4vmr(nlay-l+1,iplon)
         wkl(7,l) = o2vmr(nlay-l+1,iplon) 
...
! modify to reverse layer indexing here if necessary.
      ...
      if (iaer .ge. 1) then 
         do l = 1, nlay-1
            do ib = 1, nbndsw
               taua(ib,l) = tauaer(ib,nlay-l,iplon)
               ssaa(ib,l) = ssaaer(ib,nlay-l,iplon)
               asma(ib,l) = asmaer(ib,nlay-l,iplon)
            enddo
         enddo
      endif
...
! convert effective radius to generalized effective size using method of Mitchell, JAS, 2002:
         ...
         do l = 1, nlay-1
            do ig = 1, ngptsw
               cldfmc(ig,l) = cldfmcl(ig,nlay-l,iplon)
               taucmc(ig,l) = taucmcl(ig,nlay-l,iplon)
               ssacmc(ig,l) = ssacmcl(ig,nlay-l,iplon)
               asmcmc(ig,l) = asmcmcl(ig,nlay-l,iplon)
               fsfcmc(ig,l) = fsfcmcl(ig,nlay-l,iplon)
               ciwpmc(ig,l) = ciwpmcl(ig,nlay-l,iplon)
               clwpmc(ig,l) = clwpmcl(ig,nlay-l,iplon)
            enddo
            reicmc(l) = reicmcl(nlay-l,iplon)
            if (iceflag .eq. 3) then
               dgesmc(l) = 1.5396_r8 * reicmcl(nlay-l,iplon)
            endif
            relqmc(l) = relqmcl(nlay-l,iplon)
         enddo
