...
!      use parkind, only : jpim, jprb 
      ...
      use rrtmg_lw_cldprmc, only: cldprmc, cldprmc_4DDA
! Move call to rrtmg_lw_ini and following use association to 
! GCM initialization area
!      use rrtmg_lw_init, only: rrtmg_lw_ini
      use rrtmg_lw_rtrnmc, only: rtrnmc, rtrnmc_4DDA
...
!------------------------------------------------------------------
      ...
      subroutine rrtmg_lw &
            (ncol    ,nlay    ,icld    ,                            &
             ...
             cldfmcl ,taucmcl ,ciwpmcl , cswpmcl, clwpmcl ,reicmcl , resnmcl, relqmcl , & !linhan
             tauaer  ,                                              &
...
! ----- Input -----
      ...
      real(kind=r8), intent(in) :: ciwpmcl(:,:,:)       ! Cloud ice water path (g/m2)
                                                        !    Dimensions: (ngptlw,nlay,ncol)
      real(kind=r8), intent(in) :: cswpmcl(:,:,:)       ! Cloud snow water path (g/m2) linhan
                                                        !    Dimensions: (ngptlw,nlay,ncol)
      ...
      real(kind=r8), intent(in) :: reicmcl(:,:)         ! Cloud ice effective radius (microns)
                                                        !    Dimensions: (nlay,ncol)
      real(kind=r8), intent(in) :: resnmcl(:,:)         ! Cloud snow effective radius (microns) linhan
                                                        !    Dimensions: (nlay,ncol)
...
! Atmosphere
      ...
      real(kind=r8) :: fracs(ngptlw,nlay)       ! 
      real(kind=r8) :: taug(ngptlw,nlay)        ! gaseous optical depths
      real(kind=r8) :: taut(ngptlw,nlay)        ! gaseous + aerosol optical depths

      real(kind=r8) :: taua(nbndlw,nlay)        ! aerosol optical depth
!      real(kind=r8) :: ssaa(nbndlw,nlay)        ! aerosol single scattering albedo
                                                 !   for future expansion 
                                                 !   (lw aerosols/scattering not yet available)
!      real(kind=r8) :: asma(nbndlw,nlay+1)      ! aerosol asymmetry parameter
                                                 !   for future expansion 
                                                 !   (lw aerosols/scattering not yet available)
...
! Atmosphere - setcoef
      ...
      real(kind=r8) :: planklay(nbndlw,nlay)      ! 
      real(kind=r8) :: planklev(nbndlw,0:nlay)    ! 
...
! Atmosphere/clouds - cldprmc [mcica]
      ...
      real(kind=r8) :: cswpmc(ngptlw,nlay)      ! cloud snow water path [mcica] linhan
      ...
      real(kind=r8) :: rescmc(nlay)             ! snow particle effective radius (microns)!linhan
...
!  other input parameters.  
         ...
         call inatm (iplon, nlay, icld, iaer, &
              ...
              cfc22vmr, ccl4vmr, emis, inflglw, iceflglw, liqflglw, &
              cldfmcl, taucmcl, ciwpmcl, cswpmcl, clwpmcl, reicmcl, resnmcl, relqmcl, tauaer, &!linhan
              pavel, pz, tavel, tz, tbound, semiss, coldry, &
              wkl, wbrodl, wx, pwvcm, inflag, iceflag, liqflag, &
              cldfmc, taucmc, ciwpmc, cswpmc, clwpmc, reicmc, rescmc, dgesmc, relqmc, taua) !linhan
...
! Combine gaseous and aerosol optical depths, if aerosol active
         ...
         elseif (iaer .eq. 10) then
            do k = 1, nlay
               do ig = 1, ngptlw 
                  taut(ig,k) = taug(ig,k) + taua(ngb(ig),k)
               enddo
            enddo
         endif
...
!  Vertical indexing goes from bottom to top
         ...
         do k = 0, nlay
            uflx(k+1,iplon) = totuflux(k)
            dflx(k+1,iplon) = totdflux(k)
            uflxc(k+1,iplon) = totuclfl(k)
            dflxc(k+1,iplon) = totdclfl(k)
            uflxs(:,k+1,iplon) = totufluxs(:,k)
            dflxs(:,k+1,iplon) = totdfluxs(:,k)
         enddo
         do k = 0, nlay-1
            hr(k+1,iplon) = htr(k)
            hrc(k+1,iplon) = htrc(k)
         enddo
...
!---------linhan add 4DDA--------------------------
      subroutine rrtmg_4DDA_lw &
            (ncol    ,nlay    ,icld    ,coszrs  ,               &   !linhan add coszrs
             play    ,plev    ,tlay    ,tlev    ,tsfc    ,h2ovmr  , &
             o3vmr   ,co2vmr  ,ch4vmr  ,o2vmr   ,n2ovmr  ,          &
             cfc11vmr,cfc12vmr,                                     &
             cfc22vmr,ccl4vmr ,emis    ,inflglw ,iceflglw,liqflglw, &
             cldfmcl ,taucmcl ,ciwpmcl ,cswpmcl, clwpmcl ,reicmcl ,rescmcl, relqmcl , &!linhan
             tauaer  ,                                              &
             uflx    ,dflx    ,hr      ,uflxc   ,dflxc,  hrc, uflxs, dflxs )
...
! ----- Input -----
      ...
      integer, intent(inout) :: icld                    ! Cloud overlap method
                                                        ...
                                                        !    2: Maximum/random
                                                        !    3: Maximum
      real(kind=r8), intent(in) :: coszrs(:)                 ! cos of zenith  linhan
                                                        !    Dimensions: (ncol)
      real(kind=r8), intent(in) :: play(:,:)            ! Layer pressures (hPa, mb)
                                                        !    Dimensions: (nlay,ncol)
      real(kind=r8), intent(in) :: plev(:,:)            ! Interface pressures (hPa, mb)
      ...
      real(kind=r8), intent(in) :: reicmcl(:,:)         ! Cloud ice effective radius (microns)
                                                        !    Dimensions: (nlay,ncol)
      real(kind=r8), intent(in) :: rescmcl(:,:)         ! Cloud snow effective radius (microns)!linhan
                                                        !    Dimensions: (nlay,ncol)
      real(kind=r8), intent(in) :: relqmcl(:,:)         ! Cloud water drop effective radius (microns)
                                                        !    Dimensions: (nlay,ncol)
      real(kind=r8), intent(in) :: taucmcl(:,:,:)       ! Cloud optical depth
...
! Atmosphere/clouds - cldprmc [mcica]
      ...
      real(kind=r8) :: dgesmc(nlay)             ! ice particle generalized effective size (microns)
      real(kind=r8) :: taucmc(ngptlw,nlay)      ! cloud optical depth [mcica]
      real(kind=r8) :: ssacmc(ngptlw,nlay)     ! cloud single scattering albedo [mcica] linhan
                                                !   for future expansion 
                                                !   (lw scattering not yet available)
      real(kind=r8) :: asmcmc(ngptlw,nlay)     ! cloud asymmetry parameter [mcica] linhan
                                                !   for future expansion 
                                                !   (lw scattering not yet available)
...
!  other input parameters.  
         ...
         call inatm (iplon, nlay, icld, iaer, &
              ...
              o3vmr, co2vmr, ch4vmr, o2vmr, n2ovmr, cfc11vmr, cfc12vmr, &
              cfc22vmr, ccl4vmr, emis, inflglw, iceflglw, liqflglw, &
              cldfmcl, taucmcl, ciwpmcl, cswpmcl, clwpmcl, reicmcl,rescmcl, relqmcl, tauaer, &!linhan
              pavel, pz, tavel, tz, tbound, semiss, coldry, &
              wkl, wbrodl, wx, pwvcm, inflag, iceflag, liqflag, &
              cldfmc, taucmc, ciwpmc, cswpmc, clwpmc, reicmc, rescmc, dgesmc, relqmc, taua) !linhan
...
!  optical depth are transferred to rrtmg_lw arrays in cldprop.  
         ...
         call cldprmc_4DDA(nlay, inflag, iceflag, liqflag, cldfmc, ciwpmc, cswpmc, &!linhan
                      clwpmc, reicmc, rescmc, dgesmc, relqmc, ncbands, taucmc, & !linhan
                      ssacmc, asmcmc)
...
! For McICA, RTRNMC is called for clear and cloudy calculations.
        ...
        !------------------linhan--------------------
         call rtrnmc_4DDA(nlay, istart, iend, iout, pz, semiss, ncbands, &
                     cldfmc, taucmc, planklay, planklev, plankbnd, &
                     pwvcm, fracs, taut, &
...
! Atmosphere/clouds - cldprop
      ...
      real(kind=r8), intent(out) :: ciwpmc(:,:)         ! cloud ice water path [mcica]
                                                        !    Dimensions: (ngptlw,nlay)
      real(kind=r8), intent(out) :: cswpmc(:,:)         ! cloud snow water path [mcica] linhan
                                                        !    Dimensions: (ngptlw,nlay)
      ...
      real(kind=r8), intent(out) :: reicmc(:)           ! ice particle effective radius (microns)
                                                        !    Dimensions: (nlay)
      real(kind=r8), intent(out) :: rescmc(:)           ! snow particle effective radius (microns)linhan
                                                        !    Dimensions: (nlay)
...
!  into RRTM arrays below.
      ...
      taucmc(:,:) = 0.0_r8
      ciwpmc(:,:) = 0.0_r8
      cswpmc(:,:) = 0.0_r8 !linhan
      clwpmc(:,:) = 0.0_r8
      reicmc(:) = 0.0_r8
      rescmc(:) = 0.0_r8 !linhan
      dgesmc(:) = 0.0_r8
      relqmc(:) = 0.0_r8
      taua(:,:) = 0.0_r8
      amttl = 0.0_r8
...
!         wkl(1,l) = h2o(nlay-l,iplon)*amdw
! For h2o input in specific humidity;
!         wkl(1,l) = (h2o(nlay-l,iplon)/(1._r8 - h2o(nlay-l,iplon)))*amdw
...
! Set cross section molecule amounts from input; convert to vmr if necessary
         wx(1,l) = ccl4vmr(nlay-l+1,iplon)
         wx(2,l) = cfc11vmr(nlay-l+1,iplon)
         wx(3,l) = cfc12vmr(nlay-l+1,iplon)
         wx(4,l) = cfc22vmr(nlay-l+1,iplon)
...
! convert effective radius to generalized effective size using method of Mitchell, JAS, 2002:
         ...
         do l = 1, nlay-1
            do ig = 1, ngptlw
               ...
               cswpmc(ig,l) = cswpmcl(ig,nlay-l,iplon) !linhan
               clwpmc(ig,l) = clwpmcl(ig,nlay-l,iplon)
            enddo
            reicmc(l) = reicmcl(nlay-l,iplon)
            rescmc(l) = rescmcl(nlay-l,iplon) !linhan
...
! If an extra layer is being used in RRTMG, set all cloud properties to zero in the extra layer.
         ...
         taucmc(:,nlay) = 0.0_r8
         ciwpmc(:,nlay) = 0.0_r8
         cswpmc(:,nlay) = 0.0_r8 !linhan
         clwpmc(:,nlay) = 0.0_r8
         reicmc(nlay) = 0.0_r8
         rescmc(nlay) = 0.0_r8 !linhan
         dgesmc(nlay) = 0.0_r8
         relqmc(nlay) = 0.0_r8
         taua(:,nlay) = 0.0_r8
