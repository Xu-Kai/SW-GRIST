...
! ------- Output -------
      ...
      real(kind=r8), intent(out) :: asmcloud(:,:)     ! asymmetry parameter (delta scaled)
                                                        !    Dimensions: (jpband,nlayers)
!-------------linhan-----------------
      real(kind=r8), intent(out) :: asmcloud2(:,:)     ! second parameter of omega (ww) (delta scaled)
                                                        !    Dimensions: (jpband,nlayers)
      real(kind=r8), intent(out) :: asmcloud3(:,:)     ! third parameter of omega (ww) (delta scaled)
!-------------linhan end-------------
...
!      ncbands = 29
      ...
      do lay = 1, nlayers
         do ib = ib1 , ib2
            taucloud(ib,lay) = 0.0_r8
            ssacloud(ib,lay) = 1.0_r8
            asmcloud(ib,lay) = 0.0_r8
...
! Apply delta-M scaling here
            if (inflag .eq. 0) then
               ...
               do ib = ib1 , ib2
                  ...
                  taucldorig(ib,lay) = taucldorig_a
                  ssacloud(ib,lay) = ssacloud_a
                  taucloud(ib,lay) = taucloud_a
                  asmcloud(ib,lay) = (asmc(ib-15,lay) - ffp) / (ffp1)
               enddo
...
! Check to ensure all calculated quantities are within physical limits.
                ...
                do ib = ib1 , ib2
                   ...
                   taucldorig(ib,lay) = tauliqorig + tauiceorig
                   ...
                   taucloud(ib,lay) = tauliq + tauice

! Ensure non-zero taucmc and scatice
                   if (taucloud(ib,lay).eq.0.0_r8) taucloud(ib,lay) = cldmin
                   if (scatice.eq.0.0_r8) scatice = cldmin

                   ssacloud(ib,lay) = (scatliq + scatice) / taucloud(ib,lay)
...
! or hexagonal ice crystals.
                      istr = 1
                      asmcloud(ib,lay) = (1.0_r8/(scatliq+scatice)) * &
                         (scatliq*(gliq(ib)**istr - forwliq(ib)) / &
                         (1.0_r8 - forwliq(ib)) + scatice * ((gice(ib)-forwice(ib)) / &
                         (1.0_r8 - forwice(ib)))**istr)
!-------------linhan-----------------
                      istr = 2
                      asmcloud2(ib,lay) = (1.0_r8/(scatliq+scatice)) * &
                         (scatliq*(gliq(ib)**istr - forwliq(ib)) / &
                         (1.0_r8 - forwliq(ib)) + scatice * ((gice(ib)-forwice(ib)) / &
                         (1.0_r8 - forwice(ib)))**istr)
                      istr = 3
                      asmcloud3(ib,lay) = (1.0_r8/(scatliq+scatice)) * &
                         (scatliq*(gliq(ib)**istr - forwliq(ib)) / &
                         (1.0_r8 - forwliq(ib)) + scatice * ((gice(ib)-forwice(ib)) / &
                         (1.0_r8 - forwice(ib)))**istr)
!-------------linhan end-------------
