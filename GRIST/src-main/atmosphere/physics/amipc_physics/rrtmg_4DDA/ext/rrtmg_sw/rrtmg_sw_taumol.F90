         ...
         do ig = 1, ng17
            taug(ngs16+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 forfrac(lay) * &
                 (forref(indf+1,ig) - forref(indf,ig)))) 
!             ssa(ngs16+ig,lay) = tauray/taug(ngs16+ig,lay)
            taur(ngs16+ig,lay) = tauray
         enddo
      enddo
         ...
         do ig = 1, ng18
            taug(ngs17+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 forfrac(lay) * &
                 (forref(indf+1,ig) - forref(indf,ig)))) 
!            ssa(ngs17+ig,lay) = tauray/taug(ngs17+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs17+ig) = sfluxref(ig,js) &
               + fs * (sfluxref(ig,js+1) - sfluxref(ig,js))
            taur(ngs17+ig,lay) = tauray
         enddo
      enddo
         ...
         do ig = 1 , ng19
            taug(ngs18+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 forfrac(lay) * &
                 (forref(indf+1,ig) - forref(indf,ig)))) 
!            ssa(ngs18+ig,lay) = tauray/taug(ngs18+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs18+ig) = sfluxref(ig,js) &
               + fs * (sfluxref(ig,js+1) - sfluxref(ig,js))
            taur(ngs18+ig,lay) = tauray   
         enddo
      enddo
...
!            ssa(ngs19+ig,lay) = tauray/taug(ngs19+ig,lay)
            taur(ngs19+ig,lay) = tauray 
         ...
         do ig = 1, ng21
            taug(ngs20+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 forfrac(lay) * &
                 (forref(indf+1,ig) - forref(indf,ig))))
!            ssa(ngs20+ig,lay) = tauray/taug(ngs20+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs20+ig) = sfluxref(ig,js) &
               + fs * (sfluxref(ig,js+1) - sfluxref(ig,js))
            taur(ngs20+ig,lay) = tauray
         enddo
      enddo
         ...
         do ig = 1, ng22
            taug(ngs21+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 (forref(indf+1,ig) - forref(indf,ig)))) &
                 + o2cont
!            ssa(ngs21+ig,lay) = tauray/taug(ngs21+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs21+ig) = sfluxref(ig,js) &
                + fs * (sfluxref(ig,js+1) - sfluxref(ig,js))
            taur(ngs21+ig,lay) = tauray
         enddo
      enddo
         ...
         do ig = 1, ng23
            ...
            taug(ngs22+ig,lay) = colh2o(lay) * &
                (givfac * (fac00(lay) * absa(ind0,ig) + &
                 ...
                 forfrac(lay) * &
                 (forref(indf+1,ig) - forref(indf,ig)))) 
!            ssa(ngs22+ig,lay) = tauray/taug(ngs22+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs22+ig) = sfluxref(ig) 
            taur(ngs22+ig,lay) = tauray
         enddo
      enddo
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         do ig = 1, ng23
!            taug(ngs22+ig,lay) = colmol(lay) * rayl(ig)
!            ssa(ngs22+ig,lay) = 1.0_r8
            taug(ngs22+ig,lay) = 0._r8
            taur(ngs22+ig,lay) = colmol(lay) * rayl(ig) 
         enddo
      enddo
         ...
         do ig = 1, ng24
            ...
            taug(ngs23+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 forfrac(lay) * &
                 (forref(indf+1,ig) - forref(indf,ig))))
!            ssa(ngs23+ig,lay) = tauray/taug(ngs23+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs23+ig) = sfluxref(ig,js) &
               + fs * (sfluxref(ig,js+1) - sfluxref(ig,js))
            taur(ngs23+ig,lay) = tauray
         enddo
      enddo
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         do ig = 1, ng25
            tauray = colmol(lay) * rayl(ig)
            taug(ngs24+ig,lay) = colh2o(lay) * &
                (fac00(lay) * absa(ind0,ig) + &
                 fac10(lay) * absa(ind0+1,ig) + &
                 fac01(lay) * absa(ind1,ig) + &
                 fac11(lay) * absa(ind1+1,ig)) + &
                 colo3(lay) * abso3a(ig) 
!            ssa(ngs24+ig,lay) = tauray/taug(ngs24+ig,lay)
            if (lay .eq. laysolfr) sfluxzen(ngs24+ig) = sfluxref(ig) 
            taur(ngs24+ig,lay) = tauray
         enddo
      enddo
...
! Lower atmosphere loop
      do lay = 1, laytrop
         do ig = 1, ng26 
!            taug(ngs25+ig,lay) = colmol(lay) * rayl(ig)
!            ssa(ngs25+ig,lay) = 1.0_r8
            if (lay .eq. laysolfr) sfluxzen(ngs25+ig) = sfluxref(ig) 
            taug(ngs25+ig,lay) = 0._r8
            taur(ngs25+ig,lay) = colmol(lay) * rayl(ig) 
         enddo
      enddo
         ...
         do ig = 1, ng27
            ...
            taug(ngs26+ig,lay) = colo3(lay) * &
                (fac00(lay) * absa(ind0,ig) + &
                 ...
                 fac01(lay) * absa(ind1,ig) + &
                 fac11(lay) * absa(ind1+1,ig))
!            ssa(ngs26+ig,lay) = tauray/taug(ngs26+ig,lay)
            taur(ngs26+ig,lay) = tauray
         enddo
      enddo
         ...
         do ig = 1, ng28
            taug(ngs27+ig,lay) = speccomb * &
                (fac000 * absa(ind0,ig) + &
                 ...
                 fac011 * absa(ind1+9,ig) + &
                 fac111 * absa(ind1+10,ig)) 
!            ssa(ngs27+ig,lay) = tauray/taug(ngs27+ig,lay)
            taur(ngs27+ig,lay) = tauray
         enddo
      enddo
...
!            ssa(ngs28+ig,lay) = tauray/taug(ngs28+ig,lay)
            taur(ngs28+ig,lay) = tauray
         enddo
      enddo
