...
!===================================================================================================
 ...
 module grist_rad_constituents

    use grist_handle_error,                 only: endrun
    use grist_constants,                    only: r8, i4
    use grist_nml_module,                   only: nlev, nlevp, ntracer
    use grist_phys_prop,                    only: physprop_accum_unique_files,      &
                                                  physprop_init  
    use radconstants,                       only: nradgas, ot_length,               &
                                                  rad_gas_index
    use grist_phys_prop,                    only: physprop_get_id
    ...
    public          :: read_nml_rad_cnst         ,  &! read namelist values and parse
                       rad_cnst_get_info         ,  &! return info about climate/diagnostic lists
                       rad_cnst_get_gas          ,  &! return physical properties to mmr for gasses
                       rad_cnst_get_aer_mmr      ,  &! return physical properties to mmr for aerosols
...
! Namelist variables
    ...
    integer, parameter :: fieldname_len = 24

! LiXH: save the ncol for GRIST
    integer :: ncol


 contains

    subroutine read_nml_rad_cnst(nlfile)
    ...
    character(len=*), parameter :: subname = 'read_nml_rad_cnst'
    ...
    ! lists.
    if (mpi_rank()==0) print*, nl//subname//': Radiation constituent lists:'
    do i = 0, N_DIAG
       if (active_calls(i)) then
    ...
    if (mpi_rank()==0 .and. verbose) call print_modes(modes)

    end subroutine read_nml_rad_cnst


! Purpose: The initialization of the gas and aerosol lists is finished by
...
!             physprop module.
    subroutine rad_cnst_init(ncol_in)
    ! io
...
!    ! or physics buffer arrays.
    if (mpi_rank()==0) print*, nl//subname//': checking for radiative constituents'
...
! Purpose: Return pointer to mass mixing ratio for the gas from the specified
!          climate or diagnostic list.
    subroutine rad_cnst_get_gas(list_idx, gasname, mmr)
    ! io
    integer,                     intent(in) :: list_idx    ! index of the climate or a diagnostic list
    character(len=*),            intent(in) :: gasname
    real(r8),                    intent(out):: mmr(:,:)
    ...
    if (list_idx >= 0 .and. list_idx <= N_DIAG) then
       ...
       if(mpi_rank()==0)print*, subname//': list_idx =', list_idx
    ...
    select case( source )
    case ('A')
       mmr = pstate%tracer_mxrt_at_pc_full_level%f(idx,:,1:ncol)
    case ('G')
       mmr = pstate_cam%ghg_at_pc_full_level(idx)%f(:,1:ncol)
    case ('Z')
       mmr = zero_cols
    case default
       if(mpi_rank()==0)print*,subname//': Error in gaslist source!' 
       call endrun(subname)
    end select
...
! Purpose : Return info about gas and aerosol lists 
    ...
    if (present(aernames)) then
       ...
       if (arrlen < a_list%numaerosols) then
          if(mpi_rank()==0)print*, subname//': ERROR: naero=', a_list%numaerosols, '  arrlen=', arrlen
    ...
    if (present(gasnames)) then
       ...
       if (gaslen < g_list%ngas) then
          if(mpi_rank()==0)print*, subname//': ERROR: ngas=', g_list%ngas, '  gaslen=', gaslen
...
! Purpose: Return info about modal aerosol lists
    ...
    if (m_idx < 1 .or. m_idx > nmodes) then
       if(mpi_rank()==0)print*, subname//': ERROR - invalid mode index: ', m_idx
...
!               spec_type, spec_name, spec_name_cw)
   spec_type, spec_name, spec_name_cw, pbuf_idx)
!--------------LiXH test--------------
    ! io
    integer,                     intent(in)  :: list_idx    ! index of the climate or a diagnostic list
    integer,                     intent(in)  :: m_idx       ! index of mode in the specified list
...
!--------------LiXH test--------------
   integer,optional, intent(out) :: pbuf_idx
!--------------LiXH test--------------
    ...
    if (s_idx < 1 .or. s_idx > nspec) then
       if(mpi_rank()==0)print*, subname//': ERROR - invalid specie index: ', s_idx
...
!--------------LiXH test--------------
if(present(pbuf_idx))then
    pbuf_idx = modes%comps(mm)%idx_mmr_a(s_idx)
endif
!--------------LiXH test--------------
...
! Purpose: Return info about modes in the specified climate/diagnostics list
...
! Purpose: Return info about which climate/diagnostic calculations are requested
    ...
    do m = 1, modes%nmodes
       ...
       modes%comps(m)%idx_num_a = get_idx(modes%comps(m)%source_num_a, modes%comps(m)%camname_num_a, routine)
       modes%comps(m)%idx_num_c = get_idx(modes%comps(m)%source_num_c, modes%comps(m)%camname_num_c, routine)
      ...
      do ispec = 1, nspec
          ...
          modes%comps(m)%idx_mmr_a(ispec) = get_idx(modes%comps(m)%source_mmr_a(ispec),      &
                                                    modes%comps(m)%camname_mmr_a(ispec), routine)
          modes%comps(m)%idx_mmr_c(ispec) = get_idx(modes%comps(m)%source_mmr_c(ispec),      &
          ...
          if (modes%comps(m)%idx_props(ispec) == -1) then
              if(mpi_rank()==0)print*,routine//' : ERROR idx not found for '//trim(modes%comps(m)%props(ispec))
              call endrun('init_mode_comps')
          end if
...
! Purpose: get index of name in internal CAM array; either the constituent array
!          or the physics buffer
! LiXH modified for pstate in GRIST
    integer function get_idx(source, name, routine)
    ...
    !local
    integer :: idx, naero, ngas
 
    if (source(1:1) == 'N') then
 
       naero = pstate_cam%total_aerosol_num 
       get_idx = -1
       do idx = 1, naero
          if(trim(name) .eq. trim(pstate_cam%aerosol_at_pc_full_level(idx)%name))then
             get_idx = idx
          end if
       end do 
       ...
       if (get_idx < 0) then
           if(mpi_rank()==0)then
               print*,routine//' ERROR: cannot find aerosol '//trim(name)
               do idx = 1, naero
                  print*,'aerosol list:',idx, trim(pstate_cam%aerosol_at_pc_full_level(idx)%name)
               end do
           end if 
           call endrun('get_idx in grist_rad_constituents')
       end if
 
    elseif (source(1:1) == 'G') then
 
       ngas = pstate_cam%total_ghg_num
       get_idx = -1
       do idx = 1, ngas
          if(trim(name) .eq. trim(pstate_cam%ghg_at_pc_full_level(idx)%name))then
             get_idx = idx
          end if
       end do 
       ...
       if (get_idx < 0) then
           if(mpi_rank()==0)then
               print*,routine//' ERROR: cannot find gas '//trim(name)
               do idx = 1, ngas
                  print*,'ghg list:',idx, trim(pstate_cam%ghg_at_pc_full_level(idx)%name)
               end do
           end if 
           call endrun('get_idx in grist_rad_constituents')
       end if
       ...
       get_idx = -1
       do idx = 1, ntracer
          if(trim(name) .eq. trim(phy_tracer_info(idx)%longname))then
             get_idx = idx
          end if
       end do 
       ...
       if (get_idx < 0) then
           if(mpi_rank()==0)then
               print*,routine//' ERROR: cannot find tracer '//trim(name)
               do idx = 1, ntracer
                  print*,'tracer list:',idx, trim(phy_tracer_info(idx)%longname)
               end do 
           end if 
           call endrun('get_idx in grist_rad_constituents')
       end if
       ...
       get_idx = -1
        ...
        if(mpi_rank()==0)print*,routine//' ERROR: invalid source for specie '//trim(name) 
       call endrun('get_idx in grist_rad_constituents')
...
! Purpose: Initialize the gas and bulk and modal aerosol lists with the
...
!          is available at the time the namelist is read.
    ...
    allocate( aerlist%aer(aerlist%numaerosols),       &
    ...
    if (mpi_rank()==0 .and. verbose) then
       if (len_trim(gaslist%list_id) == 0) then
          print*, nl//' '//routine//': namelist input for climate list'
       else
          print*, nl//' '//routine//': namelist input for diagnostic list:'//gaslist%list_id
       end if
    end if
    ...
    do ii = 1, namelist%ncnst
       ...
       if (mpi_rank()==0 .and. verbose) &
          print*, "  rad namelist spec: "// trim(namelist%source(ii)) &
          //":"//trim(namelist%camname(ii))//":"//trim(namelist%radname(ii))

       ! Check that the source specifier is legal.
       !--------------LiXH add source 'G' to search for ghg data index------------>
       ...
       if (namelist%source(ii) /= 'A' .and. namelist%source(ii) /= 'M' .and. &
           ...
           namelist%source(ii) /= 'G') then
          if (mpi_rank()==0)print*,routine//": source must either be A, M, N, G, or Z:"//&
...
! Purpose : Final initialization phase gets the component indices in the constituent array
!           and the physics buffer, and indices into physprop module.
    ...
    do i = 1, gaslist%ngas
       ...
       gaslist%gas(i)%idx = get_idx(gaslist%gas(i)%source, gaslist%gas(i)%camname, routine)
    end do

    ! Loop over bulk aerosols
    do i = 1, aerlist%numaerosols
       ! locate the specie mixing ratio in the pbuf or state
       aerlist%aer(i)%idx = get_idx(aerlist%aer(i)%source, aerlist%aer(i)%camname, routine)
...
! Purpose:  Add diagnostic fields to the master fieldlist.
    ...
    do i = 1, ngas
       ...
       if (len_trim(name) > fieldname_len) then
          if(mpi_rank()==0)print*, 'rad_gas_diag_init: '//trim(name)//' longer than ', fieldname_len, ' characters'
...
! Purpose:  Add diagnostic fields to the master fieldlist.
    ...
    do i = 1, naer
       ...
       ! error check for name length
       if (len_trim(name) > fieldname_len) then
           if(mpi_rank()==0)print*, 'rad_aer_diag_init: '//trim(name)//' longer than ', fieldname_len, ' characters'
...
! Purpose: Parse the mode definition specifiers.  The specifiers are of the form:
! 
!          'mode_name:mode_type:=',
...
!          and at least 1 string for the species.
    ...
    do m = 1, nmodes
       ...
       allocate( modes%comps(m)%source_mmr_a(nspec),  &
    ...
    subroutine parse_error(msg, str)
       ...
       print*, 'rank = ', mpi_rank()
       print*, routine//': ERROR: '//msg
       print*, ' input string: '//trim(str)
...
! Purpose: Private method for parsing the radiation namelist specifiers.  The specifiers
...
! and adds that info to a structure that stores the parsed data.
    ...
    allocate(namelist_data%source (number))
    allocate(namelist_data%camname(number))
    allocate(namelist_data%radname(number))
    allocate(namelist_data%type(number))
...
! Purpose : Return pointer to mass mixing ratio for the aerosol from the specified
!           climate or diagnostic list.
    subroutine rad_cnst_get_aer_mmr_by_idx(list_idx, aer_idx, mmr,istart,istop)
    ! io
    integer,                     intent(in) :: list_idx    ! index of the climate or a diagnostic list
    ...
    integer,optional,intent(in) :: istart,istop
    !-----------------------------------------------------------------------------
    ...
    if (list_idx >= 0 .and. list_idx <= N_DIAG) then
       ...
       if(mpi_rank()==0)then
           print*, subname//': list_idx =', list_idx
           call endrun(subname//': list_idx out of bounds')
       end if 
    endif
    ...
    if (aer_idx < 1  .or.  aer_idx > aerlist%numaerosols) then
       if(mpi_rank()==0)then 
           print*, subname//': aer_idx= ', aer_idx, '  numaerosols= ', aerlist%numaerosols
    ...
    case ('A')
       ...
       mmr(:,istart:istop) = pstate%tracer_mxrt_at_pc_full_level%f(idx,:,istart:istop)
       else
       mmr = pstate%tracer_mxrt_at_pc_full_level%f(idx,:,1:ncol)
       end if
    case ('N')
       if (present(istart) .and. present(istop)) then
       mmr(:,istart:istop) = pstate_cam%aerosol_at_pc_full_level(idx)%f(:,istart:istop)
       else
       mmr = pstate_cam%aerosol_at_pc_full_level(idx)%f(:,1:ncol)
       end if
    case ('Z')
       mmr = zero_cols
    case default
       if(mpi_rank()==0)print*,subname//': Error in aerlist source!' 
...
! Purpose : Return pointer to mass mixing ratio for the modal aerosol specie from the specified
!           climate or diagnostic list.
    subroutine rad_cnst_get_mam_mmr_by_idx(list_idx, mode_idx, spec_idx, phase, mmr,istart,istop)
    ! io
    integer,                     intent(in) :: list_idx    ! index of the climate or a diagnostic lis     t
    integer,                     intent(in) :: mode_idx    ! mode index
    integer,                     intent(in) :: spec_idx    ! index of specie in the mode
    character(len=1),            intent(in) :: phase       ! 'a' for interstitial, 'c' for cloud born     e
    real(r8),                    intent(inout):: mmr(:,:)
    ...
    if (mode_idx < 1  .or.  mode_idx > mlist%nmodes) then
       if(mpi_rank()==0)print*, subname//': mode_idx= ', mode_idx, '  nmodes= ', mlist%nmodes
    ...
    if (spec_idx < 1  .or.  spec_idx > modes%comps(m_idx)%nspec) then
       if(mpi_rank()==0)print*, subname//': spec_idx= ', spec_idx, '  nspec= ', modes%comps(m_idx)%nspec
    ...
    if (phase == 'a') then
       ...
       if(mpi_rank()==0)print*, subname//': phase= ', phase
    ...
    case ('Z')
       mmr = zero_cols
    case default
       if(mpi_rank()==0)print*,subname//': Error in mlist source!' 
       call endrun(subname)
    end select
...
! Purpose : Return constituent index of mam specie mass mixing ratio for aerosol modes in
!           the climate list.
...
! Purpose : Return pointer to number mixing ratio for the aerosol mode from the specified
!           climate or diagnostic list.  
    subroutine rad_cnst_get_mode_num(list_idx, mode_idx, phase, num,istart,istop)
    ! io
    integer,                     intent(in) :: list_idx    ! index of the climate or a diagnostic lis     t
    integer,                     intent(in) :: mode_idx    ! mode index
    character(len=1),            intent(in) :: phase       ! 'a' for interstitial, 'c' for cloud born     e
    real(r8),                    intent(inout):: num(:,:)
    ...
    case ('A')
       if (present(istart) .and. present(istop)) then
       num(:,istart:istop) = pstate%tracer_mxrt_at_pc_full_level%f(idx,:,istart:istop)
       else
       num = pstate%tracer_mxrt_at_pc_full_level%f(idx,:,1:ncol)
       end if
    case ('N')
       if (present(istart) .and. present(istop)) then
       num(:,istart:istop) = pstate_cam%aerosol_at_pc_full_level(idx)%f(:,istart:istop)
       else
       num = pstate_cam%aerosol_at_pc_full_level(idx)%f(:,1:ncol)
       end if
    case ('Z')
       num = zero_cols
    case default
...
! Purpose : Return constituent index of mode number mixing ratio for the aerosol mode in
!           the climate list.
!           This is a special routine to allow direct access to information in the 
!           constituent array inside physics parameterizations that have been passed,
!           and are operating over the entire constituent array.  The interstitial phase
!           is assumed since that's what is contained in the constituent array.
    ...
    source = modes%comps(m_idx)%source_num_a
    if (source /= 'A') then
       if(mpi_rank()==0)print*, subname//': source= ', source
...
! Purpose : Return the index of aerosol aer_name in the list specified by list_idx.
    integer function rad_cnst_get_aer_idx(list_idx, aer_name)
    ! io
    integer,             intent(in) :: list_idx    ! 0 for climate list, 1-N_DIAG for diagnostic list     s
...
! Purpose : Return requested properties for the aerosol from the specified
!           climate or diagnostic list.
    ...
    use grist_phys_prop, only: physprop_get
    ...
    real(r8),          optional, intent(out) :: lw_hygro_ext(:,:)         
    ...
    real(r8),          optional, intent(out) :: lw_ext(:)         
    ...
    if (aer_idx < 1 .or. aer_idx > aerlist%numaerosols) then
       if(mpi_rank()==0)print*, subname//': aerosol list index out of range: ', aer_idx ,' list index: ',list_idx
    ...
    !LiXH adds for GRIST
    if (present(n_mu_samples))      call physprop_get(id, n_mu_samples=n_mu_samples)
...
!--------------LiXH Test---------------
if(id >9)print*,'LiXH Test:',id,m_idx,spec_idx,mpi_rank()
!--------------LiXH Test---------------
...
! Purpose : Return requested properties for the mode from the specified
!           climate or diagnostic list.
    ...
    print*,nl//' Mode Definitions'

    do m = 1, modes%nmodes

       print*, nl//' name=',trim(modes%names(m)),'  type=',trim(modes%types(m))
       print*, '  src_a=',trim(modes%comps(m)%source_num_a),'  num_a=',trim(modes%comps(m)%camname_num_a), &
               '  src_c=',trim(modes%comps(m)%source_num_c),'  num_c=',trim(modes%comps(m)%camname_num_c)
       print*, '  nspec=',modes%comps(m)%nspec 

       do i = 1, modes%comps(m)%nspec
          print*, '  src_a=',trim(modes%comps(m)%source_mmr_a(i)), '  mmr_a=',trim(modes%comps(m)%camname_mmr_a(i)), &
                  '  src_c=',trim(modes%comps(m)%source_mmr_c(i)), '  mmr_c=',trim(modes%comps(m)%camname_mmr_c(i)), &
                  '  type=',trim(modes%comps(m)%type(i))
          print*, '  prop file=', trim(modes%comps(m)%props(i))
       end do
    ...
    if (len_trim(gas_list%list_id) == 0) then
       print*, nl//' gas list for climate calculations'
    else
       print*, nl//' gas list for diag'//gas_list%list_id//' calculations'
    end if

    do i = 1, nradgas
       if (gas_list%gas(i)%source .eq. 'G') then
          print*, '  '//gas_list%gas(i)%source//':'//gascnst(i)//' has pstate_cam name:'//&
                         trim(gas_list%gas(i)%camname)
       else if (gas_list%gas(i)%source .eq. 'A') then
          print*, '  '//gas_list%gas(i)%source//':'//gascnst(i)//' has constituents name:'//&
                         trim(gas_list%gas(i)%camname)
       else
          if(mpi_rank()==0)print*,'Error: Only A or G is avaiable in gas source!' 
          call endrun('print_lists in grist_rad_constituents.F90')
       endif
    enddo

    if (len_trim(aer_list%list_id) == 0) then
       print*, nl//' bulk aerosol list for climate calculations'
    else
       print*, nl//' bulk aerosol list for diag'//aer_list%list_id//' calculations'
    end if

    do i = 1, aer_list%numaerosols
       print*, '  '//trim(aer_list%aer(i)%source)//':'//trim(aer_list%aer(i)%camname)//&
                   ' optics and phys props in :'//trim(aer_list%aer(i)%physprop_file)
    enddo

    if (len_trim(ma_list%list_id) == 0) then
       print*, nl//' modal aerosol list for climate calculations'
    else
       print*, nl//' modal aerosol list for diag'//ma_list%list_id//' calculations'
    end if

    do i = 1, ma_list%nmodes
       id = ma_list%idx(i)
       print*, '  '//trim(modes%names(id))
    enddo

    end subroutine print_lists


 end module grist_rad_constituents
