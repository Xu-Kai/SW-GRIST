...
!
!This scheme is a bulk double-moment scheme which is develodped based on MG1.5 
!A single ice approach, merging cloud ice, snow and grauple into a single
!prognostic category with environmentally-dependent physical properties, 
!is proposed.
!Description in: Zhao et al.,, 2017. JAMES
!
! for questions contact Xi Zhao, Yanluan Lin 
! e-mail: zhaoxinuist@126.com; yanluan@mail.tsinghua.edu.cn
!---------------------------------------------------------------------------------
!
...
public :: micro_mg1_5_init_lin ,  &
          micro_mg1_5_tend_lin ,  & 
          micro_mg1_5_get_cols_lin
...
real(r8), parameter :: dcs = 100.e-6_r8 !LiXH Test. default: 250.e-6_r8
...
subroutine micro_mg1_5_init_lin( &
...
end subroutine micro_mg1_5_init_lin

!===============================================================================
!microphysics routine for each timestep goes here...

subroutine micro_mg1_5_tend_lin( mgncol,   mgcols,   nlev,     top_lev,  deltatin,           &
                             tn,                 qn,                                     &
                             qcn,                          qin,                          &
                             ncn,                          nin,                          &
  ...
  ! Note that for these variables, the dust bin is assumed to be the first index. 
  ! (For example, the first dimension for dust is always size 4.)
  ...
  !--------------Lin Micro--------------
  real(r8), intent(out) :: rimo(:,:)           ! riming intensity  
  !--------------Lin Micro--------------
  ...
  !--------------Lin Micro--------------
  real(r8) :: rimtot(nlev,mgncol)            !riming of ice 
  real(r8) :: rim(nlev,mgncol)            !riming of ice 
  real(r8) :: cap_i(nlev,mgncol)         !capacitance of snow 
  real(r8) :: aa_i(nlev,mgncol)    ! snow
  real(r8) :: ba_i(nlev,mgncol)    ! snow
  real(r8) :: am_i(nlev,mgncol)    ! snow
  real(r8) :: bm_i(nlev,mgncol)    ! snow
  real(r8) :: av_i(nlev,mgncol)    ! snow
  real(r8) :: bv_i(nlev,mgncol)    ! snow
  real(r8) :: bmbv1(nlev,mgncol)    ! snow
  real(r8) :: bm1(nlev,mgncol)    ! snow
  real(r8) :: bv1(nlev,mgncol)    ! snow

  real(r8) :: nmelttot(nlev,mgncol)        ! melting of cloud ice
  real(r8) :: melt_rain(nlev,mgncol)        ! melting of ice to rain
  real(r8) :: nmelt_rain(nlev,mgncol)        ! melting of  ice number to rain
  real(r8) :: nmelto(nlev,mgncol)        ! melting of cloud ice
  !--------------Lin Micro--------------
  ...
  substepping: do it=1,iter
     ...
     micro_vert_loop: do k=1,nlev
        ...
        if (k /= 1) then
           where (qric(k-1,:).ge.qsmall .or. qiic(k-1,:).ge.qsmall)
...
!get riming intensity due to qs qc and T 
         call ice_rime(nlev, t(k,:), rho(k,:), qcic(k,:), qiic(k,:), rim(k,:))
 !------make sure Ri does not decrease downward in a column 
         do i = 1,mgncol
           if (k.ge.2) then
             rim(k,i)=max(rim(k-1,i),rim(k,i))
           end if
         end do
 !get ice property
         call ice_characteristics(nlev, mu(k,:), t(k,:), rho(k,:), qcic(k,:), &
              qiic(k,:), rim(k,:), cap_i(k,:), aa_i(k,:), ba_i(k,:), am_i(k,:), &
              bm_i(k,:),av_i(k,:),bv_i(k,:), bmbv1(k,:),bm1(k,:),bv1(k,:))
 
         call size_dist_param_ice_zx(qiic(k,:), niic(k,:), lami(k,:), n0i(k,:), am_i(k,:), bm_i(k,:),bm1(k,:))
        !where (lams(k,:) > 0._r8)
        where (lami(k,:) > 0._r8)
!---------------------------Lin Micro------------------------------------
...
!           uns(k,:) = min(asn(k,:)*cons7/lams(k,:)**bs,1.2_r8*rhof(k,:))
            ums(k,:) = min(av_i(k,:)*bmbv1(k,:) /(lami(k,:)&
                **bv_i(k,:))/bm1(k,:), 9.1_r8*rhof(k,:))
            uns(k,:) = min(av_i(k,:)*bv1(k,:)/lami(k,:)**bv_i(k,:)&
                ,9.1_r8*rhof(k,:))
!---------------------------Lin Micro------------------------------------
...
!                                      psacws(k,:), npsacws(k,:))
        ...
        call snow_self_aggregation(t(k,:), rho(k,:), av_i(k,:), qiic(k,:), niic(k,:), &
                                   nsagg(k,:),bv_i(k,:))

        call accrete_cloud_water_snow(t(k,:), rho(k,:), av_i(k,:), uns(k,:), mu(k,:), &
                                      qcic(k,:), ncic(k,:), qiic(k,:), pgam(k,:),     &
                                      lamc(k,:), lami(k,:), n0i(k,:),                 &
                                      psacws(k,:), npsacws(k,:),                      &
                                      bv_i(k,:),aa_i(k,:), ba_i(k,:))
...
!                               pracs(k,:), npracs(k,:))
        call accrete_rain_snow(t(k,:), rho(k,:), umr(k,:), ums(k,:), unr(k,:), uns(k,:),        &
                               qric(k,:), qiic(k,:), lamr(k,:), n0r(k,:), lami(k,:), n0i(k,:),  &
                               pracs(k,:), npracs(k,:))
...
!                                        pre(k,:), prds(k,:))
        call evaporate_sublimate_precip(deltat, t(k,:), p(k,:), rho(k,:), dv(k,:), mu(k,:), sc(k,:),        &
                                        q(k,:), qvl(k,:), qvi(k,:), lcldm(k,:), cldmax(k,:),                &
                                        arn(k,:), asn(k,:), qcic(k,:), qiic(k,:), qric(k,:), qiic(k,:),     &
                                        lamr(k,:), n0r(k,:), lami(k,:), n0i(k,:), cmei(k,:),                &
                                        pre(k,:), prds(k,:))
...
!           end if
           ...
           ! conservation of qi
           if (do_cldice) then
               dum = (mnuccc(k,i)+mnucct(k,i)+msacwi(k,i))*lcldm(k,i)+&
                     (bergs(k,i)+psacws(k,i))*lcldm(k,i)+&
                     (pracs(k,i)+mnuccr(k,i)+prds(k,i))*cldmax(k,i)+cmei(k,i)+berg(k,i)
 
               if (dum.lt.-qi(k,i)/deltat) then
                 if (-prds(k,i).ge.qsmall) then
                 if (-cmei(k,i).ge.qsmall) then
                   ratio = (qi(k,i)/deltat+dum-(prds(k,i)*cldmax(k,i)+cmei(k,i)))/&
                           (-prds(k,i)*cldmax(k,i)-cmei(k,i))*omsm
                   prds(k,i) = prds(k,i)*ratio
                   cmei(k,i)=cmei(k,i)*ratio
                 else
                   ratio = (qi(k,i)/deltat+dum-prds(k,i)*cldmax(k,i))/(-prds(k,i)*cldmax(k,i))*omsm
                   prds(k,i) = prds(k,i)*ratio
                 end if
                 else
                   prds(k,i) = 0._r8
               ...
               nsubs(k,i)=0._r8
               !-------------------------------------------------------------------
               dum = ((-nnucct(k,i)-nsacwi(k,i))*lcldm(k,i)+(-nsubi(k,i))*icldm(k,i)&
                     -(nnuccr(k,i)+nsubs(k,i)+nsagg(k,i))*cldmax(k,i))*deltat
 
               if (dum.gt.nie) then
                   if ((nsubs(k,i)+nsagg(k,i)+nsubi(k,i)).ge.qsmall) then
 
                  ratio = (nie/deltat+(nnucct(k,i)+nsacwi(k,i))*lcldm(k,i)+nnuccr(k,i)*cldmax(k,i))/ &  
                       ((-nsubs(k,i)-nsagg(k,i))*cldmax(k,i)+(-nsubi(k,i))*icldm(k,i))*omsm
                  nsubi(k,i) = nsubi(k,i)*ratio
                  nsubs(k,i) = nsubs(k,i)*ratio
                  nsagg(k,i) = nsagg(k,i)*ratio
                  end if
...
!                   prai(k,i))*icldm(k,i)+cmei(k,i)+berg(k,i)
               qitend(k,i) = qitend(k,i)+ &
                    (mnuccc(k,i)+mnucct(k,i)+msacwi(k,i))*lcldm(k,i)+&
                     (bergs(k,i)+psacws(k,i))*lcldm(k,i)+&
                     (pracs(k,i)+mnuccr(k,i)+prds(k,i))*cldmax(k,i)+cmei(k,i)+berg(k,i)
           end if
!---------------------------Lin Micro------------------------------------
...
!                pracs(k,i)+mnuccr(k,i))*cldmax(k,i)
           prodsnow(k,i) = prodsnow(k,i)+(mnuccc(k,i)+mnucct(k,i)+msacwi(k,i))*lcldm(k,i)+&
                            (psacws(k,i)+bergs(k,i))*lcldm(k,i)+&
                           (pracs(k,i)+mnuccr(k,i)+prds(k,i))*cldmax(k,i)+cmei(k,i)+berg(k,i)
!---------------------------Lin Micro------------------------------------
...
!                   nprai(k,i))*icldm(k,i)
                    (nnucct(k,i)+nsacwi(k,i))*lcldm(k,i)+(nsubi(k,i))*icldm(k,i)&
                    +(nnuccr(k,i)+nsubs(k,i)+nsagg(k,i))*cldmax(k,i)
           end if
...
!           preci(i) = preci(i)+qstend(k,i)*dz(k,i)*rho(k,i)/rhow
           prect(i) = prect(i)+(qrtend(k,i)*dz(k,i)*rho(k,i))/rhow 
!---------------------------Lin Micro------------------------------------
...
!           call size_dist_param_snow(qsic(k,i), nsic(k,i), lams(k,i), n0s(k,i))
           call size_dist_param_ice_zx(qiic(k,i), niic(k,i), lami(k,i), n0i(k,i),am_i(k,i), bm_i(k,i),bm1(k,i)) 

!           if (lams(k,i) > 0._r8) then
           if (lami(k,i) > 0._r8) then
...
!              uns(k,i) = min(asn(k,i)*cons7/lams(k,i)**bs,1.2_r8*rhof(k,i))
              call ice_terminal_velocity(rim(k,i),av_i(k,i), bmbv1(k,i), &
                   lami(k,i), bv_i(k,i), bm1(k,i), rhof(k,i), bv1(k,i),ums(k,i),uns(k,i))
...
!        call size_dist_param_ice(dumi(k,i), dumni(k,i), lami(k,i), n0i(k,i))
        call ice_characteristics(nlev, mu(k,i), t(k,i), rho(k,i), dumc(k,i), &
             dumi(k,i), rimtot(k,i), cap_i(k,i), aa_i(k,i), ba_i(k,i), am_i(k,i), &
             bm_i(k,i),av_i(k,i),bv_i(k,i), bmbv1(k,i),bm1(k,i),bv1(k,i))
        call size_dist_param_ice_zx(dumi(k,i), dumni(k,i), lami(k,i), n0i(k,i), am_i(k,i), bm_i(k,i),bm1(k,i)) 
!---------------------------Lin Micro------------------------------------
...
!                min(ain(k,i)*cons16/lami(k,i)**bi,1.2_r8*rhof(k,i))
            call ice_terminal_velocity(rimtot(k,i),av_i(k,i), bmbv1(k,i), &
                 lami(k,i), bv_i(k,i), bm1(k,i), rhof(k,i), bv1(k,i),ums(k,i),uns(k,i))
            if (lami(k,i) .le. 0._r8) then
                 ums(k,i)=0._r8
                 uns(k,i)=0._r8
            end if

            vtrmi(k,i)=ums(k,i)
            fi(k) = g*rho(k,i)*ums(k,i)
            fni(k) = g*rho(k,i)*min(uns(k,i),1.2_r8)
        else
           fi(k) = 0._r8
           fni(k)= 0._r8
        end if
...
!           if (t(k,i)+tlat(k,i)/cpp*deltat > tmelt) then
           ...
           if (t(k,i)+tlat(k,i)/cpp*deltat > snowmelt) then
!---------------------------Lin Micro------------------------------------
...
!                    dum = (t(k,i)+tlat(k,i)/cpp*deltat-tmelt)*cpp/xlf
                 if (t(k,i)+tlat(k,i)/cpp*deltat+dum.lt.snowmelt) then
                    dum = (t(k,i)+tlat(k,i)/cpp*deltat-snowmelt)*cpp/xlf
!---------------------------Lin Micro------------------------------------
                    dum = dum/dumi(k,i)
                    dum = max(0._r8,dum)
                    dum = min(1._r8,dum)
...
!                 qctend(k,i)=qctend(k,i)+dum*dumi(k,i)/deltat
                 qitend(k,i)=qitend(k,i)-dum*dumi(k,i)/deltat
                 nitend(k,i)=nitend(k,i)-dum*dumni(k,i)/deltat
                 !ice melt to rain-
                 !hard to change qric, since there is the substep, qric have no memory of first value, 
                 !just change the qrout(grid mean),put it after the do k=1.lev
                 qrtot(i)=qrtot(i)+dum*dumi(k,i)*dz(k,i)*rho(k,i)/deltat
                 nrtot(i)=nrtot(i)+dum*dumni(k,i)*dz(k,i)*rho(k,i)/deltat
                 prect(i) = prect(i)+(dum*dumi(k,i)/deltat*dz(k,i)*rho(k,i))/rhow

                 tlat(k,i)=tlat(k,i)-xlf*dum*dumi(k,i)/deltat
                 ! for output
                 melttot(k,i)=dum*dumi(k,i)/deltat
                 nmelttot(k,i)=dum*dumni(k,i)/deltat
                 melt_rain(k,i)=melttot(k,i)
                 nmelt_rain(k,i)=nmelttot(k,i)
...
!---------------------------Lin Micro------------------------------------
    qrout(1,:)  = qrout(1,:)  + int_to_mid(1,:)*melt_rain(1,:)
    qrout(2:,:) = qrout(2:,:) + interp_to_mid(melt_rain,int_to_mid(2:,:))
    nrout(1,:)  = nrout(1,:)  + (int_to_mid(1,:)*nmelt_rain(1,:)*rho(1,:))
    nrout(2:,:) = nrout(2:,:) + interp_to_mid(nmelt_rain *rho, int_to_mid(2:,:))
!---------------------------Lin Micro------------------------------------
...
!        if (qc(k,i).ge.qsmall) then
        if (qc(k,i).ge.qsmall .and. (nc(k,i)+nctend(k,i)*deltat).ge.qsmall) then
!---------------------------Lin Micro------------------------------------
...
!---------------------------Lin Micro------------------------------------
  call unpack_array(rimtot, mgcols,top_lev, 0._r8, rimo)
  call unpack_array(nmelttot, mgcols,top_lev, 0._r8, nmelto)
!---------------------------Lin Micro------------------------------------
end subroutine micro_mg1_5_tend_lin


!---------------------------Lin Micro------------------------------------
elemental subroutine ice_rime(nlev, t, rho, qcic, qiic, ri)
   integer, intent(in) :: nlev      ! Number of levels to use
   real(r8), intent(in) :: t    ! temperature
   real(r8), intent(in) :: rho  ! air density
   real(r8), intent(in) :: qcic  ! saturation humidity (water)
   real(r8), intent(in) :: qiic  ! saturation humidity (ice)
   real(r8), intent(inout) :: ri  ! saturation humidity (ice)    
   integer :: k
 
     if (rho*qcic .gt. 1e-5 .and. rho*qiic .gt.1e-5) then
       ri=1.0/(1.0+6e-5/(rho**1.170*qcic*qiic**0.170 ))
     else
       ri = 0._r8
     end if
   ri = max(0._r8,min(ri,1._r8))
 
 end subroutine ice_rime

 elemental subroutine ice_characteristics(nlev, mu, t, rho, qcic, qiic, ri,&
                       cap_i, aa_i, ba_i, am_i, bm_i, av_i, bv_i, bmbv1, bm1, bv1)
   integer,  intent(in) :: nlev                  ! number of layers
   real(r8), intent(in) :: mu   ! dynamic viscosity of air kg/m/s
   real(r8), intent(in) :: t    ! Temperature
   real(r8), intent(in) :: rho  ! air density
   real(r8), intent(in) :: qcic, qiic
   real(r8), intent(in) :: ri
 
   real(r8), intent(inout) ::cap_i
   real(r8), intent(inout) ::aa_i
   real(r8), intent(inout) ::ba_i
   real(r8), intent(inout) ::am_i
   real(r8), intent(inout) ::bm_i
   real(r8), intent(inout) ::av_i
   real(r8), intent(inout) ::bv_i
   real(r8), intent(inout) ::bmbv1
   real(r8), intent(inout) ::bm1
   real(r8), intent(inout) ::bv1
 
   integer :: k
   real :: tc
   real(r8), parameter :: am_c1=0.004, am_c2= 6e-5,    am_c3=0.15
   real(r8), parameter :: bm_c1=1.85,  bm_c2= 0.003,   bm_c3=1.25
   real(r8), parameter :: aa_c1=1.28,  aa_c2= -0.012,  aa_c3=-0.6
   real(r8), parameter :: ba_c1=1.5,   ba_c2= 0.0075,  ba_c3=0.5
   !---------------LiXH,Tuning Test----------------
   !real(r8), parameter :: best_a=1.08 ,best_b = 0.499, g=9.18      !Original
   real(r8), parameter :: best_a=1.5,best_b = 0.499!, g=9.18       !Test
 
     cap_i= 0.25*(1+ri)
     tc     = min(-0.1, t-273.15)
     am_i = am_c1+am_c2*tc+am_c3*ri*ri   !--- Heymsfield 2007
     am_i = max(0.000023,am_i)             !--- use the a_min in table 1of Heymsfield
     bm_i = bm_c1+bm_c2*tc+bm_c3*ri
     bm_i = min(bm_i,3.0)                  !---- capped by 3
 !---  converting from cgs to SI unit
     am_i =  10**(2*bm_i-3.0)*am_i
     aa_i = aa_c1 + aa_c2*tc + aa_c3*ri
     ba_i = ba_c1 + ba_c2*tc + ba_c3*ri
 !---  convert to SI unit ai in paper
     aa_i = (1e-2)**(2.0-ba_i)*aa_i
 !---- get v from Mitchell 1996
     av_i = best_a*mu*(2*g*am_i/rho/aa_i/(mu**2))**best_b
     bv_i = best_b*(bm_i-ba_i+2)-1
 
     bmbv1 = gamma(1._r8+bm_i+bv_i)
     bm1 = gamma(1._r8+bm_i)
     bv1 = gamma(1._r8+bv_i)
 end subroutine ice_characteristics

 ! get snow size distribution parameters
 elemental subroutine size_dist_param_ice_zx(qiic, niic, lami, n0i, am_i, bm_i, bm1)
   real(r8), intent(in) :: qiic
   real(r8), intent(in) ::am_i
   real(r8), intent(in) ::bm_i
   real(r8), intent(in) ::bm1
   ...
   real(r8), parameter :: lammini = 1._r8/2000.e-6_r8
 
   if (qiic > qsmall) then
 
      lami = (am_i*bm1*niic/qiic)**(1._r8/bm_i)
      n0i = niic * lami
 ...
 end subroutine size_dist_param_ice_zx

elemental subroutine ice_terminal_velocity(ri,av_i, bmbv1, lami, bv_i, bm1, rhof, bv1,umi,uni)
 !this velocity inculde the densit factor for unrimed ice particle
   real(r8), intent(in) ::av_i
   real(r8), intent(in) ::bv_i
   real(r8), intent(in) ::bmbv1
   real(r8), intent(in) ::bm1
   real(r8), intent(in) ::bv1
   real(r8), intent(in) :: lami
   real(r8), intent(in) :: rhof
   real(r8), intent(in) :: ri
   real(r8), intent(inout) :: umi
   real(r8), intent(inout) :: uni
   real(r8) :: density_factor
  
   if (ri .LT. 0.1_r8) then
 !    umi = min(rhof*av_i*bmbv1 /(lami**bv_i)/bm1, 9.1_r8*rhof)
 !    uni = min(rhof*av_i*bv1/lami**bv_i,9.1_r8*rhof)
     density_factor=rhof
   else
 !    umi = min(av_i*bmbv1 /(lami**bv_i)/bm1, 9.1_r8*rhof)
 !    uni = min(av_i*bv1/lami**bv_i,9.1_r8*rhof)
     density_factor=1.0_r8
   end if
 ...
 !zx160518
   if (lami > 0._r8) then
     umi = min(density_factor*av_i*bmbv1 /(lami**bv_i)/bm1, 9.1_r8*rhof)
     uni = min(density_factor*av_i*bv1/lami**bv_i,9.1_r8*rhof)
   end if
 
 end subroutine ice_terminal_velocity
...
elemental subroutine snow_self_aggregation(t, rho, av_s, qsic, nsic, nsagg, bv_s)

  real(r8), intent(in) :: t    ! Temperature
  real(r8), intent(in) :: rho  ! Density
!  real(r8), intent(in) :: asn  ! fall speed parameter for snow
  real(r8), intent(in) :: av_s  ! fall speed parameter for snow
  real(r8), intent(in) :: bv_s  ! fall speed parameter for snow
...
!          (nsic*rho)**((4._r8-bs)/3._r8) /(4._r8*720._r8*rho)
      nsagg = -1108._r8*av_s*eii* &
           pi**((1._r8-bv_s)/3._r8)*rhosn**((-2._r8-bv_s)/3._r8)* &
           rho**((2._r8+bv_s)/3._r8)*qsic**((2._r8+bv_s)/3._r8)* &
           (nsic*rho)**((4._r8-bv_s)/3._r8) /(4._r8*720._r8*rho)
      nsagg = min(max(nsagg,-nsic),0._r8) 
  else
     nsagg=0._r8
  end if
...
!                                              psacws, npsacws)
!
elemental subroutine accrete_cloud_water_snow(t, rho, av_i, uns, mu, qcic, ncic, qsic, &
                     pgam, lamc, lams, n0s, psacws, npsacws, bv_i, aa_i, ba_i)
  real(r8), intent(in) :: t   ! Temperature
  real(r8), intent(in) :: rho ! Density
!  real(r8), intent(in) :: asn ! Fallspeed parameter (snow)
  real(r8), intent(in) :: av_i  ! fall speed parameter for snow
  real(r8), intent(in) :: bv_i  ! fall speed parameter for snow
  real(r8), intent(in) :: aa_i  ! fall speed parameter for snow
  real(r8), intent(in) :: ba_i  ! fall speed parameter for snow
...
!     npsacws = pi/4._r8*asn*ncic*rho*n0s*eci*cons11 / lams**(bs+3._r8)
     psacws = aa_i*av_i*qcic*rho*n0s*eci*gamma(ba_i+bv_i+1._r8) / lams**(ba_i+bv_i+1._r8)
     npsacws = aa_i*av_i*ncic*rho*n0s*eci*gamma(ba_i+bv_i+1._r8) / lams**(ba_i+bv_i+1._r8)
  else
     psacws = 0._r8
     npsacws = 0._r8
  end if
...
pure subroutine micro_mg1_5_get_cols_lin(ncol, nlev, top_lev, qcn, qin, &
                                     mgncol, mgcols)
...
end subroutine micro_mg1_5_get_cols_lin
