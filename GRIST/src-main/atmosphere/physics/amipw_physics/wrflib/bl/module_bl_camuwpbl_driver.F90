...
module module_bl_camuwpbl_driver
  ...
  use grist_constants,    only : r8, zvir, latvap,  &
                                 latice, karman,    &
                                 mwdry, avogad,     &
                                 boltz,             &
                                 gravit=>gravity,   &
                                 cpair =>cp,        &
                                 rair  =>rdry 
  use module_cam_diffusion_solver,   only : vdiff_selector
  ...
  !! Private data !
  !! ------------ !
  ! module_cam_support, LiXH move it here
  integer, parameter :: pcols = 1   !Always have a chunk size of 1 in WRF
  integer, parameter :: pcnst = 3   !Only diffuse qv, qc, and qi, LiXH
  integer            :: pver
  integer            :: pverp
  ...
  real(r8)             :: tms_orocnst = 1._r8          !! Converts from standard deviation to height
...
CONTAINS
  ...
  subroutine camuwpbl(dt,u_phy,v_phy,th_phy,rho,qv_curr,hfx,qfx,ustar,p8w &
       ,p_phy,z,t_phy,qc_curr,qi_curr,z_at_w                              &  
       !,cldfra_old_mp
       ,cldfra,ht      &
       ,rthratenlw,exner,is_CAMMGMP_used                                  &
       ,itimestep                                                         &
       !,qnc_curr,qni_curr,wsedl3d                               &
       ,ids,ide, jds,jde, kds,kde                                         &
       ,ims,ime, jms,jme, kms,kme                                         &
       ,its,ite, jts,jte, kts,kte                                         &
!Intent-inout 
       ,tauresx2d,tauresy2d                                               &
       ,rublten,rvblten,rthblten,rqiblten                                 &
       !,rqniblten
       ,rqvblten,rqcblten     &
       ,kvm3d,kvh3d                                                       &
!Intent-out
       ,tpert2d,qpert2d,wpert2d,smaw3d,turbtype3d                         &
       ,tke_pbl,leng_pbl,pblh2d,kpbl2d                                    )  
    ...
    !  compute_eddy_diff,compute_vdiff and           !
    !  positive_moisture.
    !!---------------------------------------------------- !
    use module_cam_trb_mtn_stress,  only : compute_tms
    use module_cam_eddy_diff,       only : compute_eddy_diff
    use module_cam_molec_diff,      only : compute_molec_diff, vd_lu_qdecomp
!    use constituents,          only : qmincg, qmin
    use module_cam_diffusion_solver !!, only : compute_vdiff, any, operator(.not.)
#ifdef MODAL_AERO
    use modal_aero_data
#endif
...
!<----------------CAMMG is not available, LiXH-------------
    ...
    real, dimension( ims:ime, kms:kme, jms:jme ), intent(out) :: leng_pbl    !Turbulence length scale at all interfaces [ m ]
    ...
    real(r8) :: lengi(    pcols,kte+1)                       ! Turbulence length scale at all interfaces [ m ]
    ...
    do j    = jts , jte
       do i = its , ite
          ...
          do k  = kts,kte
             ...
             !----------------LiXH test, use q instead of mix ratio?-------------> 
             ...
             cloud( 1,kflip,1)        = max( qv_curr(i,k,j), 1.0e-30_r8 )
             cloud( 1,kflip,ixcldliq) = qc_curr(i,k,j)
             cloud( 1,kflip,ixcldice) = qi_curr(i,k,j)
             !<---------------LiXH test, use q instead of mix ratio?-------------- 
          ...
          !!----------------------------------------------------------------------- !
             ...
             call compute_eddy_diff( lchnk  , pcols  , pver   , ncol   , t8      , cloud(:,:,1)   , ztodt    ,           &
                  ...
                  cgs         , tpert       , qpert  , wpert  , tke8   , bprod   , sprod          , sfi      ,           &
...
!             call wrf_error_fatal(errstring)
          ...
          !------do_pseudocon_diff=.false. in WRF, LiXH---------->
          ...
          do k=kts,kte
             ...
             rqvblten(i,k,j)   = cloudtnd(1,kflip,1       )
             rqcblten(i,k,j)   = cloudtnd(1,kflip,ixcldliq)
             rqiblten(i,k,j)   = cloudtnd(1,kflip,ixcldice)
             !<---------------LiXH test, use q instead of mix ratio?-------------- 
          ...
          do k = kts,kte+1
             ...
             leng_pbl(i,k,j)   = lengi(1,kflip)   !Minxing length at the interfaces
...
!-----------------------------------------------------------------------------------------   
    ...
    do i = 1, ncol
       ...
       if( dqv .gt. 1.e-20_r8 ) then
          ...
          if( aa .lt. 0.5_r8 ) then
             ...
             print*, 'Full positive_moisture is impossible in vertical_diffusion'
          endif
       endif
    end do
    return
 ...
 !-----------------------------------------------------------------------------------------   
  subroutine camuwpblinit( &
    ...
    !Calls: cnst_get_ind, wrf_error_fatal,init_eddy_diff,init_tms,init_vdiff 
!-----------------------------------------------------------------------------------------

    use module_cam_eddy_diff,       only : init_eddy_diff
    use module_cam_molec_diff,      only : init_molec_diff
    use module_cam_trb_mtn_stress,  only : init_tms
    use module_cam_diffusion_solver,only : init_vdiff, vdiff_select
