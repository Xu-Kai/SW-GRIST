...
! add some features by Xiaoqi Xu,
! A: aerosol, C: cloud, E: entrainment

MODULE MODULE_MP_MORR_TWO_MOMENT_V381_ACE
#ifdef AMIPW_PHYSICS
...
! USE WRF PHYSICS CONSTANTS
  use module_wrfmodel_constants, ONLY: CP, G, R => r_d, RV => r_v, EP_2
...
! IACT = 4, Ji N(1-EXP(BS^K))  FOR ICCN = 4,5
...
! SWITCH FOR ENTRAINMENT-MIXING MECHANISMS
! IHOMO = 0, HOMOGENEOUS MIXING MECHANISMS
! IHOMO = 1, EXREMELY INHOMOGENEOUS MIXING MECHANISMS
! IHOMO = 2, CLUMIX, HOMOGENEOUS MIXING DEGREE FROM LU ET AL(2013)    added by xiaoqi 22.04

     INTEGER, PRIVATE ::  IHOMO
	 
! SWITCH FOR ACCRETION SCHEME
! IACCR = 0, KK2000
! IACCR = 1, COHARD AND PINTY 2000   added by xiaoqi 22.04
! IACCR = 2, Kogan 2013
! IACCR = 3, Thompson lookup table    added by xiaoqi 22.07

     INTEGER, PRIVATE ::  IACCR
	 
! SWITCH FOR AUTOCONVERSION SCHEME
! IAUTO = 0, KK2000
! IAUTO = 1, Liu and Daum(2004)    added by xiaoqi 22.04
! IAUTO = 2, Berry(1968)
! IAUTO = 3, Beheng(1994)

     INTEGER, PRIVATE ::  IAUTO
	 
! SWITCH FOR aerosol activation SCHEME, only for INUM = 0
! ICCN = 0, MARITIME  C1=120 K1=0.4
! ICCN = 1, CONTINENTAL  C1=1000  K1=0.5
! ICCN = 2, C1=2628  K1=0.63  uncorrected Twomey CSK  
! ICCN = 3, C1=2668  K1=0.69  corrected Twomey CSK
! ICCN = 4, N2=2512  K2=1.23  B2=-3.37 uncorrected Ji N(1-EXP(BS^K))   IACT = 4
! ICCN = 5, N2=2409  K2=1.5  B2=-4.74 corrected Ji N(1-EXP(BS^K))  IACT = 4   Wang yuan 2019 GRL
	 INTEGER, PRIVATE ::  ICCN

! SWITCH FOR RIMING SCHEME
! IRIM = 0, Morrison (default)
! IRIM = 1, Thompson lookup table    added by xiaoqi 22.07


     INTEGER, PRIVATE ::  IRIM	 
...
! CCN SPECTRA FOR IACT = 1
     ...
     REAL, PRIVATE ::      N2     ! 'N' IN NCCN = N(1-EXP(BS^K))
     REAL, PRIVATE ::      K2     ! 'K' IN NCCN = N(1-EXP(BS^K))
	 REAL, PRIVATE ::      B2     ! 'B' IN NCCN = N(1-EXP(BS^K))
...
! CONSTANTS TO IMPROVE EFFICIENCY
	 ...
	 !..Lookup table dimensions
      ...
      DOUBLE PRECISION, DIMENSION(nbi):: Dii, dti
      DOUBLE PRECISION, DIMENSION(nbr):: Dr, dtr
      DOUBLE PRECISION, DIMENSION(nbs):: Dss, dts
      DOUBLE PRECISION, DIMENSION(nbg):: Dgg, dtg
...
! ICCN = 2, C1=2628  K1=0.63  uncorrected Twomey CSK  
! ICCN = 3, C1=2668  K1=0.69  corrected Twomey CSK
! ICCN = 4, N2=2512  K2=1.23  B2=-3.37 uncorrected Ji N(1-EXP(BS^K))
! ICCN = 5, N2=2409  K2=1.5  B2=-4.74 corrected Ji N(1-EXP(BS^K))     Wang yuan 2019 GRL
...
! IACT = 4, Ji N(1-EXP(BS^K))  FOR ICCN = 4,5
      IACT = 1
  	   ...
  	   IF ((ICCN .EQ. 4) .OR. (ICCN .EQ. 5)) THEN
	    IACT = 4
	   END IF
! IBASE = 1, NEGLECT DROPLET ACTIVATION AT LATERAL CLOUD EDGES DUE TO 
!             UNRESOLVED ENTRAINMENT AND MIXING, ACTIVATE
...
!ADDED BY xiaoqi 2022.03.31  entrainment-mixing, accretion, autoconversion	  
! SWITCH FOR ENTRAINMENT-MIXING MECHANISMS
! IHOMO = 0, HOMOGENEOUS MIXING MECHANISMS
! IHOMO = 1, EXREMELY INHOMOGENEOUS MIXING MECHANISMS
! IHOMO = 2, CLUMIX, HOMOGENEOUS MIXING DEGREE FROM LU ET AL(2013)

     IHOMO = 2
	 
! SWITCH FOR ACCRETION SCHEME
! IACCR = 0, KK2000
! IACCR = 1, COHARD AND PINTY 2000
! IACCR = 2, Ko2013, Kogan (2013)
! IACCR = 3, Thompson lookup table    added by xiaoqi 22.07
...
! IF NO COUPLING WITH WRF-CHEM
      ...
      IF (INUM .EQ. 1 .AND. ICCN .EQ. 250) THEN
	   NDCNST = 250
	  END IF
	  
	  IF (INUM .EQ. 1 .AND. ICCN .EQ. 100) THEN
       NDCNST = 100
	  END IF
...
!.. radar module.
         ...
         call radar_init
!+---+-----------------------------------------------------------------+
     IF (IACCR .EQ. 3 .OR. IRIM .EQ. 1) THEN
...
! add cumulus tendencies, decouple from mu
          ...
          if(present(mu))then ! yizhang for grist
             qrcu1d(k)     = qrcuten(i,k,j)/mu(i,j)
             qscu1d(k)     = qscuten(i,k,j)/mu(i,j)
             qicu1d(k)     = qicuten(i,k,j)/mu(i,j)
          end if
...
REAL, DIMENSION(KTS:KTE) ::  DFR                ! diameter of raindrop
      REAL, DIMENSION(KTS:KTE) ::  DFC, visco                ! diameter of raindrop      
...
! FALL-SPEED PARAMETER 'A' WITH AIR DENSITY CORRECTION
	  ...
	  REAL, DIMENSION(KTS:KTE) ::  smob, smo2, smoc, smoe 
...
! MORE WORKING/DUMMY VARIABLES
     ...
     REAL DUMQC,DUMQR,RATIO,SUM_DEP,FUDGEF,DUMNC,DUMRC
     REAL BCOND,A5DR1,ADFV,A5DR2,ATC4,LNC,LRC     !XXQ
...
!##### clumixing, crm #######################################################################
 !     INTEGER, INTENT(IN)                     ::  crm_clumix_opt  ! Switch for clumixing, crm
      REAL, DIMENSION(KTS:KTE) ::  dissip_rate1d   ! dissipation rate for clumixing, crm
      REAL, DIMENSION(KTS:KTE) ::  scale_num1d     ! scale number     for clumixing, crm
	  REAL, DIMENSION(KTS:KTE) ::  Tao_reaction1d
	  REAL, DIMENSION(KTS:KTE) ::  L_trasition1d
	  REAL, DIMENSION(KTS:KTE) ::  Kol_return1d
	  REAL, DIMENSION(KTS:KTE) ::  clumixpsi31d
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXT1D         !temperature          for clumixing, crm  #xxq
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXQV1D       
      REAL, DIMENSION(KTS:KTE) ::  CLUMIXQC1D
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXS1D         !Supersaturation      for clumixing, crm  #xxq
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXN1D         !Number Concentration for clumixing, crm  #xxq
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXRV1D       !Volume-Mean radius   for clumixing, crm  #xxq
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXP1D         !Air pressure         for clumixing, crm  #xxq
	  
	  REAL, DIMENSION(KTS:KTE) ::  CLUMIXCF1D
	  REAL :: log_mixpsi, mixpsi, SUBSAT, RHGRID, QVSW, DENOM, GAMMACF, ARG, ALPHA0, RHUM, PEXP, CF, PSW4, SS4   ! mixing degree. mixpsi is used because psi is booked
!############################################################################################
		
!#####collision processes using lookup table, from Thomspon##################### added by xiaoqi 22.07.04
      REAL:: Ef_rw, Ef_sw, Ef_gw, xDs, xDgg, vtg, stoke_g, tc0, a_, b_, loga_
...
!Liu and Daum(2004)       
     	 IF (IAUTO .EQ. 1) THEN
          DUMNC = MAX(NC3D(K),1.e3)
!ε2
          DUM1 = (0.0005714*DUMNC*RHO(K)/1.E6+0.271)**2.
!	      DUM1 = 0.4**2
!p0    KG/KG/S
          DUM2 = 1.1*1.E10*(1.+DUM1*3.)*(1.+DUM1*4.)*(1.+DUM1*5.)/  &
	       ((1.+DUM1)*(1.+DUM1*2.))*QC3D(K)**3./DUMNC*RHO(K)  
 !Xc
          DUM3 =   9.7*1.e-20*DUMNC**1.5/QC3D(K)**2./RHO(K)**0.5         
 ! T
          DUM = 0.5*(DUM3**2.+DUM3*2.+2.)*(1+DUM3)*exp(-2*DUM3)
	      PRC(K) = DUM2*DUM		
         END IF		  
!Berry(1968)
         ...
         IF (IAUTO .EQ. 2) THEN  
		  DUMNC = MAX(NC3D(K),1.e3)
    	  PRC(K) = 0.035*(QC3D(K)*RHO(K))**2/(0.12+1.E-12*DUMNC/QC3D(K))/RHO(K)
		 END IF
		 
!Beheng(1994)
         IF (IAUTO .EQ. 3) THEN
!           n=10
           PRC(K) = 6.0*1.E28*10**(-1.7)*(QC3D(K)*1.E-3*RHO(K))**4.7*(NC3D(K)*1.E-6*RHO(K))**(-3.3)/RHO(K)
         END IF
...
! IACCR = 0, KK2000, KHAIROUTDINOV AND KOGAN 2000, MWR
         IF (IACCR .EQ. 0) THEN 
           DUM=(QC3D(K)*QR3D(K))
           PRA(K) = 67.*(DUM)**1.15
          END IF
		  ...
		  IF (IACCR .EQ. 1) THEN		
! IACCR = 1, CP2000, COHARD AND PINTY 2000 
! added by xiaoqi 2021.12.22
		   DFR(K) = (3*QR3D(K)/(4*PI*NR3D(K)*RHOW))**(1./3.)*2  
		   IF (DFR(K).GE.1.E-4 ) THEN 
		    PRA(K) = (PI*RHOW/RHO(K)*3.03E3*NC3D(K)*NR3D(K))/(6*LAMC(K)**3.)*  &
		    (GAMMA(PGAM(K)+7.)/(GAMMA(PGAM(K)+1.)*LAMC(K)**3.)+  &
			GAMMA(PGAM(K)+4.)/GAMMA(PGAM(K)+1.)*GAMMA(4.)/LAMR(K)**3)
		   
		   ELSE IF (DFR(K).GT.0 .AND. DFR(K).LT.1.E-4) THEN
		    PRA(K) = (PI*RHOW/RHO(K)*2.59E15*NC3D(K)*NR3D(K))/(6*LAMC(K)**3.)*  &
		    (GAMMA(PGAM(K)+10.)/(GAMMA(PGAM(K)+1.)*LAMC(K)**6.)+  &
			GAMMA(PGAM(K)+4.)/GAMMA(PGAM(K)+1.)*GAMMA(7.)/LAMR(K)**6.)
		   
		   END IF
          END IF
! IACCR = 2, Ko2013，Kogan et al. 2013        
		  IF (IACCR .EQ. 2) THEN
		   PRA(K) = QC3D(K)*QR3D(K)
		  END IF
...
! IACCR = 3, Thompson		  
		  IF (IACCR .EQ. 3) THEN
		  DFR(K) = (3*QR3D(K)/(4*PI*NR3D(K)*RHOW))**(1./3.)*2  
		  DFC(K) = (3*QC3D(K)/(4*PI*NC3D(K)*RHOW))**(1./3.)*2
		   IF (DFR(K).GE.5.E-5 .AND. DFC(K) .GE. 1.E-7) THEN		
           idx = 1 + INT(nbr*DLOG(DFR(k)/Dr(1))/DLOG(Dr(nbr)/Dr(1)))
           idx = MIN(idx, nbr)
           Ef_rw = t_Efrw(idx, INT(DFC(k)*1.E6))
           PRA(K) = SQRT(101325.0/(287.05*298.0)/RHO(K))*PI*0.25*4854.0*GAMMA(4.)*Ef_rw*QC3D(K)*RHO(K)*N0RR(K) &
                          *((LAMR(K)+195.)**(-4.))
           PRA(K) = MIN(DBLE(QC3D(k)/DT), PRA(K))
		   END IF
		  END IF
...
! HM ADD, WRF-CHEM, ADD TENDENCIES FOR C2PREC
 ...
 300  CONTINUE
! calculate variables before evaporation for clumixing    #xxq 2017.10.24
      CLUMIXT1D(K) = T3D(K)+DT*T3DTEN(K)
	  CLUMIXQV1D(K) = QV3D(K)+DT*QV3DTEN(K)
	  CLUMIXP1D(K) = PRES(K)/100
	  
      CLUMIXQC1D(K) = QC3D(K)+DT*QC3DTEN(K)
	  
!clumixcf     xxq 2020.04.07 
PSW4 =  min(0.99*pres(k),POLYSVP(CLUMIXT1D(K),0))
	  SS4 = CLUMIXQV1D(K)*PRES(K)/(0.622+0.378*CLUMIXQV1D(K))/PSW4-1.
	   CLUMIXS1D(K) = SS4	  
      IF (CLUMIXQC1D(K) .GE. QSMALL) THEN
	   CLUMIXN1D(K) = (NC3D(K)+DT*NC3DTEN(K))*RHO(K)/1.E6    !change kg-1 to cm-3      
	   CLUMIXRV1D(K) = ((3*CLUMIXQC1D(K)/(4*PI*(NC3D(K)+DT*NC3DTEN(K))*1.0))**(1./3.))*1.E5
	   
	  END IF
	  dissip_rate1d(K) = (ABS(W3D(K)))**3/100	
...
! DROPLET CONCENTRATION IS SPECIFIED !!!!!  added by xiaoqi 22.04
...
! MAKE SURE THIS VALUE ISN'T GREATER THAN TOTAL NUMBER OF AEROSOL
		   ...
		   IF (IACT.EQ.4) THEN
			DUM=DUM*100.  ! CONVERT FROM M/S TO CM/S
            DUM2 = 3.6*((1.6E-3*DUM**1.5/C1)**(1/(K1+2.)))
			DUM2 = N2*(1-EXP(B2*DUM2**K2))
            DUM2=DUM2*1.E6 ! CONVERT FROM CM-3 TO M-3
            DUM2=DUM2/RHO(K)  ! CONVERT FROM M-3 TO KG-1
            DUM2 = (DUM2-NC3D(K))/DT
            DUM2 = MAX(0.,DUM2)
...
! ACTIVATE IN CLOUD INTERIOR liuxl
...
! FIND MAXIMUM ALLOWED ACTIVATION WITH NON-EQUILIBRIUM SS
            ...
            DUM=DUM*100.  ! CONVERT FROM M/S TO CM/S
            DUMACT = 3.6*((1.6E-3*DUM**1.5/C1)**(1/(K1+2.)))
			DUMACT = N2*(1-EXP(B2*DUMACT**K2))
...
!clumix	  
       IF (IHOMO .EQ. 2)THEN
	     ...
	     call crm_clumix_scale_num(                          &
                     dissip_rate1d(K), scale_num1d(K),Tao_reaction1d(k), &
					 L_trasition1d(k), Kol_return1d(k) , CLUMIXT1D(K), CLUMIXS1D(K), &
					 CLUMIXN1D(K), CLUMIXRV1D(K), CLUMIXP1D(K))

     ! crm_clumix_opt can be used to select mixing degree eqs from namelist(didn't use now)
     
     !the third homogeneous mixing degree (psi) vs. scale number calculated 
     !from number concentration after dilution but before evaporation (N_L0)
     
	 
!      log_mixpsi=0.12*log10(scale_num1d(K))+1.70       !Cumulus
!	  log_mixpsi=0.28*log10(scale_num1d(K))+0.96       !stratocumulus
!      mixpsi=10**log_mixpsi

          mixpsi=107.19*EXP(-1.99*scale_num1d(k)**(-0.29))   !EMPM Luo	     
          clumixpsi31d(k)=MAX(0., MIN(1.,mixpsi/100.))     	  
          NSUBC(K) =(CLUMIXN1D(K)/RHO(K)*1.E6)*(((CLUMIXQC1D(K)+PCC(K)*DT)/CLUMIXQC1D(K))** &
		                       (1-clumixpsi31d(k))-1)/DT	  	   
	      NSUBC(K) = MIN(NSUBC(K),0.)
	     END IF
 	    END IF
! UPDATE TENDENCIES
...
!##########################################################################
!!	ACCRETION OF CLOUD DROPLETS ONTO SNOW/GRAUPEL using lookup table	
      
		 
    IF (IRIM .EQ. 1) THEN
	 DFC(K) = (3*QC3D(K)/(4*PI*NC3D(K)*RHOW))**(1./3.)*2
	  tc0 = MIN(-0.1, (T3D(K)-273.15))
         smob(k) = QNI3D(k)*1.0/0.069
		 smo2(k) = smob(k)
		 loga_ = sa(1) + sa(2)*tc0 + sa(3)*3.0 &
               + sa(4)*tc0*3.0 + sa(5)*tc0*tc0 &
               + sa(6)*3.0*3.0 + sa(7)*tc0*tc0*3.0 &
               + sa(8)*tc0*3.0*3.0 + sa(9)*tc0*tc0*tc0 &
               + sa(10)*3.0*3.0*3.0
         a_ = 10.0**loga_
         b_ = sb(1)+ sb(2)*tc0 + sb(3)*3.0 + sb(4)*tc0*3.0 &
              + sb(5)*tc0*tc0 + sb(6)*3.0*3.0 &
              + sb(7)*tc0*tc0*3.0 + sb(8)*tc0*3.0*3.0 &
              + sb(9)*tc0*tc0*tc0 + sb(10)*3.0*3.0*3.0
         smoc(k) = a_ * smo2(k)**b_
		 
		 loga_ = sa(1) + sa(2)*tc0 + sa(3)*2.55 &
               + sa(4)*tc0*2.55 + sa(5)*tc0*tc0 &
               + sa(6)*2.55*2.55 + sa(7)*tc0*tc0*2.55 &
               + sa(8)*tc0*2.55*2.55 + sa(9)*tc0*tc0*tc0 &
               + sa(10)*2.55*2.55*2.55
         a_ = 10.0**loga_
         b_ = sb(1)+ sb(2)*tc0 + sb(3)*2.55 + sb(4)*tc0*2.55 &
              + sb(5)*tc0*tc0 + sb(6)*2.55*2.55 &
              + sb(7)*tc0*tc0*2.55 + sb(8)*tc0*2.55*2.55 &
              + sb(9)*tc0*tc0*tc0 + sb(10)*2.55*2.55*2.55
         smoe(k) = a_ * smo2(k)**b_
		 
    if (QC3D(k).GE.QSMALL .and. DFC(K) .gt. 1.E-6) then
          xDs = 0.0
          if (QNI3D(K).GE.QSMALL) xDs = smoc(k) / smob(k)
          if (xDs .gt. D0s) then
           idx = 1 + INT(nbs*DLOG(xDs/Dss(1))/DLOG(Dss(nbs)/Dss(1)))
           idx = MIN(idx, nbs)
           Ef_sw = t_Efsw(idx, INT(DFC(K)*1.E6))
           PSACWS(K) = SQRT(101325.0/(287.05*298.0)/RHO(K))*PI*0.25*40.0*Ef_sw*QC3D(K)*smoe(k)
           NPSACWS(K) = SQRT(101325.0/(287.05*298.0)/RHO(K))*PI*0.25*40.0*Ef_sw*NC3D(K)*smoe(k)                ! Qc2M
           NPSACWS(K) = MIN(DBLE(NC3D(K)/DT), NPSACWS(K))
          endif
         
!..Graupel collecting cloud water.  In CE, assume Dc<<Dgg and vtc=~0.
         if ((T3D(K)-273.15) .ge. 0.0) then
            visco(k) = (1.718+0.0049*(T3D(K)-273.15))*1.0E-5
         else
            visco(k) = (1.718+0.0049*(T3D(K)-273.15)-1.2E-5*(T3D(K)-273.15)*(T3D(K)-273.15))*1.0E-5
         endif		

          if (QG3D(k).ge. 1.e-5 .and. DFC(K).gt. 1.E-6) then
           xDgg = 4.0/LAMG(K)
           vtg = SQRT(101325.0/(287.05*298.0)/RHO(K))*442.0*GAMMA(4.89)*1.0/GAMMA(4.0)/LAMG(K)**0.89
           stoke_g =  DFC(K)* DFC(K)*vtg*RHOW/(9.*visco(k)*xDgg)
           if (xDgg.gt. D0g) then
            ...
            PSACWG(K) = SQRT(101325.0/(287.05*298.0)/RHO(K))*PI*0.25*442.0*GAMMA(3.89)*Ef_gw*QC3D(K)*N0G(K) &
                          /LAMG(K)**3.89
            NPSACWG(K) = SQRT(101325.0/(287.05*298.0)/RHO(K))*PI*0.25*442.0*GAMMA(3.89)*Ef_gw*NC3D(K)*N0G(K)           &
                          /LAMG(K)**3.89                                ! Qc2M
            NPSACWG(K) = MIN(DBLE(NC3D(K)/DT), NPSACWG(K))
           endif
          endif
         endif	
		END IF
...
!.......................................................................
! ACTIVATION OF CLOUD DROPLETS liuxl        added by xiaoqi 22.04

!! KVW AFTER amy added code to predict droplet concentration back in 
      IF (INUM.EQ.0) THEN     
...
!+---+-----------------------------------------------------------------+   
	  SUBROUTINE crm_clumix_scale_num(               &
                     dissip_rate, scale_number_return, Tao_return,   &
					 L_return, Kol_return, clumixt, clumixs,    & 
                     clumixn, clumixrv, clumixp )                                  !added by xiaoqi 22.04

       ! Compute local scale number for mixing 
       
       ! u,v,w at stag? m/s
       ! dissipation_rate, m2s-3
       
       IMPLICIT NONE
       
       REAL :: scale_num, dissip_rate,  clumixt, clumixs,    & 
                 clumixn, clumixrv, clumixp,    &
                T_input,S_input,N_input,rv_input,&
                dissi_rate_input,pressure_input,LWC_input,&
       	        N_i, N_return,rv_return,log_psi_3,psi_3,&
       	        Tao_return,L_return,Kol_return,scale_number_return
        
       
       
       Dimension Y_f(2),D_f(2),B_f(2),C_f(2),G_f(2),E_f(2)
       Double Precision Y_f,D_f,T_f,H_f,B_f,C_f,G_f,E_f,EPS_f
       
       !some variables below are redundant; pls ignore them 
       real*8 T1(8),K1(8),D1(8),K_c(1),&
              D(1),T2(13),L2(13),L(1),top(1,5),a_leg(50000,70,1),a_leg_update(50000,70,1)
       real*8 es_T(1),r(1),turbulence(1),p(1),n(1),T(1)
       real*8 RH(1),S(1)
       real*8 Rv,row_L,B1(1),B2(1),B3(1),B(1),AA(1),R_apostrophy,epslo622,cp,xishu_f(2)
       real*8 E(1),C1(1),C2(1)
       real*8 Tao_reaction(1),L_trasition(1)
       real*8 T_miu(8),miu1(8),miu(1),row_air(1),kinematic_viscosity(1)
       real*8 Kolmogorov_length_scale(1),scale_number(1)
       real*8 max_r,max_n
       real*8 x_min_array(100,1),x_max_array(100,1),x_min_array1(100,1),x_max_array1(100,1)
       real*8 r_leg_part(1,100),turbulence_leg_part(1,100),p_leg_part(1,100),n_leg_part(1,100)
       real*8 r_max_part(1,100),n_max_part(1,100)
       real*8 a_leg_part(1,5000,70,70)
       integer i_leg_update(1)
       integer ipart_leg(100,1)
       integer ia_leg(1)
	   integer k,i,M_f
        character afile*4,atemp1*1,atemp2*2,path_out*121,path_in*108
    ...
    !    EXTERNAL GRKT2
       ...
       !-----step1--beginning---calcu Coefficient of Thermal Conductivity,Coefficient of Diffusion,Latent Heats-
       !From a short course in cloud physics by Rogers and Yau. ---P102
       data T1 /233,243,   253,   263,   273,   283,   293,   303/
        	 
       !Coefficient of Thermal Conductivity K of Air UNIT (J m-1 s-1 K-1)	 
       data K1 /2.070E-02, &
                2.160E-02,&
                2.240E-02,&
                2.320E-02,&
       ...
       !%Coefficient of Diffusion D of water vapor in Air UNIT(m2 s-1)
       data D1 /1.62E-05,&
                1.76E-05,&
                1.91E-05,&
                2.06E-05,&
         ...
         N_input=clumixn
         rv_input=clumixrv
         dissi_rate_input=dissip_rate
         pressure_input=clumixp
          ...
          T(k)=T_input !Temperature 
          S(k)=S_input !supersaturation
          n(k)=N_input !number concentration
          r(k)=rv_input !volume mean radius
          turbulence(k)=dissi_rate_input !dissipation rate
          p(k)=pressure_input !pressure
       
   	    do i=1,7 
        if (T(k)>T1(i).and.T(k)<=T1(i+1)) then
               K_c(k)=K1(i)+(K1(i+1)-K1(i))/(T1(i+1)-T1(i))*(T(k)-T1(i)) !Linear Interpolation
               D(k)=D1(i)+(D1(i+1)-D1(i))/(T1(i+1)-T1(i))*(T(k)-T1(i))
           
       	end if
       
       	end do
       
       !From a short course in cloud physics by Rogers and Yau. Page 16
       data T2 / 233,   243,   253,   263,   273,   278,   283,   288,   293,   298,   303,   308,   313/
       
       !%Latent Heats of condensation UNIT (J/g)
       data L2 /2.603E+03,&
                2.575E+03,&
                2.549E+03,&
                2.525E+03,&
       ...
       do i=1,12   !J/g not J/kg
       !write(*,*)i,T(k),T2(i),T2(i+1)
           if (T(k)>T2(i).and.T(k)<=T2(i+1))then
               L(k)=L2(i)+(L2(i+1)-L2(i))/(T2(i+1)-T2(i))*(T(k)-T2(i));!Linear Interpolation
           end if
       end do
       ...
       !%-----step2---beginning----------transition length----------------------  
       !solve Eqs (B4) and (B5) in Lu et al. JGR 2011 Appendix
       
       !calculate saturation water vapor pressure
           es_T(k)=6.11*10**(3.45e-6*L(k)*1000*((T(k)-273)/T(k))); !L unit:J/g not J/kg
       
           Rv=461.5; !A short course in cloud physics UNIT J kg-1 K-1
           row_L=1e6; !water density, UNIT: g/m3
       !B1,B2,B3,B and AA are the terms in the numerators and denominators of Eqs (B4) and (B5)
       !in Lu et al. JGR 2011 Appendix
           B1(k)=L(k)/(Rv*T(k))*1000-1;
           B2(k)=L(k)*row_L/(K_c(k)*T(k));
           B3(k)=1e-5*row_L*Rv*T(k)/(D(k)*es_T(k));
           B(k)=B1(k)*B2(k)+B3(k);
           AA(k)=1e12*1/B(k); !UNIT: mkm^2/s
       
           R_apostrophy=287; ! R'=287J kg-1 K-1  ------P2 of Rogers and Yau
           epslo622=0.622; ! ------P12 of Rogers and Yau
           cp=1005; ! J kg-1 K-1------P5 of Rogers and Yau
       
       !C1 and C2 are "-B" and "A", respectively in Eqs (B4) and (B5) in Lu et al. JGR 2011 Appendix
           E(k)=R_apostrophy*T(k)/(epslo622*es_T(k))+1e6*epslo622*L(k)**2/(p(k)*T(k)*cp); !E is just a temporary variable
           C1(k)=-1e-17*AA(k)*E(k)*row_L*4*3.1415926*n(k); !unit: s-1 mkm-1
           C2(k)=AA(k); !UNIT: mkm^2/s
       
       
       !call GRKT2 to solve Eqs (B4) and (B5) in Lu et al. JGR 2011 Appendix
           T_f=0.0
           Y_f(1)=r(k) !
           Y_f(2)=S(k)
           H_f=0.01
     ...
     Tao_reaction(k)=T_f
	  Tao_reaction(k)=-r(k)**2/(2*xishu_f(1)*Y_f(2))  !tao_evap
	  
	  if (Tao_reaction(k)>0)then
         L_trasition(k)=turbulence(k)**(1./2)*Tao_reaction(k)**(3./2);
       else
      L_trasition(k)=9999
      end if
       
       !%-----step2---end---------transition length----------------------      
       
       
       
         
       !-----step3--beginning----------calculate Kolmogorov scale----------------------
       data T_miu /233,   243,   253,   263,   273,   283,   293,   303/
       data miu1/1.512e-5,& !dynamic viscosity kg m-1s-1--p102
                 1.564e-5,&
                 1.616e-5,&
                 1.667e-5,&
                 1.717e-5,&
       ...
       do i=1,7
           if (T(k)>T_miu(i).and.T(k)<=T_miu(i+1))then
               miu(k)=miu1(i)+(miu1(i+1)-miu1(i))/(T_miu(i+1)-T_miu(i))*(T(k)-T_miu(i)) !Linear Interpolation
           end if
       end do
       
       row_air(k)=100*p(k)/R_apostrophy/T(k) !air density UNIT: kg/m3
       kinematic_viscosity(k)=miu(k)/row_air(k)!UNIT:m2s-1
       Kolmogorov_length_scale(k)=(kinematic_viscosity(k)**3/turbulence(k))**(1./4) !Kolmogorov micro scale, Unit m
       !write(*,*)'haha',kinematic_viscosity(k)
       !write(*,*)'hehe',miu(k)
       !-----step3--end----------calculate Kolmogorov scale----------------------
       
       
       !%-----step4--beginning-----------------calculate transition scale number-----------
        scale_number(k)=L_trasition(k)/Kolmogorov_length_scale(k);
       Tao_return=Tao_reaction(k)
       L_return=L_trasition(k)
       Kol_return=Kolmogorov_length_scale(k)
       scale_number_return=scale_number(k)
      
       
       END SUBROUTINE crm_clumix_scale_num
...
!..Local variables
      ...
      do i = 1, 100
         ...
         elseif (p.gt.0.25) then
          ...
          stokes = Dc(j)*Dc(j)*vtr*RHOW/(9.*1.718E-5*Dr(i))
          reynolds = 9.*stokes/(p*p*RHOW)
...
!..Local variables
      DOUBLE PRECISION:: Dss_m, vts, vtc, stokes, reynolds, Ef_sw
      ...
      do i = 1, 100
         vts = 40.0*Dss(i)**0.55 * DEXP(-100.0*Dss(i)) - vtc
         Dss_m = (0.069*Dss(i)**2.0 / PI*RHOW/6.0)**(1.0/3.0)
         p = Dc(j)/Dss_m
         if (p.gt.0.25 .or. Dss(i).lt. D0s .or. Dc(j).lt.6.E-6 &
               .or. vts.lt.1.E-3) then
          t_Efsw(i,j) = 0.0
         else
          stokes = Dc(j)*Dc(j)*vts*RHOW/(9.*1.718E-5*Dss_m)
          reynolds = 9.*stokes/(p*p*RHOW)
...
END MODULE module_mp_morr_two_moment_v381_ace
!+---+-----------------------------------------------------------------+
!+---+-----------------------------------------------------------------+
