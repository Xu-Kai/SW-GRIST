...
! ==================================================================================================
  ...
  SUBROUTINE NOAHMP_GLACIER (&
                   ...
                   EMISSI,  FPICE,    CH2B                              , & ! OUT :
                    ...
                    sfcheadrt,                                           &
#endif
                   TAUX, TAUY, FIRE, ALBD_OUT, ALBI_OUT)                    !cheyz added                      
...
!-> xinyf and cheyz
  ...
  REAL,DIMENSION(1:2)            , INTENT(OUT)   :: ALBD_OUT,ALBI_OUT 
!<- 
...
! --------------------------------------------------------------------------------------------------
  SUBROUTINE ENERGY_GLACIER (NSNOW  ,NSOIL  ,ISNOW  ,DT     ,QSNOW  ,RHOAIR , & !in
                             ...
                             SAG    ,FSA    ,FSR    ,FIRA   ,FSH    ,FGEV   , & !out
                             TRAD   ,T2M    ,SSOIL  ,LATHEA ,Q2E    ,EMISSI, CH2B, &    !out
                             TAUX   ,TAUY   ,FIRE   ,ALBD_OUT, ALBI_OUT )    !chenyz       
...
!-> xinyf and cheyz
  ...
  REAL                              , INTENT(OUT)   :: TAUY
  REAL                              , INTENT(OUT)   :: FIRE
  REAL,DIMENSION(1:2)               , INTENT(OUT)   :: ALBD_OUT
  REAL,DIMENSION(1:2)               , INTENT(OUT)   :: ALBI_OUT
!<-
...
!energy balance at surface: SAG=(IRB+SHB+EVB+GHB)
    ...
    IF(FIRE <=0.) print*, ("STOP in Noah-MP: emitted longwave <0")
...
! --------------------------------------------------------------------------------------------------
   ...
   IF (ERRSW > 0.01) THEN            ! w/m2
     ...
     print*, (trim(message))
     print*, ("Radiation budget problem in NOAHMP GLACIER")
   END IF
   ...
   IF(ERRENG > 0.01) THEN
      ...
      print*, (trim(message))
      print*, ("Energy budget problem in NOAHMP GLACIER")
   END IF
   ...
   IF(ABS(ERRWAT) > 0.1) THEN
      ...
      WRITE(message,'("    I      J     END_WB     BEG_WB       PRCP       EDIR      RUNSRF     RUNSUB")')
           ...
           print*, (trim(message))
           print*, ("Water budget problem in NOAHMP GLACIER")
        END IF
#endif
