...
!
!  zhangyi notes: some constants may not be the same as those in grist
!  ignore this now.
!=======================================================================

  use grist_constants, only: r8, rearth, rdry, rvap, gravity, cp, pi, omega

  IMPLICIT NONE
  private
   
  public :: tropical_cyclone_test, &
            dp, rp, exppr, cen_lat, cen_lon,&
...
!=======================================================================
  ...
  REAL(r8), PARAMETER ::               &
       !rearth= 6371220.0d0,           & ! Reference Earth's Radius (m)
...
!=======================================================================
  SUBROUTINE tropical_cyclone_test(lon,lat,p,z,zcoords,u,v,t,thetav,phis,ps,rho,q) &
    ...
    gr = rearth*acos(sin(cen_lat*deg2rad)*sin(lat) + &  ! Great circle radius
         (cos(cen_lat*deg2rad)*cos(lat)*cos(lon-cen_lon*deg2rad)))

    !------------------------------------------------
    !   Initialize PS (surface pressure), eq[9]
    !------------------------------------------------
    ...
    if (zcoords .eq. 1) then
       ...
       if(p.le.ps.and.p.ge.ptrop)then
          height = (T0/gamma)*(1.d0-(p/ps)**exponent) ! eq[24], only one?
       else
          height = ztrop+Rd*Ttrop*log(ptrop/p)/g
       end if

       ! If inside a certain distance of the center of the storm
       ! perform a Fixed-point iteration to calculate the height
       ! more accurately, eq[25]
    ...
    !------------------------------------------------
    !   Initialize U and V (wind components), eq[19-21]
    !------------------------------------------------
    ...
    !------------------------------------------------
    !   Initialize water vapor mixing ratio (q), eq[1]
    !------------------------------------------------
    if (height > ztrop) then
        q = qtrop
    else
    ...
    !------------------------------------------------
    !   Initialize temperature (T), eq[13]
    !------------------------------------------------
    if (height > ztrop) then
        t = Ttrop
    else
...
! eq[8], only one?
     if(zhere.ge.0._r8.and.zhere.le.ztrop)then
      fpiF = phere-(p00-dp*exp(-(gr/rp)**exppr)*exp(-(zhere/zp)**exppz)) &
             *((T0-gamma*zhere)/T0)**(g/(Rd*gamma))
     else
      fpiF = phere-(ptrop*exp(-g*(zhere-ztrop)/(Rd*Ttrop)))
     end if
...
END MODULE grist_dtp_dcmip2016_tc
