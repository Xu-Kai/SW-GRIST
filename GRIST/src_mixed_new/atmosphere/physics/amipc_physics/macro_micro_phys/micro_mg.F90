...
! qme
    ...
    subroutine read_nml_micro_mg(nlfile)
! io
...
! local
    ...
    case (1)
       ...
       case default
          call endrun("Invalid sub-version number specified for MG microphysics")
       end select
    case default
       call endrun("Invalid version number specified for MG microphysics")
    end select

    end subroutine read_nml_micro_mg


    subroutine micro_mg_init(ncol)

    use micro_mg1_0,                   only: micro_mg1_0_init
    use micro_mg1_5,                   only: micro_mg1_5_init
...
! local
   integer :: m
    ...
    case (1)
       ...
       case(0)
          ! MG version 1.0
          call micro_mg1_0_init(gravity, rdry,  rvap,  cp,    rhoh2o,    &
                                tmelt  , latvap,latice,rhmini,pi)
       case(5)
          ! MG version 1.5 - MG2 development
          call micro_mg1_5_init(gravity, rdry,  rvap,  cp,               &
    ...
    allocate(tnd_qsnow(nlev,ncol));      tnd_qsnow   = 0._r8    !CARMA
    allocate(tnd_nsnow(nlev,ncol));      tnd_nsnow   = 0._r8    !CARMA
    allocate(re_ice(nlev,ncol));         re_ice      = 0._r8    !CARMA
    allocate(relvar(nlev,ncol));         relvar      = 2._r8    !CLUBB
    allocate(accre_enhan(nlev,ncol));    accre_enhan = 1._r8    !CLUBB
...
! initialize to 0, if we have initial data, set to initial value.
    ...
    end subroutine micro_mg_end


    subroutine micro_mg_tend(ncol,dtime)
...
! check negetive qv
    ...
    do k = top_lev, nlev
       do i = 1, ncol
          ...
          !Note: iwc = iwc+snow, only used for io, LiXH.
     !     pstate_cam%microp_iwc_at_pc_full_level%f(k,i) = pstate_cam%microp_iwc_at_pc_full_level%f(k,i)  &
     !                                                   + qsout(k,i) * state_pmid(k,i) / (287.15_r8*state_t(k,i))
...
contains
    ...
    use micro_mg1_0,        only: micro_mg1_0_tend
    ...
    qc = pstate%tracer_mxrt_at_pc_full_level%f(ixcldliq,:,:ncol)
    qi = pstate%tracer_mxrt_at_pc_full_level%f(ixcldice,:,:ncol)
    nc = pstate%tracer_mxrt_at_pc_full_level%f(ixnumliq,:,:ncol)
    ni = pstate%tracer_mxrt_at_pc_full_level%f(ixnumice,:,:ncol)
 
    call micro_mg1_0_tend( microp_uniform, nlev, ncol, top_lev, dtime,                       &
                           pstate%temp_at_pc_full_level%f(:,:ncol),                          &
                           pstate%tracer_mxrt_at_pc_full_level%f(1,:,:ncol),                 &  
                           qc, qi, nc, ni,                                                   &
                           pstate%pressure_at_pc_full_level%f(:,:ncol),                      &
                           pstate%delp_at_pc_full_level%f(:,:ncol),                          &
                           pstate_cam%macrop_ast_at_pc_full_level%f(old_time_level,:,:ncol),     &
                           alst_mic, relvar, accre_enhan, aist_mic,                          &
                           pstate_cam%microp_cldo_at_pc_full_level%f(old_time_level,:,:ncol),    &
                           rate1cld, pstate_cam%microp_areo_naai%f(:,:ncol),                     &
                           pstate_cam%microp_areo_npccn%f(:,:ncol),                              &
                           pstate_cam%microp_areo_rndst%f(1:4,:,:ncol),                          &
                           pstate_cam%microp_areo_nacon%f(1:4,:,:ncol),                          &
                           tlat, qvlat, qcten, qiten, ncten, niten, effc,                    &
                           effc_fn, effi, prect, preci, nevapr, evapsnow, prain, prodsnow,   &
                           cmeice, pstate_cam%microp_dei_at_pc_full_level%f(:,:ncol),            &
                           pstate_cam%microp_mu_at_pc_full_level%f(:,:ncol),                     &
                           pstate_cam%microp_lambdac_at_pc_full_level%f(:,:ncol), qsout,         &   
                           pstate_cam%microp_des_at_pc_full_level%f(:,:ncol),                    &
                           pstate_cam%microp_res_at_pc_full_level%f(:,:ncol), rflx, sflx,        &
                           ...
                           nfice, do_cldice, tnd_qsnow, tnd_nsnow, re_ice )
 
    end subroutine micro_mg1_0_tend_intr

    subroutine micro_mg1_5_tend_intr
    use micro_mg1_5,        only: micro_mg1_5_tend, micro_mg1_5_get_cols
    ...
    call micro_mg1_5_get_cols(ncol, nlev_loc, top_lev, pstate%tracer_mxrt_at_pc_full_level%f(ixcldliq,:,:ncol), &
                              pstate%tracer_mxrt_at_pc_full_level%f(ixcldice,:,:ncol), mgncol, mgcols) 


    call micro_mg1_5_tend( mgncol,   mgcols,   nlev_loc,   top_lev,   dtime,                      &
                           pstate%temp_at_pc_full_level%f(:,:ncol),                               &
                           pstate%tracer_mxrt_at_pc_full_level%f(1,:,:ncol),                      &
