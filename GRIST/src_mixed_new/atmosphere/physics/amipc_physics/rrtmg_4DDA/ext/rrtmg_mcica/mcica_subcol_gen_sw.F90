...
!------------------------------------------------------------------
      ...
      subroutine mcica_subcol_sw(ncol, nlay, icld, permuteseed, play,         &
...
!      real(kind=r8) :: mean_fsfc_stoch(nlay,ncol)    ! cloud forward scattering fraction
...
!       do ilev = 2,nlay
!          where (CDF2(:,ilev,:) < spread(alpha (ilev,:), dim=2, nCopies=nsubcol) )
!             CDF(:,ilev,:) = CDF(:,ilev-1,:) 
!          end where
!       end do
