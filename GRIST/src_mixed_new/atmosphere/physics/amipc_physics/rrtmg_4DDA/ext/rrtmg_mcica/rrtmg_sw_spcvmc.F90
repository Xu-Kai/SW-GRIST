...
!      use parkind, only : jpim, jprb
      ...
      use rrtmg_sw_reftra, only: reftra_sw, reftra_4DDA_sw
      use rrtmg_sw_taumol, only: taumol_sw
      use rrtmg_sw_vrtqdr, only: vrtqdr_sw, vrtqdr_4DDA_sw
...
! ---------------------------------------------------------------------------
      subroutine spcvmc_sw &
            (iplon, nlayers, istart, iend, icpr, idelm, iout, &
...
!   Do REFTRA only for cloudy layers in profile, since already done for clear layers
               lrtchkcld(jk)=.false.
               lrtchkcld(jk)=(pcldfmc(iw,ikl) > repclc)
...
!               ztauo(jk) = ztaur(iw,ikl) + ztaug(iw,ikl) + ptaucmc(iw,ikl)
!               zomco(jk) = ptaucmc(iw,ikl) * pomgcmc(iw,ikl) + ztaur(iw,ikl)
!               zgco (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc(iw,ikl) + &
...
!   \/\/\/ This block of code is only needed for unscaled direct beam calculation
               if (idelm .eq. 0) then
!     
                  zclear = 1.0_r8 - pcldfmc(iw,ikl)
                  zcloud = pcldfmc(iw,ikl)
...
! Clear + Cloud
                  tauorig = ztauc(jk) + ptaormc(iw,ikl)
...
!   Use this code if cloud properties are derived in rrtmg_sw_cldprop       
               if (icpr .ge. 1) then
                  ztauo(jk) = ztauc(jk) + ptaucmc(iw,ikl)
                  zomco(jk) = ztauc(jk) * zomcc(jk) + ptaucmc(iw,ikl) * pomgcmc(iw,ikl) 
                  zgco (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc(iw,ikl) + &
...
!   Use this code if cloud properties are not derived in rrtmg_sw_cldprop       
               elseif (icpr .eq. 0) then
                  ztauo(jk) = ztaur(iw,ikl) + ztaug(iw,ikl) + ptaua(ibm,ikl) + ptaucmc(iw,ikl)
                  zomco(jk) = ptaua(ibm,ikl) * pomga(ibm,ikl) + ptaucmc(iw,ikl) * pomgcmc(iw,ikl) + &
                              ztaur(iw,ikl) * 1.0_r8
                  zgco (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc(iw,ikl) + &
...
!               zbbcddir(ikl) = zbbcddir(ikl) + zincflx(iw)*ztdbtc_nodel(jk)
               ...
               pbbfsu(ibm,ikl) = pbbfsu(ibm,ikl) + zincflx(iw)*zfu(iw,jk)
               pbbfsd(ibm,ikl) = pbbfsd(ibm,ikl) + zincflx(iw)*zfd(iw,jk)
...
! band 9 is half-NearIR and half-Visible
                  ...
                  puvcd(ikl) = puvcd(ikl) + 0.5_r8*zincflx(iw)*zcd(iw,jk)
                  puvfd(ikl) = puvfd(ikl) + 0.5_r8*zincflx(iw)*zfd(iw,jk)
                  pnicd(ikl) = pnicd(ikl) + 0.5_r8*zincflx(iw)*zcd(iw,jk)
                  pnifd(ikl) = pnifd(ikl) + 0.5_r8*zincflx(iw)*zfd(iw,jk)
                  if (idelm .eq. 0) then
                  ...
                  pnicu(ikl) = pnicu(ikl) + 0.5_r8*zincflx(iw)*zcu(iw,jk)
                  pnifu(ikl) = pnifu(ikl) + 0.5_r8*zincflx(iw)*zfu(iw,jk)
...
! linhan add ---------------------------------------------------------------------------
      subroutine spcvmc_4DDA_sw &
            (iplon, nlayers, istart, iend, icpr, idelm, iout, &
             pavel, tavel, pz, tz, tbound, palbd, palbp, &
             pcldfmc, ptaucmc, pasycmc, pasycmc2, pasycmc3, pomgcmc, ptaormc, & !linhan
             ptaua, pasya, pomga, prmu0, coldry, wkl, adjflux, &
             laytrop, layswtch, laylow, jp, jt, jt1, &
             co2mult, colch4, colco2, colh2o, colmol, coln2o, colo2, colo3, &
             fac00, fac01, fac10, fac11, &
...
! ------- Input -------
      ...
      real(kind=r8), intent(in) :: pasycmc(:,:)                ! cloud asymmetry parameter [mcica]
                                                                 !   Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: pasycmc2(:,:)                ! 2nd asymmetry parameter [mcica]
                                                                 !   Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: pasycmc3(:,:)                ! 3rd parameter [mcica]
                                                                 !   Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: pomgcmc(:,:)                ! cloud single scattering albedo [mcica]
                                                                 !   Dimensions: (ngptsw,nlayers)
      real(kind=r8), intent(in) :: ptaormc(:,:)                ! cloud optical depth, non-delta scaled [mcica]
...
!      integer, parameter :: nvs = ?? 
      ...
      real(kind=r8) :: zdbt(nlayers+1), zdbt_nodel(nlayers+1)
      real(kind=r8) :: zgc(nlayers), zgcc(nlayers), zgco(nlayers)
      real(kind=r8) :: zgcc2(nlayers), zgco2(nlayers) !linhan
      real(kind=r8) :: zgcc3(nlayers), zgco3(nlayers) !linhan
      real(kind=r8) :: zomc(nlayers), zomcc(nlayers), zomco(nlayers)
      real(kind=r8) :: zrdnd(nlayers+1), zrdndc(nlayers+1)
      real(kind=r8) :: zref(nlayers+1,2), zrefc(nlayers+1,2), zrefo(nlayers+1,2) !linhan
      real(kind=r8) :: zrefd(nlayers+1,2,2), zrefdc(nlayers+1,2,2), zrefdo(nlayers+1,2,2) !linhan
      real(kind=r8) :: zrup(nlayers+1), zrupd(nlayers+1)
      real(kind=r8) :: zrupc(nlayers+1), zrupdc(nlayers+1)
      real(kind=r8) :: zs1(nlayers+1)
      real(kind=r8) :: ztauc(nlayers), ztauo(nlayers)
      real(kind=r8) :: ztdn(nlayers+1), ztdnd(nlayers+1), ztdbt(nlayers+1)
      real(kind=r8) :: ztoc(nlayers), ztor(nlayers)
      real(kind=r8) :: ztra(nlayers+1,2), ztrac(nlayers+1,2), ztrao(nlayers+1,2)!linhan
      real(kind=r8) :: ztrad(nlayers+1,2,2), ztradc(nlayers+1,2,2), ztrado(nlayers+1,2,2) !linhan
      real(kind=r8) :: zdbtc(nlayers+1), ztdbtc(nlayers+1)
      real(kind=r8) :: zincflx(ngptsw), zdbtc_nodel(nlayers+1) 
      real(kind=r8) :: ztdbt_nodel(nlayers+1), ztdbtc_nodel(nlayers+1)
      ...
      real(kind=r8) :: zdbtmc, zdbtmo, zf, zgw, zreflect
      real(kind=r8) :: zwf, tauorig, repclc
      real(kind=r8) ::DTR(nlayers+1),DTRC(nlayers+1),DTRO(nlayers+1)  !linhan add
...
! Reinitialize g-point counter for each band if output for each band is requested.
        !linhan if (iout.gt.0.and.ibm.ge.2) iw = ngs(ibm-1)
        if (ibm.ge.2) iw=ngs(ibm-1) !linhan
...
!   Surface values
            zdbtc(klev+1) =0.0_r8
            !ztrac(klev+1,:) =0.0_r8  !linhan 20191013
            !ztradc(klev+1,:,:)=0.0_r8
            ztrac =0.0_r8
            ztradc=0.0_r8
            zrefc=0._r8
            zrefdc=0._r8
            zrefc(klev+1,:) =palbp(ibm)
            zrefdc(klev+1,:,1)=palbd(ibm)*2*u(3)*a(3)
            zrefdc(klev+1,:,2)=palbd(ibm)*2*u(4)*a(4)
            zrupc(klev+1) =palbp(ibm)
            zrupdc(klev+1)=palbd(ibm)
...
!   Surface values
            ...
            ztrado(klev+1,:,:)=0.0_r8
            zrefo(klev+1,:) =palbp(ibm)
            zrefdo(klev+1,2,1)=palbd(ibm)*2*u(3)*a(3)
            zrefdo(klev+1,2,2)=palbd(ibm)*2*u(4)*a(4)
...
!   Surface values
            ...
            ztrad(klev+1,:,:)=0.0_r8
            zref(klev+1,:) =palbp(ibm)
            zrefd(klev+1,:,1)=palbd(ibm)*2*u(3)*a(3)
            zrefd(klev+1,:,2)=palbd(ibm)*2*u(4)*a(4)
            zrup(klev+1) =palbp(ibm)
            zrupd(klev+1)=palbd(ibm)
...
! Clear-sky optical parameters including aerosols
               ...
               zomcc(jk) = ztaur(iw,ikl) * 1.0_r8 + ptaua(ibm,ikl) * pomga(ibm,ikl)
               zgcc(jk) = pasya(ibm,ikl) * pomga(ibm,ikl) * ptaua(ibm,ikl) / zomcc(jk)
               zgcc2(jk) = (pasya(ibm,ikl) * pomga(ibm,ikl) * ptaua(ibm,ikl)&
                   +(0.5_r8*1.0_r8*ztaur(iw,ikl))) / zomcc(jk) !linhan
               zomcc(jk) = zomcc(jk) / ztauc(jk)
...
! Delta scaling - clear   
               ...
               zgcc (jk) = (zgcc(jk) - zf) / (1.0_r8 - zf)
               !linhan
               zgcc2(jk) = (zgcc2(jk)**2 - zf) / (1.0_r8 - zf)
               zgcc3(jk) = (zgcc(jk)**3 - zf) / (1.0_r8 - zf)
               !linhan end
...
!   Use this code if cloud properties are derived in rrtmg_sw_cldprop       
               if (icpr .ge. 1) then
                  ...
                  zgco (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc(iw,ikl) + &
                              ztauc(jk) * zomcc(jk) * zgcc(jk)) / zomco(jk)
                  !linhan
                  zgco2 (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc2(iw,ikl) + &
                              ztauc(jk) * zomcc(jk) * zgcc2(jk)) / zomco(jk)
                  zgco3 (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc3(iw,ikl) + &
                              ztauc(jk) * zomcc(jk) * zgcc3(jk)) / zomco(jk)
...
!   Use this code if cloud properties are not derived in rrtmg_sw_cldprop       
               elseif (icpr .eq. 0) then
                  ...
                  zgco (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc(iw,ikl) + &
                              ptaua(ibm,ikl)*pomga(ibm,ikl)*pasya(ibm,ikl)) / zomco(jk)
                  zgco2 (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc2(iw,ikl) + &
                              ptaua(ibm,ikl)*pomga(ibm,ikl)*pasya(ibm,ikl)+ &
                              ztaur(iw,ikl)*1.0_r8*0.5_r8) / zomco(jk)
                  zgco3 (jk) = (ptaucmc(iw,ikl) * pomgcmc(iw,ikl) * pasycmc3(iw,ikl) + &
                              ptaua(ibm,ikl)*pomga(ibm,ikl)*pasya(ibm,ikl)) / zomco(jk)
                  zomco(jk) = zomco(jk) / ztauo(jk)
...
!   Use only if subroutine rrtmg_sw_cldprop is not used to get cloud properties and to apply delta scaling
                  ...
                  zomco(jk) = (zomco(jk) - zwf) / (1.0_r8 - zwf)
                  zgco (jk) = (zgco(jk) - zf) / (1.0_r8 - zf)
                  zgco2 (jk) = (zgco2(jk)**2 - zf) / (1.0_r8 - zf)
                  zgco3 (jk) = (zgco3(jk)**3 - zf) / (1.0_r8 - zf)
               endif 
...
! Clear sky reflectivities
            if (clear_flg) then !linhan
            call reftra_4DDA_sw (klev, &
                            lrtchkclr, zgcc, zgcc2, zgcc3, prmu0, ztauc, zomcc, & !linhan
                            zrefc, zrefdc, ztrac, ztradc, DTRC) !linhan
            endif

! Total sky reflectivities      
            call reftra_4DDA_sw (klev, &
                            lrtchkcld, zgco, zgco2, zgco3, prmu0, ztauo, zomco, & !linhan
                            zrefo, zrefdo, ztrao, ztrado, DTRO) !linhan
...
! Combine clear and cloudy contributions for total sky
               ...
               zref(jk,1) = zclear*zrefc(jk,1) + zcloud*zrefo(jk,1)
               zref(jk,2) = zclear*zrefc(jk,2) + zcloud*zrefo(jk,2)
               zrefd(jk,1,1)= zclear*zrefdc(jk,1,1) + zcloud*zrefdo(jk,1,1)
               zrefd(jk,1,2)= zclear*zrefdc(jk,1,2) + zcloud*zrefdo(jk,1,2)
               zrefd(jk,2,1)= zclear*zrefdc(jk,2,1) + zcloud*zrefdo(jk,2,1)
               zrefd(jk,2,2)= zclear*zrefdc(jk,2,2) + zcloud*zrefdo(jk,2,2)
               ztra(jk,1) = zclear*ztrac(jk,1) + zcloud*ztrao(jk,1)
               ztra(jk,2) = zclear*ztrac(jk,2) + zcloud*ztrao(jk,2)
               ztrad(jk,1,1)= zclear*ztradc(jk,1,1) + zcloud*ztrado(jk,1,1)
			   ztrad(jk,1,2)= zclear*ztradc(jk,1,2) + zcloud*ztrado(jk,1,2)
			   ztrad(jk,2,1)= zclear*ztradc(jk,2,1) + zcloud*ztrado(jk,2,1)
			   ztrad(jk,2,2)= zclear*ztradc(jk,2,2) + zcloud*ztrado(jk,2,2)
			   DTR(jk)= zclear*DTRC(jk) + zcloud*DTRO(jk)!linhan
...
! Vertical quadrature for clear-sky fluxes
            ...
            if (clear_flg) then !linhan
            call vrtqdr_4DDA_sw(klev, iw, &
                           zrefc, zrefdc, ztrac, ztradc, DTRC, & !linhan
                           zdbtc, zrdndc, zrupc, zrupdc, ztdbtc, &
                           palbp(ibm),palbd(ibm),& !linhan
                           zcd, zcu)
            endif
...
! Vertical quadrature for cloudy fluxes
            ...
            call vrtqdr_4DDA_sw(klev, iw, &
                           zref, zrefd, ztra, ztrad, DTR, & !linhan
                           zdbt, zrdnd, zrup, zrupd, ztdbt, &
                           palbp(ibm),palbd(ibm),& !linhan
                           zfd, zfu)
