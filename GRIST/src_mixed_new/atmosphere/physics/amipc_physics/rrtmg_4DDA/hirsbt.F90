...
! ----- Modules -----
    use grist_constants,    only: r8, gravit=>gravity, rair=>rdry
...
! -------------------
    ...
    subroutine hirsrtm(ncol, pp, tt, rmix, co2mmr, o3mix, ts, oro, &
                       tb_ir, britemp)
...
!
   ...
   real(r8), intent(in) :: pp(nlevp, ncol)      ! Pressure for each column and layer (hPa)
   real(r8), intent(in) :: tt(nlev, ncol)       ! Temperature of each column and layer (K)
   real(r8), intent(in) :: ts(ncol)             ! Surface temperature (K)
   real(r8), intent(in) :: rmix(nlev, ncol)     ! Water vapor mass mixing ratio (kg/kg)
   real(r8), intent(in) :: co2mmr(ncol)         ! CO2 mass mixing ratio (kg/kg)
   real(r8), intent(in) :: o3mix(nlev, ncol)    ! Ozone mass mixing ratio (kg/kg)
   real(r8), intent(in) :: oro(ncol)            ! Land surface flag, sea=0, land=1
!
! Output arguments
!
   real(r8), intent(out) :: britemp(pnf_msu, ncol)   ! HIRS brightness temperatures
   real(r8), intent(out) :: tb_ir(pnb_hirs, ncol)    ! MSU brightness temperatures
...
!     real(r8)  britemp1(pnf_msu,ncol)
!     real(r8)  britemp2(pnf_msu,ncol)
!     real(r8)  britemp3(pnf_msu,ncol)
...
!       endif
        ...
        do i=1,nlev
          ...
          dp=pp(i+1,icol)-pp(i,icol)
          tlay=tt(i,icol)
          play=sqrt(pp(i,icol)*pp(i+1,icol))
          rlay=rmix(i,icol)
          o3lay=o3mix(i,icol)
          
          rhoair=play*100._r8/(r*tt(i,icol))
          e = play*rlay/(rlay+0.6220_r8)
          delz=dp*0.1_r8/(rhoair*grav)
          
!         if(msu_flag.eq.1) then
            dp2=pp(nlev+2-i,icol)-pp(nlev+1-i,icol)
            tlay2=tt(nlev+1-i,icol)
            play2=sqrt(pp(nlev+1-i,icol)*pp(nlev+2-i,icol))
            rlay2=rmix(nlev+1-i,icol)

            rhoair2=play2*100._r8/(r*tt(nlev+1-i,icol))
...
!           britemp1(if,icol) = 0.047981*freq(if)/log(1.0 
!    $                + 1.47445e-23*freq(if)**3/rad(if))
!           britemp2(if,icol) = 0.047981*freq(if)/log(1.0 
!    $                + 1.47445e-23*freq(if)**3/rad2(if))
!           britemp3(if,icol) = 0.047981*freq(if)/log(1.0 
!    $                + 1.47445e-23*freq(if)**3/refl(if))
     
            rad3(if) = rad(if) + refl(if)
            britemp(if,icol) = 0.047981_r8*freq(if)/log(1.0_r8  &
...
!       endif
        ...
        do ib=1,pnb_hirs
          ...
          tb_ir(ib,icol)=btemp(cwn(ib),a_hirs(ib),b_hirs(ib),radir(ib))
        end do
