...
! local
   integer(i4)  :: clock_dynamics, clock_wrfphys, clock_diagio, clock_diagte, clock_tracer, clock_d2t, &
                   clock_int, clock_diffusion,clock_tracer_mif, clock_run_loop
   logical      :: is_first_restart
    clock_dynamics= clock_id('dynamics')
    ...
    clock_tracer  = clock_id('tracer')
    clock_d2t     = clock_id('d2t')
    clock_int     = clock_id('integration')
    clock_diffusion     = clock_id('diffusion')
    clock_tracer_mif  = clock_id('eval_mif')
    clock_run_loop     = clock_id('run_loop')
...
!    call grist_static_data_generate_sst(mesh)
    ...
    call clock_begin(clock_run_loop)
    DO itimestep = istep_beg, istep_end
...
! integration
!
      !call cesm_swlu_prof_init(2)
      call cesm_swlu_prof_start(2)
...
!
          ...
          call clock_begin(clock_tracer_mif)
          call tracer_transport_evaluate_mif(mesh,nlev)
          call clock_end(clock_tracer_mif)

          do idstep = 1, dstep_in_tstep
             call clock_begin(clock_int)
#ifdef MIXCODE
...
!
    ...
    END DO
    call clock_end(clock_run_loop)
      call cesm_swlu_prof_stop(2)
      call cesm_swlu_prof_print(2)
